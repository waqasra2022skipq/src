...
my $update_token_q =
  $dbh->prepare("Update USPSTokens set access_token=? WHERE access_token = ?");
...
my $api_key = "OsIAIMJ9I76boItsLnBOeRBIrpQnNOuw42AL9tuuhUEnXmKv"
  ;    # Replace with your API key

# OAuth 2.0 Credentials
my $client_id = "hTxmGg2yXrARXyTAsqKliUXUAgntRyW1G3PDAWTtYzBjYACi";
my $client_secret =
  "raM669vwDsm6wpoAVq3x3wX2NBPq4ikcqiq1KtWr2zlG4GANcasGR24tmNuzymrl";
...
sub get_access_token {
    ...
    my $body =
"grant_type=client_credentials&client_id=$client_id&client_secret=$client_secret&scope=addresses";
    $req->content($body);
...
sub uspsValidateAddress {
    ...
    try {
        ...
        my $query =
          "?streetAddress=${Addr1}&city=${City}&state=${State}&ZIPCode=${Zip5}";
        ...
        if ( $response->is_success ) {
            ...
            if (
                $response->{'error'}->{'errors'}->{'title'} eq "token_expired" )
            {
                $access_token = get_access_token();
            }
            else {
...
eval {
    ...
    if ( $validated_address->{'error'}->{'message'} eq
