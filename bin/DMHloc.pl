...
my $test = $form->{'DBNAME'} eq 'okmisorg_devmms' || $form->{'DBNAME'} eq 'okmisorg_devoays' || $form->{'DBNAME'} eq 'okmis_demo' || $form->{'DBNAME'} eq 'okmms_dev'
         ? 1 : 0;
my @notes = (''
...
my $method = SOAP::Data->name('Notifications')->attr({xmlns => 'http://tempuri.org/'});
...
#print qq|KLS: back from soap-call...\n|;
  ...
  {
    my $sClientPrAuthCDC = $dbh->prepare("select * from ClientPrAuthCDC where CDCKey=?");
    ...
    foreach $Header (@Header)
    {
      ...
      if ( $CDCKey eq '' )
      { $errmsg .= $hdrcnt == 1 ? qq|hdrcnt=${hdrcnt}| : qq|ERROR: NOCDCKey: hdrcnt=${hdrcnt}|; }
      else
      {
...
foreach my $f ( sort keys %{ $rHeader } ) { print "updCDCPA: rHeader-$f=$rHeader->{$f}\n"; }
  ...
  $rPA->{'NotificationType'} = $NotificationType if ( $NotificationType ne '' );
...
#print qq|  updCDC: PrAuthID=${PrAuthID}, Status=${Status}\n|;
  ...
  if ( ( $NotificationType == 4       # Collab not needed, other provider terminated
      ...
      || $NotificationType == 17      # Termination accepted, your PA is now active
      || $NotificationType == 22      # Health Home, Customer does not meet the high intensity
      || $NotificationType == 23 )    # Health Home, Customer has met high intensity and is unlocked
    && $Status ne 'Approved'
    && $PAnumber ne '' )
  {
  ...
  if ( $NotificationType == 8         # Termination ended your PA
    && $Status ne 'Closed'
    && $PAnumber ne '' )
  {
...
#foreach my $f ( sort keys %{$rUpdateCDC} ) { print "  updCDC: rUpdateCDC: $f=$rUpdateCDC->{$f}\n"; }
  my $CDCID = DBA->doUpdate($form,'ClientPrAuthCDC',$rUpdateCDC,"ClientPrAuthID=${PrAuthID}");
...
foreach my $f ( sort keys %{$rUpdateLog} ) { print "  updCDC: rUpdateLog: $f=$rUpdateLog->{$f}\n"; }
...
print qq|  updLines: key=${key}, ClientID=${ClientID}, PrAuthID=${PrAuthID}/$rCDC->{'ClientPrAuthID'}\n|;
  my $cnt = 0;
...
foreach my $f ( sort keys %{ $Detail } ) { print "  updLines: Detail-$f=$Detail->{$f}\n"; }
}
...
#print qq|  updLines: DELETE!\n|;
      ...
      ($rUpdate->{'BegDate'},$time) = split('T',$Detail->{'LineBegdate'});
      ($rUpdate->{'EndDate'},$time) = split('T',$Detail->{'LineEnddate'});
      $rUpdate->{'PAgroup'} = $Detail->{'PGgroup'};
      $rUpdate->{'Status'} = $Detail->{'LineStatus'};
      $rUpdate->{'Cost'} = $Detail->{'LineStatus'} eq 'K' ? 0 : $Detail->{'LineCost'};
      $rUpdate->{'Units'} = 0;
...
# update ClientPrAuth when these change based on the PAlines...
      $rUpdatePA->{'EffDate'} = $rUpdate->{'BegDate'} if ( $rUpdatePA->{'EffDate'} eq '' );
