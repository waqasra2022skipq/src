...
my $qxTable =
qq|select * from ${xtable} where Script is not null and ExpDate is null order by Category|;
...
while ( my $rxTable = $sxTable->fetchrow_hashref ) {
    ...
    $Defns .=
qq|<SCRIPT LANGUAGE="JavaScript">newtextMsg('help$rxTable->{Name}',${text});</SCRIPT>\n|;
}
my @Categories = ();
my $CatCount   = 0;
my $qxTable =
  qq|select Category from ${xtable} where ExpDate is null group by Category|;

#warn "\nqxTable=\n$qxTable\n";
my $sxTable = $cdbh->prepare($qxTable);
$sxTable->execute || myDBI->dberror($qxTable);
while ( my $rxTable = $sxTable->fetchrow_hashref ) {
    push( @Categories, $rxTable->{Category} );
    $CatCount++;
}
...
my $w       = "33%";
my $qxTable =
qq|select * from ${xtable} where Category=? and ExpDate is null order by Descr |;
$html .= qq|
...
|;
    ...
    while ( my $rxTable = $sxTable->fetchrow_hashref ) {
        ...
        $Help =
qq|<A HREF="javascript:void(0)" ONMOUSEOVER="textMsg.show('help$rxTable->{Name}')" ONMOUSEOUT="textMsg.hide()" ><IMG WIDTH="15" HEIGHT="15" BORDER="0" SRC="/images/qm1.gif"></A>|;
        if ( $ALLACCESS || SysAccess->ChkPriv( $form, "$rxTable->{Priv}" ) ) {
            my $upd =
              $rxTable->{Cron} && $AccessHR
              ? main->setUpdate( $form, $form->{LOGINPROVID}, $rxTable->{Name} )
              : '';
            my $flg = $rxTable->{Cron} ? '*' : '';
            $html .=
qq|<A HREF="javascript:ReportWindow('/src/cgi/bin/GenReport.cgi?Name=$rxTable->{Name}&mlt=$form->{mlt}&$rxTable->{Args}&xtable=$form->{xtable}','$rxTable->{Name}',900,1000)" >$rxTable->{Descr}</A>${flg} ${Help} ${upd} <BR>\n|;
            ...
            null;
        }    # don't let normal users see these.
...
sub setUpdate {
    ...
    my $type;
    my $qProviderRpts =
      qq|select * from ProviderRpts where ProvID=? and Name=?|;
    my $sProviderRpts = $dbh->prepare($qProviderRpts);
    $sProviderRpts->execute( $ProvID, $Name )
      || myDBI->dberror("execute error: UserReports/setUpdate");
