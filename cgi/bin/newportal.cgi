...
if ( !$form->{Client_ClientID} ) {
    $form->error("Client Page / denied ClientID NULL");
}
...
qq|LOGIN EXISTS!\\nAborted setting login: ${ClientID}/${Email}|;
            ...
            else { $AlertMessage = main->setLogin( $form, $ID, $Email ); }
        }
    }
...
sub setLogin {
    ...
    my $query =
      DBA->genReplace( $form, $cdbh, 'UserLogin', $rUpdate, "ID='${ID}'",
        'ID' );
    ...
    my $query = main->genReplace( $form, $cdbh, 'ClientACL', $rUpdate,
        "ClientID='${ClientID}' and ProvID='${ClientID}'" );
    warn qq|newportal: query=${query}\n|;
    my $sUpdate = $cdbh->prepare($query);
    $sUpdate->execute() || $form->dberror($query);
    $rUpdate->{'ProvID'} = 91;
    my $query = main->genReplace( $form, $cdbh, 'ClientACL', $rUpdate,
        "ClientID='${ClientID}' and ProvID='91'" );
    warn qq|newportal: query=${query}\n|;
    my $sUpdate = $cdbh->prepare($query);
    $sUpdate->execute() || $form->dberror($query);
    $sUpdate->finish();
...
sub genReplace {
    ...
    if ( $where eq '' ) { $str = DBA->genInsert( $form, $table, $record ); }
        ...
        $s->execute()
          || $form->dberror("replace: select ${table} where ${where}");
        if ( my $tst = $s->fetchrow_hashref ) {
            ...
            foreach my $fld ( sort keys %{$record} ) {
                if    ( $fld eq $recid ) { null; }
                    ...
                    push( @flds, "`${fld}`=" . $dbh->quote( $record->{$fld} ) );
                }
            }
