...
my $sInsurance = $dbh->prepare("select * from Insurance left join xInsurance on xInsurance.ID=Insurance.InsID where Insurance.ClientID=? and xInsurance.Descr LIKE '%medicaid%' order by Insurance.InsNumEffDate desc");
...
my $basefontoptions = "fontname=" . $fontname . " fontsize=" . $fontsize . " embedding encoding=unicode charref charspacing=0.3";
...
##
  $sInsurance->execute($ClientID) || $form->dberror("select Insurance: ${ClientID}");;
...
#####################
  ...
  $ypos += 13;
  $p->fit_textline("Medicaid ID:", $innermarginleft, $ypos, $basemidfontoptions);
  main->renderLine($p, 134.6, $ypos + 3, 288.5, $ypos + 3, undef, $BBGSData->{"medicaid"});

  $ypos += 40;
  $h_tf = main->renderTextflow($p, "During the past 12 months, have you become restless irritable or anxious when trying to stop/cut down on gambling?",
    $innermarginleft, $ypos, $innermarginleft + $innercontentwidth);
  $ypos += $h_tf;
  main->renderRect($p, 166, $ypos + 3, "Yes", $BBGSData->{"restlessyes"});
  main->renderRect($p, 238, $ypos + 3, "No", $BBGSData->{"restlessno"});

  $ypos += 40;
  $h_tf = main->renderTextflow($p, "During the past 12 months, have you tried to keep your family or friends from knowing how much you gambled?",
    $innermarginleft, $ypos, $innermarginleft + $innercontentwidth);
  $ypos += $h_tf;
  ...
  $ypos += 40;
  $h_tf = main->renderTextflow($p, "During the past 12 months did you have such financial trouble as a result of your gambling that you had to get help with living expenses from family, friends or welfare?",
    $innermarginleft, $ypos, $innermarginleft + $innercontentwidth);
  $ypos += $h_tf;
  ...
  $ypos += 54;
  $p->fit_textline("Can J Psychiatry. 2010 Feb;55(2):82-90.",
    $innermarginleft, $ypos, $basemidfontoptions . " boxsize={$innercontentwidth 13} position={center right}");

  $ypos += 64;
  $h_tf = main->renderTextflow($p,
    "Fax to:\n" .
...
sub renderTextflow {
  ...
  $tf = $p->create_textflow($text, $basefontoptions . " leading=130% " . ($optlist ? $optlist : ""));
  $result = $p->fit_textflow($tf, $xpos_start, $y_footer, $xpos_end, $ypos, "");
...
sub renderRect {
  ...
  $p->stroke();
  $p->fit_textline($data ? '&#x2713;' : '', $xpos + $x_offset_data, $ypos - $y_offset_data, $basecheckfontoptions);
  $p->fit_textline($label, $xpos + $w_rect + 4.8, $ypos - 1.5, $basemidfontoptions);
}

sub renderLine {
  my ($self, $p, $xpos_start, $ypos_start, $xpos_end, $ypos_end, $linewidth, $text, $textopt) = @_;
  ...
  $p->stroke();
  $p->fit_textline($text, $xpos_start + $x_offset_text, $ypos_start - $y_offset_text, $textopt ? $textopt : $basefontoptions);
}
