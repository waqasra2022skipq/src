...
##
# Access required
if ( !$form->{Provider_ProvID} ) { $form->error("Provider Page / denied ProvID NULL"); }
...
{
  my $sDelete = $dbh->prepare("delete from ProviderPrivs where ProvID=$form->{Provider_ProvID}");
  $sDelete->execute() || $form->dberror("delete ProviderPrivs");
  $sDelete->finish();
#warn qq|DELETE ProviderPrivs: ProvID=$form->{Provider_ProvID}\n|;
  my $sInsert = $dbh->prepare("INSERT INTO ProviderPrivs (ProvID,Type,Rank,CreateProvID,CreateDate,ChangeProvID) VALUES (?,?,?,?,?,?)");
  my $qxPrivileges = qq|select * from okmis_config.xPrivileges where ExpDate is null order by Descr |;
...
#warn qq|ProviderPrivs: $rxPrivileges->{Priv}-value=$form->{$rxPrivileges->{Priv}}\n|;
    ...
    {
      $sInsert->execute($form->{Provider_ProvID},$rxPrivileges->{Priv},'1',$form->{LOGINPROVID},$form->{TODAY},$form->{LOGINPROVID}) || $form->dberror("insert ProviderPrivs");
#warn qq|INSERT ProviderPrivs: $form->{Provider_ProvID}, $rxPrivileges->{Priv}\n|;
  ...
  foreach my $ClinicID ( SysAccess->getACL($form,$form->{LOGINPROVID},'Clinic') )
  {
    if ( $form->{$ClinicID} == 1 )                 # Clinic access
    {
      $sInsert->execute($form->{Provider_ProvID},$ClinicID,'9',$form->{LOGINPROVID},$form->{TODAY},$form->{LOGINPROVID}) || $form->dberror("insert ProviderPrivs");
...
# EXIT on update.
  $form->complete();
  print qq|Location: /cgi/bin/ProviderPage.cgi?Provider_ProvID=$form->{Provider_ProvID}&mlt=$form->{mlt}\n\n|;
  exit;
}
##
...
$sProvider->execute($form->{Provider_ProvID}) || $form->dberror("select Provider table in ProviderPrivs");
my $rProvider = $sProvider->fetchrow_hashref;

my $qProviderPrivs = qq|select * from ProviderPrivs where ProvID=? and Type=?|;
my $sProviderPrivs = $dbh->prepare($qProviderPrivs);
my $Defns;
my $qxPrivileges = qq|select * from okmis_config.xPrivileges where ExpDate is null order by Category|;
#warn "\nqxPrivileges=\n$qxPrivileges\n";
my $sxPrivileges = $dbh->prepare($qxPrivileges);
$sxPrivileges->execute || $form->dberror($qxPrivileges);
while ( my $rxPrivileges = $sxPrivileges->fetchrow_hashref ) 
{ 
  my $text = $rxPrivileges->{Defn} ? $dbh->quote($rxPrivileges->{Defn}) : "''";
  $Defns .= qq|<SCRIPT LANGUAGE="JavaScript">newtextMsg('help$rxPrivileges->{Priv}',${text});</SCRIPT>\n|;
}
my @Categories = ();
my $qxPrivileges = qq|select Category from okmis_config.xPrivileges where ExpDate is null group by Category|;
#warn "\nqxPrivileges=\n$qxPrivileges\n";
my $sxPrivileges = $dbh->prepare($qxPrivileges);
$sxPrivileges->execute || $form->dberror($qxPrivileges);
while ( my $rxPrivileges = $sxPrivileges->fetchrow_hashref ) 
{ 
  ...
  { push(@Categories,$rxPrivileges->{Category}); }
}
############################################################################
...
my $qxPrivileges = qq|select * from okmis_config.xPrivileges where Category=? and ExpDate is null order by Descr |;
$html .= qq|
...
|;
  ...
  while ( my $rxPrivileges = $sxPrivileges->fetchrow_hashref )
  {
    $sProviderPrivs->execute($form->{Provider_ProvID},$rxPrivileges->{Priv});
    if ( $rProviderPrivs = $sProviderPrivs->fetchrow_hashref )
    { $Checked='CHECKED'; } else { $Checked=''; }
    $Help = qq|<A HREF="javascript:void(0)" ONMOUSEOVER="textMsg.show('help$rxPrivileges->{Priv}')" ONMOUSEOUT="textMsg.hide()" ><IMG WIDTH=15 HEIGHT=15 BORDER=0 SRC="/images/qm1.gif"></A>|;
    $Descr = $Agent ? qq|$rxPrivileges->{Descr} ($rxPrivileges->{Priv})| : $rxPrivileges->{Descr};
    $html .= qq|  <LI><INPUT TYPE="checkbox" NAME="$rxPrivileges->{Priv}" VALUE="1" ${Checked} >${Descr} ${Help}</LI>|;
  }
  $html .= qq|</UL>
 </TD>
 </TR>
...
|;
  foreach my $ClinicID ( SysAccess->getACL($form,$form->{LOGINPROVID},'Clinic') )
  {
    ...
    if ( $rProviderPrivs = $sProviderPrivs->fetchrow_hashref )
    { $Checked='CHECKED'; } else { $Checked=''; }
    $html .= qq|  <LI><INPUT TYPE="checkbox" NAME="${ClinicID}" VALUE="1" ${Checked} >$rClinic->{Name}</LI>|;
  }
  $html .= qq|</UL>
 </TD>
 </TR>
...
#warn qq|setGraphUser: $ProvID\n|;
  return() if ( $ProvID <= 91 );
  my $qProviders = qq|select Provider.ProvID,Provider.FName,Provider.LName,UserLogin.Password,UserLogin.loginid from Provider left join UserLogin on UserLogin.UserID=Provider.ProvID where Provider.ProvID=?|;
  my $dbh = $form->dbconnect();
...
# update in the graph users records...
  ...
  if (my $rProvider = $sProvider->fetchrow_hashref)
  {
    if ( SysAccess->chkPriv($form,'Graphs',$ProvID) )
    {
      ...
      $r->{'role'} = SysAccess->chkPriv($form,'ClinicManager',$ProvID) ? 'manager' : 'provider';
      $r->{'provider'} = qq|$rProvider->{'LName'}, $rProvider->{'FName'}|;
      $r->{'provid'} = $ProvID;
      $r->{'mlt'} = DBUtil->genToken(12);
      my $u;
      my $q = qq|select * from users where hospital='$r->{'hospital'}' and provid='$r->{provid}'|;
      my $s = $gdbh->prepare($q);
      $s->execute() || $form->dberror($q);
      if ( my $t = $s->fetchrow_hashref )
      { $r->{'id'}=$t->{'id'}; $u = DBA->genUpdate($form,'users',$r,'id'); }
      else
      { $u = DBA->genInsert($form,'users',$r); }
#warn qq|Privileges: u=$u\n|;
    ...
    {
      my ($pfx,$hospital) = split('_',$form->{'DBNAME'});
      my $q = qq|delete from users where hospital='${hospital}' and provid='${ProvID}'|;
