...
my $NextLOC =
qq|/cgi/bin/mis.cgi?misPOP=1&mlt=$form->{mlt}&misLINKS=$form->{misLINKS}&prompt=$form->{prompt}\n\n|;
...
$qNoteTrans =
"select * from NoteTrans where TrID=? and (InsCode=? or InsCode='09') and RecDate is null order by ID";
$sNoteTrans = $dbh->prepare($qNoteTrans);
$qRecTrans =
"update NoteTrans set InsCode=?,RefID=?,RecDate=?,PaidAmt=?,ICN=?,InsPaidID=?,Code='IR',DenCode=NULL where ID=?";
...
$qRecTreatment =
"update Treatment set BillStatus=5, StatusDate=?, RecDate=?, CIPDate=NULL, DenDate=NULL, DenCode=NULL where TrID=? and RecDate is null";
...
#foreach $f (sort keys %{ $rTreatment }) { warn "Treatment-$f = $rTreatment->{$f}<BR>\n"; }
                    $sNoteTrans->execute( $TRID, $rInsPaid->{InsCode} )
                      || $form->dberror($qNoteTrans);
                    ...
                    my ( $TrID, $SCID, $code, $type ) =
                      uBill->postClaim( $form, $r835, 'CL', 'IR' );
                    my $RecAmt = $rInsPaid->{RecAmt} + $AMT;
                    $sRecInsPaidAmt->execute( $RecAmt, $ID )
                      || $form->dberror($qRecInsPaidAmt);
