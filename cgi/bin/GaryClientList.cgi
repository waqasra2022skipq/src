...
my ( $isDMH,    $StatusColor, $InsColor ) = ( 0,    '',    '' );
...
qq|/cgi/bin/ClientPage.cgi?Client_ClientID=$rClient->{ClientID}&${addLinks}|;
    ...
    elsif ( $rInsurance->{Descr} eq 'dmhsas' ) {
        $StatusColor = '#CC9966';
    }                                            # tan
...
sub setEligibility {
 ...
 # Check for PlanDesrc = 'Expansion Healthy Adult Program' is same as 'Title 19'
        my ( $EHAP, $Title19str, $Renew ) = main->chkEligibility(
            1, 1,
            $rInsurance->{InsCode},
        ...
        { $str = $Title19str; }
        elsif ( $Title19 && $isDMH )    # T19 and has DMH contract
        { $str = $Title19str; $StatusReason .= ' no MH and SA Insurance'; }
        elsif ( $Title19 || $EHAP ) { $str = $Title19str; }
