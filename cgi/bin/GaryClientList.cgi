...
# reset all these...
  ...
  elsif ( $rInsurance->{Descr} eq 'dmhsas' ) { $StatusColor = '#CC9966'; }        # tan
...
#warn qq|setEligible: $rInsurance->{InsCode}, isDMH=$isDMH\n|;
  if ( $rInsurance->{InsCode} eq 'MC' )
  {
    ...
    my ($EHAP,$Title19str,$Renew) = main->chkEligibility(1,1,$rInsurance->{InsCode},'Expansion Healthy Adult Program');
    ...
    $EBColor = 'orange' unless ( $Renew eq '' );
    if ( ($Title19 || $EHAP) && $MHSA )             # T19, Expansion Healthy Adult Program and MHSA eligibility, 
    { $str = $Title19str; }
    ...
    { $str = $MHSAstr; $StatusReason .= ' Insurance not Title 19'; $StatusColor = 'red'; }
    $Found = $str eq '' ? 0 : 1;
  }
  ...
  if ( !$Found )                                        # did not find primary insurance this month
  {
    ...
    if ( $Found ) { $StatusReason .= ' Eligibility Expired for Primary Ins'; $EBColor = 'red'; }
    else
    { 
