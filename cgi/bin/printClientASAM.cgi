...
sub renderObject {
    ...
    if ( $obj->{"type"} eq "textline" ) {
        main->renderTextline( $p, $obj->{"text"}, $obj->{"xpos"},
            $obj->{"ypos"}, $obj->{"optlist"} );
    }

    if ( $obj->{"type"} eq "textflow" ) {
        main->renderTextflow( $p, $obj->{"text"}, $obj->{"xpos_start"},
            $obj->{"xpos_end"}, $obj->{"ypos"}, $obj->{"optlist"},
            $obj->{"optlist2"} );
    }

    if ( $obj->{"type"} eq "table" ) {
        main->renderTable( $p, $obj->{"fields"}, $obj->{"ypos"},
            $obj->{"h_row"}, $obj->{"data"} );
    }

    if ( $obj->{"type"} eq "check" ) {
        main->renderCheck( $p, $obj->{"val"}, $obj->{"xposarray"},
            $obj->{"ypos"}, $obj->{"optlist"}, $obj->{"text"} );
    }

    if ( $obj->{"type"} eq "checkvert" ) {
        main->renderCheckVert( $p, $obj->{"val"}, $obj->{"xpos"},
            $obj->{"yposarray"}, $obj->{"optlist"}, $obj->{"text"} );
    }
}
