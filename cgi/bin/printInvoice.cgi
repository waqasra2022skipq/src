...
my $sProvider = $dbh->prepare("select Provider.*, ProviderControl.FinMgrID, ProviderControl.LOGO from Provider left join ProviderControl on ProviderControl.ProvID=Provider.ProvID where Provider.ProvID=?");
# get the clinic from the first InvID...
my $sFindClinic = $dbh->prepare("select * from Client left join Invoices on Invoices.ClientID=Client.ClientID where Invoices.ID=?");
...
$sFindClinic->execute($FirstInvID) || myDBI->dberror("printInvoice: select Clinic $FirstInvID");
...
my @table_headers = ( "Date", "Code", "Description / TrID", "Units", "Charge", "Type", "Payment", "PayDate", "AmtDue");
my @table_alignments = ( "left", "left", "left", "right", "right", "left", "right", "left", "right");
...
my $footertext =
    "<fontsize=".$fontsizesmall.">Please Send Payment to:\n".
    "<fontsize=".$fontsize.">${AgencyName}\n${AgencyAddr}\n\n".
    "<fontsize=".$fontsizesmall.">For questions regarding your invoice contact:\n".
    "<fontsize=".$fontsize.">";
  $sProvider->execute($rAgency->{FinMgrID}) || myDBI->dberror("printInvoice: select FinMgrID");
...
sub fetch_invoice {
    ...
    my $query_handle;
    # PREPARE THE QUERY
    my $query = "SELECT ClientID, ID, DATE_FORMAT(InvDate, '%m/%d/%y') FROM Invoices WHERE ID=?";
...
sub fetch_invoice_items {
    ...
    # PREPARE THE QUERY
    $query = "SELECT TrID, DATE_FORMAT(ContDate, '%m/%d/%y'), ScNum, Descr, Units, BillAmt, InsCode, PaidAmt, DATE_FORMAT(PaidDate, '%m/%d/%y'), AmtDue FROM InvItems WHERE InvID=$invoiceID";
    ...
    # BIND TABLE COLUMNS TO VARIABLES
    $query_handle->bind_columns(undef, \$TrID, \$ContDate, \$ScNum, \$Descr, \$Units, \$BillAmt, \$InsCode, \$PaidAmt, \$PaidDate, \$AmtDue);
...
sub create_header {
    ...
    my $optlist = "boxsize={" . $imagesize . " " .  $imagesize . "} fitmethod=meet";
    $p->fit_image($logoimage,
            $x_logo, $y_logo, $optlist);
    ...
    $optlist = $basefontoptions . " fontname=" . $fontname . " fontsize=" . $fontsize .
    " leading=105% alignment=justify lastalignment=center";
    my $tf = $p->create_textflow($senderfull, $optlist);
    $p->fit_textflow($tf, $x_senderdetail, $y_senderdetail,
            $x_senderdetail+$w_senderdetail, $y_senderdetail + $h_senderdetail, "verticalalign=bottom");
    ...
    $optlist = $basefontoptions . " alignment=justify leading=120% fontsize=" . $fontsize;
    $tf = $p->create_textflow($invoicetext, $optlist);
    $p->fit_textflow($tf, $x_invoice, $y_invoice,
            $x_invoice+$w_invoice, $y_invoice + $h_invoice, "verticalalign=center");
    $p->delete_textflow($tf);
...
sub create_client {
    ...
    $sClient->execute($clientID) || myDBI->dberror("printInvoice: select Client $clientID");
    my $rClient = $sClient->fetchrow_hashref;
    my $ClientAddr = $rClient->{'Addr2'} eq ''
                   ? qq|$rClient->{'Addr1'}| : qq|$rClient->{'Addr1'}, $rClient->{'Addr2'}|;
    $ClientAddr .= qq|, $rClient->{'City'}, $rClient->{'ST'}  $rClient->{'Zip'}|;
    my $clienttext =
    ...
    my $optlist = $basefontoptions . " fontname=" . $fontname . " fontsize=" . $fontsize .
    " leading=110%";
    my $tf = $p->create_textflow($clienttext, $optlist);
    $p->fit_textflow($tf, $x_client, $y_client,
            $x_client+$w_client, $y_client + $h_client, "verticalalign=center");
    $p->delete_textflow($tf);
}
...
sub create_table {
    ...
    my $smallfontoption = "fontname=" . $fontname . " fontsize=" .     
        $fontsizesmall . " embedding encoding=unicode";
    ...
    {
        $optlist =  "fittextline={position={" . $table_alignments[$col-1] . 
    ...
    for (my $i = 0; $i <  $#dataset+1; $i++) {
        ...
        # column 1: Date 
        $optlist = "fittextline={position={" . $table_alignments[$col-1] . 
            " center} " . $smallboldfontoption . "} margin=2";
        ...
        $optlist = "fittextline={position={" . $table_alignments[$col-1] . 
            " center} " . $smallboldfontoption . "} margin=2";
        $tbl = $p->add_table_cell($tbl, $col++, $row, $dataset[$i]{code}, $optlist);
        ...
        $optlist = "fittextline={position={" . $table_alignments[$col-1] . 
            " center}} textflow=" . $tf . " colwidth=60% margin=2";
        ...
        $optlist = "fittextline={position={" . $table_alignments[$col-1] . 
            " center} " . $smallboldfontoption . "} margin=2";
        $tbl = $p->add_table_cell($tbl, $col++, $row, $dataset[$i]{type}, $optlist);
        ...
        $optlist = "fittextline={position={" . $table_alignments[$col-1] . 
            " center} " . $smallboldfontoption . "} margin=2";
        $tbl = $p->add_table_cell($tbl, $col++, $row, $dataset[$i]{paydate}, $optlist);
    ...
    $optlist = "fittextline={position={right center} " . $basefontoptions . "} margin=2 colspan=2 matchbox={name=subtotaltitle} rowheight=16";
    $tbl = $p->add_table_cell($tbl, 7, $row, "", $optlist);

    $optlist = "fittextline={position={right center} " . $smallboldfontoption . "} margin=2 matchbox={name=subtotal} rowheight=16";
    ...
    do {
        ...
        $result = $p->fit_table($tbl,
                $x_table, $y_table, $x_table+$w_table, $y, $optlist);
        ...
        if ($result ne "_boxfull") {
            ...
            $p->fit_textline("GRAND TOTAL:", $x3-2, $y3-2, $basefontoptions . " position={right top}");

            if ($p->info_matchbox("subtotal", 1, "exists") eq 1) {
                $x3 = $p->info_matchbox("subtotal", 1, "x3");
                $y3 = $p->info_matchbox("subtotal", 1, "y3");
            }
            $p->fit_textline($roundedValue, $x3-2, $y3-2, $smallboldfontoption . " position={right top}");
        ...
        if ($result eq "_boxfull") {
            ...
            $p->fit_textline("SUB TOTAL:", $x3-2, $y3-2, $basefontoptions . " position={right top}");
