...
my $basecheckfontoptions = "fontname={Cooper Black} encoding=unicode fontsize=10 charref";
...
sub createPages {
  ...
  my $Insurance = DBA->getxref($form, 'xInsurance', $rCreds->{InsID}, 'Name');
  
  my $Provider_ID_PIN = $rCreds->{PIN};
  
  my $PIN_Qualifier = DBA->getxref($form,'xRefID',$rCreds->{RefID},'ID Descr', '', '', ' | ');

  my $Credential_Type = DBA->getxref($form,'xCredentials',$rCreds->{CredID}, 'Descr Rank', '', '', ' | ');

  my $Rank_Credential = $rCreds->{Rank};

  my $Taxonomy = DBA->getxref($form,'xTaxonomy',$rCreds->{Taxonomy}, 'ID Spec Class Type', '', '', ' | ');

  my $Restriction = DBA->getxref($form,'xSCRestrictions',$rCreds->{Restriction}, 'Descr Rank');
