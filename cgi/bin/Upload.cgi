...
warn
qq|ENTER: Upload.cgi: DocType=${DocType}, ProvID=${ProvID}, ClientID=${ClientID}, TrID=${TrID}\n|
  if ($debug);
warn
qq|DocID=${DocID}, DocTitle=${DocTitle}, DocTag=${DocTag}, DocDescr=${DocDescr}\n|
  if ($debug);
...
elsif ( ${DocType} eq 'MisEDocs' )  # Millennium Electronic Forms, HelpDesk only
{
    ...
    warn
qq|Upload: DirName=${DirName}, FileName=${FileName}, PathName=${PathName}\n|
      if ($debug);
    ...
    $Location =
qq|/cgi/bin/mis.cgi?ProviderEDocs_ID=${id}&view=MisEDocs.cgi&Provider_ProvID=${ProvID}&mlt=${mlt}&misLINKS=$misLINKS&misPop=2\n\n|;
}
...
qq|Upload: DirName=${DirName}, FileName=${FileName}, PathName=${PathName}\n|
    ...
    #warn qq|Upload.cgi: query=$query\n|;
    $Location =
qq|/cgi/bin/mis.cgi?ProviderEDocs_ID=NEWID&view=ProviderEDocs.cgi&Provider_ProvID=${ProvID}&mlt=${mlt}&misLINKS=$misLINKS\n\n|;
}
...
qq|Upload: DirName=${DirName}, FileName=${FileName}, PathName=${PathName}\n|
    ...
    #warn qq|Upload.cgi: query=$query\n|;
    $Location =
qq|/cgi/bin/mis.cgi?ClientEDocs_ID=NEWID&view=ClientEDocs.cgi&Client_ClientID=${ClientID}&mlt=${mlt}&misLINKS=$misLINKS\n\n|;
}
...
qq|Upload: DirName=${DirName}, FileName=${FileName}, PathName=${PathName}\n|
    ...
    $r->{CreateDate}   = $form->{TODAY};
    $query =
      DBA->genReplace( $form, $dbh, 'ProviderControl', $r, "ProvID=${ProvID}" );
...
qq|Upload: DirName=${DirName}, FileName=${FileName}, PathName=${PathName}\n|
      if ($debug);
    $Location =
qq|/cgi/bin/adminFiles.pl?type=${DocType}&mlt=${mlt}&misLINKS=$misLINKS&misPop=2\n\n|;
...
else         { $errmsg = qq|DataBase upload error: DocType=${DocType}|; }
