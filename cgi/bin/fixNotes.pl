...
(my $ICN = $PaidDate) =~ s/-//g;
my $sPayDate=$dbh->prepare("update NoteTrans set PaidDate='${PaidDate}' where RefID='${RefID}' and PaidDate is null");
my $sTreatment=$dbh->prepare("select * from Treatment left join xSC on xSC.SCID = Treatment.SCID where xSC.InsID=212 and BillStatus=5 and AmtDue=?");
...
#warn qq|fixNotes: TrID=${TrID}\n|;
    foreach my $tran ( split(':',$Trans) )
    {
      ...
      my $cmd = qq|/home/okmis/mis/src/cgi/bin/adjNote.pl DBNAME=$form->{DBNAME}\\&submit=1\\&TrID=${TrID}\\&PaidAmt=${Amount}\\&DenCode=${DenCode}\\&RefID=${RefID}\\&ICN=${ICN}\\&mlt=$form->{mlt}|;
      system($cmd);
    }
  }
