...
my $basefontoptions = "fontname=" . $fontname . " fontsize=" . $fontsize . " embedding encoding=unicode charref=true";
...
my $basecheckfontoptions = "fontname={DejaVuSans} encoding=unicode fontsize=10 charref=true";
...
# INSTRUCTIONS
  ...
  $h_tf = main->renderTextflow($p, "<$basefontoptions_u>Leave No Blanks <underline=false>- Where appropriate code items:", 45, $ypos, 212, "charref=true");
  $ypos += $h_tf;
  ...
  $ypos += 4;
  $h_tf = main->renderTextflow($p, "Use <$basefontoptions_u>only<underline=false> one character per item.", 45, $ypos, 180, "charref=true");
  $ypos += $h_tf;

  $ypos += 4;
  $p->fit_textline("2.", $marginleft, $ypos + 11, $basemidfontoptions);
  $h_tf = main->renderTextflow($p, "Item numbers <$basefontoptions_u>underlined<underline=false> are to be asked at follow-up. Items with an asterisk are cumulative and should be rephrased at follow-up (see Manual).", 45, $ypos, 212, "charref=true");
  $ypos += $h_tf;
...
# SEVERITY RATINGS
  ...
  $ypos += 8;
  $h_tf = main->renderTextflow($p, "Each ratings is based upon the patient’s history of problem symptoms, present condition and subjective assessment of his treatment needs in a given area. For a detailed description of severity ratings’ derivation procedures and conventions, see manual.  <$baseboldfontoptions>Note:  <$basefontoptions>These severity ratings are optional.", 218, $ypos, 394, "charref=true");
  $ypos += $h_tf;
##
...
# Addiction Severity
  ...
  $h_tf = main->renderTextflow($p, "<$baseboldmidxlargefontoptions>Addiction Severity Index <$basemidxlargefontoptions_i>Fifth Edition", 414, $ypos, 575, "charref=true");
  $ypos += $h_tf;
...
# GENERAL INFORMATION column 1
  ...
  main->renderRects($p, 200, $ypos + 2, 5, $rClientASI->{'G1'});
...
# GENERAL INFORMATION column 3
  ...
  my @SEVERITYData = ($rClientASI->{'SPMedical'}, $rClientASI->{'SPEmpSup'}, $rClientASI->{'SPAlcohol'},
    $rClientASI->{'SPDrugs'}, $rClientASI->{'SPLegal'}, $rClientASI->{'SPFamily'}, $rClientASI->{'SPPsych'});
  foreach my $i (0..7) {
    my $x = 431.9 + 20 * $i;
    ...
    if ($i > 0 && $SEVERITYData[$i - 1] ne '') {
      my $value = int($SEVERITYData[$i - 1]);
      if ($value >= 0 && $value <= 9) {
        my $y = $ypos + 14.6 * (9 - $value);
        $p->fit_textline("&#x2713;", $x - 15, $y + 12, $basecheckfontoptions . " fontsize=12");
      }
    }
  }
...
# DRUG/ALCOHOL USE column 1
  ...
  $ypos += 5;
  $h_tf = main->renderTextflow($p, "<$basefontoptions_u>Note: <underline=false>See manual for representative examples for each drug class", $xpos, $ypos, 214, , "charref=true");
  $ypos += $h_tf;

  $ypos += 5;
  $h_tf = main->renderTextflow($p, "<$basefontoptions_u>Route of Administration: <underline=false>1 = Oral, 2 = Nasal 3 = Smoking,  4 = Non IV inj.,  5 = IV inj.", $xpos, $ypos, 214, "charref=true");
  $ypos += $h_tf;
##
...
# LEGAL STATUS column 2
  ...
  $h_tf = main->renderTextflow($p, "What was it for? <$basesmallfontoptions_i>(use codes 3-16, 18-20.  If multiple charges, code most severe)", 242, $ypos, 360, "charref=true");
  main->renderRects($p, 396, $ypos + $h_rect + 4, 2, $rClientASI->{'L23'});
  $ypos += $h_tf;

  $ypos += 5;
  $p->fit_textline("L24.", $xpos, $ypos + 11, $basefontoptions_u);
  $h_tf = main->renderTextflow($p, "Are you presently awaiting charges, trial or sentence?\n<$basesmallfontoptions>0 - No  1 - Yes", 242, $ypos, 360, "charref=true");
