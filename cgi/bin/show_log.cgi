...
my $prev = ();
my $select = qq|select Log${table}.* from Log${table} where Log${table}.ID=${ID} order by Log${table}.ChangeDate|;
...
if ( $cnt > 0 )
{
  my $select = qq|select ${table}.* from ${table} where ${table}.ID=${ID} order by ${table}.ChangeDate|;
#warn qq|select=$select\n|;
  my $s = $dbh->prepare($select);
  $s->execute() || $form->dberror($select);
...
my $html = myHTML->newHTML($form,'Logged activity report','CheckPopupWindow noclock countdown_1')
    . gHTML->htmlReport($report,3) . qq|\n</BODY>\n</HTML>\n|;
print $html;
exit;
############################################################################
sub difFields
{
  ...
  $sProvider->execute($cRecord->{'ChangeProvID'}) || $form->dberror("show_log: select Provider $cRecord->{'ChangeProvID'}");
  my $rProvider = $sProvider->fetchrow_hashref;
  my $ProvName = $rProvider->{ScreenName} eq '' ? qq|$rProvider->{'FName'} $rProvider->{'LName'} $rProvider->{'Suffix'}| 
