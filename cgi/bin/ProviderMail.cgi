...
my $MailOff = 'Receipt of Mail Off. Click here to toggle Mail On. This will place your mail address in others To: list.';
my $MailOn  = 'Receipt of Mail On. Click here to toggle Mail Off. This will remove your mail address from others To: list so you will not receive mail from others.';
my $NoMailTxt = $NoMail ? $MailOff : $MailOn;
my $NoMailIMG = $NoMail ? '/images/mailto.gif' : '/images/mailpage.gif';
############################################################################
$qListMail = qq|select * from ProviderMail where ProvID=? and Flag=? order by CreateDate desc|;
...
INSERT INTO ProviderMail (ProvID,FromProvID,ToProvID,Subject,Message,DateSent,Flag,CreateProvID,CreateDate,ChangeProvID) 
    ...
    $sInsert->execute($ProvID,$ProvID,$ToProvID,$Subject,$Message,$DateSent,'sent',$ProvID,$form->{TODAY},$ProvID)
                     || $form->dberror($qInsert);
    foreach my $p ( split(chr(253),$ToProvID) )
    {
      $sInsert->execute($p,$ProvID,$ToProvID,$Subject,$Message,$DateSent,'inbox',$ProvID,$form->{TODAY},$ProvID)
                     || $form->dberror($qInsert);
    }
    $sInsert->finish();
  }
  print qq|Location: /cgi/bin/ProviderMail.cgi?mlt=$form->{mlt}\&showd=$form->{showd}\&prompt=$form->{prompt}\n\n|;
}
...
{ print qq|Location: /cgi/bin/mis.cgi?MIS_Action=MgrTree&mlt=$form->{mlt}\&showd=$form->{showd}\n\n|; }
...
#warn qq|ProviderMail: inside: $form->{MIS_Action}: FORMID=$form->{FORMID}\n|;
    $sListMail->execute($ProvID,$section) || $form->dberror("List Mail=$ProvID ($section)");
...
#warn qq|ProviderMail: $form->{MIS_Action}: ID=$ID=$rListMail->{ID},$rListMail->{Flag}\n|;
      ...
      {
        my $qMailLog = DBA->genInsert($form,'ProviderMailLog',$rListMail);
        $sMailLog = $dbh->prepare($qMailLog);
        $sMailLog->execute() || $form->dberror($qMailLog);
        $sMailDEL->execute($rListMail->{ID}) || $form->dberror($qMailDEL);
      }
      else
      { $sMailUpd->execute('deleted',$rListMail->{ID}) || $form->dberror($qMailUpd); }
    }
  }
...
sub Mail
{
  ...
  my $ToMsg = qq|Reply/Forward has all recipients (reply all)<BR>(use ctrl-key to select/deselect multiples)|;
  my ($FromList,$ToList,$SubList,$MsgList,$Focus) = ($ProvName,'','','','<SCRIPT LANGUAGE="JavaScript">document.ProviderMail.elements[0].focus();</SCRIPT>');
...
# add to list all Providers for each Agency for this Provider.
  ...
  my $selProv = DBA->makeSelect($form,$ToProvID,$list,'Provider','LName:FName');
  if ( $Action eq 'read' || $Action eq 'forward' )
  {
    ...
    if ( my $rMail = $sMail->fetchrow_hashref )
    {
      if ( $rMail->{Flag} eq 'inbox' || $rMail->{Flag} eq '' )
      {
        ...
        my $qMailRead = qq|update ProviderMail set Flag='read', DateRead='$year$month$day$hrs$min' where ID=?|;
      ...
      if ( $Action eq 'forward' )
      {
        ...
        { $ToList .= qq|${sc}$Providers{$p}{FName} $Providers{$p}{LName}|; $sc='; '; }
        $Message = qq|\n\n\n---- Original Message ----\nFrom:    $Providers{$FromProvID}{FName} $Providers{$FromProvID}{LName}\nTo:      $ToList\nSent:    $DateSent\nSubject: $Subject\n\n$Message|;
        $Subject = qq|Re: $Subject| if ( substr($Subject,0,4) ne 'Re: ' );
        ...
        $ToList = qq|    <SELECT NAME="ProviderMail_ToProvID_1" MULTIPLE SIZE=5 >${selProv}</SELECT>|;
        $SubList = qq|  <INPUT TYPE="text" NAME="ProviderMail_Subject_1" VALUE="${Subject}" ONFOCUS="select()" SIZE=60>|;
        $MsgList = qq|  <TEXTAREA NAME="ProviderMail_Message_1" COLS=80 ROWS=18 WRAP="virtual" onFocus="select()" >${Message}</TEXTAREA>|;
      ...
      else
      {
        ...
        { $FromList .= qq|${sc}$Providers{$p}{FName} $Providers{$p}{LName}|; $sc='; '; }
        my $sc='';
        foreach my $p ( split(chr(253),$rMail->{ToProvID}) )
        { $ToList .= qq|${sc}$Providers{$p}{FName} $Providers{$p}{LName}|; $sc='; '; }
        $SubList = qq|${Subject}|;
...
|;
  if ( $Action eq 'send' )
  {
    print qq|<TABLE CLASS="main fullsize" ><TR > <TD CLASS="numcol" > <INPUT TYPE="submit" NAME="MIS_Action=updatesend" VALUE="Send"> </TD> </TR></TABLE>|;
  }
  if ( $Action eq 'forward' )
  {
    print qq|<TABLE CLASS="main fullsize" ><TR > <TD CLASS="numcol" > <INPUT TYPE="submit" NAME="MIS_Action=updateforward" VALUE="Forward"> </TD> </TR></TABLE>|;
  }
print qq|
...
|;
  ...
  my $cnt=0;
  $sListMail->execute($ProvID,$Flag) || $form->dberror("List Mail=$ProvID ($Flag)");
  while ( my $rListMail = $sListMail->fetchrow_hashref )
  {
    $cnt+=1;
    ...
    my $FromProv = qq|$Providers{$rListMail->{FromProvID}}{FName} $Providers{$rListMail->{FromProvID}}{LName}|;
    my $ToProv = ''; my $sc = '';
    foreach my $p ( split(chr(253),$rListMail->{ToProvID}) )
    { $ToProv .= qq|${sc}$Providers{$p}{FName} $Providers{$p}{LName}|; $sc='; '; }
    print qq|
  <TR ${class} >
