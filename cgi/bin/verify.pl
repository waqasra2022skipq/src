...
if ( $form->{type} eq 'InsID' )
{
  ...
  my $sxInsurance = $dbh->prepare("select * from xInsurance where ID='${InsID}'");
  $sxInsurance->execute() || $form->dberror("verify: InsID: select xInsurance (${InsID})");
...
warn qq|verify: Descr=$rxInsurance->{'Descr'}\n|;
  ...
  else
  {
    ...
    else { $errmsg = qq|<FONT COLOR="red">Used only for BCBS,Medicare/Railroad,TriCare and other Insurances!</FONT>|; }
  }
}
elsif ( $form->{type} eq 'InsNum' )
{
  my $InsID = $form->{'a1'};
  (my $InsNum = $form->{'s'}) =~ s/^\s*(.*?)\s*$/$1/g;   # trim both leading/trailing
warn qq|verify: InsNum: InsID=$InsID, InsNum=$InsNum\n|;
  my $sxInsurance = $dbh->prepare("select * from xInsurance where ID='${InsID}'");
  $sxInsurance->execute() || $form->dberror("verify: InsNum: select xInsurance (${InsID})");
  my $rxInsurance = $sxInsurance->fetchrow_hashref;
warn qq|verify: Descr=$rxInsurance->{'Descr'}\n|;
  if ( $InsNum eq '' ) { $errmsg = "InsNum cannot be NULL! "; }
  elsif ( $rxInsurance->{'Descr'} =~ /medicaid/i && ($InsNum !~ /^B\d{8}$/ && $InsNum !~ /^\d{9}$/) )
  { $errmsg = qq|<FONT COLOR="red">InsNum MUST BE 9 digits or B followed by 8 digits!</FONT>|; }
}
elsif ( $form->{type} eq 'DesigProv' )
{
  my $InsID = $form->{'a1'};
  my $sxInsurance = $dbh->prepare("select * from xInsurance where ID='${InsID}'");
  $sxInsurance->execute() || $form->dberror("verify: DesigProv: select xInsurance (${InsID})");
  my $rxInsurance = $sxInsurance->fetchrow_hashref;
#warn qq|verify: Descr=$rxInsurance->{'Descr'}\n|;
  if ( $rxInsurance->{'Descr'} =~ /bcbsok/i ) { $errmsg = ''; } 
  elsif ( $rxInsurance->{'Descr'} =~ /choice/i ) { $errmsg = ''; } 
...
#warn qq|verify: s=$form->{'s'}, a1=$form->{'a1'}, a2=$form->{'a2'}\n|;
  ...
  my $where = $DistID eq '' ? '' : "Active=1 and CountyDistrictCode='${DistID}'";
#warn qq|verify: DistID=$DistID, SelectedIDs=$SelectedIDs\n|;
  $errmsg = qq|
  <SELECT ID="LastSchoolName" NAME="ClientIntake_LastSchoolName_1" >
  | .  DBA->selxTable($form,'xSchoolSites',$SelectedIDs,'SchoolSite City State Zip CountyName CountyDistrictCode SiteCode','','',$where) . qq|
  </SELECT>
|;
}
elsif ( $form->{type} eq 'EFT' )
{
  ...
  my $rxInsurance = $sxInsurance->fetchrow_hashref;
############################################################################
  my $files = qq|/home/okmis/www/forms/hesk_docs/Insurance_$rxInsurance->{Descr}_*|;
  my @Files = glob($files);
  foreach $path ( @Files )
  {
    my ($dir,$fn) = $path =~ /(.*\/)?(.+)/s;
    ...
    $errmsg .= qq|<BR><A HREF="javascript:ReportWindow(\'http://forms.okmis.com/${p2}${fn}\',\'$rxInsurance->{Descr}\')" >${Link}</A>|;
  }
  $errmsg .= qq|<BR><A HREF="javascript:ReportWindow(\'$rxInsurance->{Help}\',\'HELP\')" >Additional Help</A>| if ( $rxInsurance->{Help} ne '' );
  $sxInsurance->finish();
