...
if ( $form->{Provider_ProvID} ) {
    $priv = 'hasProviderAccess';
    $id   = $form->{Provider_ProvID};
    $c    = 'P';
    $s    = 'ProviderPage.cgi';
}
$form->{MIS_Links} =
  qq|S:${s}:${c}:${id}|;    # this can go away if I put this code in mis.cgi.

if ( !SysAccess->verify( $form, $priv, $id ) ) {
    myDBI->error("Access denied! (getHTML)");
}
...
else { $html = myHTML->getHTML( $form, "$form->{view}.dis" ); }
print $html;
