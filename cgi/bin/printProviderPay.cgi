...
sub createPages {
  ...
  $p->fit_textline("$r->{LName}, $r->{FName} $r->{Rate} ($r->{Commission})", 100, $ypos, $basefontoptions);
  ...
  $p->fit_textline($rProviderPay->{"Commission"}, 130, $ypos, $basefontoptions);

  $s = $dbh->prepare("select xSC.*, xInsurance.Name from xSC left join xInsurance on xInsurance.ID=xSC.InsID where xSC.SCID = ?");
  ...
  $p->fit_textline($rProviderPay->{"EffDate"}, 130, $ypos, $basefontoptions);

  $p->fit_textline("Expiry Date:", 230, $ypos, $baseboldfontoptions);
  $p->fit_textline($rProviderPay->{"ExpDate"}, 330, $ypos, $basefontoptions);


  $p->fit_textline("Is Manager:", 430, $ypos, $baseboldfontoptions);
  $p->fit_textline($YesNoArr{$rProviderPay->{"isMgr"}}, 500, $ypos, $basefontoptions);
