...
if ( $form->{ADJUST_TRANSID} ) {
    unless ( DBA->updSQLdone($form) ) {
        main->adjTrans( $form, $form->{ADJUST_TRANSID},
            $form->{ADJUST_AMT}, $form->{ADJUST_REASON} );
        $form->{prompt} .= chr(253)
          . qq|Transaction: $form->{ADJUST_TRANSID} for TrID: $form->{ADJUST_TRID} adjusted for: $form->{ADJUST_AMT}.\\n($form->{ADJUST_REASON})|;
    }
}
if ( $form->{WRITEOFF_TRID} ) {
    unless ( DBA->updSQLdone($form) ) {
        ...
        my ( $TrID, $SCID, $code, $type ) =
          uBill->postClaim( $form, $r835, 'CL', 'SR',
            $form->{WRITEOFF_REASON} );
        $form->{prompt} .= chr(253)
          . qq|TrID: $form->{WRITEOFF_TRID} written-off for: $form->{WRITEOFF_AMT}.\\n($form->{WRITEOFF_REASON})|;
    }
}
...
#warn qq|DENYNOTE_TRANSID=$form->{DENYNOTE_TRANSID}, DENYNOTE_TRID=$form->{DENYNOTE_TRID}, DENYNOTE_CLIENTID=$form->{DENYNOTE_CLIENTID},DENYNOTE_CONTDATE=$form->{DENYNOTE_CONTDATE}|;
    unless ( DBA->updSQLdone($form) ) {
        ...
        $r835->{'DenCode'}  = 'A1';
        my ( $TrID, $SCID, $code, $type ) =
          uBill->postClaim( $form, $r835, 'CL', 'MD',
            $form->{DENYNOTE_REASON} );
        $form->{prompt} .= chr(253)
          . qq|Transaction: $form->{DENYNOTE_TRANSID} for TrID: $form->{DENYNOTE_TRID} for: $form->{DENYNOTE_CONTDATE}/$form->{DENYNOTE_SCNUM} Denied.\\n($form->{DENYNOTE_REASON})|;
    }
}
if ( $form->{UNREVIEW_TRID} ) {
    unless ( DBA->updSQLdone($form) ) {
        ...
        $sTreatment->finish();
        my $StatusMsg =
          $rTreatment->{'StatusMsg'} . ' ' . $form->{UNREVIEW_REASON};
        my $sUnReview = $dbh->prepare(
"update Treatment set RevStatus=2, MgrProvID=NULL, MgrRevDate=NULL, MgrRevTime=NULL, StatusMsg=? where TrID=?"
        );
        $sUnReview->execute( $StatusMsg, $form->{UNREVIEW_TRID} );
        $sUnReview->finish();
        $form->{prompt} .= chr(253)
          . qq|TrID: $form->{UNREVIEW_TRID} has been UNREVIEWED. Manager APPROVAL backed-out.\\n(${StatusMsg})|;
    }
}
############################################################################
my $WithSelect  = '';
...
my (
    $SortTypeAll,        $SortTypeInprocess,     $SortTypeScholarshipped,
    $SortTypeReconciled, $SortTypeNotReconciled, $SortTypeNotBilled
) = ( '', '', '', '', '', '' );
if    ($ForTrID) { null; }   # for a specfic TrID we DON'T NEED the SortOptions.
    ...
    $WithSelect .=
      qq| and (Treatment.BillStatus != 4 and Treatment.BillStatus != 5) |;
...
my (
    $SinceDate,    $SortRangeL1M, $SortRangeL3M, $SortRangeL6M,
    $SortRangeL9M, $SortRangeL1Y, $SortRangeL2Y, $SortRangeALL
) = ( '', '', '', '', '', '', '', '' );
...
my $order =
  qq|Treatment.ContLogDate,Treatment.ContLogBegTime,Treatment.ContLogEndTime|;
...
if ($ForTrID) {
    ...
    $sClient = $dbh->prepare(
"select Treatment.*, Client.FName, Client.LName, Client.SSN, Client.ProvID as PrimaryProvID from Treatment left join Client on Client.ClientID=Treatment.ClientID where TrID=?"
    );
    $sClient->execute($TrID);
    if ( $rClient = $sClient->fetchrow_hashref ) {
        $ChartName = qq|$rClient->{FName} $rClient->{LName}|;
        $SearchName =
qq|$rClient->{FName} $rClient->{LName} ($rClient->{ClientID}) $rClient->{SSN}|;
...
|;
        $hidden =
qq|<INPUT TYPE="hidden" NAME="Client_ClientID" VALUE="$rClient->{ClientID}" >|;
        $ChartType = 'by Note';
...
#warn qq|\n TrID=$ForTrID, ProvID=$rClient->{PrimaryProvID}, ClientID=$rClient->{ClientID}\n|;
          ...
          )
        {
            if ( $form->{LOGINPROVID} != $rClient->{ProvID} ) {
                $addMessage .= 'Chart List / Access Denied!'
                  unless (
                    SysAccess->verify( $form, 'Privilege=OtherProvNotes' ) );
            }
        }
        else { $addMessage .= 'Chart List / Access Denied!'; }
    }
    $sClient->finish();
    unless ( $rClient->{ClientID} ) {
        myDBI->error("Chart List / $ForTrID Not Found!");
    }
}
elsif ($ProviderID) {
...
|;
    ...
    $hidden =
      qq|<INPUT TYPE="hidden" NAME="Provider_ProvID" VALUE="${ProviderID}" >|;
...
|;
    $hidden =
      qq|<INPUT TYPE="hidden" NAME="Client_ClientID" VALUE="${ClientID}" >|;
    $ChartType = 'by Client';
...
my $ClientListByProv =
qq|<A HREF="/src/cgi/bin/ClientList.cgi?Provider_ProvID=$rProvider->{ProvID}&${addLinks}" TITLE="Click here for <BR>${ProviderName}\'s Client List." ><IMG BORDER=0 SRC="/src/images/icon_folder.gif" HEIGTH="30" WIDTH="30" ></A>|;
my $AddNote =
  $ClientID
  ? qq|      <A HREF="/src/cgi/bin/mis.cgi?MIS_Action=Note&Client_ClientID=${ClientID}&Treatment_TrID=new&${addLinks}" TITLE="Click here to <BR>add a Note for <BR>${ClientName}." ><IMG SRC="/src/images/edit.gif" CLASS="site" HEIGHT="35" WIDTH="35" ></A>|
  : '';
my $PhysNote =
  $ClientID && DBA->isPhysician($form)
  ? qq|     <A HREF="/src/cgi/bin/mis.cgi?MIS_Action=Note&NoteType=2&Client_ClientID=${ClientID}&Treatment_TrID=new&${addLinks}" TITLE="Click here to <BR>add a Physician Note for <BR>${ClientName}." > <IMG SRC="/src/cgi/images/caduceuswhite.png" WIDTH="24" HEIGHT="24" > </A>|
  : '';
my $TestNote =
  $form->{'LOGINPROVID'} == 91
  ? qq|     <A HREF="/src/cgi/bin/mis.cgi?MIS_Action=Note&NoteType=5&Client_ClientID=${ClientID}&Treatment_TrID=new&${addLinks}" TITLE="Click here to <BR>add a Physician Note for <BR>${ClientName}." > <IMG SRC="/src/cgi/images/caduceuswhite.png" WIDTH="24" HEIGHT="24" > </A>|
  : '';
...
$sClientInsRemarksDetail = $dbh->prepare(
    "select * from ClientInsRemarksDetail where TrID=? order by CheckDate desc"
);
...
if ($BillingRemitACCESS) {
    ...
    $html .=
      myHTML->ListSel( $form, 'ListClientInsRemarks', $ClientID,
        $form->{'LINKID'}, 0 )
      if ($ClientID);
}
$html .= qq|
...
sub hdr {
    ...
    my $HelpDenials =
      $form->{SortType} eq 'notreconciled'
...
#warn qq|TrID=$r->{TrID}, BillStatus=$r->{BillStatus}, RevStatus=$r->{RevStatus}\n|;
    if ( $r->{BillStatus} > 1 ) {
        ...
        $TrIDColor = 'blue';
        $TrIDMesg  = qq|Provider approval needed.|;
    }
    elsif ( $r->{RevStatus} == 1 ) {
        $TrIDColor = 'orange';
        $TrIDMesg  = qq|Provider approval of Manager changes needed.|;
    }
    elsif ( $r->{RevStatus} == 2 ) {
        $TrIDColor = 'red';
        $TrIDMesg  = qq|Manager review/approval needed.|;
    }
    elsif ( $r->{RevStatus} == 3 ) {
        $TrIDColor = 'darkgreen';
        $TrIDMesg  = qq|Note approved for billing.|;
    }
    $TrIDMesg .= qq| $r->{StatusMsg} ${NoteMsg}|;

    #foreach my $f ( sort keys %{ $r } ) { warn qq|: r-$f=$r->{$f}\n|; }
    my $ProvLName =
      $r->{ProviderScreenName} ? $r->{ProviderScreenName} : $r->{ProviderLName};
    my $ProvName =
        $r->{ProviderScreenName}
      ? $r->{ProviderScreenName}
      : qq|$r->{ProviderFName} $r->{ProviderLName}|;
    ( my $Name = qq|$r->{FName} $r->{LName}| ) =~ s/'/\\'/g;
    my $TrIDProvider = qq|Note by ${ProvName} for $r->{ClinicName}|;

    #foreach my $f ( sort keys %{ $r } ) { warn qq|: r-$f=$r->{$f}\n|; }
    my $rxSC = cBill->getServiceCode( $form, $r->{SCID}, $r->{ContLogDate},
        $r->{ContLogBegTime}, $r->{ContLogEndTime}, $r->{TrID} );
    my $rInsurance = cBill->getInsurance( $form, $r->{ClientID}, $rxSC->{InsID},
        $r->{ContLogDate} );
    ...
    my $BillDate =
      $r->{BillDate} ? DBUtil->Date( $r->{BillDate}, 'fmt', 'MM/DD/YY' ) : 'No';
...
|;
    ...
    if ( $BillStatus =~ /Denied|Recoupment/ ) {
        ( $DenCodeDescr =
              DBA->getxref( $form, 'xDenCodes', $r->{DenCode}, 'Descr' ) ) =~
          s/'//g;
        (
            $RemarkCodeDescr = DBA->getxref(
                $form, 'xDenCodesTrans', $r->{RemarkCode}, 'Descr'
            )
        ) =~ s/'//g;
        my $sNoteTransType = $dbh->prepare(
"SELECT TransType FROM NoteTrans WHERE TrID='$r->{TrID}' AND TransType IS NOT NULL"
        );
        $sNoteTransType->execute();
        my $rTransType = $sNoteTransType->fetchrow_array;
        $BillStatus =
qq|<FONT COLOR="red"><A HREF="javascript:void(0)" TITLE="CAG CODE: ${rTransType}<BR><BR>${DenCodeDescr}<BR><BR>${RemarkCodeDescr}" >${BillStatus} $r->{DenCode} $r->{RemarkCode}</A></FONT>|;
...
qq|<FONT COLOR="red"><A  HREF="javascript:ReportWindow('/src/cgi/bin/unRecNoteForm.pl?submit=1&TrIDs=$r->{TrID}&mlt=$form->{mlt}&noteType=Sch','adjNote')" onclick="return getConsent('Are you SURE you want to UnScholarship $r->{TrID} ')" TITLE="Click here to UnScholarship $r->{TrID}" >${BillStatus}</A></FONT>|;
    ...
    my $sNoteTransHere =
      $dbh->prepare("select * from NoteTrans WHERE TrID='$r->{TrID}'");
...
qq|<FONT COLOR="red"><A  HREF="javascript:ReportWindow('/src/cgi/bin/unRecNoteForm.pl?submit=1&TrIDs=$r->{TrID}&mlt=$form->{mlt}&noteType=New','adjNote')" onclick="return getConsent('Are you SURE you want to UnBill $r->{TrID} ')" TITLE="Click here to UnBill $r->{TrID}" >${BillStatus}</A></FONT>|;
    ...
    my $StatusDate =
      $r->{StatusDate}
      ? DBUtil->Date( $r->{StatusDate}, 'fmt', 'MM/DD/YY' )
      : '&nbsp;';
    ...
    my $MgrRevDate =
      $r->{MgrRevDate}
      ? ':' . DBUtil->Date( $r->{MgrRevDate}, 'fmt', 'MM/DD/YY' )
      : '&nbsp;';
    my $MgrRevStatus =
      $r->{RevStatus} == 3 && ( $MgrRevName eq '' || $MgrRevDate eq '' )
      ? qq|<SPAN CLASS="hotmsg" >error! Status Reviewed but missing info</SPAN>|
      : $r->{RevStatus} == 3 ? ''
      :                        'none';
    my $UnReviewLabel =
qq|<SPAN CLASS="sublabel" >review:${MgrRevName}${MgrRevDate}${MgrRevStatus}</SPAN>|;
    my $UnReviewButton =
         $r->{BillStatus} < 3
      && $r->{RevStatus} == 3 && ( $AgentACCESS || $BillingAdmin )
...
#warn qq|ChartList: TrID=$r->{TrID}, RevStatus=$r->{RevStatus},MgrRevName=$MgrRevName,MgrRevDate$MgrRevDate,MgrRevStatus=$MgrRevStatus\n|;
    my $AmtDueButton =
      $r->{BillStatus} > 3 && $r->{AmtDue} > 0 && $BillingRemitACCESS
      ...
      : $ShowAmountsACCESS ? sprintf( "%.2f", $r->{AmtDue} )
      : $r->{AmtDue} > 0   ? 'yes'
      :                      'no';
    my $AmtDue =
        $ShowAmountsACCESS ? sprintf( "%.2f", $r->{AmtDue} )
...
qq|<A HREF="/src/cgi/bin/mis.cgi?MIS_Action=Note&Client_ClientID=$r->{ClientID}&Treatment_TrID=$r->{TrID}&${addLinks}&pushID=$form->{LINKID}" TITLE="${TrIDMesg} <BR>Click here to <BR>Edit Note $r->{TrID}" >${NoteIMG}|;
    $NotePrintLink .=
qq|<A HREF="javascript:ReportWindow('/src/cgi/bin/printNotes.pl?TrIDs=$r->{TrID}&${addLinks}','printNote')" TITLE="Click here to <BR>New print Note $r->{TrID} as pdf file." ><IMG SRC="/images/adobelogo_bt.gif" BORDER="0" HEIGHT="18" WIDTH="18" ></A>|;
    $NotePrintLink .=
qq|<A HREF="javascript:ReportWindow('$r->{Path}','PrintNote')" TITLE="Click here to <BR>Print the attachment for Note $r->{TrID}." ><IMG SRC="/src/images/paperclip_black.png" BORDER="0" HEIGHT="18" WIDTH="18" ></A>|
      if ( $r->{'Type'} == 3 );
    $NoteCCDALink =
...
#warn qq|ChartList: Priority=$rInsurance->{Priority}, InsID=$rInsurance->{InsID}/$rxSC->{InsID}, InsDescr=$rxSC->{InsDescr}\n|;
    ( my $CredDescr =
      ...
      s/'//g;
    my $Priority =
        $rInsurance->{'Priority'} == 1 ? 'Primary'
      : $rInsurance->{'Priority'} == 2 ? 'Secondary'
      : $rInsurance->{'Priority'} == 3 ? 'Tertiary'
      :                                  'NoPriority';
    ( my $scpopup =
qq|${Priority} $rxSC->{InsName} <BR>[$rInsurance->{InsIDNum}] <BR>$rxSC->{SCName} <BR>[$CredDescr]|
    ) =~ s/'/\\'/g;
    $out .= qq|
  <TR ${class} >
    <TD >${ClientPageAcc}</TD>
...
sub prtNoteTrans {
    ...
    while ( $rNoteTrans = $sNoteTrans->fetchrow_hashref ) {
        ...
        my $rxSC =
          cBill->getServiceCode( $form, $rNoteTrans->{SCID}, $r->{ContLogDate},
            $r->{ContLogBegTime}, $r->{ContLogEndTime}, $r->{TrID} );
        my $ICN   = $rNoteTrans->{ICN}   ? $rNoteTrans->{ICN}   : '&nbsp;';
        my $RefID = $rNoteTrans->{RefID} ? $rNoteTrans->{RefID} : '&nbsp;';
        my $ContDate =
          DBUtil->Date( $rNoteTrans->{ContDate}, 'fmt', 'MM/DD/YY' );
        ...
        my $TCodeDescr =
            $rNoteTrans->{Code} eq 'ER' ? 'Electronic Reconcile'
          : $rNoteTrans->{Code} eq 'ER-I'
          ? 'Electronic Reconcile - from previous ID'
          : $rNoteTrans->{Code} eq 'ER-T'
          ? 'Electronic Reconcile - from previous Transaction'
          : $rNoteTrans->{Code} eq 'ER-R'
          ? 'Electronic Reconcile - from previous Reconciled transaction'
          : $rNoteTrans->{Code} eq 'ER-N'
          ? 'Electronic Reconcile - from Note (no billed transaction)'
          ...
          : $rNoteTrans->{Code} eq 'MD'  ? 'Manual Denied'
          :                                'xx';
        my $TCode =
          $rNoteTrans->{Code} eq ''
          ? '&nbsp;'
          : qq|<FONT COLOR="red"><A HREF="javascript:void(0)" TITLE="${TCodeDescr}" >$rNoteTrans->{Code}</A></FONT>|;
        my ( $dlm, $ReasonCodes ) = ( '', '&nbsp;' );
        ...
        $id =
          ...
          : length($id) == 2 ? '0' . $id
          :                    $id;
        ( my $rcodedescr = DBA->getxref( $form, 'xDenCodes', $id, 'Descr' ) )
          =~ s/'//g;
        $ReasonCodes .=
qq|<FONT COLOR="red"><A HREF="javascript:void(0)" TITLE="${rcodedescr}" >${dlm}${id}</A></FONT>|;
        ...
        foreach my $rcode ( split( '/', $rNoteTrans->{ReasonCode} ) ) {
            ...
            $id =
              ...
              :                    $id;
            ( my $rcodedescr =
                  DBA->getxref( $form, 'xDenCodes', $id, 'Descr' ) ) =~ s/'//g;
            $ReasonCodes .=
qq|<FONT COLOR="red"><A HREF="javascript:void(0)" TITLE="${rcodedescr}" >${dlm}${id}</A></FONT>|;
            $dlm = '<BR>';
...
##
        ...
        ( my $scntpopup = qq|$rxSC->{InsDescr} <BR>$rxSC->{SCName}| ) =~
          s/'/\\'/g;
...
sub adjTrans {
    ...
    if ( my $rNoteTrans = $sNoteTrans->fetchrow_hashref ) {
        ...
        if ( $amt == 0 ) {
            my $sZeroTrans = $dbh->prepare(
                "update NoteTrans set PaidAmt=0, Code='COR' where ID=?");
            $sZeroTrans->execute($id);
            $sZeroTrans->finish();
            my $sAdjTrans =
              $dbh->prepare("update NoteTrans set AdjCode=0 where ID=?");
            $sAdjTrans->execute( $rNoteTrans->{RefID} )
              ;    # set it so they can adjust the associated transaction.
            $sAdjTrans->finish();
          ...
          #warn qq|adjTrans: $id, INSERT: $rNoteTrans->{TrID}: $amt, $reason\n|;
            ...
            $rNoteTrans->{Code}    = 'COR';
            $rNoteTrans->{SRC}     = 'MA';
            $rNoteTrans->{InsPaidID} =
              $InsPaidID;                     # reverse/backout negative amount
            ...
            my $sAdjTrans =
              $dbh->prepare("update NoteTrans set AdjCode=1 where ID=?");
        ...
        my ( $BilledAmt, $IncAmt, $SchAmt, $AmtDue ) =
          uBill->setBilledAmt( $form, $rNoteTrans->{'TrID'} );
...
#warn qq|adjTrans: BilledAmt=$BilledAmt, IncAmt=$IncAmt, SchAmt=$SchAmt, AmtDue=$AmtDue\n|;
        ...
        if ($InsPaidID) {
            ...
            if ( my $rInsPaid = $sInsPaid->fetchrow_hashref ) {
                my $sRecInsPaidAmt =
                  $dbh->prepare("update InsPaid set RecAmt=? where ID=?");
                my $RecAmt = $rInsPaid->{RecAmt} -
                  $InsPaidAMT;    # +amt adds back, -amt takes out
...
qq|<A HREF="/cgi/bin/ClientList.cgi?SearchType=ClientID&SearchString=${ClientID}&${addLinks}" ONMOUSEOVER="window.status='Client List this Client only'; return true;" ONMOUSEOUT="window.status='';" ><IMG BORDER=0 ALT="Client-List" SRC="/images/icon_folder.gif" ></A>|;
        ...
        while ( $rInsPaid = $sInsPaid->fetchrow_hashref ) {
            ...
            my $class = $even ? qq|CLASS="alternate"|         : '';
            my $StartDate =
              DBUtil->Date( $rInsPaid->{StartDate}, 'fmt', 'MM/DD/YY' );
            my $EndDate =
              DBUtil->Date( $rInsPaid->{EndDate}, 'fmt', 'MM/DD/YY' );
            my $TransDate =
              DBUtil->Date( $rInsPaid->{TransDate}, 'fmt', 'MM/DD/YY' );
