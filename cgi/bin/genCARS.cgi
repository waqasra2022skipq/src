...
'select * from Contracts left join xInsurance on xInsurance.ID=Contracts.InsID where ProvID=? and xInsurance.Descr=?'
);
my $sProvider = $dbh->prepare('select * from Provider where ProvID=?');
$qClient =
qq|select * from Client left join ClientIntake on ClientIntake.ClientID=Client.ClientID where Client.ClientID=?|;
my $sClient        = $dbh->prepare($qClient);
my $sClientLegalPP = $dbh->prepare(
    'select * from ClientLegalPP where ClientID=? order by CreateDate');
my $sClientReferrals =
...
if ( $form->{ClientCARSReview_ID} ) {
    my $sCARSReviewMonth =
      $dbh->prepare("select * from ClientCARSReview where ID=?");
    $sCARSReviewMonth->execute( $form->{ClientCARSReview_ID} )
      || myDBI->dberror("select CARSReviewMonth");
    my $rCARSReviewMonth = $sCARSReviewMonth->fetchrow_hashref;
    foreach my $f ( sort keys %{$rCARSReviewMonth} ) {
        warn ": rCARSReviewMonth-$f=$rCARSReviewMonth->{$f}\n";
    }
...
my $sClientCARSReview = $dbh->prepare(
    "select * from ClientCARSReview where ClientID=? and Month='${YearMonth}'");
my $qFirstContact =
  qq|select ContLogDate from Treatment where ClientID=? order by ContLogDate|;
...
sub printPDF(\%) {
    ...
    $sClientLegalPP->execute($ClientID)
      || myDBI->dberror("select ClientLegalPP ${ClientID}");
    my $rClientLegalPP = $sClientLegalPP->fetchrow_hashref;
    $sClientReferrals->execute($ClientID)
      || myDBI->dberror("select ClientReferrals ${ClientID}");
    my $rClientReferrals = $sClientReferrals->fetchrow_hashref;
    my $RefAgency        = DBA->getxref( $form, 'xReferralTypes',
        $rClientReferrals->{'ReferredBy1Type'}, 'Descr' );
...
#warn "$ClientID\t$rClient->{LName}\t$rClient->{FName}\n";
    ...
    my (
        $Ind,      $IndRehab, $Grp,    $GrpRehab, $Fam, $TPDevel,
        $TPReview, $Para,     $Assess, $Others,   $TotHrs
    ...
    while ( my $rHours = $sHours->fetchrow_hashref ) {
        my $Duration = DBUtil->getDuration( $rHours->{ContLogBegTime},
            $rHours->{ContLogEndTime} );
...
#warn "$rHours->{TrID}, $rHours->{SCID}, $rHours->{SCNum}, $rHours->{SCName}, $rHours->{Type}, ContLogDate=$rHours->{ContLogDate}, TotHrs=$TotHrs\n";
        ...
        if ( $rHours->{SCNum} eq 'XNOSHOW' ) {
            $NSCnt++;
            $NoShows->{$NSCnt} =
              DBUtil->Date( $rHours->{ContLogDate}, 'fmt', 'MM/DD/YYYY' );
        }
    }

    #warn "1.TotHrs=$TotHrs\n";
    my $FirstContDateAmr =
      DBUtil->Date( ${FirstContDate}, 'fmt', 'MM/DD/YYYY' );
    $Ind      = sprintf( "%.2f", $Ind );
...
##
    ...
    $sClientCARSReview->execute( $rClient->{ClientID} )
      || myDBI->dberror("select ClientCARSReview");
    my $rClientCARSReview = $sClientCARSReview->fetchrow_hashref;
    my $fontsize = main->sizefont( $rClientCARSReview->{ProgEvidence}, 190 );
    $out .= "/R10 ${fontsize} Tf\n";
    $out .=
      $pdf->flowText( 42, 419, 529, 190, $rClientCARSReview->{ProgEvidence},
        $fontsize );
    $fontsize = main->sizefont( $rClientCARSReview->{Issues}, 100 );
    $out .= "/R10 ${fontsize} Tf\n";
    $out .= $pdf->flowText( 42, 189, 529, 100, $rClientCARSReview->{Issues},
        $fontsize );
