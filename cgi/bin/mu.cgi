...
elsif ( $form->{method} eq 'setClientDetails' ) {
    $html = main->setClientDetails($form);
}
...
sub searchClient {
    ...
    my $FOR = qq| and ( LName LIKE "%${pattern}%" or FName LIKE "%${pattern}" )|
      if ( $pattern ne '' );
    ...
    my $opts =
      main->selmatch( $form, $dbh, "select * from Client where ClientID=?",
        $value, 'ClientID', @Display );
    my $q =
qq|select * from Client where ClientID is not null ${FOR} ${DOB} order by LName, FName|;
...
##
    ...
    my $sClient = $dbh->prepare(
        "select * from Client where ${where} order by LName, FName");
    $sClient->execute();
    while ( my $rClient = $sClient->fetchrow_hashref ) {
        ...
        if ( my $rTreatment = $sTreatment->fetchrow_hashref ) {
            $xml .=
              gXML->setXML( $form, $rTreatment->{ProvID}, $rClient->{ClientID},
                $rTreatment->{TrID}, '', '', 'CCDA nonulls' );
        }
...
<FORM NAME="MU" ACTION="/src/cgi/bin/rest.cgi" METHOD="POST" >
