...
# KLS: ||||| remove these next lines to be moved to where the read for $rClientASAM is...
# KLS: |||||  these lines of code where added here because the AdultChild,Type,Level 
# KLS:        can't be passed in. They must be read and since multiple records (IDs)
# KLS:        an be feed to this routine move this and the open_pdi_document to after the read SQL
my ($ID1,$rest) = split(' ',$IDs);
$sClientASAM->execute($ID1) || myDBI->dberror("INITIAL select ClientASAM ${ID1}");
my $rClientASAM1 = $sClientASAM->fetchrow_hashref;
$AdultChild = $rClientASAM1->{'AdultChild'};      # ie: ADULT|CHILD
$Type = $rClientASAM1->{'Type'};                  # ie: DIS|ADM
# Level: 1 or 2.1 or .05 etc...
($Level = $rClientASAM1->{'Level'}) =~ s/\.//g;
$Level = length($Level) == 2 ? $Level : $Level.'0';
my $pdfname = qq|Print${AdultChild}_${Type}_L${Level}.pdf|;
my $pdfpath = qq|$form->{'DOCROOT'}/forms/${pdfname}|;
# KLS: ^^^^ remove above lines of code having made them work after SQL read.
...
# KLS: need to move these lines of code to after the SQL read so we can set
# KLS:  pdfpath from fields in the read record... 
	...
	main->openPdiPages($p);
# KLS: ^^^^^ move the above lines to after the SQL read in printClientASAM sub-routine???
...
# KLS: change from determining name of pdf to use from arguments given
# KLS: to here after read the table record and built the pdfname from
# KLS: fields in the sql record 'AdultChild', 'Type', and 'Level'
# KLS: use this code snippet to assign pdfname/pdfpath...
...
#                       my $pdfname = qq|Print${AdultChild}_${Type}_L${Level}.pdf|;
#                       my $pdfpath = qq|$form->{'DOCROOT'}/forms/${pdfname}|;
# KLS: move open_pdi_document to here (or best way maybe move the 'foreach my $ID' outside....using $pdfpath to open...
