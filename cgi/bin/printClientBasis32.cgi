...
############################################################################
my $qClientBasis32 =
qq|select * from ClientBasis32 left join Client on Client.ClientID=ClientBasis32.ClientID where ClientBasis32.ID=?|;
...
foreach my $ID ( split( ' ', $form->{'IDs'} ) ) {
    ...
    if ( my $rClientBasis32 = $sClientBasis32->fetchrow_hashref ) {
        &genClientBasis32($rClientBasis32);
        $Cnt++;
    }
}
if ( ${Cnt} == 0 ) {
...
##
    my $AgencyID =
      MgrTree->getManager( $form, $rClientBasis32->{clinicClinicID} );
    ...
    $pdf->addElement(
        31,
        655,
"$rClientBasis32->{FName} $rClientBasis32->{LName} ($rClientBasis32->{ClientID})",
        '/R5',
        12
    );
