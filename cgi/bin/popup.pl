...
#warn qq|popup: pAllergy: pattern=${pattern}=, value=${value}=\n|;
  ...
  $q .= qq| or Descr REGEXP "${pattern}" and Active=1 | unless ( $pattern eq '' );
...
elsif ( $form->{method} eq 'xOccupationSnomed' )
{
  my $autocomplete = $form->{'Autocomplete'};
  my $target = $form->{'Target'};
  my ($err,$script,$FLDS) = ('','','Code Description CodeSystem CodeSystemVers CodeSystemOID');
  my $dbh = $form->connectdb('okmis_config');
  ...
  while ( my $rxOccupationSnomed = $sxOccupationSnomed->fetchrow_hashref )
  {
    my ($name,$dlm,$row) = ('','','');
    foreach my $fld ( split(' ',$FLDS) )
    { $row .= qq|$dlm"$rxOccupationSnomed->{$fld}"|; $dlm=','; }
    $script .= qq|[$row],|;
  }
  $script =~ s/\R//g;
  $script = qq|
...
elsif ( $form->{method} eq 'xRelationship' )
{
  my $autocomplete = $form->{'Autocomplete'};
  my $target = $form->{'Target'};
  my ($err,$script,$FLDS) = ('','','Descr ID SNOMEDID HL7OID HL7code');
  my $dbh = $form->connectdb('okmis_config');
  ...
  while ( my $rxRelationship = $sxRelationship->fetchrow_hashref )
  {
    my ($name,$dlm,$row) = ('','','');
    foreach my $fld ( split(' ',$FLDS) )
    { $row .= qq|$dlm"$rxRelationship->{$fld}"|; $dlm=','; }
    $script .= qq|[$row],|;
  }
  $script =~ s/\R//g;
  $script = qq|
...
#warn qq|: pattern=$pattern,value=$value, FINDING=${FINDING}, DISORDER=${DISORDER}\n|;
  my ($selected,$err,$FLDS,$bynum) = ('','','sctName SNOMEDID icdName ICD10 Type',0);
  my $items = ();
  my $found = ();
  my $dbh = $form->connectdb('okmis_config');
  my $with = $value eq '' && $pattern eq '' ? ''
           : $pattern eq '' ? qq|and ID="${value}"|
           : $value eq '' ? qq| and (sctName REGEXP "${pattern}" or icdName REGEXP "${pattern}" or ICD10 REGEXP "${pattern}" or SNOMEDID REGEXP "${pattern}")|
           : qq| and (ID="${value}" or (sctName REGEXP "${pattern}" or icdName REGEXP "${pattern}" or ICD10 REGEXP "${pattern}" or SNOMEDID REGEXP "${pattern}") )|;
  my $q = qq|select * from misICD10 where Active=1 ${with} group by SNOMEDID,ICD10|;
#warn qq|: q=$q\n|;
...
#warn qq|setxTable: ID=$ID, name=$name, match=$match\n|;
  ...
  $smisICD10->finish();
# just uses items->{name} to sort
  my $unSel = PopUp->unMatched($form,$value,$found,'misICD10','sctName SNOMEDID icdName ICD10 Type');
  my $Sel = PopUp->makeSelect($items,$bynum);
  my $SelStmt = $unSel.$Sel;
#warn qq|SelStmt=$SelStmt\n|;
  $out = $err eq '' ? qq|
...
elsif ( $form->{method} eq 'pProblem' )
{
  ...
  my ($err,$cnt) = ('',0);
  my $dbh = $form->connectdb('okmis_config');
  my $s = $dbh->prepare("select sctName,SNOMEDID,icdName,ICD10 from misICD10 where ID=?");
  ...
  my $q = qq|select * from misICD10 where Active=1 and ICD10=? and SNOMEDID=?|;
...
#warn qq|pProblem: ID=$ID, descr=$descr, match=$match\n|;
    $html .= qq|  <INPUT TYPE="radio" NAME="$form->{'name'}" VALUE="${ID}" ${checked} > ${descr}<BR><BR>\n|;
  }
  $smisICD10->finish();
...
elsif ( $form->{method} eq 'ListClientProblems' )
{
  ...
  my $id = $form->{'id'};
  my $addWHERE = $form->{'active'} == 1 ? "and ClientProblems.ResolvedDate is null" : '';
...
#warn qq|popup: addWHERE=${addWHERE}\n|;
  if ( $id )
  {
    ...
    $sUpdate->execute($value,$id) || $form->dberror("popup: update ClientProblems (Priority=${value})");
    $sUpdate->finish();
    PostUpd->renumClientProblems($form,$ClientID);
  }
  my $list = myHTML->ListSel($form,'ListClientProblems',$ClientID,$LINKID,$Locked,'',$addWHERE);
#warn qq|list=$list\n|;
  $out = $err eq '' ? qq|
  <command method="setcontent">
    <target>${target}</target>
...
#warn qq|popup: id=$form->{id}\n|;
  my $Locked = $form->{'Locked'};
  my $list = gHTML->setClientNoteProblems($form,$Locked,$ClientID,$TrID,$ContDate);
#warn qq|list=$list\n|;
  $out = $err eq '' ? qq|
  <command method="setcontent">
    <target>${target}</target>
...
#warn qq|popup: ListClientTrPlanS: DELETE=${value}/$form->{value}, ID=$form->{'sid'}\n|;
    ...
    $sDelete->execute($form->{'sid'}) || $form->dberror("popup: delete ClientTrPlanS ($form->{sid})");
    $sDelete->finish();
  }
...
#warn qq|popup: ListClientTrPlanS: ListTrPlan: TrPlanID=${TrPlanID}\n|;
  my $list = myHTML->ListSel($form,'ListClientTrPlanS',$TrPlanID,$form->{'LINKID'},$Locked,$row);
...
#warn qq|popup: row=${row}, target=${target}\n|;
  ...
  $sUpdate->execute($value,$id) || $form->dberror("popup: update ClientTrPlanPG (Priority=${value})");
  $sUpdate->finish();
  PostUpd->renumClientTrPlanPG($form,$TrPlanID);
  my $Locked = $form->{'Locked'};
  my $list = myHTML->ListSel($form,'ListClientTrPlanPG',$TrPlanID,$form->{'LINKID'},$Locked,$row);
#warn qq|list=$list\n|;
  $out = $err eq '' ? qq|
  <command method="setcontent">
    <target>${target}</target>
...
#warn qq|popup: row=${row}, target=${target}\n|;
  ...
  $sUpdate->execute($value,$id) || $form->dberror("popup: update ClientTrPlanOBJ (Priority=${value})");
  $sUpdate->finish();
  PostUpd->renumClientTrPlanOBJ($form,$TrPlanPGID);
  my $Locked = $form->{'Locked'};
  my $list = myHTML->ListSel($form,'ListClientTrPlanOBJ',$TrPlanPGID,$form->{'LINKID'},$Locked,$row);
#warn qq|list=$list\n|;
  $out = $err eq '' ? qq|
  <command method="setcontent">
    <target>${target}</target>
...
elsif ( $form->{method} eq 'ListClientFamilyProblems' )
{
  ...
  my $id = $form->{'id'};
  my $addWHERE = $form->{'active'} == 1 ? "and ClientFamilyProblems.ResolvedDate is null" : '';
  my $Locked = $form->{'Locked'};
  my $LINKID = $form->{'LINKID'};
#warn qq|popup: ClientID=$form->{Client_ClientID}\n|;
#warn qq|popup: id=$form->{id}/${id}\n|;
#warn qq|popup: addWHERE=${addWHERE}\n|;
  if ( $id )
  {
    ...
    $sUpdate->execute($value,$id) || $form->dberror("popup: update ClientFamilyProblems (Priority=${value})");
    $sUpdate->finish();
    PostUpd->renumClientFamilyProblems($form,$ClientFamilyID);
  }
  my $list = myHTML->ListSel($form,'ListClientFamilyProblems',$ClientFamilyID,$LINKID,$Locked,'',$addWHERE);
#warn qq|list=$list\n|;
  $out = $err eq '' ? qq|
  <command method="setcontent">
    <target>${target}</target>
...
#warn qq|popup: id=$form->{id}/${id}\n|;
#warn qq|popup: addWHERE=${addWHERE}\n|;
  my $list = myHTML->ListSel($form,'ListClientPDMeds',$ClientID,$LINKID,$Locked,'',$addWHERE);
#warn qq|list=$list\n|;
  $out = $err eq '' ? qq|
  <command method="setcontent">
    <target>${target}</target>
...
#warn qq|popup: id=$form->{id}/${id}\n|;
#warn qq|popup: addWHERE=${addWHERE}\n|;
  my $list = myHTML->ListSel($form,'ShowClientMeds',$ClientID,$LINKID,$Locked,'',$addWHERE);
#warn qq|list=$list\n|;
  $out = $err eq '' ? qq|
  <command method="setcontent">
    <target>${target}</target>
...
#warn qq|popup: id=$form->{id}/${id}\n|;
#warn qq|popup: addWHERE=${addWHERE}\n|;
  my $list = myHTML->ListSel($form,'ListClientAdMeds',$ClientID,$LINKID,$Locked,'',$addWHERE);
#warn qq|list=$list\n|;
  $out = $err eq '' ? qq|
  <command method="setcontent">
    <target>${target}</target>
...
elsif ( $form->{method} eq 'ListClientAllergies' )
{
  my $ClientID = $form->{'Client_ClientID'};
  my $id = $form->{'id'};
  my $addWHERE = $form->{'active'} == 1 ? "and ClientAllergies.EndDate is null" : '';
  my $Locked = $form->{'Locked'};
  my $LINKID = $form->{'LINKID'};
#warn qq|popup: ClientID=$form->{Client_ClientID}\n|;
#warn qq|popup: id=$form->{id}/${id}\n|;
#warn qq|popup: addWHERE=${addWHERE}\n|;
  my $list = myHTML->ListSel($form,'ListClientAllergies',$ClientID,$LINKID,$Locked,'',$addWHERE);
#warn qq|list=$list\n|;
  $out = $err eq '' ? qq|
  <command method="setcontent">
    <target>${target}</target>
...
#warn qq|popup: ClientID=$form->{Client_ClientID}\n|;
#warn qq|popup: id=$form->{id}/${id}\n|;
  my $list = myHTML->set1CheckBoxColumn($form,"select * from Client where ProvID='$id' order by LName,FName",'ClientIDs','ClientID','LName~Last Name~strcol:FName~First Name~strcol:DOB~~hdrcol','Select Clients');
  $out = $err eq '' ? qq|
  <command method="setcontent">
    <target>${target}</target>
    <content><![CDATA[${list}]]></content>
...
elsif ( $form->{method} eq 'Medications' )
{
  ...
  my $opts = main->selmatch($form,$cdbh,"select * from xMedNames where ID=?",$value,'ID',@Display);
  $opts .= main->seloptions($form,$cdbh,"select * from xMedNames where TradeName ${For} order by TradeName",$pattern,'ID',@Display);
...
elsif ( $form->{method} eq 'Physicians' )
{
  (my $pattern = $form->{'pattern'}) =~ s/"//g;
  my $For = qq| like "%${pattern}%" |;
  @Display = ('ProvLastName',',','ProvFirstName',',','Addr1',',','City',',','ST','[','NPI',']');
  my $opts = main->selmatch($form,$cdbh,"select * from xNPI where NPI=?",$value,'NPI',@Display);
  my $q = $pattern eq '*'
...
elsif ( $form->{method} eq 'Axis1' )
{
  ...
  my $For = qq| like "%${pattern}%" |;
  @Display = ('ICD9',':','Descr');
  my $opts = main->selmatch($form,$cdbh,"select * from xAxis1 where ID=?",$value,'ID',@Display);
  my $q=qq|select * from xAxis1 where Active=1 and (ICD9 ${For} or Descr ${For}) order by ICD9,Descr|;
...
#warn qq|Handicap=$Handicap\n|;
  @Display = ('ConceptName');
  my $opts = main->selmatch($form,$cdbh,"select * from xFunctionalStatus where ID=?",$value,'ID',@Display);
  my $q=qq|select * from xFunctionalStatus where Active=1 and Handicap='${Handicap}' order by ConceptName|;
...
#warn qq|popup: id=$form->{id}/${id}\n|;
  if ( $id )
  {
    ...
    $sUpdate->execute($value,$id) || $form->dberror("popup: update ProviderCDAparms (Priority=${value})");
    $sUpdate->finish();
    PostUpd->renumProviderCDAparms($form,$ProvID);
  }
  my $list = myHTML->ListSel($form,'ListProviderCDAparms',$ProvID,$LINKID,$Locked);
  $out = $err eq '' ? qq|
  <command method="setcontent">
    <target>${target}</target>
    <content><![CDATA[${list}]]></content>
...
#warn qq|popup: value=${value}\n|;
  ...
  {
    my $dbh = $form->dbconnect();
    my $sUpdate = $dbh->prepare("update ProviderCDAparms set Visible=? where ID=?");
    my $toggle = $value eq 'off' ? 0 : 1;
    $sUpdate->execute($toggle,$id) || $form->dberror("popup: update ProviderCDAparms (Visible=${value}/${toggle})");
    $sUpdate->finish();
  }
  my $list = myHTML->ListSel($form,'ListProviderCDAparms',$ProvID,$LINKID,$Locked);
  $out = $err eq '' ? qq|
...
elsif ( $form->{method} eq 'Procedure' )
{
  ...
  my $opts = main->selmatch($form,$cdbh,"select * from xProcedures where ConceptCode=?",$value,'ConceptCode',@Display);
  my $q = $pattern eq '*'
  ...
  $opts .= main->seloptions($form,$cdbh,$q,$pattern,'ConceptCode',@Display);
  my $list = qq|<SELECT NAME="$form->{'name'}" ${size} ${multiple} >\n${opts}</SELECT>\n|;
  $out = $err eq '' ? qq|
  <command method="setcontent">
    <target>${target}</target>
