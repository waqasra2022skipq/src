...
my $labelfont = qq|<nounderline fontname={Times New Roman} alignment=left encoding=unicode>|;
my $responsefont = qq|<nounderline fontname={Arial} alignment=left encoding=unicode>|;
my $hdrfont = qq|<underline fontname={Times New Roman} alignment=center encoding=unicode>|;
...
print qq|add ID: |.$TrID."\n";
    ...
    my $dbilled = DBUtil->Date($rTreatment->{'BillDate'},'fmt','MM/DD/YYYY');
    ...
    for ($pageno = 1; $pageno <= $no_of_input_pages; $pageno++)
    {
      ...
      my $grpsize = $rNote->{'GrpSize'} >= 1
                    ...
                    : qq|\n|;
      my $str = qq|Name: ${responsefont}$rClient->{'LName'}, $rClient->{'FName'} (${ClientID})|;
      $str   .= qq|               ${labelfont}Group #: ${responsefont}${groupno}\n|;
      $str   .= qq|${labelfont}Type Of Service: ${responsefont}$rxSC->{SCName} ${grpsize}|;
      $str   .= qq|${labelfont}Date Of Service: ${responsefont}${dos}|;
      $str   .= qq|               ${labelfont}Start Time: ${responsefont}${stime}|;
      $str   .= qq|               ${labelfont}End Time: ${responsefont}${etime}|;
      $str   .= qq|               ${labelfont}Total Minutes: ${responsefont}${DurationInMinutes}|;
      if ($p->fill_textblock($pagehandles[$pageno], 'noteinfo', $str, $optlist) == -1)
      { printf("Warning: %s\n", $p->get_errmsg()); }
...
${labelfont}Describe Below:\n${responsefont}|.$rNote->{'NewProblems'};
        ...
        if ( $rxSC->{'Type'} eq 'CI' )
        {
          ...
          $notetext .= qq|LEVEL OF FUNCTIONING ASSESSMENT GAF: CURRENT: ${cGAF} ${labelfont}RECENT PAST (30 DAYS): ${pGAF}
|.$rNote->{'CrisisText'};
        }
      }
      ...
      { printf("Warning: %s\n", $p->get_errmsg()); }
      my $completedby = "Electronically Signed By: ".DBA->setProvCreds($form,$rTreatment->{'ProvID'},$rxSC->{InsID},$rTreatment->{'ProvOKDate'},$rTreatment->{'ProvOKTime'});
      ...
      my $approvedby = "                   Approved By: ".DBA->setProvCreds($form,$rTreatment->{'MgrProvID'},$rxSC->{InsID},$rTreatment->{'MgrRevDate'},$rTreatment->{'MgrRevTime'});
      my $enteredby = "                      Entered By: ".DBA->setProvCreds($form,$rTreatment->{'EnteredBy'},$rxSC->{InsID},$rTreatment->{'ChartEntryDate'});
      $str = $completedby."\n".$approvedby."\n".$enteredby."\n";
...
# make 0 null, to set starting at 1...
  ...
  my $fontsize = 10;
  my $basefontoptions = "fontname=TimesNewRomanPSMT fontsize=" . $fontsize . " embedding encoding=unicode";
  my $optlist;
  my $x_table = 32;
  my $tablewidth = 545;
...
#print qq|buf=${buf}, row=${row}, col=${col}\n|;
    $optlist = "fittextline={position={" . $alignments[$col] . 
                " center} " . $basefontoptions . "} colwidth=60% margin=2";
    $tbl = $p->add_table_cell($tbl, $col, $row, $buf, $optlist);
...
#print qq|buf=${buf}, row=${row}, col=${col}\n|;
    $optlist = "fittextline={position={" . $alignments[$col] . 
                " center} " . $basefontoptions . "} colwidth=15% margin=2";
    $tbl = $p->add_table_cell($tbl, $col, $row, $buf, $optlist);
...
sub getPG
{
  ...
  my $NoteType = $rTreatment->{'Type'};
  my $out = qq|SPECIFIC PROBLEM(S) ADDRESSED (As Identified On Comprehensive Treatment Plan):
${responsefont}|;
...
sub getParticipants
{
  ...
  my $Participants = '';
  my $row = 0;
  $sClientNoteFamilyI->execute($TrID) || $form->dberror("ClientMU: select ClientNoteFamilyI ${TrID}");
  ...
  $sClientNoteFamilyP->execute($TrID) || $form->dberror("ClientMU: select ClientNoteFamilyP ${TrID}");
...
#$cnt = 1 unless($cnt);
  my $out = qq|${labelfont}Date\tHeight\tWeight\tWaist\tTemp\tBP\tPulse\tOximetry\tBloodSugar\tRespiration\n| if ( $cnt>0 );
  my $row = 0;
  while ( my $r = $sClientVitalSigns->fetchrow_hashref )
  {
    ...
    $out .= qq|${responsefont}$r->{VDate}\t${Height}\t${Weight}\t$r->{Waist}\t${Temp}\t${BP}\t$r->{Pulse}\t$r->{Oximetry}\t$r->{BloodSugar}\t$r->{Respiration}\n|;
...
sub getMeds
{
  my ($self,$p,$rTreatment) = @_;
  my $TrID = $rTreatment->{'TrID'};
  $sClientNoteMeds->execute($TrID) || $form->dberror("PrintNotes: select ClientNoteMeds");
