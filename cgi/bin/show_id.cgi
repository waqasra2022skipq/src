...
if ( $table eq 'ClientPrAuth' || $table eq 'ClientDischarge' )
{
  ...
  $s2->finish();
  $s3 = $dbh->prepare("select * from ${table}CDCSent where ${table}CDCID=? and xmlstring is not null order by ChangeDate desc");
  $s3->execute($r2->{ID});
  if ( $r3 = $s3->fetchrow_hashref )
  {
    ...
    $url = qq|<TR><TD>xmlfile: <A HREF="javascript:ReportWindow('${printfile}','PrintWindow')" ONMOUSEOVER="textMsg.show('print')" ONMOUSEOUT="textMsg.hide()" ><IMG ALT="print" SRC="/images/icon_print.gif" BORDER="0" ></A></TD></TR>|;
...
my $html = myHTML->newHTML($form,'Show Record','CheckPopupWindow noclock countdown_1') . qq|
<TABLE CLASS="main" >
