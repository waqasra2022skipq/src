...
if ( $form->{'ClientID'} eq '' )
{ myDBI->error("Chanage SSN / NO ID!"); }

my $ClientID = $form->{'ClientID'};
my $sClient = $dbh->prepare("select * from Client where ClientID=?");
$sClient->execute($ClientID) || myDBI->dberror("chgSSN: select Client ${ClientID}");
...
warn qq|validSSN: READ: ssn=${ssn}\n|;
    ...
    $sClient->execute() || myDBI->dberror("validate: SSN: select Client (${ssn})");
    my $rClient = $sClient->fetchrow_hashref;
...
# Start out the display.
  my $html = myHTML->newHTML($form,'Check SSN','CheckPopupWindow noclock countdown_2') . qq|
<TABLE CLASS="main" >
...
#foreach my $f ( sort keys %{$rUpdate} ) { warn "chgSSN 1: rUpdate-$f=$rUpdate->{$f}\n"; }
    my $ID1 = DBA->doUpdate($form,'Client',$rUpdate,"ClientID='${ClientID}'");
  }
