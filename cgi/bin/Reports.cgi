...
{
  my $sDelete = $dbh->prepare("delete from ProviderRpts where ProvID=$form->{Provider_ProvID}");
  $sDelete->execute() || $form->dberror("delete ProviderRpts");
  $sDelete->finish();
#warn qq|DELETE ProviderRpts: ProvID=$form->{Provider_ProvID}\n|;
  my $sInsert = $dbh->prepare("INSERT INTO ProviderRpts (ProvID,Name,CreateProvID,CreateDate,ChangeProvID) VALUES (?,?,?,?,?)");

  my $ReportNames;
  my $qxReports = qq|select * from xReports where Cron=1 and ExpDate is null order by Descr |;
  ...
  while ( my $rxReports = $sxReports->fetchrow_hashref )
  {
    ...
    {
      $sInsert->execute($form->{Provider_ProvID},$rxReports->{Name},$form->{LOGINPROVID},$form->{TODAY},$form->{LOGINPROVID}) || $form->dberror("insert ProviderRpts");
...
# send notice to support to add reports to cron.
  my $Subject = qq|Provider Reports: $form->{Provider_FName_1} $form->{Provider_LName_1}|;
...
$sProvider->execute($form->{Provider_ProvID}) || $form->dberror("Provider in ProviderRpts");
my $rProvider = $sProvider->fetchrow_hashref;

my $qProviderRpts = qq|select * from ProviderRpts where ProvID=? and Name=?|;
my $sProviderRpts = $dbh->prepare($qProviderRpts);
my $Defns;
my $qxReports = qq|select * from xReports where Cron=1 and Script is not null and ExpDate is null order by Category|;
#warn "\nqxReports=\n$qxReports\n";
my $sxReports = $cdbh->prepare($qxReports);
$sxReports->execute || $form->dberror($qxReports);
while ( my $rxReports = $sxReports->fetchrow_hashref ) 
{ 
  ...
  $Defns .= qq|<SCRIPT LANGUAGE="JavaScript">newtextMsg('help$rxReports->{Name}',${text});</SCRIPT>\n|;
}
my @Categories = ();
my $qxReports = qq|select Category from xReports where Cron=1 and ExpDate is null group by Category|;
#warn "\nqxReports=\n$qxReports\n";
my $sxReports = $cdbh->prepare($qxReports);
$sxReports->execute() || $form->dberror($qxReports);
while ( my $rxReports = $sxReports->fetchrow_hashref ) 
...
my $qxReports = qq|select * from xReports where Cron=1 and Category=? and ExpDate is null order by Descr |;
...
#warn qq|CHECK: $rxReports->{Name}, $rxReports->{Priv}\n|;
      ...
      { $Checked='CHECKED'; $Value=1; $Text='Yes'; } else { $Checked=''; $Value=0; $Text='No'; }
      $Help = qq|<A HREF="javascript:void(0)" ONMOUSEOVER="textMsg.show('help$rxReports->{Name}')" ONMOUSEOUT="textMsg.hide()" ><IMG WIDTH=15 HEIGHT=15 BORDER=0 SRC="/images/qm1.gif"></A>|;
    if ( SysAccess->verify($form,"Privilege=$rxReports->{Priv}") )
    {
      $html .=  qq|  <LI><INPUT TYPE="checkbox" NAME="$rxReports->{Name}" VALUE="1" ${Checked} >$rxReports->{Descr} ${Help}</LI>|;
    }
    else
    {
      $html .=  qq|  <LI><INPUT TYPE="hidden" NAME="$rxReports->{Name}" VALUE="${Value}" >[${Text}] $rxReports->{Descr} ${Help}</LI>|;
    }
  }
  $html .=  qq|</UL>
    </TD>
