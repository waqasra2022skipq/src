...
if ( $form->{method} eq 'vInsID' )
{
  ...
  { $InsID = $form->{'i'}; $DesigProvID = $value; }
...
#warn qq|validate: Descr=$rxInsurance->{'Descr'}\n|;
  ...
  else
  {
    ...
    {
      $warn = qq|Used only for BCBS,Medicare/Railroad,TriCare and other Insurances!|;
...
{
  my $target = $form->{'target'};
  (my $InsNum = $form->{'value'}) =~ s/^\s*(.*?)\s*$/$1/g;   # trim both leading/trailing
  my $InsID = $form->{'i'};
#warn qq|validate: InsNum: InsID=$InsID, InsNum=$InsNum\n|;
  my $dbh = $form->dbconnect();
  my $sxInsurance = $dbh->prepare("select * from xInsurance where ID='${InsID}'");
  $sxInsurance->execute() || $form->dberror("validate: InsNum: select xInsurance (${InsID})");
  my $rxInsurance = $sxInsurance->fetchrow_hashref;
#warn qq|validate: Descr=$rxInsurance->{'Descr'}\n|;
  ...
  elsif ( $rxInsurance->{'Descr'} =~ /medicare/i && ($InsNum !~ /\b[1-9][AC-HJKMNP-RT-Yac-hjkmnp-rt-y][AC-HJKMNP-RT-Yac-hjkmnp-rt-y0-9][0-9]-?[AC-HJKMNP-RT-Yac-hjkmnp-rt-y][AC-HJKMNP-RT-Yac-hjkmnp-rt-y0-9][0-9]-?[AC-HJKMNP-RT-Yac-hjkmnp-rt-y]{2}\d{2}\b/) )
  { $err = qq|Insurance Number NOT IN Medicare Beneficiary Identifier (MBI) Format!|; }
  $sxInsurance->finish();
...
elsif ( $form->{method} eq 'calcPG' )
{
  ...
  {
    $PAgroup = $target eq 'TransType' && $value == 21 ? 'PG038'
...
elsif ( $form->{method} eq 'HL7Report' )
{
  my $target = $form->{'target'};
  my $value = $form->{'value'};
  my $rpt = qq|Tag\tConceptCode\tConceptName\tValueSetOID\tCodeSystemOID\tp\n|;
...
#warn qq|r: $r->{ConceptCode}\n|;
    $rpt .= qq|$r->{Tag}\t$r->{ConceptCode}\t$r->{ConceptName}\t$r->{ValueSetOID}\t$r->{CodeSystemOID}\t$r->{Popup}\n|;
  }
  $sxHL7->finish();
  $dbh->disconnect();
#warn qq|rpt: $rpt\n|;
