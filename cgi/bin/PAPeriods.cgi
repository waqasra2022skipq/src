...
my $qPrAuth=qq|select ClientPrAuth.*,Client.FName,Client.LName,Client.DOB,Client.ProvID,Client.clinicClinicID,Insurance.InsID from ClientPrAuth left join Client on Client.ClientID=ClientPrAuth.ClientID left join Insurance on Insurance.InsNumID=ClientPrAuth.InsuranceID where ID=?|;
...
if ( my $rPrAuth = $sPrAuth->fetchrow_hashref )
{
  ...
  my $InsDescr = DBA->getxref($form,'xInsurance',$rPrAuth->{InsID},'Name');
  my $inv=Inv->InvPA($form,$rPrAuth);
  $inv=$inv->InvNotes($form,$rPrAuth->{ClientID},$rPrAuth->{InsID});
  my ($cnt,$out,$hdrline) = (0,"$rPrAuth->{FName} $rPrAuth->{LName} ($rPrAuth->{ClientID}) - ${InsDescr}\n",1);
  foreach my $Dates ( @{$inv->{PADates}} )
  {
    ...
    if ( $cnt == 1 )
    {
      if ( $inv->{UnitsAuth} > 0 )
      {
        ...
        $hdrline++;
        $out .= qq|StartDate\tEndDate\tAuth\tUsed\tLeft\tPctLeft\tType\tServiceCode\tTrID\tContactDate\tUnits\tAmount\n|;
        $hdrline++;
        foreach $SCID ( keys %{ $inv->{PASCID} } )
        {
          my $UnitsAuth = sprintf("%.2f",$inv->{$SCID}->{UnitsAuth});
          my $UnitsUsed = sprintf("%.2f",$inv->{$SCID}->{UnitsUsed});
          my $UnitsLeft = sprintf("%.2f",$inv->{$SCID}->{UnitsAuth} - $inv->{$SCID}->{UnitsUsed});
          my $PctUnits = $UnitsAuth > 0 ?  sprintf("%.1f",(($UnitsLeft / $UnitsAuth) * 100)) : '0.0';
          $out .= qq|$inv->{fPAEffDate}\t$inv->{fPAExpDate}\t${UnitsAuth}\t${UnitsUsed}\t${UnitsLeft}\t${PctUnits}\tAUTH\t$inv->{PASCID}->{$SCID}\t \t \t \t \n|;
        }
      }
      else
      {
...
{ $form->error("ClientPrAuth Access Page / No PrAuth ID"); }
...
#foreach my $f ( sort keys %{$inv->{$CurPeriod}} ) { warn "  main: inv-${CurPeriod}-${f} is $inv->{$CurPeriod}->{$f}\n"; }
    ...
    $PctUnits = $UnitsAuth > 0 ?  sprintf("%.1f",(($UnitsLeft / $UnitsAuth) * 100)) : '0.0';
    ...
    foreach my $SCID ( sort keys %{ $inv->{$CurPeriod}->{INPA} } )
    {
      ...
      foreach my $TrID ( sort keys %{ $inv->{$CurPeriod}->{$SCID} } )
      {
        ...
        $out .= qq|${tabs}${TrID}\t$inv->{$CurPeriod}->{$SCID}->{$TrID}->{ContDate}\t$inv->{$CurPeriod}->{$SCID}->{$TrID}->{Units}\t|;
        $out .= $BillingReportFlag ? qq|$inv->{$CurPeriod}->{$SCID}->{$TrID}->{Amt}\t| : " \t";
        $out .= qq|\n|;
        $tabs = "\t\t\t\t\t\t\t\t";
      }
    }
