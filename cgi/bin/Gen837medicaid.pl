...
my $pwd=cwd();
print qq|\n--------------------------------------\n|;
print qq|billdir: $BILLDIR billdate: $billDate database: ${dbname} pwd=${pwd}\n|;

my $sInsContracts = $dbh->prepare("select xInsurance.Descr from Contracts left join xInsurance on xInsurance.ID=Contracts.InsID where Contracts.BillFlag=1 and Contracts.BillType='EL' and xInsurance.Descr='medicaid'  group by xInsurance.Descr");
$sInsContracts->execute();
while ( my $rInsContracts = $sInsContracts->fetchrow_hashref )
{
  print qq|\n    generate $rInsContracts->{Descr} '837'...\n|;
  system("${BIN}/gen837 DBNAME=${dbname}\\&InsDescr=$rInsContracts->{Descr}\\&BillDate=${billDate}\\&mlt=$form->{mlt}");
}
