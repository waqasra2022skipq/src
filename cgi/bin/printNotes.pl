...
my $responsefont = qq|<nounderline leftindent 0 fontname={Arial} fontsize=9 alignment=justify encoding=unicode>|;
...
${responsefont}|;
  ...
  while ( my $rClientNoteTrPlanPG = $sClientNoteTrPlanPG->fetchrow_hashref )
  {
    ...
    while ( my $rClientTrPlanPG = $sClientTrPlanPG->fetchrow_hashref )
    {
      ...
      $out .= "${labelfont}#".$Priority.": Problem: ${responsefont} <leftindent=22>".${PROB}."\n";
      $out .= "${labelfont}${leftindent1}Goal: ${responsefont} <leftindent=30>".${GOAL}."\n";
      my $cnt = 0;
...
#warn qq|printNotes: ClientNoteTrPlanOBJ: ID=$rClientTrPlanOBJ->{ID}\n|;
        $out .= "${labelfont}${leftindent2}Objective ${Priority}${ltr}: ${responsefont} <leftindent=50>".${OBJ}."\n";
      }
    }
  }
  return($out);
...
sub getVitals
{
  ...
  my $optlist =
        ...
        "hortabmethod=ruler>";
  my $out = qq|${labelfont}${optlist}Date\tHeight\tWeight\tWaist\tTemp\tBP\tPulse\tOximetry\tBloodSugar\tRespiration\n| if ( $cnt>0 );
  my $row = 0;
...
sub getMeds
{
  ...
  my $optlist =
        ...
        "hortabmethod=ruler>";
  my $out = qq|${labelfont}${optlist}DrugID\tFrequency\t Route\t Schedule\t PrescriptionDate\t Refills\tDrugInfo\n| if ( $cnt>0 );
  my $row = 0;
  ...
  while ( my $rClientNoteMeds = $sClientNoteMeds->fetchrow_hashref )
  {
    ...
    $out .= qq|${responsefont}$rClientNoteMeds->{DrugID}\t$rClientNoteMeds->{DosageFrequencyDescription}\t $rClientNoteMeds->{Route}\t $rClientNoteMeds->{ExternalDrugSchedule}\t $pdate\t $rClientNoteMeds->{Refills}\t<leftindent=370>$rClientNoteMeds->{DrugInfo}\n|;
  }
  return("$out");
}
