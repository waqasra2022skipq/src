...
my $sProviderPrefs =
  $dbh->prepare("select * from ProviderPrefs where ProvID=?");
...
else { $html .= main->genTreeFlat( $form, $form->{LOGINUSERID} ); }
$html .= qq|
    </TD>
  </TR>
</TABLE>
...
|;
    ...
    $out .= $mtree;
    $out .= qq|  </TABLE>\n|;
    $out .= gHTML->misSiteMsg($form) . qq|
    </TD>
    <TD CLASS="home subtitle" >
|
      . gHTML->misGuide($form) . gHTML->misFeatures($form) . qq|
|;
    $sManagerTree->finish();
    return ($out);
}
    ...
    while ( my $r = $sManagerTree->fetchrow_hashref ) {
        ...
        if ( $AgencyCnt > 1 )    # roll up Agencys.
        {
            if ( $r->{Type} == 2 )    # new Agency.
            {
                ...
                my $Initials = '';
                $Initials .= uc($1) while $Name =~ /(\w)\w+\s?/g;
...
sub genHTML {
    ...
    $sUnreviewed->execute( $r->{ListProvID} )
      || myDBI->dberror("select Unreviewed where ProvID=$r->{ListProvID}");
    if ( $rUnreviewed = $sUnreviewed->fetchrow_hashref ) {
        $UnRevCnt  = $rUnreviewed->{Count};
        $UnBillCnt = $rUnreviewed->{Unbilled};
    }
    my $UnRev =
      $UnRevCnt
...
|;
    ...
    if ( $rProviderPrefs->{'ListClients'} ) {
        my $sClientAccess = $dbh->prepare(
"select distinct ClientAccess.ClientID,Client.LName,FName from ClientAccess left join Client on Client.ClientID=ClientAccess.ClientID where (ClientAccess.ProvID=? or Client.ProvID=?) and Client.Active=1 order by Client.LName, Client.FName"
        );
