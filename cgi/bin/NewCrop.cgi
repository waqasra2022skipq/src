...
# Specify the URL...
my $UseProduction = $form->{'DBNAME'} eq 'okmis_demo' || $form->{'DBNAME'} eq 'okmis_dev'
                 || $form->{'LOGINPROVID'} == 2501 || $form->{'LOGINPROVID'} == 2502 || $form->{'LOGINPROVID'} == 2503
                  ? 0 : 1;
my $demo = $UseProduction ? '' : 'demo';
my $webpage = $UseProduction 
...
#warn qq|setRxInput: PatientID=${PatientID}, ClientID=${ClientID}\n|;
  ...
  my $height = ($rClientVitalSigns->{'HeightFeet'} * 12) + $rClientVitalSigns->{'HeightInches'};
  ...
  my $Prescriber = NewCrop->isPrescriber($form)
                 ? NewCrop->Prescriber($form)
                 : NewCrop->Staff($form);
  my $SupervisingPhysician = NewCrop->provider('code') eq 'M'
...
| . NewCrop->Credentials($demo) . NewCrop->UserRole($form) . qq|
  ...
  <Account ID="|.NewCrop->agency('AccountID').qq|">
    ...
    <accountPrimaryPhoneNumber>|.NewCrop->agency('accountPrimaryPhoneNumber').qq|</accountPrimaryPhoneNumber>
    <accountPrimaryFaxNumber>|.NewCrop->agency('accountPrimaryFaxNumber').qq|</accountPrimaryFaxNumber>
  </Account>
  <Location ID="|.NewCrop->clinic('LocationID').qq|">
    ...
    <primaryPhoneNumber>|.NewCrop->clinic('primaryPhoneNumber').qq|</primaryPhoneNumber>
    <primaryFaxNumber>|.NewCrop->clinic('primaryFaxNumber').qq|</primaryFaxNumber>
    <pharmacyContactNumber>|.NewCrop->clinic('pharmacyContactNumber').qq|</pharmacyContactNumber>
...
sub checkForErrors
{
  ...
  if ( NewCrop->agency('ST') eq '' )
  { $err .= qq|>>>Agency ST MISSING!\n|; }
  ...
  if ( NewCrop->clinic('ST') eq '' )
  { $err .= qq|>>>Clinic ST MISSING!\n|; }
  ...
  if ( $rClient->{'City'} eq '' )
  { $err .= qq|>>>Patient City MISSING!\n|; }
  if ( $rClient->{'ST'} eq '' )
  { $err .= qq|>>>Patient ST MISSING!\n|; }
  if ( ${Zip} eq '' )
  { $err .= qq|>>>Patient Zip MISSING!\n|; }
  if ( ${HomePhone} eq '' )
  { $err .= qq|>>>Patient HomePhone MISSING!\n|; }
  if ( ${DateOfBirth} eq '' )
  { $err .= qq|>>>Patient DateOfBirth MISSING!\n|; }
  if ( $rClient->{'Gend'} eq '' )
  { $err .= qq|>>>Patient Gender MISSING!\n|; }
  if ( $weight eq '' )
  { $err .= qq|>>>Patient Weight MISSING!\n|; }
...
sub ptGuarantor
{
  ...
  $sGuarantor->execute($ClientID) || myDBI->dberror("select Guarantor ${ClientID}");
...
sub ptInsurance
{
  ...
  my $xml = '';
  my $sInsurance = $dbh->prepare("select * from Insurance where ClientID=? and Priority=? and InsNumEffDate<=curdate() and (curdate()<=InsNumExpDate or InsNumExpDate is NULL)");
  $sInsurance->execute($ClientID,$Priority) || myDBI->dberror("select Insurance ${ClientID}");
  if ( my $rInsurance = $sInsurance->fetchrow_hashref )
  {
    my $sxInsurance = $dbh->prepare("select * from xInsurance where ID=?");
    $sxInsurance->execute($rInsurance->{InsID}) || myDBI->dberror("select xInsurance $rInsurance->{InsID}");
    my $rxInsurance = $sxInsurance->fetchrow_hashref;
    my ($pzip,$pzip4) = split('-',$rxInsurance->{'Zip'});
    my $InsProv = main->getinsuranceServiceProvider($rxInsurance->{'Name'});
...
sub ptAllergies
{
  ...
  $sClientAllergies->execute($ClientID) || myDBI->dberror("select ClientAllergies ${ClientID}");
...
sub ptMedications
{
  ...
  my $ClientID = $rClient->{'ClientID'};
  my $xml = '';
  my $sClientMeds = $dbh->prepare("select * from ClientMeds where ClientID=?");
  $sClientMeds->execute($ClientID) || myDBI->dberror("select ClientMeds ${ClientID}");
...
#  return();
  ...
  my $sPDMed = $dbh->prepare("select * from PDMed where ClientID=? and MedActive=1 and Locked=0");
  ...
  while ( my $rPDMed = $sPDMed->fetchrow_hashref )
  {
    ...
    my $Freq = DBA->getxref($form,'xMedFreq',$rPDMed->{MedFreq},'Text');
    my $rPhys = DBA->selxref($form,'xNPI','NPI',$rPDMed->{PhysNPI});
    my $doctorName = $rPhys->{'ProvPrefix'}
                    .' '.$rPhys->{'ProvLastName'}
...
|;
    ...
    $sLock->execute($rPDMed->{PDMedID}) || myDBI->dberror("NewCrop: Lock PDMed:$rPDMed->{PDMedID}");
    $sLock->finish();
...
sub getinsuranceServiceProvider
{
  ...
  my $ins = $insurancename =~ /medicaid/ ? 'Medicaid'
