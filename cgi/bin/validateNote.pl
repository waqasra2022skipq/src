...
#warn qq|vNote: ContDate=$ContDate,min=$min,max=$max\n|;
  ...
  unless ( $ContDate eq '' )
  {
    $msg .= main->DUP('ProvID',$ProvID,$TrID,$ContDate,$btime,$InsID);
    $msg .= main->DUP('ClientID',$ClientID,$TrID,$ContDate,$btime,$InsID);
    $msg .= main->DUP('ProvID',$ProvID,$TrID,$ContDate,$etime,$InsID);
    $msg .= main->DUP('ClientID',$ClientID,$TrID,$ContDate,$etime,$InsID);
# check duplicate note for Client/ContDate/ServiceCode
    my ($chkcode,$chkmsg) = cBill->CheckClientDUP($form,$ClientID,$ProvID,$TrID,$SCID,$ContDate);
#warn qq|vNote: chkcode=$chkcode, chkmsg=$chkmsg\n|;
    $msg .= qq|!!! WARNING !!!\n|.$chkmsg if ( $chkcode);
    $msg .= main->INPA($ProvID,$ClientID,$InsID,$TrID,$ContDate,$SCID);
...
#warn qq|vProvID: Mod4=$Mod4,ContDate=$ContDate\n|;
  ...
  my $Mod4Sel = SysAccess->chkPriv($form,'TeleMedicine',$ProvID) ?
...
#warn qq|ProvID=$ProvID,ClientID=$ClientID,btime=$btime,etime=$etime\n|;
    ...
    ($err,$ContDate) = DBUtil->vDate($value,$form->{'TODAY'},$min,$max);
  }
...
#warn qq|vContDate: chkcode=$chkcode, chkmsg=$chkmsg\n|;
  ...
  my $plist = gHTML->setClientNoteProblems($form,0,$ClientID,$TrID,$ContDate);
  my $tlist = gHTML->setClientNoteTrPlanPG($form,0,$ClientID,$TrID,$ContDate);
#warn qq|list=$list\n|;
#warn qq|msg=$msg\n|;
  my $out = $err eq '' ? qq|
  <command method="setvalue">
...
#warn qq|vContTime: target=$target,value=$value\n|;
  ...
  my $timeid = $target eq 'BeginTime' ? 'Treatment_ContLogBegTime_1'
...
#warn qq|err=$err\n|;
    ...
    if ( $target eq 'EndTime' )
    { ($btime,$etime) = ($itime,$ctime); }
#warn qq|ProvID=$ProvID,ClientID=$ClientID,btime=$btime,etime=$etime\n|;
    $msg .= main->DUP('ProvID',$ProvID,$TrID,$ContDate,$btime,$InsID);
    $msg .= main->DUP('ClientID',$ClientID,$TrID,$ContDate,$btime,$InsID);
    $msg .= main->DUP('ProvID',$ProvID,$TrID,$ContDate,$etime,$InsID);
    ...
    if ( $btime ne '' && $etime ne '' )
    {
      ...
      { $msg .= "!!! WARNING !!!\nTreatment duration is LESS THAN 15 minutes.\n"; }
      elsif ( $ehour-$bhour > 8 || ($ehour-$bhour == 8 && $eminute > $bminute) )
...
#warn qq|INPA: NON-BILLABLE: SCID=$SCID,SCNum=$SCNum\n|;
  ...
  my ($found,$PRAUTHNUM,$PRAUTHDAYS,$INSDESCR,$AXIS1ACODE,$TRPLANID) = (0,'',0,'','','');
  my $dbh = $form->dbconnect();
  my $sPrAuthRVU = $dbh->prepare("select SCID from PrAuthRVU where PrAuthID=?");
  my $sClientPrAuth = $dbh->prepare("select ClientPrAuth.ID,ClientPrAuth.PAgroup,ClientPrAuth.PAnumber,to_days(ClientPrAuth.ExpDate)-to_days(curdate()),xInsurance.Descr from ClientPrAuth left join Insurance on Insurance.InsNumID=ClientPrAuth.InsuranceID left join xInsurance on xInsurance.ID=Insurance.InsID where ClientPrAuth.ClientID=? and Insurance.InsID=? and ? between ClientPrAuth.EffDate and ClientPrAuth.ExpDate order by ClientPrAuth.EffDate desc,ClientPrAuth.ExpDate");
  $sClientPrAuth->execute($ClientID,$InsID,$ContDate) || $form->dberror("vSCID: select ClientPrAuth");
  while ( my ($PrAuthID,$PAgroup,$PAnumber,$PrAuthDays,$InsDescr) = $sClientPrAuth->fetchrow_array )
  {
    $PrAuthDays += 0;
    $PRAUTHNUM = $PAnumber;
    ...
    if ( $PAgroup eq '' )
    {
      $sPrAuthRVU->execute($PrAuthID) || $form->dberror("INPA: select RVU ${PrAuthID}");
...
#warn qq|\n\nDUP1: fld=$fld, ID=$ID, ContDate=$ContDate, ContTime=${ContTime}\n|;
  ...
  return() if ( $InsID eq '356' );   # Don't check for Note with Health Home Insurance
  return() if ( $InsID eq '391' );   # Don't check for Note with Meaningful Use Insurance
  my $dbh = $form->dbconnect();
...
# Skip groups codes for Providers...
  my $Groups = $fld eq 'ProvID' ? "and xSC.Type!='GC' and xSC.Type!='GR'" : '';
  my $sTreatment = $dbh->prepare("select TrID,ContLogBegTime,ContLogEndTime,xSC.SCID,xSC.SCNum from Treatment left join xSC on xSC.SCID=Treatment.SCID where xSC.SCNum NOT LIKE 'X%' ${SKIPINS} ${T1012} ${H0001} ${Groups} and ${fld}=? and TrID!='${MyTrID}' and ContLogDate=? and ContLogBegTime<=? and ContLogEndTime>=?");
...
#warn qq|DUP2: fld=${fld}, TrID=${MyTrID}, Groups=${Groups}, T1012=${T1012}, H0001=${H0001}\n|;
  ...
  my ($TrID,$BeginTime,$EndTime,$SCID,$SCNum) = $sTreatment->fetchrow_array;
  $sTreatment->finish();
...
#warn qq|\n\nMAX: SCID=${SCID}, ContDate=${ContDate}, BeginTime=${BeginTime}, EndTime=${EndTime}\n|;
  ...
  my $warn = '';
  my $rxSC = cBill->getServiceCode($form,$SCID,$ContDate,$BeginTime,$EndTime);
#warn qq|MAX: rxSC->{'SCNum'}, Units=$rxSC->{Units}\n|;
  if ( $rxSC->{'SCNum'} =~ /^H0004/ && ($rxSC->{'SCNum'} =~ /HE$/ || $rxSC->{'SCNum'} =~ /HF$/) )
  { $warn = $rxSC->{Units} > 4 ? qq|!!! WARNING !!!\nIndividual Units over 4 Units may not pay!\n(${BeginTime}/${EndTime})\n| : ''; }
  elsif ( $rxSC->{'SCNum'} =~ /^H0004/ && ($rxSC->{'SCNum'} =~ /HE HR$/ || $rxSC->{'SCNum'} =~ /HF HR$/) )
  { $warn = $rxSC->{Units} > 4 ? qq|!!! WARNING !!!\nFamily Units over 4 Units may not pay!\n(${BeginTime}/${EndTime})\n| : ''; }
  elsif ( $rxSC->{'SCNum'} =~ /^H0004/ && ($rxSC->{'SCNum'} =~ /HE HQ$/ || $rxSC->{'SCNum'} =~ /HF HQ$/) )
  { $warn = $rxSC->{Units} > 6 ? qq|!!! WARNING !!!\nGroup Units over 6 Units may not pay!\n(${BeginTime}/${EndTime})\n| : ''; }
...
#warn qq|CheckTrPlan: with=${with}\nmsg=${msg}|;
  my $s = $dbh->prepare("select ID from ClientTrPlan where ClientID=? ${with} order by EffDate desc");
  $s->execute($ClientID) || $form->dberror("CheckTrPlan: select ClientTrPlan ${ClientID}");
  my $rows = $s->rows;
...
#warn qq|CheckTrPlan: rows=${rows}, warn=${warn}\n|;
  ...
  my $s = $dbh->prepare("select ID from ClientTrPlanS where ProvID=? and ClientID=? and TrPlanID=?");
  ...
  unless ( $rows )
  {
    my $s = $dbh->prepare("select FName,LName from Provider where ProvID=?");
    $s->execute($ProvID);
...
sub checkHospice {
  ...
  return($warn) unless ( $ClientID );
  my $dbh = $form->dbconnect();
  my $s = $dbh->prepare("SELECT HospiceCheck FROM ClientEmergency WHERE ClientID=?");
  $s->execute($ClientID);
