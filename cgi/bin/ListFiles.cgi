...
$xref='Clinics';
  $s=$dbh->prepare("select * from Provider where Type=3 or Type=4 or ProvID=91");
  $s->execute();
...
|;
$MainLinks = '';
  foreach my $Section ( @Sections ) { $MainLinks .= qq|<A HREF="\#$Section->{name}" >$Section->{name}</A> |; }
...
#warn qq|sortkey=$sortkey\n|;
    unless ( $sortkey eq '' )
    {
      my $odate = substr($sortkey,5,2).substr($sortkey,8,2).substr($sortkey,0,4); 
      my $oddate = $sortkey eq '' ? '' : substr($sortkey,5,2).'&#47;'.substr($sortkey,8,2).'&#47;'.substr($sortkey,0,4); 
      $SectionLinks .= qq|<A HREF="#$Section->{name}${odate}" >${oddate}</A> |;
    }
  }
  ...
  foreach my $sortkey ( sort { $b cmp $a } keys %{$Files} )
  { 
    my $odate = $sortkey eq '' ? '' : substr($sortkey,5,2).substr($sortkey,8,2).substr($sortkey,0,4); 
    my $oddate = $sortkey eq '' ? '' : substr($sortkey,5,2).'&#47;'.substr($sortkey,8,2).'&#47;'.substr($sortkey,0,4); 
    my $SectionLink = $odate eq '' ? '' : qq|<A NAME="$Section->{name}${odate}">|;
    $out .= qq|
${SectionLink}
...
| if ( $SectionLinks );
    ...
    { 
      ...
      my $name = $Section->{sort} eq 'bydate'
...
sub getFiles()
{
  ...
  my $Dir = qq|$form->{DOCROOT}/$Section->{dir}/$form->{Type}_$Section->{name}_*|;
#warn qq|Dir=$Dir\n|;
...
#warn qq|filename=$filename, filesuffix=$filesuffix\n|;
      my($type,$section,$clinicid,$insid,$daterange,$reccnt,$stamp,$token) = 
      ...
      my $rptforname = $clinicname eq ''
                       ? $providerlname eq ''
                         ? $provid : qq|${providerfname} ${providerlname}|
                       : $clinicname;
...
#warn qq|s=$section, c=$clinicid, i=$insid, d=$daterange\n|;
      ...
      my $rptid = qq|${rptforname}_${insurancename}_${daterange}_${reccnt}_${stamp}_${token}|;
#warn qq|sort=$Section->{sort}, sortkey=$sortkey, rptid=$rptid, d=$daterange\n|;
      if ( $Acc eq '' )
      {
        if ( $form->{LOGINPROVID} == 91 || $form->{LOGINPROVID} == $clinicid )
