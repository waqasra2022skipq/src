...
$sInsurance = $dbh->prepare(
"select Insurance.*,xInsurance.Name from Insurance left join xInsurance on xInsurance.ID=Insurance.InsID where ClientID=? and Priority=1 order by InsNumEffDate desc"
);
...
foreach my $ID ( split( ' ', $form->{IDs} ) ) {
    ...
    $sClient->execute($ID)
      || $form->dberror("PrintPharmacyEnroll: select Client ${ID}");
...
if ( $form->{LOGINPROVID} == 91 ) {
    open XML, ">C:/xampp/htdocs/src/debug/PrintPharmacyEnroll.out"
...
##
    ...
    $sProvider->execute($AgencyID)
      || $form->dberror("printPharmacyEnroll: select Provider $AgencyID");
...
##
    $sProvider->execute( $rClient->{'ProvID'} )
      || $form->dberror(
        "printPharmacyEnroll: select PrimaryProvider $rClient->{'ProvID'}");
...
##
    $sProvider->execute( $rClient->{'ProvID'} )
      || $form->dberror(
        "printPharmacyEnroll: select Clinician $rClient->{'ProvID'}");
...
##
    $sInsurance->execute( $rClient->{'ClientID'} )
      || $form->dberror(
        "printPharmacyEnroll: select Insurance $rClient->{'ClientID'}");
...
sub ptAllergies {
    ...
    $sClientAllergies->execute($ClientID)
      || $form->dberror("select ClientAllergies ${ClientID}");
    while ( my $rClientAllergies = $sClientAllergies->fetchrow_hashref ) {
        $xml .= DBA->getxref( $form, 'xAllergies', $rClientAllergies->{'AID'},
            'Descr' ) . ';';
    }
