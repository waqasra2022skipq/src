...
if ( $ProvID == 90 )             # Help Desk Global Forms
{
  ...
  $LISTS->{'90'}->{'Section 1'} = 'Medicaid:OHCA:DMH:Clinical_Pathways:Medicare:BCBS:Tricare:SAMHSA:MCO Humana:MCO Complete Health:MCO Atena';
  $LISTS->{'90'}->{'Section 2'} = 'MyHealth:CARF:Testing:Charting:Library:Federal:State:Curricula:Legal';
}
else
{
...
#warn qq|LOOP: ProvID=$rProvider->{ProvID}\n|;
    ...
    $sProviderEDocs->execute($p) || myDBI->dberror("select ProviderEDocs ${p}");
    while ( my $rProviderEDocs = $sProviderEDocs->fetchrow_hashref )
    {
      $cnt++;
      $name = $rProviderEDocs->{'Name'} eq ''
