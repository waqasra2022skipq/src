...
sub copy
{
  ...
  { $text .= qq|<BR>Missing From/To Provider!|; return($text); }

  my $q = qq|select * from ProviderPay where ExpDate is null and ProvID=?|;
  my $s = $dbh->prepare($q);
  $s->execute($FromProvID) || myDBI->dberror("select ProviderPay $FromProvID");
  my $Cnt = $s->rows;
  while (my $r = $s->fetchrow_hashref)
  {
    ...
    $sNew->execute() || myDBI->dberror("insert ProviderPay $FromProvID/$ToProvID");
    $sNew->finish();
  }
  $s->finish();
  $text .= qq|<BR>Copy from '${FromName}' to '${ToName}' <BR>$Cnt records copied.|;
