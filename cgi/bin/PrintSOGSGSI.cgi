...
foreach my $ID ( split( ' ', $form->{IDs} ) ) {
    $sSOGSGSI->execute($ID)
      || $form->dberror("PrintSOGSGSI: select SOGSGSI $ID");
...
##
    ...
    $sClient->execute($ClientID)
      || $form->dberror("PrintSOGSGSI: select Client $ClientID");
...
##
    ...
    $sProvider->execute( $r->{ProvID} )
      || $form->dberror("PrintSOGSGSI: select Provider $r->{'ProvID'}");
    ...
    while ( my $rshow = $sshow->fetchrow_hashref ) {
        ...
        else { $txt .= qq|   <$f>| . DBA->subxml( $r->{$f} ) . qq|</$f>\n|; }
    }
    $page4 = $txt;
    $sshow->finish();
    my $rInsurance =
      cBill->getInsurance( $form, $r->{ClientID}, 'primary', $r->{TransDate} );
    my $staffsig1 =
      DBA->setProvCreds( $form, $r->{'ProvID'}, $rInsurance->{InsID} );
    warn
      qq|ProvID=$r->{ProvID},InsID=$rInsurance->{InsID},staffsig1=$staffsig1\n|;
