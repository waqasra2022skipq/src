...
${UploadButton}
|
  . main->listFiles( '271', "*.rsp" ) . qq||
  . main->listFiles( '835', "*.rsp" ) . qq||
  . main->listFiles( '837', "*.rsp" ) . qq|
...
|;
        foreach
          my $filepath ( main->getFiles("${ADMINDIR}/${dir}/${wildcard}") )
        {
            #warn qq|\nfilepath=$filepath\n|;
            $count++;
...
sub getFiles() {
    ...
    my @Files       = ();
    my @FilesByDate = ();
    foreach my $filepath (@TmpFiles) { push( @Files, $filepath ); }
    for my $file (
        sort {
            my $a_stat = stat($a);
            my $b_stat = stat($b);
