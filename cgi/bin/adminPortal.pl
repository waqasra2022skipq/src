...
sub RebillTask {
    my $html = '';
    $Rebill_Button =
qq|<FONT COLOR="red"><A STYLE="background-color:white; padding:0.2em;" HREF="javascript:ReportWindow('/src/cgi/bin/runCMDBash.pl?AgencyID=&mlt=$form->{mlt}','adjNote')"  TITLE="Click here to change In Process notes to Rebill" >Rebill Notes</A></FONT>|;
    $Gen837_Button =
qq|<FONT COLOR="red"><A STYLE="background-color:white; padding:0.2em;" id="gen837" HREF="javascript:ReportWindow('/src/cgi/bin/Gen837medicaid.pl?mlt=$form->{mlt}','adjNote')"  TITLE="Click here to Generate 837 files for last billing run" >Gen837 files</A></FONT>|;
