...
open DBG, ">/home/okmis/mis/src/debug/genTrPlan.out" or die "Couldn't open file: $!";
...
my $sTrPlanS = $dbh->prepare("select * from TrPlanS where TrPlanID=? order by SignDate");
my $sPrAuth = $dbh->prepare("select * from ClientPrAuth where ClientPrAuth.ID=?");
...
my $sClientPrAuthCDC = $dbh->prepare("select * from ClientPrAuthCDC where ClientPrAuthCDC.ClientPrAuthID=?");
my $sPrAuthRVU = $dbh->prepare("select * from PrAuthRVU left join xSC on xSC.SCID=PrAuthRVU.SCID where PrAuthRVU.PrAuthID=? order by xSC.SCNum");
my $sInsurance = $dbh->prepare("select * from Insurance left join xInsurance on xInsurance.ID=Insurance.InsID where Insurance.InsNumID=?");
...
$pdtpath = '/home/okmis/mis/src/pdf/TrPlan.pdt';
if ( $form->{IDs} )
{
  $pdf = PDF->start($pdtpath);
  &printPDF($form->{'IDs'});
  $pdf->finish();
  my $file = 'genTrPlan_' . DBUtil->genToken() . '_' . DBUtil->Date('','stamp') . '.pdf';
...
print DBG "ClientID=$rPrAuth->{ClientID}, ClinicID=$rClient->{clinicClinicID}\n";
  $sClinic->execute($ClinicID);
  my $rClinic = $sClinic->fetchrow_hashref;
print DBG "$ClinicID: $rClinic->{Name} ($rClient->{FName} $rClient->{LName})\n";
...
print DBG "$rClient->{ClientID}\t$rClient->{LName}\t$rClient->{FName}\n";
  ...
  my $DateRange = 'for '   . DBUtil->Date($rPrAuth->{EffDate},'fmt','MM/DD/YYYY')
                . ' thru ' . DBUtil->Date($rPrAuth->{ExpDate},'fmt','MM/DD/YYYY');
...
# 290 (width of page) / 2 (in half) * 5 (boldness&size of font)
  ...
  $pdf->addElement(120,666,"$rClient->{LName}, $rClient->{FName} $rClient->{MName}",'/R10',11);
  ...
  my $ReqType = DBA->getxref($form,'xPrAuthReqType',$rPrAuth->{ReqType},'Descr');
  ...
  $sClientPrAuthCDC->execute($PrAuthID) || $form->dberror("select ClientPrAuthCDC ${PrAuthID}");
  if ( my $rClientPrAuthCDC = $sClientPrAuthCDC->fetchrow_hashref )
  { 
    $pdf->addElement(430,633,"CDCKey: $rClientPrAuthCDC->{CDCKey}",'/R10',10);
    my $StatusDate = DBUtil->Date($rClientPrAuthCDC->{StatusDate},'fmt','MM/DD/YY');
    $pdf->addElement(30,613,"$rClientPrAuthCDC->{Status}: ${StatusDate}",'/R10',10);
  }
...
print DBG "$SCName, Cnt=$Cnt, m=$m, y=$y\n";
    ...
    $pdf->addText(30,495,430,50,'* means Service is approved for InterChangeable',8);
  }
  else
  {
  ...
  my $y = 538; my $x = 35; my $Cnt = 0;
  $sTrPlanS->execute($rTrPlan->{'TrPlanID'})
         || $form->dberror("genTrPlan: select TrPlanS ");
  while ( my $rTrPlanS = $sTrPlanS->fetchrow_hashref )
  {
    $Cnt++;
    my $signby = DBA->setProvCreds($form,$rTrPlanS->{'ProvID'},$rInsurance->{InsID},$rTrPlanS->{'SignDate'},$rTrPlanS->{'SignTime'});
