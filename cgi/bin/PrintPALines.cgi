...
############################################################################
my $select =
qq|select PALines.* from PALines left join ClientPrAuth on ClientPrAuth.ID=PALines.PrAuthID left join Client on Client.ClientID=PALines.ClientID ${where} ${ProviderSelection} order by BegDate,EndDate|;
