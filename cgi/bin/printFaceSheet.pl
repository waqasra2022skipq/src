...
my $sClientFamily = $dbh->prepare(
    "select * from ClientFamily where ClientID=? and EmerContact=1");
my $sClientEmergency =
...
sub printFS {
    ...
    foreach my $ClientID ( split( ' ', $IDs ) ) {
        ...
        $sClient->execute($ClientID)
          || myDBI->dberror("printFaceSheet: select Client ${ClientID}");
...
sub createPages {
    ...
    $sProvider->execute($AgencyID)
      || myDBI->dberror("setAgency: select Provider ${AgencyID}");
...
##
    ...
    $sClientIntake->execute($ClientID)
      || myDBI->dberror("printFaceSheet: select ClientIntake ${ClientID}");
    my $rClientIntake = $sClientIntake->fetchrow_hashref;
    $sClientReferrals->execute($ClientID)
      || myDBI->dberror("printFaceSheet: select ClientReferrals ${ClientID}");
    my $rClientReferrals = $sClientReferrals->fetchrow_hashref;
    $sClientAdmit->execute($ClientID)
      || myDBI->dberror("printFaceSheet: select ClientAdmit ${ClientID}");
    my $rClientAdmit = $sClientAdmit->fetchrow_hashref;

    # IntakeProvID
    my $IntakeProvID = $rClientAdmit->{'ProvID'};
    $sProvider->execute($IntakeProvID)
      || myDBI->dberror("printFaceSheet: select IntakeProvID ${IntakeProvID}");
    ...
    my $rClientEmergencyCare =
      $sClientEmergency->fetchrow_hashref;    # NOT rClientEmergency, see above.
    ...
    my $rReferral =
      $RefSourceID ? DBA->selxref( $form, 'xNPI', 'NPI', $RefSourceID ) : ();
...
######################################
    ...
    $p->fit_textline( "REFERRAL SOURCE / REASON",
        $marginleft, $ypos, $baseboldmidlargefontoptions );

    $ypos += 18;
    $p->fit_textline( "Referred Date:",
        $marginleft, $ypos, $baseboldfontoptions );
    $p->fit_textline( $rClientReferrals->{'RefDate'},
        97.9, $ypos, $basefontoptions );
    $p->fit_textline( "Referred By:", 151.8, $ypos, $baseboldfontoptions );
    $p->fit_textline(
"$rReferral->{'ProvOrgName'} $rReferral->{'ProvPrefix'} $rReferral->{'ProvFirstName'} $rReferral->{'ProvLastName'}",
        208, $ypos, $basefontoptions
    );
    $p->fit_textline( "Phone:", 393.2,           $ypos, $baseboldfontoptions );
    $p->fit_textline( $rReferral->{'WkPh'}, 426, $ypos, $basefontoptions );

    $ypos += 14.8;
    $p->fit_textline( "Reason for Referral:",
        $marginleft, $ypos, $baseboldfontoptions );
    $tf = $p->create_textflow( $rClientReferrals->{'RefReason'},
        $basefontoptions . " leading=120% alignment=justify" );
    $h_tf =
      main->renderTextflow( $p, $tf, $marginleft, $contentwidth, $HeaderInfo );
    $ypos += $h_tf;

    main->renderTextline( $p, "Initial Intake Information",
        $marginleft, 18, $baseboldfontoptions_u, $HeaderInfo );

    main->renderTextline( $p, "Intake Date:", $marginleft, 14.8,
        $baseboldfontoptions, $HeaderInfo );
    my $admitdate =
      DBUtil->Date( $rClientAdmit->{AdmitDate}, 'fmt', 'MM/DD/YYYY' );
    my $admittime = substr( $rClientAdmit->{AdmitTime}, 0, 5 );
    $p->fit_textline( "${admitdate} @ ${admittime}",
        87, $ypos, $basefontoptions );
    $p->fit_textline( "Staff:", 180, $ypos, $baseboldfontoptions );
    ( my $staff =
qq|$rIntakeProvider->{'Pref'} $rIntakeProvider->{'FName'} $rIntakeProvider->{'LName'}|
    ) =~ s/\d//g;
    $p->fit_textline( ${staff}, 206, $ypos, $basefontoptions );

    main->renderTextline( $p, "IDENTIFYING INFORMATION",
        $marginleft, 27, $baseboldmidlargefontoptions, $HeaderInfo );

    my $name =
qq|$rClient->{'LName'}, $rClient->{'FName'} $rClient->{'MName'} (${ClientID})|;
    ...
    main->renderTextline( $p, "Address:", $marginleft, 14.8,
        $baseboldfontoptions );
    $p->fit_textline( ${addr}, 75, $ypos, $basefontoptions );

    main->renderTextline( $p, "Home Phone:", $marginleft, 14.8,
        $baseboldfontoptions );
    ...
    main->renderTextline( $p, "Place of Birth:",
        $marginleft, 14.8, $baseboldfontoptions );
    ...
    my $height =
qq|$rClientVitalSigns->{'HeightFeet'} ft. $rClientVitalSigns->{'HeightInches'} in.|;
    main->renderTextline( $p, "Height:", $marginleft, 14.8,
        $baseboldfontoptions );
    ...
    main->renderTextline( $p, "GUARANTOR INFORMATION",
        $marginleft, 27, $baseboldmidlargefontoptions );

    my $rGuarantor = ();
    $sInsurance->execute( $ClientID, 1 );
    if ( my $rInsurance = $sInsurance->fetchrow_hashref ) {
        $rGuarantor = $rInsurance->{'ClientRel'} eq 'I'
          || $rInsurance->{'ClientRel'} eq '' ? $rClient : $rInsurance;
        ...
        $p->fit_textline( "Email:", 393, $ypos, $baseboldfontoptions );
        $p->fit_textline( $rGuarantor->{'Email'}, 423, $ypos,
            $basefontoptions );

        main->renderTextline( $p, "Employer:", $marginleft, 14.8,
            $baseboldfontoptions );
        ...
        main->renderTextline( $p, "No Primary Insurance reported.",
            $marginleft, 14.8, $HeaderInfo );
    }

    main->renderTextline( $p, "IN CASE OF AN EMERGENCY",
        $marginleft, 27, $baseboldmidlargefontoptions );
    $p->fit_textline(
        '(Parent/Guarantor if client is under 18 or under legal guardianship)',
        207, $ypos - 1, $basemidfontoptions
    );

    my $emername =
      qq|$rClientEmergency->{'FName'} $rClientEmergency->{'LName'}|;
    my $emeraddr = $rClientEmergency->{'Addr1'};
    $emeraddr .= $rClientEmergency->{'Addr2'}
      if ( $rClientEmergency->{'Addr2'} ne '' );
    $emeraddr .= qq|, $rClientEmergency->{'City'}|
      if ( $rClientEmergency->{'City'} ne '' );
    $emeraddr .= qq|, $rClientEmergency->{'ST'}|
      if ( $rClientEmergency->{'ST'} ne '' );
    $emeraddr .= qq| $rClientEmergency->{'Zip'}|
      if ( $rClientEmergency->{'Zip'} ne '' );
    my $emerwkph = $rClientEmergency->{'WkPh'};
    my $erelation =
      DBA->getxref( $form, 'xRelationship', $rClientEmergency->{'Rel'},
        'Descr' );
    ...
    main->renderTextline( $p, "Relationship:", $marginleft, 14.8,
        $baseboldfontoptions );
    $p->fit_textline( ${erelation}, 93, $ypos, $basefontoptions );
    $p->fit_textline( "Special Instructions:",
        213, $ypos, $baseboldfontoptions );
    $p->fit_textline( $rClientEmergency->{'Comments'},
        305, $ypos, $basefontoptions );

    main->renderTextline( $p, "Health Care Information / Resources",
        235, 27, $baseboldfontoptions_u, $HeaderInfo );
    ...
    if ( $PhysNPI eq '' ) {
        ...
        $physname =
qq|$rPhysician->{'ProvPrefix'} $rPhysician->{'ProvFirstName'} $rPhysician->{'ProvLastName'}|;
        $physaddr = $rPhysician->{'Addr1'};
        $physaddr .= $rPhysician->{'Addr2'} if ( $rPhysician->{'Addr2'} ne '' );
        $physaddr .= qq|, $rPhysician->{'City'}|
          if ( $rPhysician->{'City'} ne '' );
        $physaddr .= qq|, $rPhysician->{'ST'}| if ( $rPhysician->{'ST'} ne '' );
        $physaddr .= qq| $rPhysician->{'Zip'}|
          if ( $rPhysician->{'Zip'} ne '' );
        $physwkph = $rPhysician->{'WkPh'};
    }

    main->renderTextline( $p, "Primary Care Physician:",
        $marginleft, 18, $baseboldfontoptions );
    ...
    main->renderTextline( $p, "Insurance", 287, 27, $baseboldfontoptions_u,
        $HeaderInfo );
...
#foreach my $f ( sort keys %{$rInsurance} ) { warn ": rInsurance-$f=$rInsurance->{$f}\n"; }
            ...
            $Ins[$Priority][3] =
                 $rInsurance->{'ClientRel'} eq ''
              || $rInsurance->{'ClientRel'} eq 'I'
              ? qq|self|
              : qq|$rInsurance->{'FName'} $rInsurance->{'MName'} $rInsurance->{'LName'}|;
        }

        #warn qq|Ins=$Ins[$Priority][1]\n|;
    }

    main->renderTextline( $p, "Primary:", $marginleft, 18,
        $baseboldfontoptions );
    ...
    main->renderTextline( $p, "Policy Number:",
        $marginleft, 14.8, $baseboldfontoptions );
    ...
    main->renderTextline( $p, "Policy Holder:",
        $marginleft, 14.8, $baseboldfontoptions );
...
sub renderTextflow {
    ...
    do {
        $result =
          $p->fit_textflow( $tf, $xpos, $y_footer, $xpos + $width, $ypos, "" );
...
sub renderTextline {
    my (
        $self, $p,       $text,       $xpos,
        $h_tl, $optlist, $HeaderInfo, $TitleInfo,
        $bg,   $h_bg,    $margin
    ) = @_;
    ...
    if ( $ypos + $h_tl > $y_footer ) {
        ...
        if ($TitleInfo) {
            ...
            $p->fit_textline( $TitleInfo->{"title"}, $xpos, $ypos,
                $TitleInfo->{"optlist"} );
        }
    }
    $ypos += $h_tl;
