...
if ( $rProvider->{Type} == 4 ) {
    ...
    $newDir = Rollup->createDirectory($form, '/Agency/EDocs/', $managerAgencyID, 2);

    # Create the Directory for the Clinic of the provider if not created
    Rollup->createDirectory($form, "/Agency/EDocs/${managerAgencyID}/Clinics/", $ClinicID, 3);
...
sub doClinicRollup {
    ...
    Rollup->createDirectory($form, "/Agency/EDocs/${agencyID}/Clinics/", $ClinicID, 3);
    main->doRollup($form, $ClinicID, "/Agency/EDocs/${agencyID}/Clinics/$ClinicID");
...
sub doProvidersRollup {
    ...
    Rollup->createDirectory($form, "/Agency/EDocs/${agencyID}/Clinics/${ClinicID}/Providers/", $Provider_ID, 4);
    main->doRollup($form, $Provider_ID, "/Agency/EDocs/${agencyID}/Clinics/$ClinicID/Providers/$Provider_ID");

    main->setUpClientRollUP($form, $Provider_ID, "/Agency/EDocs/${agencyID}/Clinics/$ClinicID/Providers/$Provider_ID/Clients/");
}

sub setUpClientRollUP {
    my ($self, $form, $Provider_ID, $ClientsFolder) = @_;
    my $qClients = qq|select distinct ClientID from Client where ProvID='$Provider_ID' and Active=1|;
    ...
    while ( $rClient = $sClient->fetchrow_hashref )
    {
        ...
        Rollup->edocs($form,$ClientID,'ClientAdmit','2017-01-01', $SingleClientFolder);
        ...
        Rollup->edocs($form,$ClientID,'ClientRiskAssessment', '', $SingleClientFolder);
        ...
        Rollup->edocs($form,$ClientID,'ClientGAD7', '', $SingleClientFolder);
        Rollup->edocs($form,$ClientID,'ClientBasis32', '', $SingleClientFolder);
        ...
        Rollup->Notes($form, $ClientID, '', $SingleClientFolder); # now Type does not matter
