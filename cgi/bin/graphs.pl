...
my $newDates =
  $form->{'FromDate'} ne ''
  ? 1
  : 0;    # if given a From/To Date then reset daterange.
...
sub ClientsConflictUnbilledWeek {
    ...
    $SUBTITLE =
qq| Note conflict- Client Unbillable (This week) ${fdayname} $f_from - ${tdayname} $f_to|;
...
sub ProvidersConflictUnbilledWeek {
    ...
    $yLabel    = 'Counts of TrID';
    $TITLE =
      qq|${ReportHeader}  Note conflict- Provider Unbillable (This week)|;
    ...
    my $DateRange = qq|from ${fdayname} $f_from - ${tdayname} $f_to|;
    $SUBTITLE =
qq| Note conflict- Provider Unbillable (This week) ${fdayname} $f_from - ${tdayname} $f_to|;
...
sub AgeMis {
    ...
    while ( $r = $q->fetchrow_hashref ) {
        ...
        $cnt++;
        $PROG{$key}
          { $r->{'MyX'} }++;    #{'deterioration minimal':{'Y-M': 'count(10)'}}
    }
    $q->finish();
...
sub PAExpire {
    ...
    my $DateRange = qq|from ${fdayname} $f_from - ${tdayname} $f_to|;
    $SUBTITLE =
      qq| PA Expire Report form ${fdayname} $f_from - ${tdayname} $f_to|;
  ...
  select xInsurance.Name, Insurance.InsIDNum from Insurance 
    ...
    while ( my $rClient = $sClient->fetchrow_hashref ) {
        ...
        my $SubtractDays =
          $dbh->prepare("select to_days(?) - to_days(curdate())");
...
sub Pachg {
    ...
    $yLabel    = 'Count';
    $TITLE =
      qq|${ReportHeader} Change Prior Authorizations from Rehab to NO REHAB|;
    my $ActiveSelection = qq| and Client.Active=1| if ( $form->{Active} );
    my $s               = qq|
...
|;
    ...
    while ( $rPDDom = $sPDDom->fetchrow_hashref ) {
        ...
        my $key =
qq|$rPDDom->{LName}, $rPDDom->{FName}_$rPDDom->{ClientID}_$rPDDom->{ClinicName}|;
    ...
    foreach my $key ( sort keys %Scores ) {
        ...
        for my $i ( 1 .. 9 ) {
            ...
            my $Diff =
              $Score1 && $Score2 ? sprintf( "%.2f", $Score2 - $Score1 ) : 0;
...
sub ProgressByAverage {
    ...
    while ( $month <= $month_starts ) {
        ...
        my $q2 =
          qq| Progress, DATE_FORMAT(Treatment.ContLogDate,'%Y-%m') as months
              FROM ProgNotes
                left join Treatment on Treatment.TrID=ProgNotes.NoteID
                left join Client on Client.ClientID=ProgNotes.ClientID
                left join ClientACL on ClientACL.ClientID=Client.ClientID
...
sub ProgressByDatePercent {
    ...
    my $dataset = ();
    ( $cnt, $dataset ) =
      main->selProgressPercentage( $form, $dataset, $addsel );
    $WIDTH = $cnt * 15 if ( $cnt > 65 );    # widen the graph is many elements.
    my $data = graphs->setData($dataset);
    return ($data);
}
