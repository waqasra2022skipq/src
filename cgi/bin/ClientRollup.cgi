...
Rollup->createDirectory($form, "/Agency/EDocs/${managerAgencyID}/Clinics/${ClinicID}/Providers/", $ProvID, 4);

# Create the Directory for the Client if not created
Rollup->createDirectory($form, "/Agency/EDocs/${managerAgencyID}/Clinics/${ClinicID}/Providers/$ProvID/Clients/", $ClientID);

# ProgNotes all already getting rolled up at Notes so no need to do it separately
my $SingleClientFolder = "/Agency/EDocs/${managerAgencyID}/Clinics/${ClinicID}/Providers/$ProvID/Clients/${ClientID}";
