...
my $url =
qq|/cgi/bin/mis.cgi?view=vInsID.cgi&title=Reconcile or Scholarship Notes&action=Remit&mlt=$form->{'mlt'}&misLINKS=$form->{'misLINKS'}|;

############################################################################
# Remit.cgi does 1 Insurance type at a time.
my $sInsurance = $dbh->prepare("select * from xInsurance where ID=?");
$sInsurance->execute($InsID)
  || myDBI->dberror("Remit: select xInsurance ${InsID}");
my $rInsurance = $sInsurance->fetchrow_hashref;
my $Title      = qq|$rInsurance->{Name} Billed NOT Reconciled or AmtDue Notes|;

my $html =
    $form->{'submit'} ? main->submit( $InsID, $url )
...
sub html() {
    ...
    my $Hdr =
'Please note: for each treatment note you may either; input reconciled amount, check scholarship, or select deny code. Priority is in that order as to which function is performed. A note may have to be denied before it can be rebilled.';

    #  my $html =  myHTML->new($form,"Reconcile");
    my $html =
      myHTML->newHTML( $form, 'Reconcile',
        'CheckPopupWindow noclock countdown_30' )
      . qq|
...
" );
    foreach my $f ( sort keys %{$form} ) {
        ...
        $sTreatment->execute($TrID)
          || myDBI->dberror("Remit: select Treatment ${TrID}");
        ...
        warn
qq|Remit: pass2: TrID=${TrID}, PaidAmt=${PaidAmt}, DenCode=${DenCode}\n|;
    ...
    # add to uBill->selTrans that if TrID, look up note first by TrID (FindNote)
        ...
        warn
qq|Remit: TrID/trid=${TrID}/${trid}, scid=${scid}, code=${code}, type=${type}\n|;
