...
sub searchFor {
    ...
    if ( $form->{'SearchType'} eq 'ProviderID' ) {
        ...
        $str =
qq|select distinct Provider.* from Provider where Provider.ProvID = '$form->{SearchString}' order by Provider.LName, Provider.FName|;
    }
        ...
        #warn "ProviderList.cgi: SearchName=$SearchName\n";
        $str =
qq|select distinct Provider.* from Provider where Provider.FName like '$SearchStr' order by Provider.LName, Provider.FName|;
    }
        ...
        $str =
qq|select distinct Provider.* from Provider where Provider.LName like '$SearchStr' order by Provider.LName, Provider.FName|;
    }
    else { $form->error("Provider List / Access Denied"); }
    return ($str);
}
############################################################################
...
sub ProviderList() {
    ...
    my $page =
      qq|/cgi/bin/ProviderPage.cgi?Provider_ProvID=$r->{ProvID}&${addURL}|;
