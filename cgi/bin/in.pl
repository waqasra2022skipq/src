...
my $sInsurance = $dbh->prepare("select Insurance.ClientID,Insurance.InsIDNum,Insurance.InsID,xInsurance.Name from Insurance left join xInsurance on xInsurance.ID=Insurance.InsID where Insurance.ClientID=? and Insurance.InsID=? and Insurance.InsNumEffDate<=? and (?<=Insurance.InsNumExpDate or Insurance.InsNumExpDate is NULL) order by Insurance.InsNumEffDate desc");
my $sClientNoteProblems = $dbh->prepare("select ClientNoteProblems.ID,ClientNoteProblems.Locked,misICD10.ICD10, misICD10.icdName, misICD10.sctName, DATE_FORMAT(ClientNoteProblems.InitiatedDate,'%m/%d/%Y') as InitiatedDate, DATE_FORMAT(ClientNoteProblems.ResolvedDate,'%m/%d/%Y') as ResolvedDate, ClientNoteProblems.Priority from ClientNoteProblems left join okmis_config.misICD10 on misICD10.ID = ClientNoteProblems.UUID where ClientNoteProblems.TrID=? order by ClientNoteProblems.Priority LIMIT 4");
my $sClientNoteTrPlanPG = $dbh->prepare("select * from ClientNoteTrPlanPG where TrID=? order by Priority");
my $sClientTrPlanPG = $dbh->prepare("select * from ClientTrPlanPG where ID=? order by Priority");
my $sClientTrPlanOBJ = $dbh->prepare("select * from ClientTrPlanOBJ where TrPlanPGID=? order by Priority");
my $sClientVitalSigns = $dbh->prepare("select * from ClientVitalSigns where ClientID=? and VDate<=? order by VDate desc");
my $sClientNoteMeds = $dbh->prepare("select * from ClientNoteMeds left join ClientMeds on ClientMeds.ID=ClientNoteMeds.ClientMedsID where TrID=? order by ClientMeds.DrugInfo, ClientMeds.PrescriptionDate");
...
#warn qq|test ID: |.$TrID."\n";
  $sTreatment->execute($TrID) || myDBI->dberror("printNotes: select Treatment ${TrID}");
...
#warn qq|add ID: |.$TrID."\n";
    ...
    {
      $sProgNotes->execute($TrID) || myDBI->dberror("PrintNotes: select ProgNotes ${TrID}");
      $rNote = $sProgNotes->fetchrow_hashref;
    }
    elsif ( $NoteType == 2 )                        # physician
    {
      $sPhysNotes->execute($TrID) || myDBI->dberror("PrintNotes: select PhysNotes ${TrID}");
      $rNote = $sPhysNotes->fetchrow_hashref;
    }
    my $dbilled = $rTreatment->{'BillDate'} eq ''
                  ? 'New'
                  : DBUtil->Date($rTreatment->{'BillDate'},'fmt','MM/DD/YYYY');
    my $ClientID = $rTreatment->{'ClientID'};
    $sClient->execute($ClientID) || myDBI->dberror("printNotes: select Client $ClientID");
    my $rClient = $sClient->fetchrow_hashref;
    my $rxSC = cBill->getServiceCode
                 ($form,$rTreatment->{'SCID'},$rTreatment->{'ContLogDate'},$rTreatment->{'ContLogBegTime'},$rTreatment->{'ContLogEndTime'},$TrID,$rTreatment->{'BillDate'});
    $sProvider->execute($rTreatment->{'ClinicID'}) || myDBI->dberror("select Clinic $rTreatment->{'ClinicID'}");
    my $rClinic = $sProvider->fetchrow_hashref;
    $sClientLegal->execute($ClientID) || myDBI->dberror("printNotes: select ClientLegal $ClientID");
...
#warn qq|sInsurance->execute($rTreatment->{ClientID},$rxSC->{InsID},$rTreatment->{ContLogDate},$rTreatment->{ContLogDate})\n|;
    $sInsurance->execute($rTreatment->{ClientID},$rxSC->{InsID},$rTreatment->{ContLogDate},$rTreatment->{ContLogDate}) || myDBI->dberror("select Insurance ${TrID} ${ClientID}");
    my $rInsurance = $sInsurance->fetchrow_hashref;

    my $groupno = $rInsurance->{InsIDNum} !~ /^(\d{3})-(\d{2})-(\d{4})/ ? $rInsurance->{InsIDNum} : '';
    my $dos = DBUtil->Date($rTreatment->{ContLogDate},'fmt','MM/DD/YYYY');
    ...
    for ($pageno = 1; $pageno <= $no_of_input_pages; $pageno++)
    {
      ...
      { printf("Warning: %s\n", $p->get_errmsg()); }
      if ($p->fill_textblock($pagehandles[$pageno], 'myTitle', "$rInsurance->{'Name'} Note # ${TrID} <fontsize=8>(${NoteDescr})", $optlist) == -1)
      { printf("Warning: %s\n", $p->get_errmsg()); }
      my $grpsize = $rNote->{'GrpSize'} >= 1
                    ? qq|               ${labelfont}Group Size:${responsefont} $rNote->{'GrpSize'}\n|
                    : qq|\n|;
      my $noteinfo = qq|${labelfont}Name:${responsefont} $rClient->{'LName'}, $rClient->{'FName'} (${ClientID})|;
      $noteinfo   .= qq|               ${labelfont}Group #:${responsefont} ${groupno}\n|;
      $noteinfo   .= qq|${labelfont}Type Of Service:${responsefont} $rxSC->{SCName} ${grpsize}|;
      $noteinfo   .= qq|${labelfont}Date Of Service:${responsefont} ${dos}|;
      $noteinfo   .= qq|               ${labelfont}Start Time:${responsefont} ${stime}|;
      $noteinfo   .= qq|               ${labelfont}End Time:${responsefont} ${etime}|;
      $noteinfo   .= qq|               ${labelfont}Total Minutes:${responsefont} ${DurationInMinutes}|;
      if ($p->fill_textblock($pagehandles[$pageno], 'noteinfo', $noteinfo, $optlist) == -1)
      { printf("Warning: %s\n", $p->get_errmsg()); }
      ...
      my $notetext = main->getPG($p,$rTreatment,$rxSC->{'Type'});
          my $llx = $p->pcos_get_number($indoc,"pages[".($pageno-1)."]/blocks/problems/rect[0]");
          my $lly = $p->pcos_get_number($indoc,"pages[".($pageno-1)."]/blocks/problems/rect[1]");
          my $urx = $p->pcos_get_number($indoc,"pages[".($pageno-1)."]/blocks/problems/rect[2]");
          my $ury = $p->pcos_get_number($indoc,"pages[".($pageno-1)."]/blocks/problems/rect[3]");
...
${responsefont}|.$rNote->{'Methods'};
        ...
        $notetext .= qq|\n\n${labelfont}INTERVENTION:${responsefont} |.DBA->getxref($form,'xNoteInt',$int,'Descr').qq|
${labelfont}SPECFIC TECHNIQUES:|;
        foreach my $tech ( split(chr(253),$rNote->{'Techniques'}) )
        { $notetext .= "\n${responsefont}".DBA->getxref($form,$xtable,$tech,'Descr'); }
      }
      if ( $NoteType =~ /1|2|4/ )                    # progress | physician | medicare
      {
        $notetext .= qq|\n\n${labelfont}PROGRESS MADE TOWARDS GOALS:${responsefont} |.DBA->getxref($form,'xProgress',$rNote->{Progress},'Descr').qq|
...
${labelfont}Describe Below:\n${responsefont}|.$rNote->{'NewProblems'};
        ...
        if ( $rxSC->{'Type'} eq 'CI' )
        {
          my $cGAF = $rNote->{'GAFCurrent'} eq '' ? "${responsefont} None"
                                                  : "${responsefont} $rNote->{'GAFCurrent'}";
          my $pGAF = $rNote->{'GAFRecent'} eq '' ? "${responsefont} None"
                                                 : "${responsefont} $rNote->{'GAFRecent'}";
          $notetext .= qq|${labelfont}LEVEL OF FUNCTIONING ASSESSMENT GAF: CURRENT:${cGAF} ${labelfont}RECENT PAST (30 DAYS):${pGAF}
|.$rNote->{'CrisisText'};
        }
      }
      $notetext .= qq|\n\n${labelfont}Patient Education Performed:${responsefont} Yes| if ( $rTreatment->{'EdPerformed'} );
      if ($p->fill_textblock($pagehandles[$pageno], 'notetext', $notetext, $optlist) == -1)
      { printf("Warning: %s\n", $p->get_errmsg()); }
      my $completedby = "${labelfont}Electronically Signed By: ${responsefont}".DBA->setProvCreds($form,$rTreatment->{'ProvID'},$rxSC->{InsID},$rTreatment->{'ProvOKDate'},$rTreatment->{'ProvOKTime'});
      my $approvedby = "${labelfont}                        Approved By: ${responsefont}".DBA->setProvCreds($form,$rTreatment->{'MgrProvID'},$rxSC->{InsID},$rTreatment->{'MgrRevDate'},$rTreatment->{'MgrRevTime'});
      my $enteredby = "${labelfont}                           Entered By: ${responsefont}".DBA->setProvCreds($form,$rTreatment->{'EnteredBy'},$rxSC->{InsID},$rTreatment->{'ChartEntryDate'});
      $str = $completedby."\n".$approvedby."\n".$enteredby."\n";
      if ($p->fill_textblock($pagehandles[$pageno], 'completedby', $str, $optlist) == -1)
      { printf("Warning: %s\n", $p->get_errmsg()); }
      ...
      for (my $j = 0; $j < $nblocks; $j++)
      {
        ...
        $objtype = $p->pcos_get_string($indoc, "type:pages[" . ($pageno-1) . "]/blocks/" . $formblocks[$j]);
        if ($objtype eq "dict")
        {
          my $str = $j.' <fontname={NotoSerif-Regular} encoding=unicode fontsize=20> this is block: '.$formblocks[$j];
          if ($p->fill_textblock($pagehandles[$pageno], $formblocks[$j], $str, $optlist) == -1)
          { printf("Warning: %s\n", $p->get_errmsg()); }
        }
      }
      my $CustAgency = DBA->getxref($form,'xCustAgency',$rClientLegal->{'CustAgency'},'Descr');
      ...
      my $cagency = qq|${CustAgency} ${jolts} ${caseid}|;         # Append to CustAgency.
#$rClinic->{'Name'} .= qq|REDUCE LINE TEST|;
#$cagency .= qq|TEST TO REDUCE LINE|;
      $str = qq|${labelfont}Service Code:${responsefont} $rxSC->{'SCNum'}|;
      $str   .= qq|     ${labelfont}Total Units:${responsefont} $rTreatment->{'Units'}|;
...
# make 0 null, to set starting at 1...
  ...
  my $fontsize = 10;
  my $basefontoptions = "fontname={Times New Roman Bold} fontsize=" . $fontsize . " embedding encoding=unicode";
  my $optlist;
  my $x_table = 32;
  my $tablewidth = 545;
  ...
  $basefontoptions = "fontname={Arial} fontsize=" . $fontsize . " embedding encoding=unicode";
...
#warn qq|buf=${buf}, row=${row}, col=${col}\n|;
    $optlist = "fittextline={position={" . $alignments[$col] . 
                " center} " . $basefontoptions . "} colwidth=10% margin=2";
...
#warn qq|buf=${buf}, row=${row}, col=${col}\n|;
    $optlist = "fittextline={position={" . $alignments[$col] . 
                " center} " . $basefontoptions . "} colwidth=70% margin=2";
    $tbl = $p->add_table_cell($tbl, $col, $row, $buf, $optlist);
...
#warn qq|buf=${buf}, row=${row}, col=${col}\n|;
  ...
  my $result = $p->fit_table($tbl, $x_table, $top, $x_table+$tablewidth, 20, $optlist);
  if ($result eq "_error") { throw new Exception("Couldn't place table: " . $p->get_errmsg()); }
  $p->delete_table($tbl, "");
  return();
}
sub getPG
{
  ...
  my $out = qq|${labelfont}SPECIFIC PROBLEM(S) ADDRESSED (As Identified On Comprehensive Treatment Plan):
${responsefont}|;
  $out .= qq|Develop / Review Treatment Plan\n| if ( $SCType eq 'TP' );
  $out .= qq|Crisis Response\n| if ( $SCType eq 'CI' );
  $sClientNoteTrPlanPG->execute($TrID) || myDBI->dberror("printNotes: select ClientNoteTrPlanPG ${TrID}");
  while ( my $rClientNoteTrPlanPG = $sClientNoteTrPlanPG->fetchrow_hashref )
  {
    $sClientTrPlanPG->execute($rClientNoteTrPlanPG->{'TrPlanPGID'}) 
                      || myDBI->dberror("printNotes: select ClientTrPlanPG $rClientNoteTrPlanPG->{'TrPlanPGID'}");
    while ( my $rClientTrPlanPG = $sClientTrPlanPG->fetchrow_hashref )
    {
      ...
      $out .= "${labelfont}#".$Priority.": Problem: ${responsefont}".${PROB}."\n";
      $out .= "${labelfont}${leftindent1}Goal: ${responsefont}".${GOAL}."\n";
      my $cnt = 0;
      $sClientTrPlanOBJ->execute($rClientTrPlanPG->{'ID'}) || myDBI->dberror("printNotes: select ClientTrPlanOBJ $rClientTrPlanPG->{'ID'}");
...
#warn qq|printNotes: ClientNoteTrPlanOBJ: ID=$rClientTrPlanOBJ->{ID}\n|;
        $out .= "${labelfont}${leftindent2}Objective ${Priority}${ltr}: ${responsefont}".${OBJ}."\n";
      }
    }
  }
  return($out);
...
sub getParticipants
{
  ...
  {
    $row++;
    $Informants .= $rClientNoteFamilyI->{'FName'}.' '.$rClientNoteFamilyI->{'LName'}.' ('.DBA->getxref($form,'xRelationship',$rClientNoteFamilyI->{Rel},'Descr').') ';
  }
  $out .= $Informants eq '' ? '' : qq|\n${labelfont}Informants: ${Informants}|;
  ...
  {
    $row++;
    my $Participants .= $rClientNoteFamilyP->{'FName'}.' '.$rClientNoteFamilyP->{'LName'}.' ('.DBA->getxref($form,'xRelationship',$rClientNoteFamilyP->{Rel},'Descr').') ';
  }
  $out .= $Participants eq '' ? '' : qq|\n${labelfont}Participants: ${Participants}|;
...
#$cnt = 1 unless($cnt);
  ...
  my $out = qq|${labelfont}Date${leftindent1}Height${leftindent2}Weight${leftindent3}Waist${leftindent4}Temp${leftindent5}BP${leftindent6}Pulse${leftindent7}Oximetry${leftindent8}BloodSugar${leftindent9}Respiration\n| if ( $cnt>0 );
  my $row = 0;
  while ( my $r = $sClientVitalSigns->fetchrow_hashref )
  {
    ...
    my $BP = $r->{'BPSystolic'} eq '' && $r->{'BPDiastolic'} eq ''
             ? ''
             : $r->{'BPSystolic'} eq ''
               ? qq|missing / $r->{'BPDiastolic'}|
               : qq|$r->{'BPSystolic'} / $r->{'BPDiastolic'}|;
    $out .= qq|${responsefont}$r->{VDate}${leftindent1}${Height}${leftindent2}${Weight}${leftindent3}$r->{Waist}${leftindent4}${Temp}${leftindent5}${BP}${leftindent6}$r->{Pulse}${leftindent7}$r->{Oximetry}${leftindent8}$r->{BloodSugar}${leftindent9}$r->{Respiration}\n|;
...
sub getMeds
{
  my ($self,$p,$rTreatment) = @_;
  my $TrID = $rTreatment->{'TrID'};
  $sClientNoteMeds->execute($TrID) || myDBI->dberror("PrintNotes: select ClientNoteMeds");
  my $cnt = $sClientNoteMeds->rows;
#$cnt = 1 unless($cnt);
  my $out = qq|${labelfont}DrugID\tDrugInfo\tFrequency\tRoute\tSchedule\tPrescriptionDate\tRefills\n| if ( $cnt>0 );
  my $row = 0;
  while ( my $rClientNoteMeds = $sClientNoteMeds->fetchrow_hashref )
  {
    $row++;
    $out .= qq|${responsefont}$rClientNoteMeds->{DrugID}\t$rClientNoteMeds->{DrugInfo}\t$rClientNoteMeds->{DosageFrequencyDescription}\t$rClientNoteMeds->{Route}\t$rClientNoteMeds->{ExternalDrugSchedule}\t$rClientNoteMeds->{PrescriptionDate}\t$rClientNoteMeds->{Refills}\n|;
#    $rNote->{Text}
  }
