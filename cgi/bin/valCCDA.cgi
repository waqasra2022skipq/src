...
$sPhiMailAttachments->execute( $form->{'IDs'} )
  || $form->dberror("valCCDA: select PhiMailAttachments");
...
else { warn qq|NOT FOUND\n|; }
my $file = qq|$form->{DOCROOT}$rPhiMailAttachments->{filename}|;
my ( $directory, $filename ) = $file =~ m/(.*\/)(.*)$/;
my $url =
qq|-X POST --header 'Content-Type: multipart/form-data' --header 'Accept: application/json' -F ccdaFile=\@${file} -F validationObjective=NegativeTesting_CarePlan -F referenceFileName=NT_CP_Sample4_r21_v4.xml https://ttpds.sitenv.org:8443/referenceccdaservice/|;
warn qq|curl $url\n|;
my ( $cnt, $err ) = ( 0, '' );
my $jsonData = `curl $url 2>/tmp/kls`;
if ( $jsonData eq '' ) { $err .= qq|jsonData IS NULL!\n|; }
else {
...
#  serviceError=0
#  serviceErrorMessage=
    $err .= $rJSON->{'resultsMetaData'}->{'serviceErrorMessage'}
      if ( $rJSON->{'resultsMetaData'}->{'serviceError'} == 1 );
...
#  xPath=/ClinicalDocument/component/structuredBody/component/section/entry[3]/act/code
    foreach my $f (@ccdaValidationResults) {
        if (   $f->{"type"} eq "C-CDA MDHT Conformance Error"
            or $f->{"type"} eq
