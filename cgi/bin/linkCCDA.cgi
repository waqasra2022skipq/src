...
# prepare selects...
##
my $sPhiMailAttachments =
  $dbh->prepare("select * from PhiMailAttachments where ID=?");
$sPhiMailAttachments->execute( $form->{'IDs'} )
  || $form->dberror("linkCCDA: select PhiMailAttachments");
my $html = '';
my ( $cnt, $list, $err ) = ( 0, '', '' );
...
if ( -f $fromfile ) {
    if ( $form->{link} ) {
        $list = main->link( $form, $ClientID, $rPhiMailAttachments );
    }
...
sub link {
    ...
    copy( $fromfile, $tofile )
      or $list .= "<<< ERROR >>>: Copy ${fromfile} failed: $!<BR>";
    my $r = ();
    $r->{'ClientID'} = $ClientID;
    $r->{'Type'}     = 41;                            # CCDA
    ...
    $NEWID               = DBA->doUpdate( $form, "ClientEDocs", $r,
        "ClientID='${ClientID}' and Path='$r->{htmfile}'" );
...
"select * from Client where FName LIKE '%${FName}%' and LName LIKE '%${LName}%';"
        );
        warn
qq|list: select * from Client where FName LIKE '%${FName}%' and LName LIKE '%${LName}%';|;
        $sClient->execute() || $form->dberror("linkCCDA: select Client");
        my $rows = $sClient->rows;
        $list =
qq|${rows} Client(s) List for: ${FName} ${LName} ${Gender} ${DOB}<BR>|;
...
qq|$rClient->{'FName'} $rClient->{'LName'} $rClient->{'Gend'} $rClient->{'DOB'}
        ...
        if ( $fromfile eq '' ) {
            $err  = qq|fromfile name empty in link!|;
            $list = qq|Unable to search for client, Aborted!|;
        }
        else { $err = qq|fromfile NOT FOUND on disk!|; $list = qq|Aborted!|; }
    }
    return ($list);
}
