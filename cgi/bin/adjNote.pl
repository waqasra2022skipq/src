...
select Treatment.*,xSC.SCNum,xInsurance.InsCode 
 ...
 where TrID=?
");
$sTreatment->execute($TrID) || myDBI->dberror("adjNote: select Treatment ${TrID}");
...
#warn qq|update....TrID=$TrID\n|;
    my $RecDate = $form->{'TODAY'};
      ...
      $rNoteTrans->{'Code'} = 'BI';       # 'BI' for an instance, get updated below...
      $rNoteTrans->{'SRC'} = 'CL';
#foreach my $f ( sort keys %{$r} ) { warn "r: $f=$r->{$f}\n"; }
      my $TransID = DBA->doUpdate($form,'NoteTrans',$rNoteTrans);        # insert the record (no where)
...
#foreach my $f ( sort keys %{$r835} ) { warn "r835: $f=$r835->{$f}\n"; }
      my ($trid,$scid,$code,$type) = uBill->postClaim($form,$r835,'CL','ADJ');
#warn qq|adjNote: CALLED postClaim: TrID=${trid}, SCID=${scid}, code=${code}, type=${type}\n|;
      my $code = uBill->setPayDate($form,$RecDate,$TransID);        # remove from Payroll, TransID, not TrID
  }
  return($html);
}
sub html
{
  ...
  $sClient->execute($rTreatment->{'ClientID'}) || myDBI->dberror("adjNote: select Client $rPrAUth->{'ClientID'}");
...
# Start out the display.
  my $html = myHTML->newHTML($form,'Manually Add Transaction','CheckPopupWindow noclock countdown_10') . qq|
