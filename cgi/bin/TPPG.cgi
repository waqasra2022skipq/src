...
$sxNS->execute($form->{'id'}) || $form->dberror("TPPG: select id error ($form->{'id'}");
if ( my $rxNS = $sxNS->fetchrow_hashref )
{
  $hdr = $rxNS->{'Descr'};
  my $sxPG = $cdbh->prepare("select * from xPG where NSID=? order by Num");
  $sxPG->execute($rxNS->{'ID'}) || $form->dberror("TPPG: select PG error ($rxNS->{'ID'}");
  while ( my $rxPG = $sxPG->fetchrow_hashref )
  {
    ...
    my $rOBJ = (); my $cnt = 0;
    my $sxOBJ = $cdbh->prepare("select * from xOBJ where PGID=? order by Num");
    $sxOBJ->execute($rxPG->{'ID'}) || $form->dberror("TPPG: select OBJ error ($rxPG->{'ID'}");
...
{
  my ($self,$form) = @_;
  my $viewedby = $form->{LOGINUSERTYPE} > 3 ? $form->{LOGINUSERNAME} : $form->{LOGINNAME};
  print qq|Content-type: text/html\n\n
<!DOCTYPE html>
<HTML>
<HEAD> <TITLE>Provider Reports</TITLE>
  <LINK REL="stylesheet" TYPE="text/css" HREF="|.myConfig->cfgfile('main.css',1).qq|" >
...
|;
  ...
  foreach my $Num ( @Nums )
  {
    print qq|<TR CLASS="home" ><TD CLASS="strcol" >${Num}</TD><TD CLASS="strcol" >$Probs[$i]<TD CLASS="strcol" >$Goals[$i]</TD>\n|;
    my $rOBJ = $Objs[$i];
    foreach my $cnt ( sort { $a <=> $b } keys %{ $rOBJ } )
    { print qq|<TR CLASS="home" ><TD CLASS="strcol" >&nbsp;</TD><TD CLASS="strcol" >Objective ${cnt}:</TD><TD CLASS="strcol" >$rOBJ->{$cnt}</TD>\n|; }
    $i++;
  }
  print qq|</TABLE>\n|;
