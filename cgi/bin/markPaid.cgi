...
# '95_Payroll_MarkPaid_20090429220714_S3fqZb.xls';
  ...
  my $Y = substr($Stamp,0,4); my $M = substr($Stamp,4,2); my $D = substr($Stamp,6,2);
  my $SDATE = DBUtil->Date("$Y-$M-$D",'fmt','MM/DD/YYYY');
  my $h = substr($Stamp,8,2); my $m = substr($Stamp,10,2); my $s = substr($Stamp,12,2);
  my $STIME = $h . ':' . $m . ':' . $s;
...
#warn qq|\nType=$Type, ProvID=$ProvID, Stamp=$Stamp, Token=$Token\n|;
  ...
  $qMarkTran = qq|update NoteTrans set PaidDate='${MarkDate}' where ID=? and PaidDate is null|;
  $sMarkTran = $dbh->prepare($qMarkTran);
  $qMarkNote = qq|update Treatment set PaidDate='${MarkDate}' where TrID=? and PaidDate is null|;
  $sMarkNote = $dbh->prepare($qMarkNote);
  $qMarkPaid = qq|update NotePaid set PaidDate='${MarkDate}' where TrID=? and PaidDate is null|;
  $sMarkPaid = $dbh->prepare($qMarkPaid);
  if ( open(MARKFILE, $markfile) )
  {
    my @items; my $trans=0; my $notes=0;
...
#warn qq|markPaid: p1=$p1, p2=$p2\nType=$Type, ProvID=$ProvID, Stamp=$Stamp, Token=$Token\n|;
    ...
    my $MarkedFile = "../payroll/${ProvID}_Payroll_MarkedPaid_${Stamp}_${Token}_${Marked}.${p2}";
    $text .= qq|<BR>move MarkPaid_${Stamp} to MarkedPaid_${Marked}|;
    move($markfile,$MarkedFile) or $text .= "<<< ERROR >>>: Move ${file} failed: $!";;
    my $SummaryFrom = "${ProvID}_Payroll_Summary_${Stamp}_${Token}.xls";
    my $SummaryTo = "../payroll/${ProvID}_Payroll_Summary_${Stamp}_${Token}_${Marked}.xls";
    $text .= qq|<BR>move Summary_${ProvID} to Summary_${Marked}|;
    move($SummaryFrom,$SummaryTo) or $text .= "<<< ERROR >>>: Move ${SummaryFrom} failed: $!";;
    my $DetailFrom = "${ProvID}_Payroll_Detail_${Stamp}_${Token}.xls";
    my $DetailTo = "../payroll/${ProvID}_Payroll_Detail_${Stamp}_${Token}_${Marked}.xls";
    $text .= qq|<BR>move Detail_${Stamp} to Detail_${Marked}|;
    move($DetailFrom,$DetailTo) or $text .= "<<< ERROR >>>: Move ${DetailFrom} failed: $!";;
  }
