...
$sPrAuth->execute($PrAuthID) || myDBI->dberror("authPA: select PrAuth ${PrAuthID}");
my $rPrAuth = $sPrAuth->fetchrow_hashref;
my $sPrAuthCDC = $dbh->prepare("select * from ClientPrAuthCDC where ClientPrAuthID=?");
$sPrAuthCDC->execute($PrAuthID) || myDBI->dberror("authPA: select PrAuthCDC ${PrAuthID}");
...
sub html
{
  ...
  $sClient->execute($rPrAuth->{'ClientID'}) || myDBI->dberror("authPA: select Client $rPrAUth->{'ClientID'}");
...
# Start out the display.
  my $html = myHTML->newHTML($form,'Manual Auth','CheckPopupWindow noclock countdown_2') . qq|
<TABLE CLASS="main" >
