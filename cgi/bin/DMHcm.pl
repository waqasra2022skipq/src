...
my $location = qq|Location: /cgi/bin/ClientList.cgi?Provider_ProvID=${ProvID}&Client_ClientID=${ClientID}&mlt=$form->{mlt}&misLINKS=$form->{misLINKS}\n\n|;
...
{ 
  $url = 'https://dmhwebservices.odmhsas.org/ErrorListWebService/Service.asmx';
}
...
# modify the cipher list because of handshake issues with ASP.net
  ...
  my $method = SOAP::Data->name('ProgramEligibilityStringList')->attr({xmlns => 'http://tempuri.org/'});
...
#foreach my $f ( sort keys %{ $node } ) { warn ": node-$f=$node->{$f}\n"; }
      ...
      { 
        ...
        if ( $f eq 'RECIPIENT_ID' ) { $rUpdate->{'RECIPIENTID'} = $node->{$f}; }
        elsif ( $f eq 'TXIX' ) { $rUpdate->{'TXIX'} = $node->{$f}; }
        elsif ( $f eq 'DMH' ) { $rUpdate->{'DMH'} = $node->{$f}; }
        elsif ( $f eq 'REHAB' ) { $rUpdate->{'REHAB'} = $node->{$f}; }
        elsif ( $f eq 'CASEMGMT' ) { $rUpdate->{'CASEMGMT'} = $node->{$f}; }
        elsif ( $f eq 'CASEMGMTUNITS' ) { $rUpdate->{'CASEMGMTUNITS'} = $node->{$f}; }
        elsif ( $f eq 'CCBHC' ) { $rUpdate->{'CCBHC'} = $node->{$f}; }
        elsif ( $f eq 'BAD_ADDRESS' ) { $rUpdate->{'BADADDRESS'} = $node->{$f}; }
        elsif ( $f eq 'VALID_ID' ) { $rUpdate->{'Status'} = $node->{$f}; }
      ...
      $sInsurance->execute($rUpdate->{'RECIPIENTID'}) || myDBI->dberror("DMHcm: select Insurance: $rUpdate->{'RECIPIENTID'}");
      $rInsurance = $sInsurance->fetchrow_hashref;
      $rUpdate->{'ClientID'} = $rInsurance->{'ClientID'};
      $rUpdate->{'StatusDate'} = $form->{'TODAY'};
      $alertmsg = qq|Response is:<BR>|;
      foreach my $f ( sort keys %{ $rUpdate } ) { $alertmsg .= qq|$f=$rUpdate->{$f}<BR>|; }
      if ( $rUpdate->{'Status'} == 0 )
      {
        $alertmsg = qq|Not a valid Member ID!<BR>$rUpdate->{'RECIPIENTID'}|;
        $rUpdate->{'Fail'} = qq|Not a valid Member ID!|;
      }
#foreach my $f ( sort keys %{ $rUpdate } ) { warn ": rUpdate-$f=$rUpdate->{$f}\n"; }
      my $ID = DBA->doUpdate($form,"EligibleDMH",$rUpdate,"RECIPIENTID='$rUpdate->{'RECIPIENTID'}'");
    }
  }
}
...
if ( $error_nodes ne '' ) { DBUtil->email($form,'support@okmis.com',"ERROR DMHcm: $form->{'DBNAME'}: ${ClientID}",$error_nodes); }
print $location;
exit;
  ...
  my @params = (
        SOAP::Data->name('i')->value(
                \SOAP::Data->value(
                        ...
                        SOAP::Data->name('_recipientlist')->value(
                                \SOAP::Data->value(
                                        @clients
                                )
                        )
