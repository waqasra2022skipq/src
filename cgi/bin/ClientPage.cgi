...
my $BackLinks = gHTML->setLINKS($form,'back',2);     # don't pop back to here on this page.
...
$stmt = qq|select Client.*,ClientLegal.JOLTS,ClientEmergency.Alert, ClientEmergency.HospiceCheck from Client left join ClientLegal on ClientLegal.ClientID=Client.ClientID left join ClientEmergency on ClientEmergency.ClientID=Client.ClientID where Client.ClientID=?|;
...
if($rClient->{HospiceCheck} == 1) {
  $Alert .= $rClient->{Alert} eq '' ? '' : qq|<BR><FONT COLOR="red" >Alert: *HOSPICE* $rClient->{Alert}</FONT>|;
} else {
...
$Clinic_Name = $rClinic->{Name} ? $rClinic->{Name} : "<FONT COLOR=red>NO CLINIC ASSIGNED</FONT>";
...
my $AddNote = $ClientInfo ? qq|     <SCRIPT type="text/javascript" >newtextMsg('ClientNote','Use this button to enter a new note for ${ClientName}.');</SCRIPT> <A HREF="/cgi/bin/mis.cgi?MIS_Action=Note&Client_ClientID=$form->{Client_ClientID}&Treatment_TrID=new&${addLinks}" ONMOUSEOVER="textMsg.show('ClientNote');" ONMOUSEOUT="textMsg.hide()" ><IMG BORDER=0 SRC="/images/edit.gif" WIDTH="35" HEIGHT="35" ></A>| : '';
my $PhysNote = $Physician ? qq|     <A HREF="/cgi/bin/mis.cgi?MIS_Action=Note&NoteType=2&Client_ClientID=$form->{Client_ClientID}&Treatment_TrID=new&${addLinks}" ONMOUSEOVER="textMsg.show('physnote')" ONMOUSEOUT="textMsg.hide()" > <IMG SRC="/cgi/images/caduceuswhite.png" WIDTH="24" HEIGHT="24" > </A>| : '';

############################################################################
# output the HTML
$form->{Client_Age} = $Age;          # set for getHTML.
my $ClientPageMenu = $ClientInfo ? myHTML->getHTML($form,'ClientPage.menu',1) : '';
...
if ( $medAlerts ne '' )
{
  $sClientRuleAlerts = $dbh->prepare("select count(*) from ClientRuleAlerts where ClientID=?");
...
my $html = myHTML->newHTML($form,'Client Page','clock mail managertree collapseipad mismenu') . qq|
...
|.myHTML->ListSel($form,'ListClientProblems',$form->{'Client_ClientID'},$form->{'LINKID'},0).qq|
</SPAN>
    </TD>
  </TR>
  <TR>
...
|.myHTML->ListSel($form,"ListClientTrPlan$form->{'FULL'}",$form->{'Client_ClientID'},$form->{'LINKID'},0).qq|
</SPAN>
    </TD>
  </TR>
  <TR>
...
|.myHTML->ListSel($form,"ListClientInsurance$form->{'FULL'}",$form->{'Client_ClientID'},$form->{'LINKID'},0).qq|
</SPAN>
    </TD>
  </TR>
</TABLE>
