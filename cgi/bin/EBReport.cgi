...
my $out = qq|Eligibility for: $form->{FName} $form->{LName} ($form->{ClientID}) ${EBDate}
InsCode\tBenefit\tBenefitDescr\tPlanDescr\tPolicyID\tPolicyName\tOtherInsCode\tOtherInsName\tPhone\tFromDate\tToDate\tRenewDate\n|;
my ($cnt,$hdrline) = (0,2);

my $qEligible=qq|select * from Eligible where ClientID=? and (? between FromDate and ToDate) order by InsCode,Benefit,PlanDescr|;
...
{
  $cnt++;
  $out .= qq|$rEligible->{InsCode}\t$rEligible->{Benefit}\t$rEligible->{BenefitDescr}\t$rEligible->{PlanDescr}\t$rEligible->{PolicyID}\t$rEligible->{PolicyName}\t$rEligible->{OtherInsCode}\t$rEligible->{OtherInsName}\t$rEligible->{Ph}\t$rEligible->{FromDate}\t$rEligible->{ToDate}\t$rEligible->{RenewDate}\n|;
}
