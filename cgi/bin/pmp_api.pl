...
$sClient->execute($ClientID)
  || $form->dberror("DMHws: select Client: ${ClientID}");
my $rClient = $sClient->fetchrow_hashref;
my $ProvID  = $rClient->{'ProvID'};
$sProvider->execute($ProvID)
  || $form->dberror("PMP: select Provider: ${ProvID}");
my $rProvider = $sProvider->fetchrow_hashref;
$sProviderLicenses->execute($ProvID)
  || $form->dberror("PMP: select ProviderLicenses: ${ProvID}");
...
if ( $form->{'method'} eq 'GetOpioidRegistry' ) {
    my %json = ();
    my $patient_link =
      main->patient_request( $rClient, $rProvider, $rProviderLicenses );
    my $report_link =
      main->report_request( $rProvider, $rProviderLicenses, $patient_link );
...
<PatientRequest xmlns="http://xml.appriss.com/gateway/v5_1" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
  ...
  <PrescriptionRequest>
    ...
    if ( $response->is_success ) {
        ...
        $patientLink =
          $data->{Report}->{ReportRequestURLs}->{ViewableReport}->{content};
    }
...
sub generate_nonce {
    ...
    my $nonce =
"@octets[0..3]-@octets[4..5]-@octets[6..7]-@octets[8..+9]-@octets[10..15]";
    $nonce =~ s/ //g;
