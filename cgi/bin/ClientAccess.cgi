...
# Access required
##
if ( !$form->{Client_ClientID} ) { myDBI->error("Client ACL / denied ClientID NULL"); }
...
my $sClient = $dbh->prepare("select Client.*,ClientLegal.JOLTS from Client left join ClientLegal on ClientLegal.ClientID=Client.ClientID where Client.ClientID=?");
...
if ( $form->{UpdateTables} ) { &doUpdate; } else { &doHTML; }
...
#warn "ClientAccess: PPSelect=$PPSelect\n";
  ...
  $clinicClinicSelVal = DBA->selClinics($form,'',$rClient->{clinicClinicID},'',1);
...
#warn qq|PID=$form->{LOGINPROVID}, CID=$rClient->{ProvID}\n|;
  if ( $form->{LOGINPROVID} == $rClient->{ProvID} 
    || $form->{LOGINPROVID} == MgrTree->getManager($form,$rClient->{ProvID}) )
  {
    if ( $form->{LOGINPROVID} == $rClient->{ProvID} ) 
    { $HeaderACL = 'Access List for Primary Provider'; $DisplayACL = 'Providers'; }
    else 
    { $HeaderACL = 'Access List for Immediate Managers'; $DisplayACL = 'Managers / HR'; }
    ...
    { $ACLSelect = DBA->selClinicManager($form,$ACL_ProviderIDs); }
#warn "ClientAccess: ACL_ProviderIDs=$ACL_ProviderIDs\nACLSelect=$ACLSelect\n";
  }
  elsif ( SysAccess->verify($form,'Privilege=Agent') )
  {
    $HeaderACL = 'Access List for Agent'; $DisplayACL = 'All Providers';
    ($ACL_ProviderIDs, $delm) = ('','');
    my $sACL = $dbh->prepare("select * from ClientAccess where ClientID=?");
    $sACL->execute($form->{Client_ClientID});
    while ( $rACL=$sACL->fetchrow_hashref )
...
# selection is just like on FORM
  my $sDelete = $dbh->prepare("delete from ClientAccess where ProvID=? and ClientID=?");
  my $ACList;
  if ( $form->{LOGINPROVID} == $rClient->{ProvID} )
  { $ACList = DBA->selClinicProvider($form,$form->{ACL_ProviderIDs},1); }
  elsif ( $form->{LOGINPROVID} == MgrTree->getManager($form,$rClient->{ProvID}) )
...
#warn "ClientAccess: ACL_ProviderIDs=$form->{ACL_ProviderIDs}\n";
  my $sInsert = $dbh->prepare("insert into ClientAccess (ProvID,ClientID) values (?,?)");
...
#warn "ClientAccess,insert into ClientAccess (ProvID,ClientID) VALUES (${ProvID},$form->{Client_ClientID})\n";
    $sDelete->execute($ProvID,$form->{Client_ClientID}); # needed to delete because of inactive Providers not in list.
...
#   and we just changed the CLientAccess table...
  SysAccess->rebldClientACL($form,$form->{Client_ClientID},$form->{Client_ProvID},1);
  foreach my $ProvID ( split(chr(253),$form->{ACL_ProviderIDs}) )
  { 
#warn "ClientAccess: rebld: ${ProvID}, $form->{Client_ClientID})\n";
    SysAccess->rebldClientACL($form,$form->{Client_ClientID},$form->{Client_ProvID},1,$ProvID);
  }

# Change Primary Provider for this Client?
  if ( $form->{Client_ProvID} && $form->{Client_ProvID} ne $rClient->{ProvID} )
  {
#warn "ClientAccess,update: Client set ProvID='$form->{Client_ProvID}'\n";
    my $sUpdate = $dbh->prepare("update Client set ProvID=? where ClientID=?");
...
# Change Clinic for this Client?
  if ( $form->{Client_clinicClinicID} && $form->{Client_clinicClinicID} ne $rClient->{clinicClinicID} )
  {
#warn "ClientAccess,update: Client set clinicClinicID='$form->{Client_clinicClinicID}'\n";
    my $sUpdate = $dbh->prepare("update Client set clinicClinicID=? where ClientID=?");
    $sUpdate->execute($form->{Client_clinicClinicID},$form->{Client_ClientID});
    $sUpdate->finish();
