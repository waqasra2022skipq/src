...
if ( !$form->{Provider_ProvID} ) {
    $form->error("Provider Triggers Page / denied ProvID NULL");
}
...
sub hPT {
    ...
    my $sProviderTriggers =
      qq|select * from ProviderTriggers where ProvID='${ProvID}'|;
...
<DIV CLASS="blackonwhite" >
  <DIV CLASS="blackonwhite txtleft" >
    ...
    <div data-role="main" class="ui-content"> |
      . myHTML->setCheckBoxColumns( $form, 'zTriggers', $sProviderTriggers,
        'Available Triggers' )
      . qq|
    </div>
  </DIV>
...
sub uPT {
    ...
    my $sDelete =
      $dbh->prepare("delete from ProviderTriggers where ProvID='${ProvID}'");
    ...
    while ( my $r = $szTriggers->fetchrow_hashref ) {
        ...
        if ( $form->{$ID} ) {
            ...
            foreach my $f ( sort keys %{$rProviderTriggers} ) {
                warn "uPT: rProviderTriggers-$f=$rProviderTriggers->{$f}\n";
            }
            my $Insert =
              DBA->genInsert( $form, 'ProviderTriggers', $rProviderTriggers );
