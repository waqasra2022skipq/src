...
"select * from Eligible where Eligible.ClientID=? and Eligible.Benefit=1 and Eligible.PlanDescr = ? and ('${EBDate}' between Eligible.FromDate and Eligible.ToDate)"
);
my $sClientReferrals =
  $dbh->prepare("select * from ClientReferrals where ClientID=?");
my $sClientLegal = $dbh->prepare("select * from ClientLegal where ClientID=?");
my $sClientJournals =
  $dbh->prepare("select * from ClientJournals where ClientID=? and Active=1");
my $sClientDischargeCDC = $dbh->prepare(
"select ClientID,Status,StatusDate,TransType,TransDate,TransTime from ClientDischargeCDC where ClientID=? and Status='Approved' order by StatusDate desc"
);
...
##
$SearchOptions =
  qq|&SearchType=$form->{SearchType}&SearchString=$form->{SearchString}|;
##
...
#warn qq|\nBEGIN: ClientID=$rClient->{ClientID}, StatusColor=$StatusColor, PAColor=$PAColor\n|;
    next
      if (
        !SysAccess->verify( $form, 'hasClientAccess', $rClient->{ClientID} ) );
    ...
    ( $ClientName = qq|$rClient->{LName}, $rClient->{FName} $rClient->{MName}| )
      =~ s/\"/&quot/g;
    $ClientName =~ s/\'//g;
    $ClientName .= ' (' . $rClient->{ClientID} . ')';
    if ( $rClient->{'Active'} ) { $ClientActiveStatus = ''; }
    else {
        $ClientActiveStatus =
          qq| <SPAN STYLE="color: red" >(NotActive)</SPAN><BR>|;
    }

    $sClientDischargeCDC->execute( $rClient->{ClientID} )
      || myDBI->dberror("select ClientDischargeCDC $rClient->{ClientID}");
    if ( my $rClientDischargeCDC = $sClientDischargeCDC->fetchrow_hashref ) {
        my $Active    = $rClient->{'Active'} ? ' [ButActive]' : '';
        my $TransType = DBA->getxref( $form, 'xCDCTransTypes',
            $rClientDischargeCDC->{'TransType'}, 'Descr' );
        my $TransDate =
          $rClientDischargeCDC->{'TransDate'} eq ''
          ? 'Missing Date'
          : DBUtil->Date( $rClientDischargeCDC->{'TransDate'},
            'fmt', 'MM/DD/YYYY' );
        my $TransTime = substr( $rClientDischargeCDC->{'TransTime'}, 0, 5 );
        my $DischargeStatus =
          qq|${TransType} Approved ${TransDate} ${TransTime}${Active}|;
        $ClientDischargeStatus =
          qq| <SPAN STYLE="color: red" >(${DischargeStatus})</SPAN><BR>|;
    }
...
qq|/src/cgi/bin/ClientPage.cgi?Client_ClientID=$rClient->{ClientID}&${addLinks}|;
    $ClinicName =
      ...
      : qq|<SPAN STYLE="color: red" >NO CLINIC ASSIGNED</SPAN>|;
    $PrimaryProvName = "$rProvider->{LName}, $rProvider->{FName}";
    $CustAgency =
      $rClientLegal->{CustAgency} ? $rClientLegal->{CustAgency} : 'none';
    $PrimaryRef = 'none';
    if ( $rClientReferrals->{'ReferredBy1NPI'} ) {
        my $rPriRef = DBA->selxref( $form, 'xNPI', 'NPI',
            $rClientReferrals->{'ReferredBy1NPI'} );
        $PrimaryRef = $rPriRef->{'ProvOrgName'};
    }
    $ClientSigned = $rTrPlan->{TrPlanClSigDate};
    $ClientSigned =
      $ClientSigned ? DBUtil->Date( $ClientSigned, 'fmt', 'MM/DD/YYYY' ) : '';
    $ProviderSigned = $rTrPlan->{TrPlanPhSigDate};
    $ProviderSigned =
      $ProviderSigned
      ? DBUtil->Date( $ProviderSigned, 'fmt', 'MM/DD/YYYY' )
      : '';

    # reset all these...
    $isDMH =
      DBA->isDMH( $form, $rClient->{ClientID} ); # Client/Clinic is DMH Contract
    if ( $rInsurance->{Descr} =~ /medicaid/i ) {
        $StatusColor = $isDMH ? '#CC9966' : '#00CC00';
    }                                            # tan else green
    elsif ( $rInsurance->{Descr} eq 'medicare' ) {
        $StatusColor = '#C0C0C0';
    }                                            # gray
    elsif ( $rInsurance->{Descr} eq 'tricare' ) {
        $StatusColor = '#6633CC';
    }                                            # purple
    elsif ( $rInsurance->{Descr} eq 'bcbsok' ) {
        $StatusColor = '#0066FF';
    }                                            # blue
    elsif ( $rInsurance->{Descr} eq 'bluelinc' ) {
        $StatusColor = '#0066FF';
    }                                            # blue
    elsif ( $rInsurance->{Descr} eq 'healthchoice' ) {
        $StatusColor = '#87CEEB';
    }                                            # skyblue

    elsif ( $rInsurance->{Descr} eq 'mcohhh' ) {
        $StatusColor = '#4C7838';
    }                                            # green
    elsif ( $rInsurance->{Descr} eq 'mcookch' ) {
        $StatusColor = '#CB187D';
    }                                            # pink
    elsif ( $rInsurance->{Descr} eq 'mcoaetnabhok' ) {
        $StatusColor = '#5B2E6F';
    }                                            # purple

    elsif ( ( $rInsurance->{Descr} eq 'dmhsas' ) && !$SQLQuery ) {
        $StatusColor = '#CC9966';
    }    # tan if NOT 'Title 19, Expansion Health ..., Sooner Choice
    elsif ( $rInsurance->{InsType} eq 'private' ) {
        $StatusColor = '#FF66CC';
    }    # pink
    else { $StatusColor = 'white'; $InsColor = 'black'; }
    $InsColor = $StatusColor eq 'white' ? 'black' : $StatusColor;
    ( $PARemDays, $PAColor, $CurPAColor, $EBColor ) =
      ( 0, 'green', 'green', 'black' );
    ( $StatusReason, $ActiveStatus, $PAStatus, $CurPAStatus ) =
      ( '', '', '', '' );

    #warn qq|call0: StatusColor=$StatusColor, PAColor=$PAColor\n|;
    if (   $rInsurance->{Descr} ne 'mcoaetnabhok'
        && $rInsurance->{Descr} ne 'mcookch'
        && $rInsurance->{Descr} ne 'mcohhh' )
    {
        ( $UnitsPopup, $UnitsAvailable ) =
          main->setInventory( $rClient->{ClientID} );
    }
...
|;
        ...
        $str =
qq|select * from Client where ClientID = '${ClientID}' order by LName, FName|;
    }
    elsif ( $form->{'SearchType'} eq 'ClientID' ) {
        $hdr = qq|Client ID: $form->{SearchString}|;
        $str =
qq|select * from Client where ClientID = '$form->{SearchString}' order by LName, FName|;
    }
    elsif ( $form->{'SearchType'} eq 'ClientSSN' ) {
        $hdr = qq|Client SSN: $form->{SearchString}|;
        $str =
qq|select * from Client where SSN = '$form->{SearchString}' order by LName, FName|;
    }
        ...
        $SearchStr =~ s/\.{3}/\%/g;
        $str =
qq|select * from Client where FName like '$SearchStr' order by LName, FName|;
    }
        ...
        $SearchStr =~ s/\.{3}/\%/g;
        $str =
qq|select * from Client where LName like '$SearchStr' order by LName, FName|;
    }
        ...
        $str =
qq|select distinct Client.* from Client left join Insurance on Insurance.ClientID=Client.ClientID where Insurance.InsIDNum ${op} '$SearchStr' and Insurance.InsNumEffDate<=curdate() and (curdate()<=Insurance.InsNumExpDate or Insurance.InsNumExpDate is NULL) order by Client.LName, Client.FName|;
    }
    else { myDBI->error("Client List / Access Denied"); }
...
qq|$rClient->{FName} $rClient->{MName} $rClient->{LName} ($rClient->{ClientID})|
    ...
    my ( $Found, $str, $Renew ) =
      main->chkEligibility( 1, 0, $rInsurance->{InsCode} );
    $StatusColor = $InsColor;

    if (   $rInsurance->{InsID} ne '451'
        && $rInsurance->{InsID} ne '452'
        && $rInsurance->{InsID} ne '453' )
    {
        if ( !$Founnd ) {
...
sub setEligibility {
 ...
 # Check for PlanDesrc = 'Expansion Healthy Adult Program' is same as 'Title 19'
        my ( $EHAP, $EHAPstr, $RenewEHAP ) = main->chkEligibility(
            1, 1,
            $rInsurance->{InsCode},
            'Expansion Healthy Adult Program'
        );

        # Check for PlanDesrc = 'SoonerCare Choice' is same as 'Title 19'
        my ( $SCC, $SCCstr, $RenewSCC ) = main->chkEligibility(
            1, 1,
            $rInsurance->{InsCode},
            'SoonerCare Choice'
        );

        my ( $Title19, $Title19str, $Renew ) =
          main->chkEligibility( 1, 1, $rInsurance->{InsCode}, 'Title 19' );

        $SQLQuery = ( $Title19 || $EHAP || $SCC );

        $EBColor = 'orange' unless ( $Renew eq '' );
        my ( $MHSA, $MHSAstr, $Renew ) = main->chkEligibility(
            1, 1,
            $rInsurance->{InsCode},
        ...
        if ( ( $Title19 || $EHAP || $SCC )
            && $MHSA
          )    # T19, Expansion Healthy Adult Program and MHSA eligibility,
        {
            $str =
                $Title19str
              . $EHAPstr . ' '
              . $SCCstr
              ; # . ' isDMH-' . $isDMH .' - '.$rInsurance->{Descr}.' '.$InsColor;
                # $StatusColor = '#00CC00';  # Set color Green
            $InsColor = '#00CC00';    # Set color Green
            $EBColor  = '#00CC00';
        }
        elsif ( ( $Title19 || $EHAP || $SCC )
            && $isDMH )               # T19 and has DMH contract
        ...
        {
            $str = $MHSAstr;
            $StatusReason .= ' Insurance not Title 19';
            $StatusColor = 'red';
            if ($SQLQuery) { $StatusColor = '#00CC00'; }
        }
        $Found = $str eq '' ? 0 : 1;

    }
    elsif ( $rInsurance->{InsCode} eq 'MB' ) {
        ( $Found, $str, $Renew ) = main->chkEligibility(
            1, 1,
            $rInsurance->{InsCode},
            'Medicare Part B'
        );
        ...
        ( $Found, $str, $Renew ) =
          main->chkEligibility( 1, 1, $rInsurance->{InsCode} );
    }
    ...
    if ( !$Found )    # did not find primary insurance this month
    {
        $StatusColor = 'red'
          if ( $rInsurance->{InsCode} eq 'MC'
            || $rInsurance->{InsCode} eq 'MB' );
        ( $Found, $str, $Renew ) =
          main->chkEligibility( 1, 0, $rInsurance->{InsCode} )
          ;    # any date primary insurance
        if ($Found) {
            $StatusReason .= ' Eligibility Expired for Primary Ins ';
            $EBColor     = 'red';
            $StatusColor = 'red';
        }
        else {
            ( $Found, $str, $Renew ) =
              main->chkEligibility( 1, 1 );    # this date any insurance
            if ($Found) { $StatusReason .= ' Eligibility Not Primary Ins'; }
            else {
                $StatusReason .= ' Eligibility not found.';
                $EBColor    = 'red';
                $isEligible = 0;
...
sub chkEligibility {
    ...
    my $qEligible =
qq|select * from Eligible where Eligible.ClientID='$rClient->{ClientID}' and Eligible.Benefit=${Active}|;
    $qEligible .=
      qq| and ('${EBDate}' between Eligible.FromDate and Eligible.ToDate)|
      if ($today);
    ...
    if ( $rEligible = $sEligible->fetchrow_hashref ) {
        $flag = 1;
        $EBDates =
            DBUtil->Date( $rEligible->{FromDate}, 'fmt', 'MM/DD/YYYY' ) . '-'
          . DBUtil->Date( $rEligible->{ToDate},   'fmt', 'MM/DD/YYYY' );
...
#foreach my $f ( sort keys %{ $inv } ) { warn "  main: inv-${f} is $inv->{$f}\n"; }
        ...
        my $UnitsLeft =
          sprintf( "%.2f", $inv->{UnitsAuth} - $inv->{UnitsUsed} );
        ...
        my $HealthHome =
          ...
          ? qq|<SPAN STYLE="color: green" >Health Home</SPAN>|
          : '';
        my $HealthHomeImg =
          $inv->{'InsDescr'} =~ /healthhome/i
...
qq|<BR>${HealthHomeImg} $inv->{'PAgroup'} <A HREF="javascript: ReportWindow('/src/cgi/bin/PAPeriods.cgi?IDs=${PrAuthID}&mlt=$form->{mlt}','PAPeriods',600,1100)" ONMOUSEOVER="textMsg.show('pa${ClientID}_${PrAuthID}')" ONMOUSEOUT="textMsg.hide()" ><SPAN STYLE="color: ${PAColor}" >$r->{Priority}-$inv->{InsDescr}: $inv->{fPAEffDate}-$inv->{fPAExpDate}: $inv->{PAnumber}: |;
        ...
        if ( $UnitsAuth > 0 ) {
            $popmsg .=
qq|$inv->{AUTHLIST}<BR>${UnitsAuth} - ${UnitsUsed} = ${UnitsLeft} left |;
        }
        elsif ( $AmtLeft == 0 && $ShowAmountsFlag ) {
            $popmsg .=
qq|<BR>&#36;${AmtAuth} - &#36;${AmtUsed} = &#36;${AmtLeft} left (${PctLeft}&#37;) |;
        }
        elsif ( $AmtLeft == 0 ) {
...
qq|<BR>&#36;${AmtAuth} - &#36;${AmtUsed} = &#36;${AmtLeft} left (${PctLeft}&#37;) |;
        ...
        else { $popmsg .= qq|<BR>(${PctLeft}&#37; left)|; }
        $popmsg .= qq|<BR>${PAStatus}|;
        $POPUP .=
qq|<SCRIPT LANGUAGE="JavaScript">newtextMsg('pa${ClientID}_${PrAuthID}','${popmsg}');</SCRIPT>\n|;
        $HREF .= qq|${text}</SPAN></A>|;

        # now the current/monthly period of the PA...
        my $CurPeriod = $inv->{CurPeriod};
        next
          if ( $CurPeriod eq '-' || $CurPeriod eq '' )
          ;    # skip if '-' or FUTURE PA.
        my $INPA  = scalar keys %{ $inv->{$CurPeriod}->{INPA} };
        my $NONPA = scalar keys %{ $inv->{$CurPeriod}->{NONPA} };
        next
          if ( $r->{PAnumber} eq '' && $INPA == 0 && $NONPA == 0 )
          ;    # skip status bar if NOT APPROVED
...
#warn qq|setInventory: PctLeft=$PctLeft\n|;
        my $text =
qq|<A HREF="javascript:void(0);" ONMOUSEOVER="textMsg.show('curpa${ClientID}_${PrAuthID}')" ONMOUSEOUT="textMsg.hide()" ><SPAN STYLE="color: ${CurPAColor}" >${CurPeriod}: |;
        my $popmsg = qq|Current Inventory|;

        #warn qq|setInventory: PAgroup=$inv->{'PAgroup'}\n|;
        if ( $UnitsAuth > 0 ) {
            $text   .= qq|${UnitsLeft} unitsleft |;
            $popmsg .= qq|<BR>${UnitsAuth} - ${UnitsUsed} = ${UnitsLeft} left |;
        }
        elsif ( $AmtLeft == 0 && $ShowAmountsFlag ) {
            $text .= qq|(${PctLeft}&#37; left) |;
            $popmsg .=
qq|<BR>&#36;${AmtAuth} - &#36;${AmtUsed} = &#36;${AmtLeft} left (${PctLeft}&#37;) |;
        }
        elsif ( $AmtLeft == 0 ) {
            $text   .= qq|zero left (${PctLeft}&#37;) |;
            $popmsg .= qq|<BR>Nothing left (${PctLeft}&#37;) |;
        }
        elsif ($ShowAmountsFlag) {
            $text .= qq|&#36;${AmtLeft} left (${PctLeft}&#37;) |;
            $popmsg .=
qq|<BR>&#36;${AmtAuth} - &#36;${AmtUsed} = &#36;${AmtLeft} left (${PctLeft}&#37;) |;
        }
        else {
        ...
        $POPUP .=
qq|<SCRIPT LANGUAGE="JavaScript">newtextMsg('curpa${ClientID}_${PrAuthID}','${popmsg}');</SCRIPT>\n|;
        ...
        foreach my $SCID ( sort keys %{ $inv->{$CurPeriod}->{INPA} } ) {
            ...
            if ( $inv->{$SCID}->{UnitsAuth} > 0 ) {
                ...
                my $UnitsLeft = sprintf( "%.2f",
                    $inv->{$SCID}->{UnitsAuth} - $inv->{$SCID}->{UnitsUsed} );
                if ( $PAReq && $UnitsLeft <= $min ) {
                    ( $Redpfx, $Redsfx ) = ( '<FONT COLOR=red >', '</FONT>' );
                }
            }
            my $popmsg =
qq|INPA<BR>TrID  ContactDate  Units  Amount<BR>$inv->{$CurPeriod}->{$SCID}->{LIST} Total: ${UNITS} |;
            $popmsg .= qq|$inv->{$CurPeriod}->{$SCID}->{AMT} |
              if ($ShowAmountsFlag);
            $POPUP .=
qq|<SCRIPT LANGUAGE="JavaScript">newtextMsg('scid${ClientID}_${PrAuthID}_${SCID}','${popmsg}');</SCRIPT>\n|;
            my $id = $Agent ? qq|[${SCID}]| : '';
            $HREF .=
qq|    <A HREF="javascript:void(0);" ONMOUSEOVER="textMsg.show('scid${ClientID}_${PrAuthID}_${SCID}')" ONMOUSEOUT="textMsg.hide()" >${Redpfx} $inv->{$CurPeriod}->{INPA}->{$SCID} ${id} ${Redsfx}</A>|;
        }
        foreach my $H2017 ( sort keys %{ $inv->{$CurPeriod}->{INPAH2017} } ) {
            ...
            { ( $Redpfx, $Redsfx ) = ( '<FONT COLOR=red >', '</FONT>' ); }
            elsif ($inv->{'PAgroup'} =~ /PG003|PG043/
                && $UNITS > 48 )                                       # Level 2
            { ( $Redpfx, $Redsfx ) = ( '<FONT COLOR=red >', '</FONT>' ); }
            elsif ($inv->{'PAgroup'} =~ /PG004|PG044/
                && $UNITS > 64 )                                       # Level 3
            { ( $Redpfx, $Redsfx ) = ( '<FONT COLOR=red >', '</FONT>' ); }
            my $popmsg =
qq|INPA<BR>TrID  ServiceCode  ContactDate  Units  Amount<BR>$inv->{$CurPeriod}->{H2017}->{LIST} Total: ${UNITS} |;
            $popmsg .= qq|$inv->{$CurPeriod}->{H2017}->{AMT} |
              if ($ShowAmountsFlag);
            $POPUP .=
qq|<SCRIPT LANGUAGE="JavaScript">newtextMsg('scid${ClientID}_${PrAuthID}_H2017','${popmsg}');</SCRIPT>\n|;
            $HREF .=
qq|    <A HREF="javascript:void(0);" ONMOUSEOVER="textMsg.show('scid${ClientID}_${PrAuthID}_H2017')" ONMOUSEOUT="textMsg.hide()" >${Redpfx} $inv->{$CurPeriod}->{INPAH2017}->{$H2017} [combined] ${Redsfx}</A>|;
        }
        foreach my $SCID ( sort keys %{ $inv->{$CurPeriod}->{NONPA} } ) {
            ...
            my $popmsg =
qq|NONPA<BR>TrID  ContactDate  Units  Amount<BR>$inv->{$CurPeriod}->{$SCID}->{LIST} Total: $inv->{$CurPeriod}->{$SCID}->{UNITS} |;
            $popmsg .= qq|$inv->{$CurPeriod}->{$SCID}->{AMT} |
              if ($ShowAmountsFlag);
            $POPUP .=
qq|<SCRIPT LANGUAGE="JavaScript">newtextMsg('scid${ClientID}_${PrAuthID}_${SCID}','${popmsg}');</SCRIPT>\n|;
            my $id = $Agent ? qq|[${SCID}]| : '';
            $HREF .=
qq|    <A HREF="javascript:void(0);" ONMOUSEOVER="textMsg.show('scid${ClientID}_${PrAuthID}_${SCID}')" ONMOUSEOUT="textMsg.hide()" >${Redpfx} $inv->{$CurPeriod}->{NONPA}->{$SCID} ${id} ${Redsfx}</A>|;
        }
        foreach my $F2F ( sort keys %{ $inv->{$CurPeriod}->{INPAF2F} } ) {
            ...
            { ( $Redpfx, $Redsfx ) = ( '<FONT COLOR=green >', '</FONT>' ); }
            elsif ($inv->{'PAgroup'} =~ /G9009/
                && $UNITS > 2 )    # Child Moderate
            { ( $Redpfx, $Redsfx ) = ( '<FONT COLOR=green >', '</FONT>' ); }
            elsif ( $inv->{'PAgroup'} =~ /G9010/ && $UNITS > 4 )    # Child High
            { ( $Redpfx, $Redsfx ) = ( '<FONT COLOR=green >', '</FONT>' ); }
            my $popmsg =
qq|INPA<BR>TrID  ServiceCode  ContactDate  Units  Amount<BR>$inv->{$CurPeriod}->{F2F}->{LIST} Total: ${UNITS} |;
            $popmsg .= qq|$inv->{$CurPeriod}->{F2F}->{AMT} |
              if ($ShowAmountsFlag);
            $POPUP .=
qq|<SCRIPT LANGUAGE="JavaScript">newtextMsg('scid${ClientID}_${PrAuthID}_F2F','${popmsg}');</SCRIPT>\n|;
            $HREF .=
qq|    <A HREF="javascript:void(0);" ONMOUSEOVER="textMsg.show('scid${ClientID}_${PrAuthID}_F2F')" ONMOUSEOUT="textMsg.hide()" >${Redpfx} $inv->{$CurPeriod}->{INPAF2F}->{$F2F} [combined] ${Redsfx}</A>|;
        }
    }
    if ( $CNT == 0 ) {
        $StatusColor = 'red';
        $StatusReason .= ' No Primary PA ';
        $PAColor  = "red";
        $PAStatus = "No PA found!";
    }
    elsif ( $PARemDays > 15 ) { null; }
    elsif ( $PARemDays > 10 ) { $StatusColor = 'orange'; }
    elsif ( $PARemDays > 0 ) {
        $StatusColor = 'red';
        $StatusReason .= ' PA less than 10 days ';
    }
    else {
        $StatusColor = 'red';
        $StatusReason .= ' No Primary Authorized PA ';
    }
...
#warn qq|setPA: Priority=$Priority, PPACnt=$i->{PPACnt}, Color/msg=${PAColor}/${PAStatus}\n|;
    ...
    if ( $i->{PPACnt} == 0 ) { $PAColor = "red"; $PAStatus = "No PA found!"; }
    elsif ( $i->{UnitsAuth} > 0 && $i->{UnitsLeft} <= 0 ) {
        $PAColor  = "red";
        $PAStatus = "No Units available for PA.";
    }
    elsif ( $i->{UnitsAuth} == 0 && $i->{AmtAuth} > 0 && $i->{AmtLeft} <= 0 ) {
        $PAColor  = "red";
        $PAStatus = "PA Authorized Amount used.";
    }
    elsif ( $i->{PARemDays} > 15 ) {
        $PAColor  = "green";
        $PAStatus = "PA has more than 15 days ($i->{PARemDays} days).";
    }
    elsif ( $i->{PARemDays} > 10 ) {
        $PAColor  = "red";
        $PAStatus = "PA has less than 15 days ($i->{PARemDays} days).";
    }
    else {
        $PAColor  = "red";
        $PAStatus = "PA has 10 days or less ($i->{PARemDays} days).";
    }
    ...
    if ( $i->{CurUnitsAuth} > 0 ) {
        $CurPAColor  = $PAColor;
        $CurPAStatus = "Current PA has $i->{CurRemDays} days remaining.";
    }
    elsif ( $i->{CurAmtLeft} > $PctAmtNotify ) {
        $CurPAColor = $PAColor;
        $CurPAStatus =
"Current PA has more than 20&#37; left<BR>and $i->{CurRemDays} days remaining.";
    }
    elsif ( $i->{CurAmtLeft} > 0 ) {
        $CurPAColor = "orange";
        $CurPAStatus =
"Current PA reached less than 20&#37; left<BR>and $i->{CurRemDays} days remaining.";
    }
    else {
        $CurPAColor = "red";
        $CurPAStatus =
"Current PA fell below zero(0)<BR>and $i->{CurRemDays} days remaining.";
    }
