...
##
  ...
  if ( $form->{LOGINACLID} )
  {
    ...
    { $qACL .= qq| ${conj} ProvID='${id}' |; $conj = qq|or|; }
...
#warn qq|ACLID=$form->{ACLID}, q=$qACL\n|;
    ...
    { 
      ...
      $NoMe = 0 if ( index($html,"Provider_ProvID=$form->{LOGINPROVID}") > 0 );
#warn qq|NoMe=$NoMe: $id=$id\n|;
    }
  }
#warn qq|NoMe=$NoMe, LOGINPROVID=$form->{LOGINPROVID}\n|;
  if ( $NoMe )
  { $html .= main->getBranch($form,$form->{LOGINPROVID},$Index); }

  $html .= gHTML->misSiteMsg($form) .
  qq|
    </TD>
  </TR>
</TABLE>
...
sub genHTML
{
  ...
  my $UnRev = $UnRevCnt ? "<FONT COLOR=red >${UnRevCnt}</FONT>"
                        : "<FONT COLOR=black >${UnRevCnt}</FONT>";
  my $UnBill = $UnBillCnt ? "<FONT COLOR=black >${UnBillCnt}</FONT>"
                          : "<FONT COLOR=red >${UnBillCnt}</FONT>";
  my $ClientListURL = qq|/cgi/bin/ClientList.cgi?Provider_ProvID=$r->{ProvID}|;
  my $ClientListIMG = 
  ...
  if ( $r->{Type} == 4 ) { $ProvName = qq|$r->{'LName'}, $r->{'FName'} $r->{'Suffix'}|; }
  my $ProvInfo = '';
  my $JobTitle = '';
  my $Info = '';
...
#warn "genHTML: Name=$ProvName, ProvID=$r->{ProvID}, Index=$r->{Index}, W=$Width\n";
  my $out .= qq|
    <TR>
      |;
  if ( $r->{Active} )
  { $ProvInfo .= qq|${ClientListIMG} ${ChartListIMG}  <FONT COLOR=black >${ProvName}</FONT> |; }
  else
  {
    my $ws = $dbh->quote("${ProvName} Information ID=$r->{ProvID}");
    $ProvInfo .= qq| <A HREF="/cgi/bin/ProviderPage.cgi?Provider_ProvID=$r->{ProvID}&${addURL}" ONMOUSEOVER="window.status=${ws}; return true;" ONMOUSEOUT="window.status=''" >${ProvName}</A> |; 
  }
  $out .= qq|
