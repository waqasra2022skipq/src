...
sub createPages {
    ...
    $p->fit_textline( $YesNoArr{ $rClientEDocs->{"Loinc"} },
        300, $ypos, $basefontoptions );

    my $type = DBA->getxrefWithDef( $form, 'xEDocType', $rClientEDocs->{"Type"},
        'Descr' );
    ...
    $p->fit_textline( $rClientEDocs->{"CreateDate"},
        120, $ypos, $basefontoptions );
    ...
    if ( !( -f $PDF_FilePath ) ) {
        ...
        $p->fit_textline( "$PDF_FilePath Not Found",
            80, $ypos + 15, $basefontoptions );
...
sub createHeader {
    ...
    $sProvider->execute($AgencyID)
      || myDBI->dberror(
        "printClientEDocs: select Provider AgencyID=${AgencyID}");
