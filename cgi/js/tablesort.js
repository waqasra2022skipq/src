...
var Table = (function(){
  ...
  var table = {
    ...
    SortedAscendingClassName:"table-sorted-acend",
    SortedDescendingClassName:"table-sorted-dcend",
  ...
  table.sort = function(o,args) {
    ...
    if (def(tdata.lastcol) && tdata.lastcol==tdata.col && def(tdata.lastdcend)) {
      tdata.dcend = !tdata.lastdcend;
    }
    else {
      tdata.dcend = !!args.dcend;
    }

    // Store the last sorted column so clicking again will reverse the sort order
    tdata.lastcol=tdata.col;
    tdata.lastdcend=!!tdata.dcend;
      ...
      function(cell) {
        if (hasClass(cell,this.SortableClassName)) {
          ...
          if (tdata.col==table.getActualCellIndex(cell) && (classValue(cell,table.SortableClassName))) {
            addClass(cell,tdata.dcend?this.SortedAscendingClassName:this.SortedDescendingClassName);
          }
        }
      }
    );
