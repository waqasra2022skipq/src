...
function AjaxRequest() {
	...
	this.loadXMLDoc = function (url, params) {
		...
		if (req) {
			...
			if (nocache) {
				params +=
					params != "" ? "&" + new Date().getTime() : new Date().getTime();
			}
			if (method == "POST") {
					...
					case "setdefault":
						...
						if (target) {
							document.getElementById(target).value =
								document.getElementById(target).defaultValue;
						}
						break;
...
const toggleSelectBox = (selectChange) => {
	...
	if (anyChecked) {
		...
		if (selectChange == "selectChange") {
			...
			alert(
				"Cannot Select This Service Code as Interactive Therapy is already checked"
			);
		}
	}
};
