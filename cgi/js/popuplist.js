...
//alert("idx="+this.idx+" value="+this.value+" text="+this.text);
  this.set = function(pattern)
  {
    ...
    var params = "method=pAllergy&pattern=" + encodeURIComponent(pattern) + "&value=" + document.ClientAllergies.ClientAllergies_AID_1.value;
...
//alert("idx="+this.idx+" value="+this.value+" text="+this.text);
  this.set = function(pattern)
  {
    var req = new AjaxRequest();
    var script = "/cgi/bin/popup.pl";
    var params = "method=sProblem&pattern=" + encodeURIComponent(pattern) + "&value=" + document.ClientProblems.ICD10Search.value + "&FINDING=" + document.ClientProblems.LIMSEL[0].checked + "&DISORDER=" + document.ClientProblems.LIMSEL[1].checked;
//alert("url="+script+" params="+params+" value="+this.value);
    req.loadXMLDoc(script, params);
  }
}
