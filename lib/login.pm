...
#warn qq|1. RenewLogin=$form->{RenewLogin} = $self->{RenewLogin}\n|;
  $self->renewLogin($form) if ( $form->{RenewLogin} );
# Login comes from Login screen (Login='Login').
# mlt is set upon successful login.
  $self->setLogin($form) if ( $form->{Login} || $form->{mlt} eq '' );
...
# get login from 'mlt' token and 'Login' table.
  unless ( $self->getLogin($form) )
  {
#warn qq|!. login failed: mlt=$form->{mlt}\n|;
    $form->complete();
    print $self->login($form,'Please Login');
    exit;
  }
...
#warn qq|setLogin: LOGINID=$self->{LOGINID}\n|;
  if ( $self->{LOGINID} eq '' )
  { $Location = $self->login($form,'Please Login'); }
  elsif ( $self->{xx} > 3 )
  {
    $self->{xx} = 0;
#warn qq|setLogin: Location=Please Login - too many attempts\n|;
    $Location = $self->login($form,'Please Login - too many attempts');
  }
  else
  {
...
#warn qq|setLogin: LOGINID=$self->{LOGINID}, pass=$self->{pass}\n|;
    ...
    my $q = qq|select * from UserLogin where BINARY UserLogin.loginid='$self->{LOGINID}' and UserLogin.dbname='$form->{DBNAME}' |;
    my $qpass = $dbh->quote($self->{pass});
    $q .= qq|and BINARY UserLogin.Password=${qpass}| if ( $ENV{HTTP_USER_AGENT} );
#warn qq|setLogin: q=${q}\n|;
    my $s = $dbh->prepare($q);
    $s->execute() || $self->error("sql error: setLogin checkpass");
...
#warn qq|setLogin: loginid=$r->{loginid}, renew=$r->{renew}\n|;
      $s->finish();
      $self->renewPasswd($form,$r) if ( $r->{renew} && $ENV{HTTP_USER_AGENT} );
      ...
      my $s = $dbh->prepare("insert into Login (Token,UserID,Name,Browser) values ('$self->{mlt}','$self->{LOGINUSERID}','$self->{LOGINID}','$self->{Browser}')");
      $s->execute() || $self->error("sql error: setLogin insert");
      $s->finish();
      my $LoginScreen = $form->{'LOGINSCREEN'} eq ''
                      ? $r->{'loginscreen'} eq '' ? 'ManagerTree.cgi' : $r->{'loginscreen'}.'.cgi'
                      : $form->{'LOGINSCREEN'};
#warn qq|login: setLogin: LOGINSCREEN=$form->{'LOGINSCREEN'}\n|;
#warn qq|login: setLogin: loginscreen=$r->{'loginscreen'}\n|;
#warn qq|login: setLogin: LoginScreen=${LoginScreen}\n|;
      $Location = qq|Location: /cgi/bin/${LoginScreen}?mlt=$self->{mlt}\n\n|;
#warn qq|login: setLogin: Location=$Location\n|;
#warn qq|login: setLogin: ID=$r->{ID}\n|;
...
#warn qq|setLogin: Location=Login Failed!\n|;
      $Location = $self->login($form,'Login Failed!');
    }
  }
...
#warn qq|setLogin: END: Location=${Location}\n|;
  ...
  { $form->complete(); print $Location; exit; }
  return($form);
}
...
#warn qq|getLogin: self mlt=$self->{mlt}\n|;
  my $sLogin = $dbh->prepare("select * from Login left join UserLogin on UserLogin.UserID=Login.UserID where Login.Token='$self->{mlt}'");
...
#foreach my $f ( sort keys %{$rLogin} ) { warn qq|getLogin 1: rLogin: $f=$rLogin->{$f}\n|; }
    ...
    $self->{LOGINPROVID} = $rLogin->{UserID};    # preserve until all changed to LOGINUSERID
    my $Browser = $ENV{HTTP_USER_AGENT} ? $ENV{HTTP_USER_AGENT} : 'Telnet Session';
    ($self->{Browser},$rest) = split(/ /,$Browser);
    my $sReplace = $dbh->prepare("replace into Login (Token,UserID,Name,Browser) values ('$self->{mlt}','$self->{LOGINUSERID}','$self->{LOGINID}','$self->{Browser}')");
    $sReplace->execute() || $self->error("getLogin: replace");
    $sReplace->finish();
#warn qq|login: getLogin: reread: ID=$rLogin->{ID}\n|;
#warn qq|login: getLogin: reread: loginscreen=$rLogin->{loginscreen}\n|;
#warn qq|login: getLogin: reread: LOGINSCREEN=$form->{LOGINSCREEN}\n|;
    ...
    #$form->{LOGINSCREEN} = $form->{'LOGINSCREEN'} eq ''
    #                     ? $rLoginr->{'loginscreen'} eq '' ? 'ManagerTree.cgi' : $rLogin->{'loginscreen'}.'.cgi'
    #                     : $form->{'LOGINSCREEN'};
...
#warn qq|getLogin: return1: ok=$ok, DBNAME=$form->{'DBNAME'} LOGINPROVID=$form->{'LOGINPROVID'}\n|;
  ...
  { my $syslock = 0; return($ok) unless ( $syslock ); }     # stop ALL account access.
#warn qq|getLogin: exit: ok=$ok, DBNAME=$form->{DBNAME}\n|;
  $form->complete();
  print $self->login($form,'System Unavailable: Upgrades/Enhancements: <BR>Please try again around 5am on 03/05/2019.');
  exit;
}
############################################################################
sub renewPasswd
...
#warn qq|renewPasswd: xx=$self->{xx}, newpass=$self->{newpass}, verpass=$self->{verpass}\n|;
  ...
  elsif ( $self->{newpass} eq $self->{pass} ) { $msg =  "Please change your Password."; }
  elsif ( $self->{newpass} ne $self->{verpass} ) { $msg =  "Passwords don't match, please reenter both."; }
  elsif ( length($self->{newpass}) < 6 ) { $msg =  "Passwords must be at least 6 characters."; }
  elsif ( $self->{newpass} !~ /[A-Z]/ ) { $msg = qq|Passwords must contain at least 1 capital letter.\n|; }
  elsif ( $self->{newpass} !~ /[a-z]/ ) { $msg = qq|Passwords must contain at least 1 lower-case letter.\n|; }
  elsif ( $self->{newpass} !~ /\d/ ) { $msg = qq|Passwords must contain at least 1 digit.\n|; }
  elsif ( $self->{newpass} !~ /[\@\#\%\=\-\+\:\.\!\_]/ ) { $msg = qq|Passwords must contain at least 1 special character.\n|; }
  elsif ( $self->{newpass} )
  {
...
#warn qq|renewPasswd: SET: Password=$self->{newpass}, UserID=$rPasswd->{UserID}, dbname=$form->{DBNAME}\n|;
    my $dbh = myDBI->dbconnect($form->{'DBNAME'});
    my $s = $dbh->prepare("update UserLogin set Password='$self->{newpass}', renew=0 where UserID=$rPasswd->{UserID} and dbname='$form->{DBNAME}'");
    $s->execute() || $self->error("sql error: renewPasswd update ");
    $s->finish();
    return(1);      # good password change.
  }
  elsif ( $self->{xx} > 3 )
  { $renew = 0; $self->{xx} = 0; $msg = 'Please Login - too many renew attempts'; }
  $form->complete();
  print $self->login($form,$msg,$renew);
  exit;
}
sub renewLogin
{ 
...
#warn qq|renewLogin: DBNAME=$form->{DBNAME}, user=$form->{user}\n|;
  ...
  {
    my $dbh = myDBI->dbconnect($form->{'DBNAME'});
    my $sUserLogin = $dbh->prepare("select * from UserLogin where BINARY UserLogin.loginid='$form->{user}' and dbname='$form->{DBNAME}'");
    $sUserLogin->execute() || $self->error("renewLogin: $form->{user} $form->{DBNAME}");
    if ( my $rUserLogin = $sUserLogin->fetchrow_hashref )
    {
      my ($DB,$ID) = split(':',$rUserLogin->{'ID'});
      my $mdbh = myDBI->dbconnect($DB);
      my $qUser = $rUserLogin->{'type'} == 0
...
#warn qq|renewLogin: DB=${DB}, ID=${ID}, qUser=${qUser}\n|;
      ...
      $sUser->execute($rUserLogin->{'UserID'}) || $self->error("renewLogin: ${qUser} $rUserLogin->{'UserID'}");
...
#warn qq|renewLogin: ssn=$form->{ssn}, SSN=$rUser->{SSN}\n|;
      if ( $form->{ssn} eq $rUser->{SSN} )
      {
        ...
        my $sUpd = $dbh->prepare("update UserLogin set Password='${Password}', renew=1 where UserID=$rUserLogin->{UserID} and dbname='$form->{DBNAME}'");
        $sUpd->execute() || $self->error("renewLogin: ERROR: update $form->{user}/$rUserLogin->{UserID}, Password=${Password}");
        $sUpd->finish();
        $msg = qq|Temporary password sent via email.<BR>use that password to login.|;
        ...
        $form->saveLINK('renewLogin',"$ENV{SCRIPT_NAME}?$ENV{QUERY_STRING}");
      }
  ...
  {
    $msg = qq|Enter 'LOGIN ID' to reset.<BR>Email with temporary password will be sent to provider's email on file.|;
    $renew=2;
  }
  $form->complete();
  print $self->login($form,$msg,$renew);
...
{ 
  ...
  $s->execute() || $self->error("sql error: chklogin SITENAME");
  ($self->{SITENAME}) = $s->fetchrow_array;
  $self->{SITENAME} = "DEVELOPMENT<BR>$self->{SITENAME}<BR>DEVELOPMENT<BR>DO NOT USE THIS SITE FOR PRODUCTION WORK" if ( substr($form->{DBNAME},0,3) eq 'dev' );
  $self->{SITENAME} = "DEVELOPMENT<BR>$self->{SITENAME}<BR>DEVELOPMENT<BR>DO NOT USE THIS SITE FOR PRODUCTION WORK" if ( $form->{DBNAME} =~ /_dev/ );
#warn qq|login: DBNAME=$form->{DBNAME}\n|;
  $s->finish();
  $self->{LOGO} = myConfig->cfgfile('logo',1);
  $self->{LOGINSCREEN} = $form->{LOGINSCREEN};
  return($self->logintxt($msg,$renew));
}
sub logintxt
{
  my ($self,$msg,$renew) = @_;
  my $fn = "/home/okmis/mis/src/html/$self->{DBNAME}.login";
  unless ( -f $fn ) { $fn = qq|/home/okmis/mis/src/html/default.login| }
  if ( $renew == 1 )
  { 
    $fn = "/home/okmis/mis/src/html/$self->{DBNAME}.renewpasswd";
    unless ( -f $fn ) { $fn = qq|/home/okmis/mis/src/html/default.renewpasswd| }
  }
  elsif ( $renew == 2 )
  { 
    $fn = "/home/okmis/mis/src/html/$self->{DBNAME}.renewlogin";
    unless ( -f $fn ) { $fn = qq|/home/okmis/mis/src/html/default.renewlogin| }
  }
