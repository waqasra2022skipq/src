...
sub vNoteServiceType {
    ...
    my $qxSC =
qq|select xSC.SCID, xSC.SCNum, xInsurance.Name, xSC.SCName, xSC.ServiceType, xCredentials.Abbr, xSCRestrictions.Descr as ResDescr, xSC.Restriction from xSC left join xInsurance on xInsurance.ID=xSC.InsID left join okmis_config.xCredentials on xCredentials.ID=xSC.CredID left join okmis_config.xSCRestrictions on xSCRestrictions.ID=xSC.Restriction where xSC.Active=1 |;
    if ( $ReqAccess =~ /INSID=ALL/i ) {
        null;
    }    # allow call for all insurance service codes - ProviderPay
    elsif ( $ReqAccess =~ /INSID=/i
      )    # allow call for one insurance service codes - xSC, xPAgroups, etc.
    {
        my ( $tag, $InsID ) = split( '=', $ReqAccess );
        $qxSC .= qq|and xSC.InsID='${InsID}' |;
    }
    else {
        $qxSC .= DBA->withCredIDs( $form, $ForProvID, 'xSC', 'and' )
          . DBA->withClientInsIDs( $form, $ClientID, 1, 'xSC', 'and' );
    }
    my $qAll = $qxSC
      . qq| order by xInsurance.Name, xSC.ServiceType, xSC.SCName, xSC.SCNum|;
...
qq|${delm0}\nnew Array(0,$r->{SCID},"$r->{Name} $r->{ServiceType} $r->{SCName} $r->{SCNum} \($r->{Abbr}\) \($r->{ResDescr}\)")|;
    ...
    my $delm1 = '';
    my $qSF =
qq|select ServiceType from xSC where ServiceType is not null group by ServiceType|;
    ...
    while ( my ($ServiceType) = $sSF->fetchrow_array ) {
        ...
        my $delm2 = '';
        my $q     = $qxSC
          . qq| and (ServiceType=? or ServiceType is null) order by xInsurance.Name, xSC.ServiceType, xSC.SCName, xSC.SCNum|;
...
sub setNoteMsg {
    ...
    if ( $form->{Treatment_TrID_1} eq '' ) {
        return (
qq|<TABLE CLASS="home fullsize" ><TR ><TD CLASS="strcol" >** New Note</TD></TR></TABLE>|
        );
    }
    my $dbh = myDBI->dbconnect( $form->{'DBNAME'} );
    my $ProvName =
      DBA->getxref( $form, 'Provider', $form->{'Treatment_ProvID_1'},
        'FName LName' );
    my $EnteredBy =
      DBA->getxref( $form, 'Provider', $form->{'Treatment_EnteredBy_1'},
        'FName LName' );
    my $out = qq|
...
|;
    if ( SysAccess->getRule( $form, 'NoteEdit', $form->{Treatment_ProvID_1} ) )
    {
        ...
        my $msg =
          $d eq ''
          ? qq|Not yet approved by ${ProvName}|
          : qq|OK'd by ${ProvName} on ${d} at ${t}|;
        $out .= qq|
...
|;
    ...
    $sSCID->finish();
    my $InsText =
      $InsName ? qq|Service for ${InsName} Insurance| : qq|NO INSURANCE!|;
    $out .= qq|  <TR ><TD CLASS="strcol" >** ${InsText}</TD></TR>\n|;

    # check for Medication Allergies...
    $sClientIntake = $dbh->prepare(
"select * from ClientIntake where ClientID='$form->{Treatment_ClientID_1}'"
    );
    $sClientIntake->execute()
      || myDBI->dberror(
        "setNoteMsg: select ClientIntake $form->{Treatment_ClientID_1}");
    my $rClientIntake = $sClientIntake->fetchrow_hashref;
    my $MAR =
      $rClientIntake->{MAR} eq ''
      ? ''
    ...
    if ( $form->{Treatment_ClinicID_1} eq '' ) {
        $msg =
qq|<FONT COLOR=red>NO CLINIC ASSIGNED TO CLIENT</FONT>Notes prohibited!|;
    }
        ...
        $msg =
qq|Note has been signed by Provider and Billed. Text portion of note cannot be changed.|;
    }
    elsif ( $form->{Treatment_BillStatus_1} == 6 ) {
        ...
        $msg =
qq|Note has been Billed and is In Process. It cannot be changed until denied.|;
    }
        ...
        $msg =
qq|Note has been reviewed and Locked (to change note must be UNreviewed by Manager).|;
    }
...
|;
    my $review =
      DBA->getxref( $form, 'Provider', $form->{Treatment_MgrProvID_1},
        'FName LName' );
    my $status =
      DBA->getxref( $form, 'xBillStatus', $form->{Treatment_BillStatus_1} );
    $out .= qq|
  <TR >
...
<TABLE CLASS="home fullsize" >
|;
    my $SCIDSel = DBA->selServiceCodes(
        $form, $form->{'Treatment_SCID_1'},
        0,
        $form->{'LOGINPROVID'},
        $form->{'Client_ClientID_1'}, 'Agent'
    );
    my $CurType =
      DBA->getxref( $form, 'xSC', $form->{Treatment_SCID_1}, 'Type' );
    my $ClinicName =
      DBA->getxref( $form, 'Provider', $form->{'Treatment_ClinicID_1'},
        'Name' );
    my $ProvName =
      DBA->getxref( $form, 'Provider', $form->{'Treatment_ProvID_1'},
        'FName LName' );
 ...
 # LOCKED note...unless mine or reviewok or Inprocess/Scholarshipped/Reconciled.
    ...
    warn
qq|ClientID=$form->{Treatment_ClientID_1}, TrID=$form->{Treatment_TrID_1}, EntBy=$EntBy, MyNote=$MyNote, RevOK=$RevOK, BS=$BillStatus\n|
      if ($debug);
    my $Locked = ( !$EntBy && !$MyNote && !$RevOK )
      || ( $BillStatus == 3 || $BillStatus == 4 || $BillStatus == 5 ) ? 1 : 0;
...
#  my $Locked = $form->{Treatment_BillStatus_1} == 3 || $form->{Treatment_BillStatus_1} == 4 || $form->{Treatment_BillStatus_1} == 5 ? 1 : 0;
    ...
    if ( ${Locked} ) {
        ...
        my $SCNum =
          DBA->getxref( $form, 'xSC', $form->{Treatment_SCID_1}, 'SCNum' );
        my $Mod4 =
          DBA->getxref( $form, 'xSCMod4', $form->{Treatment_Mod4_1}, 'Descr' );

        #warn qq| Mod4=$form->{Treatment_Mod4_1}/${Mod4}\n|;
        my $SCName =
          DBA->getxref( $form, 'xSC', $form->{Treatment_SCID_1}, 'SCName' );
        my $InsID =
          DBA->getxref( $form, 'xSC', $form->{Treatment_SCID_1}, 'InsID' );
        my $InsName = DBA->getxref( $form, 'xInsurance', $InsID, 'Name' );
        my $CredID =
          DBA->getxref( $form, 'xSC', $form->{Treatment_SCID_1}, 'CredID' );
        my $CredAbbr = DBA->getxref( $form, 'xCredentials', $CredID, 'Abbr' );
        my $RestID   = DBA->getxref( $form, 'xSC', $form->{Treatment_SCID_1},
            'Restriction' );
        my $ResDescr =
          DBA->getxref( $form, 'xSCRestrictions', $RestID, 'Descr' );
        my $SCID3 = '';

        if ( $form->{Treatment_SCID3_1} ) {
            my $SC3Num =
              DBA->getxref( $form, 'xSC', $form->{Treatment_SCID3_1}, 'SCNum' );
            my $SC3Name =
              DBA->getxref( $form, 'xSC', $form->{Treatment_SCID3_1},
                'SCName' );
            $SCID3 = qq|
  <TR >
    <TD COLSPAN="3" >
...
|;
        ...
        my $ServTypeSel =
          SysAccess->getRule( $form, 'NoteServiceType' )
          ? qq|<TR><TD COLSPAN="3" ><SELECT NAME="Treatment_ServiceType" ONCHANGE="setSelect(this,servicetypearray,this.form.Treatment_SCID_1)" > |
          . DBA->selNoteServiceTypes( $form, $form->{Treatment_ServType} )
          . qq| </SELECT></TD></TR>|
          : qq|<INPUT TYPE="hidden" NAME="Treatment_ServType" VALUE="$form->{'Treatment_ServType'}" >|;
##    $ServTypeSel = $form->{LOGINID} eq 'root' ? qq|<TR><TD COLSPAN="3" ><SELECT NAME="Treatment_ServiceType" ONCHANGE="setSelect(this,servicetypearray,this.form.Treatment_SCID_1)" > | . DBA->selNoteServiceTypes($form,$form->{Treatment_ServType}) . qq| </SELECT></TD></TR>| : $ServTypeSel;
       # Mod4 is add modifier for TeleMedicine: i.e.: GT modifier (from xSCMod4)
        ...
        my $Mod4Sel =
          SysAccess->chkPriv( $form, 'TeleMedicine',
            $form->{'Treatment_ProvID_1'} )
        ...
        my $POSSel =
          DBA->selxTable( $form, 'xPOS', $form->{'Treatment_POS_1'} );
        my $SCID3 = '';
        if ( $form->{Treatment_SCID3_1} ) {
            my $SC3Num =
              DBA->getxref( $form, 'xSC', $form->{Treatment_SCID3_1}, 'SCNum' );
...
|;
        my $BTime =
          gHTML->ifld( $form, 'Treatment_ContLogBegTime_1', 'displayonly' );
        my $ETime =
          gHTML->ifld( $form, 'Treatment_ContLogEndTime_1', 'displayonly' );
        $out .= qq|
  <TR >
    <TD WIDTH="50%" >&nbsp;</TD>
...
# Rebill note...check for something that shows it was denied...gone thru billing process.
            if ( $form->{Treatment_BillStatus_1} > 5 ) {
                $out .=
qq|      <INPUT TYPE="submit" NAME="Treatment_BillStatus_1=1&Treatment_StatusDate_1=$form->{TODAY}&Treatment_CIPDate_1=&Treatment_DenDate_1=&Treatment_DenCode_1=&UpdateTables=all&rebill_note=1&misPOP=1" VALUE="Update/Rebill" > |;
                $BillCnt++;
            }
...
# NO: write off anything? even a new note that will not be billed(0) or is not billable(2), even AmtDue=0 (NonBill).
            if (   $form->{Treatment_BillStatus_1} < 3
                || $form->{Treatment_BillStatus_1} > 5 )
            {
                $out .=
qq|      <INPUT TYPE="submit" NAME="scholarship_note=1&misPOP=1" VALUE="Scholarship" >|;
                $BillCnt++;
            }
        }
        $out .= qq|
...
|
      . $self->setClientNoteProblems( $form, $Locked, $ClientID, $TrID,
        $ContDate )
      . qq|
      </SPAN>
    </TD>
...
|
      . $self->setClientNoteTrPlanPG( $form, $Locked, $ClientID, $TrID,
        $ContDate )
      . qq|
      </SPAN>
    </TD>
...
|;
    ...
    my $header =
      ${Locked} ? 'Problems addressed' : 'Check all problems addressed';
    my $blur =
      $Locked ? qq|ONCLICK="return vUNDO(this,'Note has been locked.');" | : '';
    $out .= qq|
<TABLE CLASS="home fullsize" >
...
#  my $Locked = $form->{Treatment_BillStatus_1} > 0 || $form->{Treatment_MgrRevDate_1} ? 1 : 0;
    $Locked = 0 if ( $form->{LOGINPROVID} == 91 );
    if ( $NoteType == 1 ) {
        $out .= gHTML->setNoteTxt1( $form, $Locked );
    }    # Progress

#  elsif ( $NoteType == 2 ) { $out .= gHTML->setNoteTxt2($form,$Locked); }         # Physician
    elsif ( $NoteType == 3 ) {
        $out .= gHTML->setNoteTxt3( $form, $Locked );
    }    # Electronic
    elsif ( $NoteType == 4 ) {
        $out .= gHTML->setNoteTxt4( $form, $Locked );
    }    # Medicare
    elsif ( $NoteType == 5 ) {
        $out .= gHTML->setNoteTxt5( $form, $Locked );
    }    # TFC
    return ($out);
}
...
|;
    my $Techs = '<BR>'
      . DBA->matchNoteInt(
        $form,
        $form->{'ProgNotes_Intervention_1'},
        $form->{'ProgNotes_Techniques_1'}
      );
    $Techs =~ s/;/<BR>/g;
...
sub setAddional_SCID {
    ...
    if (   $form->{'NoteType'} ne 2
        && $form->{'NoteType'} ne 3
        && $rInsurance->{InsID} ne '212'
        && !$Physician )
    {
# Bail If Client Primary insurance is not Medicare and Not Physician Note and Electronic Note
    ...
    if ( $SCID_Num eq '2' ) {
        ...
        $desc =
'Alternative Secondary Service Code - Service Name (use ONLY if Interactive Therapy is NOT selected)';
    }
  ...
  |;
    if ($Locked) {
        ...
        my $CredAbbr = DBA->getxref( $form, 'xCredentials', $CredID, 'Abbr' );
        my $RestID =
          DBA->getxref( $form, 'xSC', $form->{$column}, 'Restriction' );
        my $ResDescr =
          DBA->getxref( $form, 'xSCRestrictions', $RestID, 'Descr' );
        $out .= qq|
      <TR >
    ...
    |;
        ...
        my $add_SC_IDS =
"(15970, 12785, 12789, 12788, 15888, 21483, 15756, 21500, 21501, 21502, 21503, 21504, 21505, 21506)";
        my $SCIDSel = DBA->selServiceCodes(
            $form,
            $form->{$column},
            0,
            ...
            'Agent',
"and xSC.SCNum NOT LIKE 'X%' and (xSC.SCID IN $add_SC_IDS OR `SCNum` LIKE '%90833%' OR `SCNum` LIKE '%96372 25%' OR `SCNum` LIKE '%93000 25%'  OR `SCNum` LIKE '%90840%' OR `SCNum` LIKE '%9920 33%')"
        );
        $out .= qq|
      <TR >
        <TD >
...
sub setIntComp {
    ...
    my $IndPrimaryProvID =
      DBA->isIndividual( $form, $form->{'Client_ProvID_1'} );
...
sub setClientNoteProblems {
    ...
    my $with =
      $ContDate eq ''
      ? ''
      : qq| and ClientProblems.InitiatedDate <= '${ContDate}' and (ClientProblems.ResolvedDate is null or ClientProblems.ResolvedDate <= '${ContDate}')|;
    my $out .= qq|
<TABLE CLASS="home fullsize" >
  <TR >
...
##
    my $s = $dbh->prepare(
"select misICD10.ICD10,misICD10.icdName,misICD10.sctName,ClientNoteProblems.UUID,ClientNoteProblems.Priority,ClientNoteProblems.InitiatedDate,ClientNoteProblems.ResolvedDate from ClientNoteProblems inner join okmis_config.misICD10 on misICD10.ID=ClientNoteProblems.UUID where ClientNoteProblems.ClientID=? and ClientNoteProblems.TrID=? order by ClientNoteProblems.Priority"
    );
    $s->execute( $ClientID, $TrID )
...
"select * from ClientNoteTrPlanPG where ClientID=? and TrID=? and TrPlanPGID=?"
    );
    my $sPG = $dbh->prepare(
        "select * from ClientTrPlanPG where TrPlanID=? order by Priority");
    my $sOBJ = $dbh->prepare(
    ...
    my $s = $dbh->prepare(
"select * from ClientTrPlan where ClientID=? and Locked=1 ${with} order by EffDate desc"
    );
    $s->execute($ClientID)
      || myDBI->dberror("setClientNoteTrPlan: select ClientTrPlan ${ClientID}");
    while ( my $r = $s->fetchrow_hashref ) {
        $rows++;
        my $class    = int( $rows / 2 ) == $rows / 2 ? 'rpteven' : 'rptodd';
        my $TrPlanID = $r->{'ID'};
        $sPG->execute($TrPlanID)
          || myDBI->dberror(
            "setClientNoteTrPlan: select ClientTrPlanPG ${TrPlanID}");
        while ( my $rPG = $sPG->fetchrow_hashref ) {
            ...
            $sClientNoteTrPlanPG->execute( $ClientID, $TrID, $PGID )
              || myDBI->dberror(
"setClientNoteTrPlan: select ClientNoteTrPlanPG ${ClientID} ${TrID} ${PGID}"
              );
            if ( my $rTest = $sClientNoteTrPlanPG->fetchrow_hashref ) {
                $Checked = 'CHECKED';
            }
            $out .= qq|
  <TR >
...
|;
            $sOBJ->execute( $rPG->{'ID'} )
              || myDBI->dberror(
                "setClientNoteTrPlan: select ClientTrPlanOBJ $r->{ID}");
...
|;
    ...
    my $dateheader =
      $rows
      ? qq|${effdate} - ${expdate}|
      : qq|NO Treatment Plan FOUND!<BR>(possibly not signed by Primary Provider?)|;
    my $html = qq|
<TABLE CLASS="home fullsize" >
...
##
    ...
    #   if there then it was CHECK when note was saved last time
##
    my $sClientNoteFamilyI = $dbh->prepare(
"select * from ClientNoteFamilyI where ClientID=? and TrID=? and ClientFamilyID=?"
    );
    my $sClientNoteFamilyP = $dbh->prepare(
"select * from ClientNoteFamilyP where ClientID=? and TrID=? and ClientFamilyID=?"
    );
    my $sClientFamily = $dbh->prepare(
"select ClientFamily.*,xRelationship.Descr as Relation from ClientFamily left join okmis_config.xRelationship on xRelationship.ID=ClientFamily.Rel where ClientID=? order by Age"
    );
    $sClientFamily->execute($ClientID)
      || myDBI->dberror(
        "setClientNoteFamilyP: select ClientFamily ${ClientID}");
    while ( my $rClientFamily = $sClientFamily->fetchrow_hashref ) {
        ...
        $sClientNoteFamilyI->execute( $ClientID, $TrID, $rClientFamily->{'ID'} )
          || myDBI->dberror(
"setClientNoteFamilyI: select ClientNoteFamilyI ${ClientID} ${TrID} $rClientFamily->{'ID'}"
          );
        if ( my $rTest = $sClientNoteFamilyI->fetchrow_hashref ) {
            $CheckedI = 'CHECKED';
        }
        my $CheckedP = '';
        $sClientNoteFamilyP->execute( $ClientID, $TrID, $rClientFamily->{'ID'} )
          || myDBI->dberror(
"setClientNoteFamilyP: select ClientNoteFamilyP ${ClientID} ${TrID} $rClientFamily->{'ID'}"
          );
        if ( my $rTest = $sClientNoteFamilyP->fetchrow_hashref ) {
            $CheckedP = 'CHECKED';
        }
        $out .= qq|
  <TR CLASS="${class}" >
...
#warn qq|gHTML.pm: TrID=$form->{Treatment_TrID_1}, BillStatus=$form->{Treatment_BillStatus_1}\n|;
    ...
    elsif ( $form->{Treatment_ClinicID_1} eq '' ) { null; }
    else    # some update ok.
    {
        #   DELETE?  # must exist and new notes only.
        if (
            $form->{Treatment_TrID_1}
            && (   $form->{Treatment_BillStatus_1} == 0
                || $form->{Treatment_BillStatus_1} == 1
                || $form->{Treatment_BillStatus_1} == 2 )
          )
        {
            $out .=
qq|      <INPUT TYPE="submit" ONCLICK="return vDELETE('Are you sure you want to delete this note?')" NAME="delete_note=1&Treatment_DELETE_1=1&UpdateTables=all&misPOP=1" VALUE="Delete Note" > |;
        }
...
#warn qq|gHTML: ProvID=LOGINPROVID: RevStatus=$form->{Treatment_RevStatus_1}\n|;
            if ( $form->{Treatment_RevStatus_1} == 1 ) {
                $out .=
qq| <INPUT TYPE="submit" ONCLICK="return validate(this.form)" NAME="approve_note=1&misPOP=1" VALUE="Approve Changes" > |;
                $out .=
qq| <INPUT TYPE="submit" ONCLICK="return validate(this.form)" NAME="reject_note=1&misPOP=1" VALUE="Reject Changes" > |;
            }
            elsif ($form->{Treatment_RevStatus_1} == 0
                || $form->{Treatment_RevStatus_1} == 2 )
            {
                if ( SysAccess->verify( $form, 'Privilege=NoteReview' ) ) {
                    $out .=
qq| <INPUT TYPE="submit" ONCLICK="return validate(this.form)" NAME="UpdateTables=all&manager_review=1&misPOP=1" VALUE="Manager Update/Review" > |;
                }
                $out .=
qq| <INPUT TYPE="submit" ONCLICK="return validate(this.form)" NAME="UpdateTables=all&misPOP=1" VALUE="Add/Update" > |;
            }
            elsif ($form->{Treatment_RevStatus_1} == 3
                && $form->{Treatment_BillStatus_1} < 3 )
            {
                if ( $form->{Treatment_MgrProvID_1} ==
                    $form->{LOGINPROVID} )    # must have had NoteReview access.
                {
                    $out .=
qq| <INPUT TYPE="submit" ONCLICK="return validate(this.form)" NAME="UpdateTables=all&manager_unreview=1&misPOP=1" VALUE="Manager UNREVIEW" > |;
                }
            }
        }
...
qq| <INPUT TYPE="submit" ONCLICK="return validate(this.form)" NAME="UpdateTables=all&misPOP=1" VALUE="Add/Update" > |;
      ...
      # entered by Data Entry - wait on Provider
        ...
        # someone comes in before reviewed to update (considered Manager review)
            elsif ( $form->{Treatment_RevStatus_1} ==
                2 )    # can't allow just update, then skips Logging.
            {
                $out .=
qq| <INPUT TYPE="submit" ONCLICK="return validate(this.form)" NAME="UpdateTables=all&manager_review=1&misPOP=1" VALUE="Manager Update/Review" > |;
            }
...
# Manager comes back to update, has to UNREVIEW note first (comes back in above)
            elsif ($form->{Treatment_MgrProvID_1} == $form->{LOGINPROVID}
                && $form->{Treatment_BillStatus_1} < 3 )
            {
                $out .=
qq| <INPUT TYPE="submit" ONCLICK="return validate(this.form)" NAME="UpdateTables=all&manager_unreview=1&misPOP=1" VALUE="Manager UNREVIEW" > |;
...
|;
    ...
    if ( $TrID ne ''
        && SysAccess->getRule( $form, 'NoteEdit', $form->{Treatment_ProvID_1} )
      )
    {
        my $log =
          $form->{Treatment_Type_1} == 2
          ? 'ShowPhysNotesLog'
          : 'ShowProgNotesLog';

        #warn qq|KLS: TrID=${TrID}, log=${log}\n|;
        $out .=
          '<BR>' . myHTML->ListSel( $form, $log, $TrID, $form->{'LINKID'}, 1 );
    }
    return ($out);
}
...
|;
    ...
    $s->execute($ClientID)
      || myDBI->dberror("Physician Note: select ClientVitalSigns");
    while ( my $r = $s->fetchrow_hashref ) {
        $row++;
        my $class  = int( $row / 2 ) == $row / 2 ? 'rpteven' : 'rptodd';
...
|
  ...
  <TR>
    ...
    #===== PLAN SECTION ==========================
    my $DocMedsDescr = DBA->getxref(
        $form, 'xProcedureRejected',
        $form->{'PhysNotes_DocMeds_1'},
...
|;
    ...
    my $row = 0;
    my $s   = $dbh->prepare(
"select * from ClientMeds where ClientID=? and Active=1 order by Active desc, DrugInfo, PrescriptionDate"
    );
    $s->execute($ClientID)
      || myDBI->dberror("Physician Note: select ClientMeds");
    while ( my $r = $s->fetchrow_hashref ) {
        $row++;
        my $class = int( $row / 2 ) == $row / 2 ? 'rpteven' : 'rptodd';
        $html .= qq|
...
sub ClientMU {
    ...
    $sClient->execute($ClientID)
      || myDBI->dberror("ClientMU: select Client ${ClientID}");
    my $rClient   = $sClient->fetchrow_hashref;
    my $sProvider = $dbh->prepare("select * from Provider where ProvID=?");
    $sProvider->execute( $rClient->{'ProvID'} )
      || myDBI->dberror(
        "ClientMU: select PrimaryProvider $rClient->{'ProvID'}");
    my $rPrimaryProvider = $sProvider->fetchrow_hashref;
    my $PriProvFullName  = DBA->subxml( $rPrimaryProvider->{'FName'} ) . ' '
      . DBA->subxml( $rPrimaryProvider->{'LName'} );
    my $sClientReferrals =
      $dbh->prepare("select * from ClientReferrals where ClientID=?");
    $sClientReferrals->execute($ClientID)
      || myDBI->dberror("ClientMU: select ClientReferrals ${ClientID}");
...
#warn qq|ClientMU: RefPhysNPI=$rClientReferrals->{RefPhysNPI}, ReferredBy1NPI=$rClientReferrals->{ReferredBy1NPI}\n|;
    my $RefProvNPI =
        $rClientReferrals->{'RefPhysNPI'} eq ''
...
#warn qq|ClientMU: EntityTypeCode=$rRef->{EntityTypeCode}, ProvOrgName=$r->{ProvOrgName}\n|;
    my $PriRefFullName =
      $rRef->{'EntityTypeCode'} == 1
      ? DBA->subxml( $rRef->{'ProvFirstName'} ) . ' '
      . DBA->subxml( $rRef->{'ProvLastName'} )
    ...
    $sProvider->execute( $rClient->{'clinicClinicID'} )
      || myDBI->dberror("ClientMU: select Clinic $rClient->{'clinicClinicID'}");
    my $rClinic        = $sProvider->fetchrow_hashref;
    my $ClinicFullName = DBA->subxml( $rClinic->{'FName'} ) . ' '
      . DBA->subxml( $rClinic->{'LName'} );
    ...
    my $sTreatment = $dbh->prepare(
        "select * from Treatment where ClientID=? order by ContLogDate desc");
    $sTreatment->execute($ClientID)
      || myDBI->dberror("ClientMU: select Treatment ${ClientID}");
    ...
    my $Duration = DBUtil->getDuration( $rTreatment->{'ContLogBegTime'},
        $rTreatment->{'ContLogEndTime'} ) / 60;
    $sProvider->execute( $rTreatment->{'ProvID'} )
      || myDBI->dberror("ClientMU: select Provider $rTreatment->{'ProvID'}");
    my $rNoteProv        = $sProvider->fetchrow_hashref;
    my $NoteProvFullName = DBA->subxml( $rNoteProv->{'FName'} ) . ' '
      . DBA->subxml( $rNoteProv->{'LName'} );
    ...
    $sProvider->execute( $rTreatment->{'EnteredBy'} )
      || myDBI->dberror(
        "ClientMU: select EnteredBy $rTreatment->{'EnteredBy'}");
    my $rEnteredBy        = $sProvider->fetchrow_hashref;
    my $EnteredByFullName = DBA->subxml( $rEnteredBy->{'FName'} ) . ' '
      . DBA->subxml( $rEnteredBy->{'LName'} );
    ...
    my $InformantFullName =
        DBA->subxml( $rClientNoteFamilyI->{'FName'} ) . ' '
      . DBA->subxml( $rClientNoteFamilyI->{'LName'} ) . ' ('
...
"select Provider.FName,Provider.LName from ClientTrPlanS left join ClientTrPlan on ClientTrPlan.ID=ClientTrPlanS.TrPlanID left join Provider on Provider.ProvID=ClientTrPlanS.ProvID where ClientTrPlan.ClientID=? and '$rTreatment->{ContLogDate}' between ClientTrPlan.EffDate and ClientTrPlan.ExpDate"
    );
    $sClientTrPlanS->execute($ClientID)
      || myDBI->dberror(
        "ClientMU: select ClientTrPlanS ${ClientID}/${ContDate}");

    while ( my $rClientTrPlanS = $sClientTrPlanS->fetchrow_hashref ) {
        $CareTeamMembers .= DBA->subxml( $rClientTrPlanS->{'FName'} ) . ' '
          . DBA->subxml( $rClientTrPlanS->{'LName'} ) . '<BR>';
    }
    my $ParticipantsList = '';
    ...
    while ( my $rClientNoteFamilyP = $sClientNoteFamilyP->fetchrow_hashref ) {
        $ParticipantsList .=
            DBA->subxml( $rClientNoteFamilyP->{'FName'} ) . ' '
          . DBA->subxml( $rClientNoteFamilyP->{'LName'} ) . ' ('
          . DBA->getxref( $form, 'xRelationship', $rClientNoteFamilyP->{Rel},
            'Descr' )
          . ')<BR>'
          . DBA->subxml( $rClientNoteFamilyP->{'FName'} ) . '<BR>'
          . DBA->subxml( $rClientNoteFamilyP->{'LName'} ) . '<BR>';
    }
    my $ProblemsList = '';
    my $s            = $dbh->prepare(
"select misICD10.ICD10,misICD10.icdName,misICD10.sctName,misICD10.SNOMEDID,ClientNoteProblems.UUID,ClientNoteProblems.Priority,ClientNoteProblems.InitiatedDate,ClientNoteProblems.ResolvedDate from ClientNoteProblems inner join okmis_config.misICD10 on misICD10.ID=ClientNoteProblems.UUID where ClientNoteProblems.ClientID=? and ClientNoteProblems.TrID=? order by ClientNoteProblems.Priority"
    );
    $s->execute( $ClientID, $TrID )
      || myDBI->dberror("Physician Note: select ClientNoteProblems");
    while ( my $r = $s->fetchrow_hashref ) {
...
sub setASAMButtons {
    ...
    my $Buttons =
qq|       <INPUT TYPE=submit ONCLICK="return validate(this.form,'${NewLabel} information','${vmsg}')" ONMOUSEOVER="window.status='add button'; return true;" ONMOUSEOUT="window.status=''" NAME="${url}&UpdateTables=all&pushID=$form->{LINKID}" VALUE="${label}" >|;
...
##
    my $xTFCTimes = {};
    my $sTFCTimes =
      $dbh->prepare("select * from TFCTimes where NoteID=? order by BegTime");
    $sTFCTimes->execute( $form->{Treatment_TrID_1} )
      || myDBI->dberror("TFCTimes select");
...
|;
    ...
    while ( my $r = $s->fetchrow_hashref ) {
        ...
        my $Goal    = $r->{Goal};
        my @Ints    = (
            $r->{Help1}, $r->{Help2}, $r->{Help3},
            $r->{Help4}, $r->{Help5}, $r->{Help6}
        );
        $out .= qq|
  <TR >
    <TD CLASS="hdrcol" WIDTH=10% >
...
|;
                ...
                $out .=
                  qq|            <LI> __:__:__ / __:__:__ ${plusInt} </LI>|
                  unless ( $j > 1 );
            }
            $idx++;
        }
...
|;
    ...
    if ( SysAccess->chkPriv( $form, 'MUAgent' )
      )    # Meaningful use testing on Get Well Agency only
    {
        $html .= qq|
...
sub RestrictedContractsFields {
    ...
    my $UseSpecialty, $UseReferring, $UseRendering, $UseSFacility,
      $ContractType, $ContractCode, $ServMeasure, $SourceCode;
    if ( SysAccess->verify( $form, $Priv ) ) {
        $userid2 =
qq|<INPUT TYPE="text" NAME="Contracts_DMHuserid2_1" VALUE="$form->{Contracts_DMHuserid2_1}" ONFOCUS="select()" SIZE="60" >|;
        $password2 =
qq|<INPUT TYPE="text" NAME="Contracts_DMHpassword2_1" VALUE="$form->{Contracts_DMHpassword2_1}" ONFOCUS="select()" SIZE="60" >|;
        if ( $form->{Contracts_UseSpecialty_1} ) {
            $UseSpecialtyYes = qq|CHECKED|;
        }
        else { $UseSpecialtyNo = qq|CHECKED|; }
        if ( $form->{Contracts_UseReferring_1} ) {
            $UseReferringYes = qq|CHECKED|;
        }
        else { $UseReferringNo = qq|CHECKED|; }
        if ( $form->{Contracts_UseRendering_1} ) {
            $UseRenderingYes = qq|CHECKED|;
        }
        else { $UseRenderingNo = qq|CHECKED|; }
        if ( $form->{Contracts_UseSFacility_1} ) {
            $UseSFacilityYes = qq|CHECKED|;
        }
        else { $UseSFacilityNo = qq|CHECKED|; }
        $UseSpecialty =
qq| <INPUT TYPE="radio" NAME="Contracts_UseSpecialty_1" VALUE="1" ${UseSpecialtyYes} > yes <INPUT TYPE="radio" NAME="Contracts_UseSpecialty_1" VALUE="0" ${UseSpecialtyNo} > no (HIPAA 837 2310A)|;
        $UseReferring =
qq| <INPUT TYPE="radio" NAME="Contracts_UseReferring_1" VALUE="1" ${UseReferringYes} > yes <INPUT TYPE="radio" NAME="Contracts_UseReferring_1" VALUE="0" ${UseReferringNo} > no (HIPAA 837 2310A)|;
        $UseRendering =
qq| <INPUT TYPE="radio" NAME="Contracts_UseRendering_1" VALUE="1" ${UseRenderingYes} > yes <INPUT TYPE="radio" NAME="Contracts_UseRendering_1" VALUE="0" ${UseRenderingNo} > no (HIPAA 837 2310B)|;
        $UseSFacility =
qq| <INPUT TYPE="radio" NAME="Contracts_UseSFacility_1" VALUE="1" ${UseSFacilityYes} > yes <INPUT TYPE="radio" NAME="Contracts_UseSFacility_1" VALUE="0" ${UseSFacilityNo} > no (HIPAA 837 2310D)|;
        $ContractType = qq|
      <SELECT NAME="Contracts_ContractType_1" >
        |
          . DBA->selxTable( $form, 'xContractType',
            $form->{Contracts_ContractType_1},
            'ID Descr' )
          . qq|
      </SELECT>|;
        $ContractCode = qq|
      <SELECT NAME="Contracts_ContractCode_1" >
        |
          . DBA->selxTable( $form, 'xContractCode',
            $form->{Contracts_ContractCode_1},
            'ID Descr' )
          . qq|
      </SELECT>|;
        $ServMeasure = qq|
      <SELECT NAME="Contracts_ServMeasure_1" >
        |
          . DBA->selxTable( $form, 'xServMeasure',
            $form->{Contracts_ServMeasure_1},
            'ID Descr' )
          . qq|
      </SELECT>|;
        $SourceCode = qq|
      <SELECT NAME="Contracts_SourceCode_1" >
        |
          . DBA->selxTable( $form, 'xContractSource',
            $form->{Contracts_SourceCode_1},
            'ID Descr' )
          . qq|
      </SELECT>|;
    }
        ...
        $userid2   = 'hidden';
        $password2 = 'hidden';
        if   ( $form->{Contracts_UseSpecialty_1} ) { $UseSpecialty = qq|Yes|; }
        else                                       { $UseSpecialty = qq|No|; }
        if   ( $form->{Contracts_UseReferring_1} ) { $UseReferring = qq|Yes|; }
        else                                       { $UseReferring = qq|No|; }
        if   ( $form->{Contracts_UseRendering_1} ) { $UseRendering = qq|Yes|; }
        else                                       { $UseRendering = qq|No|; }
        if   ( $form->{Contracts_UseSFacility_1} ) { $UseSFacility = qq|Yes|; }
        else                                       { $UseSFacility = qq|No|; }
        $ContractType =
          DBA->getxref( $form, 'xContractType',
            $form->{Contracts_ContractType_1},
            'ID Descr' );
        $ContractCode =
          DBA->getxref( $form, 'xContractCode',
            $form->{Contracts_ContractCode_1},
            'ID Descr' );
        $ServMeasure =
          DBA->getxref( $form, 'xServMeasure', $form->{Contracts_ServMeasure_1},
            'ID Descr' );
        $SourceCode =
          DBA->getxref( $form, 'xContractSource',
            $form->{Contracts_SourceCode_1},
            'ID Descr' );
    }
    my $str = qq|
...
sub RestrictedCredentialsFields {
    ...
    my $userid2 =
qq|<INPUT TYPE="text" NAME="Credentials_DMHuserid2_1" VALUE="$form->{Credentials_DMHuserid2_1}" ONFOCUS="select()" SIZE="60" >|;
    my $password2 =
qq|<INPUT TYPE="text" NAME="Credentials_DMHpassword2_1" VALUE="$form->{Credentials_DMHpassword2_1}" ONFOCUS="select()" SIZE="60" >|;
    my $html = qq|
<TABLE CLASS="home fullsize" >
  <TR><TD CLASS="port hdrtxt" " COLSPAN="2" >Restricted Electronic Data</TD></TR>
  <TR>
...
sub setLINKS {
    ...
    return ('&nbsp;')
      if ( $form->{'NONAVIGATION'} )
      ;    # newHTML withOUT left/right/back navigation
    if ( $type =~ /back/i ) {
...
sub bldTab {
    ...
    my $out =
qq|<DIV STYLE="display:${style}" ID="${tab}${num}" >\n  <UL class="${cls}">\n|;
    for ( $i = $min ; $i <= $max ; $i++ ) {
        if ( $i == $num ) {
            $out .=
qq|    <LI CLASS="${cls}_selected" ONCLICK="showTab('${tab}',${i},${min},${max});">$labels[$i-1]</LI>\n|;
        }
        else {
            $out .=
qq|    <LI ONCLICK="showTab('${tab}',${i},${min},${max});">$labels[$i-1]</LI>\n|;
        }
    }
    $out .= qq|  </UL>\n${text}\n</DIV>\n|;
...
|;
    my $qxMessages =
qq|select EffDate,Message from xMessages where ExpDate is null order by EffDate desc|;
...
#foreach my $f ( sort keys %{$form} ) { warn "gHTML-htmB32: form-$f=$form->{$f}\n"; }
    ...
    $sClientBasis32 = $dbh->prepare(
        "select * from ClientBasis32 where ClientID=? order by EffDate desc");
...
#for ($k=1; $k<=5; $k++) { warn qq|htmB32: cnt:${cnt} k=${k}, tot=$counts[$k][2] / cnt=$counts[$k][1]\n|; }
        if ( $cnt == 1 ) {
            $FirstAvg[1] = sprintf( "%.2f",
                $counts[1][1] ? $counts[1][2] / $counts[1][1] : 0 );
            $FirstAvg[2] = sprintf( "%.2f",
                $counts[2][1] ? $counts[2][2] / $counts[2][1] : 0 );
            $FirstAvg[3] = sprintf( "%.2f",
                $counts[3][1] ? $counts[3][2] / $counts[3][1] : 0 );
            $FirstAvg[4] = sprintf( "%.2f",
                $counts[4][1] ? $counts[4][2] / $counts[4][1] : 0 );
            $FirstAvg[5] = sprintf( "%.2f",
                $counts[5][1] ? $counts[5][2] / $counts[5][1] : 0 );
        }
        if ( $cnt <= 2 )    # either 1st or 2nd
        {
            $SecondAvg[1] = sprintf( "%.2f",
                $counts[1][1] ? $counts[1][2] / $counts[1][1] : 0 );
            $SecondAvg[2] = sprintf( "%.2f",
                $counts[2][1] ? $counts[2][2] / $counts[2][1] : 0 );
            $SecondAvg[3] = sprintf( "%.2f",
                $counts[3][1] ? $counts[3][2] / $counts[3][1] : 0 );
            $SecondAvg[4] = sprintf( "%.2f",
                $counts[4][1] ? $counts[4][2] / $counts[4][1] : 0 );
            $SecondAvg[5] = sprintf( "%.2f",
                $counts[5][1] ? $counts[5][2] / $counts[5][1] : 0 );
        }
        $LastAvg[1] =
          sprintf( "%.2f", $counts[1][1] ? $counts[1][2] / $counts[1][1] : 0 );
        $LastAvg[2] =
          sprintf( "%.2f", $counts[2][1] ? $counts[2][2] / $counts[2][1] : 0 );
        $LastAvg[3] =
          sprintf( "%.2f", $counts[3][1] ? $counts[3][2] / $counts[3][1] : 0 );
        $LastAvg[4] =
          sprintf( "%.2f", $counts[4][1] ? $counts[4][2] / $counts[4][1] : 0 );
        $LastAvg[5] =
          sprintf( "%.2f", $counts[5][1] ? $counts[5][2] / $counts[5][1] : 0 );
    }
...
<THEAD>|;
    for ( $i = 0 ; $i <= $#colhdrs ; $i++ ) {
        $hdr .=
          qq|    <TH CLASS="table-sortable:$coljust[$i]" >$colhdrs[$i]</TH>\n|;
    }
    $html .= qq|
  <TR >\n${hdr}\n</TR>
</THEAD>
...
|;
    ...
    foreach my $line ( split( "\n", $rpt ) ) {
        ...
        if    ( $cnt < $hdrcnt ) { $html .= qq|<DIV>${line}</DIV>\n|; }
        elsif ( $cnt == $hdrcnt ) {
            $html .=
qq|<TABLE class="chartsort table-autosort table-stripeclass:alternate" >|;
            ...
            for ( my $i = 0 ; $i <= $#colhdrs ; $i++ ) {
                ...
                $hdrs .=
qq|    <TH CLASS="table-sortable:${sort}" >$colhdrs[$i]</TH>\n|;
            }
            $html .= qq|<THEAD><TR >\n${hdrs}\n</TR></THEAD>\n<TBODY>\n|;
...
sub selClinics {
    ...
    my $dbh       = myDBI->dbconnect( $form->{'DBNAME'} );
    my $q =
qq|select SiteACL.*,Provider.Name from SiteACL left join Provider on Provider.ProvID=SiteACL.AccessID where SiteACL.ProvID=? and SiteACL.AccessOK=1 and SiteACL.Type='Clinic' and Provider.Active=1|;
    ...
    my $SelStmt =
      DBA->makeSelect( $form, $SelectedIDs, $list, 'Provider', 'Name' );
    return ($SelStmt);
}
...
sub selProviders {
    ...
    my $dbh       = myDBI->dbconnect( $form->{'DBNAME'} );
    my $q =
qq|select SiteACL.*,Provider.LName,Provider.FName from SiteACL left join Provider on Provider.ProvID=SiteACL.AccessID where SiteACL.ProvID=? and SiteACL.AccessOK=1 and SiteACL.Type='Provider' and Provider.Active=1|;
    ...
    return ($list) if ($rtnList);
    my $SelStmt =
      DBA->makeSelect( $form, $SelectedIDs, $list, 'Provider', 'LName:FName' );
    return ($SelStmt);
}
#############################################################################
sub InsuranceSelect {
    ...
    if ( !$AGENT && $ID ne 'new' && $InsID ne '' ) {
        my $qPrAuth =
qq|select ClientPrAuth.*, xInsurance.Name from ClientPrAuth left join xInsurance on xInsurance.ID='${InsID}' where ClientPrAuth.InsuranceID='${ID}'|;
        ...
        while ( my $rPrAuth = $sPrAuth->fetchrow_hashref ) {
            if ( $rPrAuth->{PAnumber} eq '' ) {
                if ( CDC->isLocked( $form, $rPrAuth->{ID} ) ) {
                    $SelStmt =
qq|<INPUT TYPE="hidden" ID="InsID" NAME="LockedIns" VALUE="${InsID}" >$rPrAuth->{Name}   (Insurance locked waiting for PA authorization)|;
                }
            }
            else {
                $SelStmt =
qq|<INPUT TYPE="hidden" ID="InsID" NAME="LockedIns" VALUE="${InsID}" >$rPrAuth->{Name}   (Insurance locked with Authorized PA)|;
            }
            last if ( $SelStmt ne '' );
        }
        $sPrAuth->finish();
...
select Contracts.PIN,Provider.Name,ProviderControl.NPI
 ...
 order by Contracts.PIN, xInsurance.Name|;
    ...
    if ( $rContracts = $sContracts->fetchrow_hashref ) {
        $AgencyName =
          qq|Agency: $rContracts->{Name} ($rContracts->{NPI}/${PIN})|;
    }
    else {
        my $qCredentials = qq|
select Credentials.*,Provider.FName,Provider.LName,ProviderControl.NPI
 ...
 order by Credentials.Rank|;
        ...
        if ( $rCredentials = $sCredentials->fetchrow_hashref ) {
            $AgencyName =
qq|Provider: $rCredentials->{LName}, $rCredentials->{FName} ($rCredentials->{NPI}/${PIN})|;
        }
        $sCredentials->finish();
    }
    $sContracts->finish();
...
sub getTag {
    ...
    my $dbh = myDBI->dbconnect( $form->{'DBNAME'} );
    my $s =
      $dbh->prepare("select AgencyTag from ProviderControl where ProvID=?");
    $s->execute($AID) || myDBI->dberror("getTag: $AID");
    my ($Tag) = $s->fetchrow_array;
    if ( $Tag eq '' ) {
        my ( $pfx, $db ) = split( '_', $form->{'DBNAME'} );
        $Tag = $db;
    }
    $s->finish();
    return ($Tag);
...
sub ifld() {
    ...
    if ( $type =~ /readonly/i ) {
        $html .=
qq|<INPUT TYPE="text" ID="${name}" NAME="${name}" VALUE="${val}" READLONLY >\n|;
    }
    elsif ( $type =~ /displayonly/i ) {
        $html .=
qq|<INPUT TYPE="hidden" ID="${name}" NAME="${name}" VALUE="${val}" > <SPAN ID="${name}_display">${val}</SPAN>\n|;
    }
    return ($html);
}
#############################################################################
