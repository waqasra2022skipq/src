...
sub new {
   ...
   # foreach my $f ( sort keys %{$FORM} ) { warn qq|1 FORM: $f=$FORM->{$f}\n|; }
    ...
    if ( exists $FORM->{'code'} && $FORM->{'code'} !~ /^\s*$/ ) {
        ...
        my $token_url =
'https://login.microsoftonline.com/b717b7ec-b71d-460b-9c8a-a1b4d05dbbfb/oauth2/v2.0/token';

        my $params = {
            "client_id"     => '90499e5d-418e-4a90-81a0-946f54a25199',
            "code"          => $FORM->{'code'},
            "redirect_uri"  => "https://dev.okmms.com/cgi/bin/mis.cgi",
            "grant_type"    => "authorization_code",
            "client_secret" => "uq9uJk4..-vC3-W2~6Kv54NZAUFePTNXB~"
        };

        $obj_useragent->default_header(
        ...
        if ($resp_content) {
            my $token_data = undef;
            eval { $token_data = from_json($resp_content); };
            if ( exists $token_data->{'access_token'}
                && $token_data->{'access_token'} !~ /^\s*$/ )
            {
                $obj_useragent->default_header(
                    'graph' => 'graph.microsoft.com' );
                $obj_useragent->default_header( 'Authorization' => 'Bearer '
                      . $token_data->{'access_token'} );
                my $sso_guest_profile_url =
                ...
                eval {
                    ...
                    if ($resp_content) {
                        ...
                        eval { $office_user_data = from_json($resp_content); };

                        if ( exists $office_user_data->{'userPrincipalName'}
                            && $office_user_data->{'userPrincipalName'} !~
                            /^\s*$/ )
                        {
                            ###code here further
                            $FORM->{user} =
...
#warn qq|myForm: DOCUMENT_ROOT=$ENV{'DOCUMENT_ROOT'}\n|;
    ...
    if    ($in_string) { $query_string = $in_string; }
    elsif ( $ARGV[0] ) { $query_string = join( ' ', @ARGV ); }
...
#warn qq|setUserLogin: LOGINUSERID=$FORM->{LOGINUSERID}\n| if ( $DB eq 'okmis_dev' );
    $sUser->execute( $FORM->{'LOGINUSERID'} )
      || myDBI->error("renewLogin: ${qUser} $FORM->{'LOGINUSERID'}");
...
#foreach my $f ( sort keys %{$FORM} ) { warn qq|saveHIST FORM: $f=$FORM->{$f}\n|; }
    ...
    elsif ( $str =~ /MIS_Action=ChartList/i ) {
        return ();
    }                                                       # catch on next one.
    elsif ( $str =~ /MIS_Action=MgrTree/i ) { return (); }  # catch on next one.
    elsif ( $str =~ /MIS_Action=ClientList/i ) {
        return ();
    }                                                       # catch on next one.
    elsif ( $str =~ /MIS_Action=ClientPage/i ) {
        return ();
    }                                                       # catch on next one.
    elsif ( $str =~ /MIS_Action=LoginScreen/i ) {
        return ();
    }                                                       # catch on next one.
    ...
    elsif ( $str =~ /view=vInsID.cgi/i ) {
        $str = 'view=Reconcile/Scholarship';
    }
    ...
    my $q =
qq|insert into History (ID,ProvID,Name,Descr,Link,Token) values (NULL,$FORM->{LOGINPROVID},${Key},${Value},$URL,$Token)|;
...
sub saveLINK    # save on login new()
{
    ...
    my $q =
qq|insert into Links (ProvID,pgm,url,browser,ip) values ($FORM->{LOGINPROVID},$pgm,?,'$FORM->{browser}','$ENV{REMOTE_ADDR}')|;
    my $s      = $dbh->prepare($q);
    my $newurl = $url;
    ...
    if ( $url eq '/cgi/bin/mis.cgi?' ) {
        ...
        if ( $FORM->{fwdTABLE} )    # save and switch to fwdTABLE
        {
            ...
            $newurl =
                substr( $url, 0, index( $url, '?' ) ) . '?'
              . myForm->genLink( $FORM->{fwdTABLE}, $FORM->{view} )
              . "&mlt=$FORM->{mlt}&misLINKS=$FORM->{misLINKS}";
        }
    }
...
#warn qq|popLINK BEGIN: cnt=$cnt, misLINKS=$FORM->{misLINKS}, fwdLINK=$FORM->{fwdLINK}\n|;
    ...
    $url =
qq|/cgi/bin/mis.cgi?MIS_Action=ManagerTree&mlt=$FORM->{mlt}&default=mgrtree|
      if ( $url eq '' );
...
#warn qq|INSIDE: HDRIDval1 ne '': inTable=$inTable, DETID=$DETID, HDRIDval=$HDRIDval\n|;
    ...
    if ( $inTable eq 'Treatment' ) {
        $FORM->{'BeginTime'} =
          DBUtil->AMPM( $FORM->{'Treatment_ContLogBegTime_1'} );
        $FORM->{'EndTime'} =
          DBUtil->AMPM( $FORM->{'Treatment_ContLogEndTime_1'} );
    }
        ...
        $FORM->{'PrAuthRVU_PANum_1'} = $FORM->{'PrAuth_PAnumber_1'}
          if ( $FORM->{'PrAuthRVU_PANum_1'} eq '' );
...
#foreach my $f ( sort keys %{$FORM} ) { warn qq|FORM: setDefaults: $f=$FORM->{$f}\n|; }
    ...
    if ( $inTable eq 'Provider' ) {
        ...
        $FORM->{'Provider_Type_1'} =
          '4';    # means Provider (Group=1,Agency=2,Clinic=3)
        ...
        $FORM->{'Client_clinicClinicID_1'} =
          MgrTree->getClinic( $FORM, $FORM->{'LOGINPROVID'} );
    }
        ...
        my $r = DBA->getLAST(
            ...
            '',
            $inTable,
            "where ClientTrPlan.ClientID='$FORM->{Client_ClientID_1}'",
...
#warn qq|myForm: setDefaults: ClientTrPlan: delete ${inTable}_${ID}_1=$FORM->{"${inTable}_${ID}_1"}\n|;
            ...
            my $q =
qq|select S1,S2,S3,S4,L1,L2,L3,L4,Prefs from ClientSummary where ClientID='$FORM->{Client_ClientID_1}'|;

#warn qq|myForm: setDefaults: ClientTrPlan: ClientID=$FORM->{Client_ClientID_1}\n${q}\n|;
            my $s = $dbh->prepare($q);
            $s->execute() || myDBI->dberror($q);
            my ( $S1, $S2, $S3, $S4, $L1, $L2, $L3, $L4, $Prefs ) =
              $s->fetchrow_array;
            ...
            $s->finish();
            my $q =
qq|select Problem,Summary,Services,Referrals from ClientIntake where ClientID='$FORM->{Client_ClientID_1}'|;

#warn qq|myForm: setDefaults: ClientTrPlan: ClientID=$FORM->{Client_ClientID_1}\n${q}\n|;
            my $s = $dbh->prepare($q);
            $s->execute() || myDBI->dberror($q);
            my ( $Problem, $Summary, $Services, $Referrals ) =
              $s->fetchrow_array;
        ...
        my $s = $dbh->prepare(
"select CopyID from ClientTrPlan where ID='$FORM->{'ClientTrPlan_ID_1'}'"
        );
        $s->execute()
          || myDBI->dberror("setDefault: select ClientTrPlan CopyID");
        my ($PrevID) = $s->fetchrow_array;
        $s->finish();

#warn qq|myForm: setDefault:  ${inTable}: PrevID=${PrevID}, TrPlanID=$FORM->{'ClientTrPlan_ID_1'}\n|;
        my $s = $dbh->prepare(
"select count(*) from ClientTrPlanPG where TrPlanID='$FORM->{'ClientTrPlan_ID_1'}'"
        );
        $s->execute()
          || myDBI->dberror("setDefault: select ClientTrPlanPG count");
        ...
        my ( $rLast, $i ) = ( '', 0 );
        my $s = $dbh->prepare(
            "select * from ClientTrPlanPG where TrPlanID='$PrevID'");
        $s->execute()
          || myDBI->dberror("setDefault: select CLientTrPlanPG ${PrevID}");
        ...
        my $s = $dbh->prepare(
"select CopyID from ClientTrPlanPG where ID='$FORM->{'ClientTrPlanPG_ID_1'}'"
        );
        $s->execute()
          || myDBI->dberror("setDefault: select ClientTrPlanPG CopyID");
        my ($PrevID) = $s->fetchrow_array;
        $s->finish();

#warn qq|myForm: setDefault:  ${inTable}: PrevID=${PrevID}, TrPlanPGID=$FORM->{'ClientTrPlanPG_ID_1'}\n|;
        my $s = $dbh->prepare(
"select count(*) from ClientTrPlanOBJ where TrPlanPGID='$FORM->{'ClientTrPlanPG_ID_1'}'"
        );
        $s->execute()
          || myDBI->dberror("setDefault: select ClientTrPlanOBJ count");
        my ($cnt) = $s->fetchrow_array;
        $s->finish();
        ...
        my ( $rLast, $i ) = ( '', 0 );
        my $s = $dbh->prepare(
"select * from ClientTrPlanOBJ where TrPlanPGID='$PrevID' and ResolvedDate is null"
        );
        $s->execute()
          || myDBI->dberror("setDefault: select CLientTrPlanOBJ ${PrevID}");
        while ( my $r = $s->fetchrow_hashref ) {
            $i++;
            $rLast = $r if ( $cnt < $i );
            last        if ( $i == $cnt + 1 );
        ...
        $FORM->{'ClientPrAuth_ReqType_1'} =
          DBA->setPrAuthReqType( $FORM, $ClientID );
        my ( $isAdult, $TL ) =
          TLevel->getTreatmentLevel( $FORM, $ClientID, '18' );
        ...
        $FORM->{'ClientPrAuth_ExpDate_1'} =
          DBUtil->Date( $TDate, $months, $days );
        ...
        my $r = DBA->getLAST(
            ...
            '',
            $inTable,
            "where ClientPrAuth.ClientID='$FORM->{Client_ClientID_1}'",
...
#   these are only updated in billing routines
        ...
        $FORM->{'Treatment_StatusDate_1'} =
          $FORM->{TODAY};                         # Billed Status = new date.
...
#warn qq|myForm: setDefaults: Treatment: DupNote\n| if ( SysAccess->verify($FORM,'Privilege=DupNote') );
        if ( $FORM->{'AppointmentID'} ) {
            ...
            my $r = DBA->getLAST(
                ...
                '',
                $inTable,
"where Treatment.ClientID='$FORM->{Client_ClientID_1}' and Treatment.ProvID='$FORM->{LOGINPROVID}' and Treatment.Type!=3",
                "order by Treatment.ContLogDate desc"
            );
            if ($r) {
        ...
        if ( $FORM->{'AppointmentID'} ) {
            ...
            my $s = $dbh->prepare(
                "select * from ProgNotes where NoteID='$FORM->{useTrID}'");
...
#warn qq|myForm: setDefaults: delete ${inTable}_${ID}_1=$FORM->{"${inTable}_${ID}_1"}\n|;
        ...
        {
            my $s = $dbh->prepare(
                "select * from PhysNotes where NoteID='$FORM->{useTrID}'");
            $s->execute() || myDBI->dberror("setDef: useTrID=$FORM->{useTrID}");
            my $r = $s->fetchrow_hashref;
            if ($r) {
                myForm->setTable( $inTable, $r );
...
#warn qq|myForm: setDefaults: delete ${inTable}_${ID}_1=$FORM->{"${inTable}_${ID}_1"}\n|;
    ...
    #warn qq|myForm: setDefaults: useTrID: $FORM->{'useTrID'} h=${h}, w=${w}\n|;
        ...
        if ( $h eq '' || $w eq '' )    # get them from Intake
        {
            ...
            $s->execute()
              || myDBI->dberror("setDef PhysNotes: ClientID=${ClientID}");
            my $r = $s->fetchrow_hashref;
            $h                            = $r->{'Height'} if ( $h eq '' );
            $w                            = $r->{'Weight'} if ( $w eq '' );
...
#warn qq|myForm: setDefaults: PhysNotes: height=$h,$height,$f,$i,$hi; weight=$w\n|;
        $FORM->{'PhysNotes_BMI_1'} =
          $hi == 0 ? 0 : sprintf( "%.2f", ( $w / ( $hi * $hi ) ) * 703 );
        my $r = DBA->getLAST(
            $FORM,
            '',
            'PDDiag',
...
#warn qq|myForm: setDefaults: ClientTherapyNotes: useTrID=$FORM->{'useTrID'}\n|;
        if ( $FORM->{'useTrID'} ) {
            my $s = $dbh->prepare(
"select * from ClientTherapyNotes where NoteID='$FORM->{useTrID}'"
            );
            $s->execute() || myDBI->dberror("setDef: useTrID=$FORM->{useTrID}");
            my $r = $s->fetchrow_hashref;
            if ($r) {
...
#warn qq|myForm: setDefaults: delete ${inTable}_${ID}_1=$FORM->{"${inTable}_${ID}_1"}\n|;
        ...
        my $qClientIntake =
qq|select * from ClientIntake where ClientID='$FORM->{Client_ClientID}'|;
        ...
        $FORM->{ClientDischarge_ServiceFocus_1} =
          $rClientIntake->{ServiceFocus};
        $FORM->{ClientDischarge_InitCond_1} = $rClientIntake->{Problem};
        $FORM->{ClientDischarge_IDNeeds_1} =
          DBA->getxref( $FORM, 'xProblems', $rClientIntake->{'Problem1'},
            'Descr' );
        $FORM->{ClientDischarge_IDNeeds_1} .= "; "
          . DBA->getxref( $FORM, 'xProblems', $rClientIntake->{'Problem2'},
            'Descr' )
          if ( $rClientIntake->{'Problem2'} ne '' );
        $FORM->{ClientDischarge_IDNeeds_1} .= "; "
          . DBA->getxref( $FORM, 'xProblems', $rClientIntake->{'Problem3'},
            'Descr' )
          if ( $rClientIntake->{'Problem3'} ne '' );
        $sClientIntake->finish();
        my $rClientTrPlan = DBA->getLAST(
            $FORM,
            '',
            'ClientTrPlan',
            "where ClientTrPlan.ClientID='$FORM->{Client_ClientID_1}'",
...
#warn qq|myForm: setDefaults: ClientID=$rClientTrPlan->{ClientID}, ID=$rClientTrPlan->{ID}\n|;
        ...
        $FORM->{'ClientDischarge_Needs_1'} .= qq|; $rClientTrPlan->{'SA2'}|
          if ( $rClientTrPlan->{'SA2'} ne '' );
        $FORM->{'ClientDischarge_Needs_1'} .= qq|; $rClientTrPlan->{'SA3'}|
          if ( $rClientTrPlan->{'SA3'} ne '' );
        $FORM->{'ClientDischarge_Needs_1'} .= qq|; $rClientTrPlan->{'SA4'}|
          if ( $rClientTrPlan->{'SA4'} ne '' );
        $FORM->{'ClientDischarge_Needs_1'} .= qq|; $rClientTrPlan->{'L1'}|
          if ( $rClientTrPlan->{'L1'} ne '' );
        $FORM->{'ClientDischarge_Needs_1'} .= qq|; $rClientTrPlan->{'L2'}|
          if ( $rClientTrPlan->{'L2'} ne '' );
        $FORM->{'ClientDischarge_Needs_1'} .= qq|; $rClientTrPlan->{'L3'}|
          if ( $rClientTrPlan->{'L3'} ne '' );
        $FORM->{'ClientDischarge_Needs_1'} .= qq|; $rClientTrPlan->{'L4'}|
          if ( $rClientTrPlan->{'L4'} ne '' );
        $FORM->{'ClientDischarge_DischargePlan_1'} =
          $rClientTrPlan->{'TransitionPlan'};
        my $sClientDevl =
          $dbh->prepare("select * from ClientDevl where ClientID=?");
        $sClientDevl->execute( $FORM->{Client_ClientID} )
          || myDBI->dberror(
"setDefault: ClientDischarge: select ClientDevl $FORM->{Client_ClientID}"
          );
        ...
        $sClientDevl->finish();
        my $sClientSocial =
          $dbh->prepare("select * from ClientSocial where ClientID=?");
        $sClientSocial->execute( $FORM->{Client_ClientID} )
          || myDBI->dberror(
"setDefault: ClientDischarge: select ClientSocial $FORM->{Client_ClientID}"
          );
        ...
        unless ( $ClientID eq 'new' ) {
            my $val = DBA->getLastID( $FORM, $inTable, "G1=$ClientID",
                "CreateDate desc" );
            if ($val) {
            ...
            my $multidel = 'ý';
            my $q =
qq|select SSN,Gend,DOB,SUBSTRING_INDEX(Race,'${multidel}',1) as Race from Client where ClientID='$ClientID'|;
            my $s = $dbh->prepare($q);
            $s->execute() || myDBI->dberror($q);
            ...
            $q =
              qq|select StaffID from ClientIntake where ClientID='$ClientID'|;
            $s = $dbh->prepare($q);
            $s->execute() || myDBI->dberror($q);
            ...
            $q =
qq|select MonthsTechEd from ClientEducation where ClientID='$ClientID'|;
            $s = $dbh->prepare($q);
            $s->execute() || myDBI->dberror($q);
            ...
            $q =
qq|select HospOverNight from ClientHealth where ClientID='$ClientID'|;
            $s = $dbh->prepare($q);
            $s->execute() || myDBI->dberror($q);
            ...
            $q =
qq|select WhoSpendTime,SatSpendTime,NumCloseFriends from ClientSocial where ClientID='$ClientID'|;
            $s = $dbh->prepare($q);
            $s->execute() || myDBI->dberror($q);
            my ( $WhoSpendTime, $SatSpendTime, $NumCloseFrieds ) =
              $s->fetchrow_array;
            ...
            $q =
qq|select MarStatY,MarStatM,ResAdmitDate,FamUsualLivArr,SatUsualLivArr from ClientRelations where ClientID='$ClientID'|;
            $s = $dbh->prepare($q);
            $s->execute() || myDBI->dberror($q);
            my ( $MarStatY, $MarStatM, $ResAdmitDate, $FamUsualLivArr,
                $SatUsualLivArr )
              = $s->fetchrow_array;
            ...
            $q =
qq|select ValidDL,AutoForUse,RegSupport,MajSupport from ClientResources where ClientID='$ClientID'|;
            $s = $dbh->prepare($q);
            $s->execute() || myDBI->dberror($q);
            my ( $ValidDL, $AutoForUse, $RegSupport, $MajSupport ) =
              $s->fetchrow_array;
        ...
        my $ClientID = $FORM->{'Client_ClientID'};
        unless ( $ClientID eq 'new' ) {
            my $val = DBA->getLastID( $FORM, $inTable, "ClientID=$ClientID",
                "AdmDate desc" );
##$val=0;   ## don't know why asked to turn this off, now turn back on 07/11/2016
            ...
            my $multidel = 'ý';
            my $q =
qq|select Client.*,SUBSTRING_INDEX(Client.Race,'${multidel}',1) as Race,ClientSocial.ReligionName from Client left join ClientSocial on ClientSocial.ClientID=Client.ClientID where Client.ClientID=?|;
            my $s = $dbh->prepare($q);
            ...
            $q =
qq|select count(*) from ClientLegalHx where ClientID=? and OutCome='J'|;
            my $s = $dbh->prepare($q);
            $s->execute($ClientID) || myDBI->dberror($q);
...
#$FORM->{'Contracts_RefID_1'} = '1D';               # Medicaid Provider Number for PIN
        $FORM->{'Contracts_Taxonomy_1'} =
          '261QM0801X';    # Mental Health including Community
        $FORM->{'Contracts_ServMeasure_1'} = 'UN';    # Units
        $FORM->{'Contracts_BillType_1'}    = 'EL';    # Electronic
        $FORM->{'Contracts_UseAgency_1'} =
          '0';    # Not Agency Address - Clinic Address (Zip+4)
      ...
      #foreach my $f ( sort keys %{$FORM} ) { warn qq|FORM: $f=$FORM->{$f}\n|; }
        ...
        my ( $TransDate, $TransTime ) =
          DBA->getPrAuthTransDT( $FORM, $ClientID, $FORM->{TODAY} );
        ...
        $FORM->{'ClientPrAuthCDC_CDCOK_1'} =
          $ClientID eq 'new' ? 1 : CDC->required( $FORM, $InsID ) ? 0 : 1;
    }
      ...
      # as of last note...
        my $qTreatment =
qq|select * from Treatment where ClientID='$FORM->{Client_ClientID}' order by ContLogDate desc|;
        my $sTreatment = $dbh->prepare($qTreatment);
        $sTreatment->execute() || myDBI->dberror($qTreatment);
        my $rTreatment = $sTreatment->fetchrow_hashref;
        $sTreatment->finish();
        $FORM->{'ClientDischargeCDC_TransDate_1'} =
          $rTreatment->{'ContLogDate'};
        $FORM->{'ClientDischargeCDC_TransTime_1'} =
          $rTreatment->{'ContLogBegTime'};
    ...
    my $fld            = 'ClientID';
    $key = $inTable . '_' . $fld . '_1';
    if ( exists( $TableFieldDefs->{$fld} ) && $FORM->{$key} eq '' ) {
        $FORM->{$key} = $FORM->{Client_ClientID};
    }
    my $fld = 'CreateProvID';
    $key = $inTable . '_' . $fld . '_1';
    if ( exists( $TableFieldDefs->{$fld} ) ) {
        $FORM->{$key} = $FORM->{LOGINPROVID};
    }
    my $fld = 'CreateDate';
    $key = $inTable . '_' . $fld . '_1';
...
#warn "myForm: BEGIN: TBLselect: inTable=$inTable, id=$id, val=$val, AccFunc=$AccFunc\n";
    SysAccess->verify( $FORM, $AccFunc )
      || myDBI->error("Access DENIED to ${inTable} (TBLselect)");
...
#warn qq|TBLselect: inTable=$inTable, DBCFGNAME=${DBCFGNAME}, DBNAME=${DBNAME}\n|;
    ...
    while ( my $r = $s->fetchrow_hashref ) {
        ...
        foreach my $f ( keys %{$r} ) {
            ...
            $FORM->{$key} = $r->{$f};
            $FORM->{$key} = ''
              if ( $f =~ /date/i && $FORM->{$key} eq '0000-00-00' );
