...
#warn qq|DBForm: DOCUMENT_ROOT=$ENV{'DOCUMENT_ROOT'}\n|;
    ...
    if    ($InString)  { $query_string = $InString; }
...
#warn qq|setUserLogin: qUser=${qUser}, LOGINUSERID=$self->{LOGINUSERID}\n| if ( $DB eq 'okmis_demo' );
    $sUser->execute( $self->{'LOGINUSERID'} )
      || $self->error("renewLogin: ${qUser} $self->{'LOGINUSERID'}");
...
#warn qq|setUserLogin: FOUND: FName=$rUser->{FName}\n| if ( $DB eq 'okmis_demo' );
        ...
        $self->{LOGINAGENCY} =
          $self->{'LOGINTYPE'} == 0
          ? MgrTree->getAgency( $self, $self->{LOGINUSERID} )
          : '';
    }
...
sub setURL {
    ...
    elsif ( $URL =~ /[?]/ ) { $URL .= qq|&mlt=$self->{mlt}|; }
    else                    { $URL .= qq|?mlt=$self->{mlt}|; }
    return ($URL);
}
...
sub getHTML {
    ...
    while ( $line =~ /\[\[\[(.+?)\]\]\]/ ) {
        ...
        $line_copy .=
          $self->exFunc($whatmatch);    #   plus what the function returns
        ####################################
    }
    $html = $line_copy . $line;
...
#warn qq|DBForm:BEGIN: TBLread: inTable=$inTable, hdrtable=$hdrtable, HDRIDval=$HDRIDval, HDRIDval1=$HDRIDval1\n|;
    if    ( $IDval =~ /new/i ) { $self->setDefaults( $inTable, $ID ); }
    elsif ( $IDval ne '' )     { $self->TBLselect( $inTable, $ID, $IDval ); }
    elsif ( $HDRIDval =~ /new/i ) {
        $self->{"${inTable}_${ID}"} = 'new';
...
#   to set a new detail record set IDval='new', ie ClientPrAuth_ID='new'
    ...
    if ( $inTable eq 'Treatment' ) {
        $self->{'BeginTime'} =
          DBUtil->AMPM( $self->{'Treatment_ContLogBegTime_1'} );
        $self->{'EndTime'} =
          DBUtil->AMPM( $self->{'Treatment_ContLogEndTime_1'} );
    }
        ...
        $self->{'PrAuthRVU_PANum_1'} = $self->{'PrAuth_PAnumber_1'}
          if ( $self->{'PrAuthRVU_PANum_1'} eq '' );
...
sub setDefaults {
    ...
    if ( $inTable eq 'Provider' ) {
        ...
        $self->{'Client_clinicClinicID_1'} =
          MgrTree->getClinic( $self, $self->{'LOGINPROVID'} );
    }
        ...
        my $r = DBA->getLAST(
            ...
            '',
            $inTable,
            "where ClientTrPlan.ClientID='$self->{Client_ClientID_1}'",
...
#warn qq|setDefaults: ClientTrPlan: delete ${inTable}_${ID}_1=$self->{"${inTable}_${ID}_1"}\n|;
            ...
            my $q =
qq|select S1,S2,S3,S4,L1,L2,L3,L4,Prefs from ClientSummary where ClientID='$self->{Client_ClientID_1}'|;
...
#warn qq|setDefaults: ClientTrPlan: ClientID=$self->{Client_ClientID_1}\n${q}\n|;
            ...
            $s->finish();
            my $q =
qq|select Problem,Summary,Services,Referrals from ClientIntake where ClientID='$self->{Client_ClientID_1}'|;
...
#warn qq|setDefaults: ClientTrPlan: ClientID=$self->{Client_ClientID_1}\n${q}\n|;
        ...
        my $s = $dbh->prepare(
"select CopyID from ClientTrPlan where ID='$self->{'ClientTrPlan_ID_1'}'"
        );
        $s->execute()
          || $self->dberror("setDefault: select ClientTrPlan CopyID");
        my ($PrevID) = $s->fetchrow_array;
        $s->finish();

#warn qq|setDefault:  ${inTable}: PrevID=${PrevID}, TrPlanID=$self->{'ClientTrPlan_ID_1'}\n|;
        my $s = $dbh->prepare(
"select count(*) from ClientTrPlanPG where TrPlanID='$self->{'ClientTrPlan_ID_1'}'"
        );
        $s->execute()
          || $self->dberror("setDefault: select ClientTrPlanPG count");
        ...
        $s->execute()
          || $self->dberror("setDefault: select CLientTrPlanPG ${PrevID}");
        ...
        my $s = $dbh->prepare(
"select CopyID from ClientTrPlanPG where ID='$self->{'ClientTrPlanPG_ID_1'}'"
        );
        $s->execute()
          || $self->dberror("setDefault: select ClientTrPlanPG CopyID");
        my ($PrevID) = $s->fetchrow_array;
        $s->finish();

#warn qq|setDefault:  ${inTable}: PrevID=${PrevID}, TrPlanPGID=$self->{'ClientTrPlanPG_ID_1'}\n|;
        my $s = $dbh->prepare(
"select count(*) from ClientTrPlanOBJ where TrPlanPGID='$self->{'ClientTrPlanPG_ID_1'}'"
        );
        $s->execute()
          || $self->dberror("setDefault: select ClientTrPlanOBJ count");
        my ($cnt) = $s->fetchrow_array;
        $s->finish();
...
"select * from ClientTrPlanOBJ where TrPlanPGID='$PrevID' and ResolvedDate is null"
        ...
        $s->execute()
          || $self->dberror("setDefault: select CLientTrPlanOBJ ${PrevID}");
        while ( my $r = $s->fetchrow_hashref ) {
            $i++;
            $rLast = $r if ( $cnt < $i );
            last        if ( $i == $cnt + 1 );
        ...
        $self->{'ClientPrAuth_ReqType_1'} =
          DBA->setPrAuthReqType( $self, $ClientID );
        my ( $isAdult, $TL ) =
          TLevel->getTreatmentLevel( $self, $ClientID, '18' );
        ...
        || $inTable eq 'PDDom' )
    {
        my $r = DBA->getLAST(
            ...
            '',
            $inTable,
            "where ClientPrAuth.ClientID='$self->{Client_ClientID_1}'",
...
#warn qq|setDefault: PDDiag ${inTable}_Axis1ACode_1=$self->{"${inTable}_Axis1ACode_1"}\n|;
        ...
        if ( $inTable eq 'PDDiag' ) {
            ...
            my $q =
qq|select Axis5Curr,Axis5High from ClientSocial where ClientID='$self->{Client_ClientID}'|;
            my $s = $dbh->prepare($q);
            $s->execute() || $self->dberror($q);
...
#   these are only updated in billing routines
        ...
        $self->{'Treatment_StatusDate_1'} =
          $self->{TODAY};                         # Billed Status = new date.
...
#warn qq|Treatment: DupNote\n| if ( SysAccess->verify($self,'Privilege=DupNote') );
        if ( $self->{'AppointmentID'} ) {
            ...
            $self->{'Treatment_ContLogDate_1'} =
              $self->{'AppointmentContactDate'};
            $self->{'Treatment_ContLogBegTime_1'} =
              $self->{'AppointmentBeginTime'};
        }
            ...
            my $r = DBA->getLAST(
                ...
                '',
                $inTable,
"where Treatment.ClientID='$self->{Client_ClientID_1}' and Treatment.ProvID='$self->{LOGINPROVID}' and Treatment.Type!=3",
        ...
        if ( $self->{'AppointmentID'} ) {
            ...
            my $s = $dbh->prepare(
                "select * from ProgNotes where NoteID='$self->{useTrID}'");
        ...
        {
            my $s = $dbh->prepare(
                "select * from PhysNotes where NoteID='$self->{useTrID}'");
            $s->execute() || $self->dberror("setDef: useTrID=$self->{useTrID}");
            my $r = $s->fetchrow_hashref;
            if ($r) {
                $self->setTable( $inTable, $r );
        ...
        if ( $h eq '' || $w eq '' )    # get them from Intake
        {
            ...
            $s->execute()
              || $self->dberror("setDef PhysNotes: ClientID=${ClientID}");
            my $r = $s->fetchrow_hashref;
            $h                            = $r->{'Height'} if ( $h eq '' );
            $w                            = $r->{'Weight'} if ( $w eq '' );
        ...
        if ( $self->{'useTrID'} ) {
            my $s = $dbh->prepare(
"select * from ClientTherapyNotes where NoteID='$self->{useTrID}'"
            );
            $s->execute() || $self->dberror("setDef: useTrID=$self->{useTrID}");
            my $r = $s->fetchrow_hashref;
            if ($r) {
        ...
        my $qClientIntake =
qq|select * from ClientIntake where ClientID='$self->{Client_ClientID}'|;
        ...
        $self->{ClientDischarge_ServiceFocus_1} =
          $rClientIntake->{ServiceFocus};
        $self->{ClientDischarge_InitCond_1} = $rClientIntake->{Problem};
        $self->{ClientDischarge_IDNeeds_1} =
          DBA->getxref( $self, 'xProblems', $rClientIntake->{'Problem1'},
            'Descr' );
        $self->{ClientDischarge_IDNeeds_1} .= "; "
          . DBA->getxref( $self, 'xProblems', $rClientIntake->{'Problem2'},
            'Descr' )
          if ( $rClientIntake->{'Problem2'} ne '' );
        $self->{ClientDischarge_IDNeeds_1} .= "; "
          . DBA->getxref( $self, 'xProblems', $rClientIntake->{'Problem3'},
            'Descr' )
       ...
       #warn qq|ClientID=$rClientTrPlan->{ClientID}, ID=$rClientTrPlan->{ID}\n|;
        ...
        $self->{'ClientDischarge_Needs_1'} .= qq|; $rClientTrPlan->{'SA2'}|
          if ( $rClientTrPlan->{'SA2'} ne '' );
        $self->{'ClientDischarge_Needs_1'} .= qq|; $rClientTrPlan->{'SA3'}|
          if ( $rClientTrPlan->{'SA3'} ne '' );
        $self->{'ClientDischarge_Needs_1'} .= qq|; $rClientTrPlan->{'SA4'}|
          if ( $rClientTrPlan->{'SA4'} ne '' );
        $self->{'ClientDischarge_Needs_1'} .= qq|; $rClientTrPlan->{'L1'}|
          if ( $rClientTrPlan->{'L1'} ne '' );
        $self->{'ClientDischarge_Needs_1'} .= qq|; $rClientTrPlan->{'L2'}|
          if ( $rClientTrPlan->{'L2'} ne '' );
        $self->{'ClientDischarge_Needs_1'} .= qq|; $rClientTrPlan->{'L3'}|
          if ( $rClientTrPlan->{'L3'} ne '' );
        $self->{'ClientDischarge_Needs_1'} .= qq|; $rClientTrPlan->{'L4'}|
          if ( $rClientTrPlan->{'L4'} ne '' );
        $self->{'ClientDischarge_DischargePlan_1'} =
          $rClientTrPlan->{'TransitionPlan'};
        my $sClientDevl =
          $dbh->prepare("select * from ClientDevl where ClientID=?");
        $sClientDevl->execute( $self->{Client_ClientID} )
          || $self->dberror(
"setDefault: ClientDischarge: select ClientDevl $self->{Client_ClientID}"
          );
        ...
        $sClientSocial->execute( $self->{Client_ClientID} )
          || $self->dberror(
"setDefault: ClientDischarge: select ClientSocial $self->{Client_ClientID}"
          );
        ...
        unless ( $ClientID eq 'new' ) {
            my $val = DBA->getLastID( $self, $inTable, "G1=$ClientID",
                "CreateDate desc" );
            if ($val) {
...
qq|select ValidDL,AutoForUse,RegSupport,MajSupport from ClientResources where ClientID='$ClientID'|;
        ...
        my $ClientID = $self->{'Client_ClientID'};
        unless ( $ClientID eq 'new' ) {
            my $val = DBA->getLastID( $self, $inTable, "ClientID=$ClientID",
                "AdmDate desc" );
...
#$self->{'Contracts_RefID_1'} = '1D';               # Medicaid Provider Number for PIN
      ...
      #foreach my $f ( sort keys %{$self} ) { warn qq|self: $f=$self->{$f}\n|; }
        ...
        my ( $TransDate, $TransTime ) =
          DBA->getPrAuthTransDT( $self, $ClientID, $self->{TODAY} );
        ...
        $self->{'ClientPrAuthCDC_CDCOK_1'} =
          $ClientID eq 'new' ? 1 : CDC->required( $self, $InsID ) ? 0 : 1;
    }
        ...
        # as of last note...
        my $qTreatment =
qq|select * from Treatment where ClientID='$self->{Client_ClientID}' order by ContLogDate desc|;
        my $sTreatment = $dbh->prepare($qTreatment);
        $sTreatment->execute() || $self->dberror($qTreatment);
        my $rTreatment = $sTreatment->fetchrow_hashref;
        $sTreatment->finish();
        $self->{'ClientDischargeCDC_TransDate_1'} =
          $rTreatment->{'ContLogDate'};
        $self->{'ClientDischargeCDC_TransTime_1'} =
...
#warn "BEGIN: TBLselect: inTable=$inTable, id=$id, val=$val, AccFunc=$AccFunc\n";
    SysAccess->verify( $self, $AccFunc )
      || $self->error("Access DENIED to ${inTable} (TBLselect)");
    ...
    while ( my $r = $s->fetchrow_hashref ) {
        ...
        foreach my $f ( keys %{$r} ) {
            ...
            $self->{$key} = $r->{$f};
            $self->{$key} = ''
              if ( $f =~ /date/i && $self->{$key} eq '0000-00-00' );
...
sub TMPwrite {
    ...
    $hidden .=
      qq|<INPUT TYPE="hidden" NAME="LINKID" VALUE="$self->{LINKID}">\n|;
    $hidden .=
      qq|<INPUT TYPE="hidden" NAME="misLINKS" VALUE="$self->{misLINKS}">\n|;
    $hidden .=
qq|<INPUT TYPE="hidden" NAME="NONAVIGATION" VALUE="$self->{NONAVIGATION}">\n|;
    ...
    my $pathname =
      $self->{DOCROOT} . '/tmp/' . $self->{LOGINID} . '_' . $FORMID;
    if ( !open( TEMPLATE, ">$pathname" ) ) {
...
sub exFunc {
    ...
    my ( $function_arguments, $return_string ) =
      split( /\)/, $function_arguments, 2 );
...
#warn qq|SET FUNCTION: 1=|.$function_args[1].qq|\n|;
    ...
    if ( $acc && $neg && $funcval ) {
        $self->error(
            "Access Denied to Page<BR>(${view})<BR>($function_args[1])");
    }
    elsif ( $acc && !$neg && !$funcval ) {
        $self->error(
            "Access Denied to Page<BR>(${view})<BR>($function_args[1])");
    }
    elsif ($acc) { $acc = 0; }    # just to do nothing
    elsif (
        $return_string eq 'return0' ) # this is needed incase funcval is 0 or ''
    { $rtnval = $funcval; }
    elsif ($neg) {
        $rtnval = $return_string eq '' ? !$funcval : $return_string;
    }
    elsif ( !$neg
        && $funcval )    # will return funcval or return_string, but not 0
    { $rtnval = $return_string eq '' ? $funcval : $return_string; }
...
sub updLINK     # call myForm->
{
    ...
    $myForm::FORM = $self;
    my ( $ProvID, $pgm, $url, $browser, $ip ) =
      myForm->updLINK( $lid, $old, $new );
    $self = $myForm::FORM;
    return ( $ProvID, $pgm, $url, $browser, $ip );
}
