...
############################################################################
# warnings come from...
# Using an array as a reference is deprecated at /home/okmis/mis/src/lib/PDF.pm line 178.
# Using an array as a reference is deprecated at /home/okmis/mis/src/lib/PDF.pm line 225.
# Using an array as a reference is deprecated at /home/okmis/mis/src/lib/PDF.pm line 250.
# Using an array as a reference is deprecated at /home/okmis/mis/src/lib/PDF.pm line 268.
# Using an array as a reference is deprecated at /home/okmis/mis/src/lib/PDF.pm line 287.
# Using an array as a reference is deprecated at /home/okmis/mis/src/lib/PDF.pm line 299.
...
sub addText
{
  ...
  my $linelen = afm::stringwidth(' ' . $line, $pointsize, %{$self->{times}});
  foreach my $word (@words)
  {
    my $wordlen = afm::stringwidth(' ' . $word, $pointsize, %{$self->{times}});
    if ($linelen + $wordlen > $width)
    {
      ...
      $line = $word;
      $linelen = afm::stringwidth($word, $pointsize, %{$self->{times}});
    }
    else
    {
      $line .= ' ' . $word;
...
endobj
";
  my $curlen = @{$self->{objectStart}}->[$#{$self->{objectStart}}];
...
sub setPagesObj($) 
{
  ...
  { $pageobj+=1; $out .= "${pageobj} 0 R\n"; }
  my $pagecnt = $pageobj - $pagesobj;

  $out .= "]\n/Count ${pagecnt}\n>>\nendobj\n";
  $self->{outText} .= $out;
  my $lastlen = @{$self->{objectStart}}->[$#{$self->{objectStart}}];
...
sub setXRef($$$)
{
  ...
  my $xrefoffset = @{$self->{objectStart}}->[$#{$self->{objectStart}}];
