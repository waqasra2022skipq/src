...
sub CheckNote {
    ...
    my $q =
qq|select *, Treatment.ProvID as ProvID, Client.ProvID as PrimaryProvID from Treatment left join Client on Client.ClientID=Treatment.ClientID left join xSC on xSC.SCID=Treatment.SCID where TrID='${TrID}'|;
    my $s = $dbh->prepare(
"select *, Treatment.ProvID as ProvID, Client.ProvID as PrimaryProvID from Treatment left join Client on Client.ClientID=Treatment.ClientID where Treatment.TrID=?"
    );
    ...
    my $rxSC =
      $self->getServiceCode( $form, $r->{SCID}, $r->{ContLogDate},
...
#foreach my $f ( sort keys %{$rxSC} ) { warn "CheckNote: rxSC-${f}=$rxSC->{$f}\n"; }
    ...
    if ( $r->{'ContLogDate'} eq '' || $r->{'ContLogDate'} eq '0000-00-00' ) {
        $msg .= qq| MISSING Contact Date! |;
        $code++;
    }

    # Check for valid Service Code and rates...
    ( $chkcode, $chkmsg ) =
      $self->CheckServiceCode( $form, $r->{SCID}, $r->{ContLogDate},
        $r->{ContLogBegTime}, $r->{ContLogEndTime}, $r->{TrID},
        $r->{BillDate} );
    if    ( $chkcode && $chkmsg ) { $msg .= $chkmsg; $code++; }
    elsif ($chkmsg)               { $msg .= $chkmsg; }

    # Check for Duplicate Service Code for Client on same date...
    ( $chkcode, $chkmsg ) =
      $self->CheckClientDUP( $form, $r->{ClientID}, $r->{ProvID}, $r->{TrID},
        $r->{SCID}, $r->{ContLogDate} );
    if    ( $chkcode && $chkmsg ) { $msg .= $chkmsg; $code++; }
    elsif ($chkmsg)               { $msg .= $chkmsg; }
    ...
    ( $chkcode, $chkmsg ) =
      $self->CheckInsurance( $form, $r->{ClientID}, $rxSC->{'InsID'},
        $r->{ContLogDate} );
    if    ( $chkcode && $chkmsg ) { $msg .= $chkmsg; $code++; }
    elsif ($chkmsg)               { $msg .= $chkmsg; }
    ...
    ( $chkcode, $chkmsg ) =
      $self->CheckContract( $form, $r->{ClinicID}, $rxSC->{'InsID'},
        $r->{ProvID}, $r->{ClientID}, $r->{ContLogDate} );
    if    ( $chkcode && $chkmsg ) { $msg .= $chkmsg; $code++; }
    elsif ($chkmsg)               { $msg .= $chkmsg; }
    ...
    if ( $rxSC->{PAReq} ) {
        ( $chkcode, $chkmsg ) =
          $self->CheckAuth( $form, $r->{ClientID}, $rxSC->{'InsID'},
            $r->{ContLogDate}, $r->{SCID} );
        if    ( $chkcode && $chkmsg ) { $msg .= $chkmsg; $code++; }
        elsif ($chkmsg)               { $msg .= $chkmsg; }
    }
    ...
    if ( $POSFederal =~ /21|51|61/ ) {
        ...
        $sClientRelations->execute( $r->{'ClientID'} )
          || $form->dberror(
            "ChkNote: select ClientRelations ($r->{'ClientID'}");
        ...
        if ( $rClientRelations->{'ResAdmitDate'} eq '' ) {
            ...
            $msg .=
qq| MISSING Admit Date for PlaceOfService: ${POSDescr} (change Date on Relations screen)|;
            $code++;
    ...
    ( $chkcode, $chkmsg ) =
      $self->CheckProblems( $form, $r->{ClientID}, $r->{TrID} );
...
sub CheckServiceCode {
    my ( $self, $form, $SCID, $ContDate, $BegTime, $EndTime, $TrID, $BillDate )
      = @_;

    my $rxSC =
      $self->getServiceCode( $form, $SCID, $ContDate, $BegTime, $EndTime,
        $TrID, $BillDate );
...
sub CheckClientDUP {
    ...
    return ( 0, '' )
      if ( $ClientID < 10 || $ClientID == 67585 );    # ODMH and NONID Clients
    ...
    return ( 0, '' )
      if ( $SCNum eq 'S0215 HF TG' );    # Medicaid Home/Community Based Travel.
    return ( 0, '' )
      if ( $SCNum eq 'H0001 HF TG U1' )
      ;    # Medicaid Residential Screening and Referral (ODASL).
    return ( 0, '' )
      if ( $SCNum eq 'H0001 HF QJ' )
      ;    # Drug Court (2nd & 3rd evemts) (Prison codes)
    return ( 0, '' )
      if ( $SCNum eq 'T1017 TS' );    # HealthHome Targeted Case Management.
    return ( 0, '' )
      if ( $SCNum eq 'S9482 HE' )
      ;    # HealthHome Clinical Evaluation and Assessment Children
...
#warn qq|CheckClientDUP: ClientID=${ClientID},ProvID=${ProvID},TrID=${TrID},SCNum=${SCNum},ContDate=${ContDate}\n|;
    ...
    ##my $addInsCheck = 'and';
    if ( $addInsCheck eq
        'and' )    ## NO DUPlicate on different Insurances. (added 8/10/17)
    {
        my $sTreatment = $dbh->prepare(
"select * from Treatment left join xSC on xSC.SCID=Treatment.SCID where Treatment.TrID='${TrID}'"
        );
        $sTreatment->execute()
          || $form->dberror(
            "CheckClientDUP: select error ($ClientID/$ContDate/$SCNum}");
        my $rTreatment = $sTreatment->fetchrow_hashref;
        $addInsCheck .= qq| xSC.InsID='$rTreatment->{'InsID'}'|;
    }
    my $s = $dbh->prepare(
"select * from Treatment left join xSC on xSC.SCID=Treatment.SCID where Treatment.ClientID='${ClientID}' and Treatment.ProvID='${ProvID}' and Treatment.TrID!='${TrID}' and Treatment.ContLogDate='${ContDate}' and Treatment.BillStatus!=4 and xSC.SCNum='${SCNum}' ${addInsCheck}"
    );
    $s->execute()
      || $form->dberror(
"CheckClientDUP: select error (${ClientID}/${ContDate}/${SCNum}/${addInsCheck}"
      );
    ...
    if ( $r->{TrID} ) {
        return ( 1,
"Duplicate Note: $r->{TrID}! Same Service Code for Client on same day! Combine duplicate notes into 1 note for Client for same Service Code on the same Contact Date. Document services inside of note. "
        );
    }
    return ( 0, '' );
}
...
sub CheckClient {
    ...
    $s->execute($ClientID)
      || $form->dberror("CheckClient: select error ($ClientID}");
    my $r = $s->fetchrow_hashref;
    $s->finish();
    if ( $r->{clinicClinicID} eq '' ) {
        return ( 1, 'Client Clinic is NULL! ', $r );
    }
    if ( $r->{FName} eq '' ) { return ( 1, 'Client FName is NULL! ', $r ); }
    if ( length( $r->{FName} ) < 2 ) {
        unless ( $r->{FName} =~ /[A-Z]/ ) {
            return ( 1, 'Client FName invalid! ', $r );
        }
    }
    if ( $r->{LName} eq '' ) { return ( 1, 'Client LName is NULL! ', $r ); }
    if ( length( $r->{LName} ) < 2 ) {
        unless ( $r->{LName} =~ /[A-Z]/ ) {
 ...
 # Do city/state checks only if the Client is Not Homeless and Client Zip is Not
        if ( $r->{City} eq '' ) { return ( 1, 'Client City is NULL! ', $r ); }
        if ( $r->{City} eq '.' ) {
            return ( 1, 'Client City is a period! ', $r );
        }
        if ( $r->{ST} eq '' ) { return ( 1, 'Client State is NULL! ', $r ); }
    }

    if ( $r->{Zip} eq '' ) { return ( 1, 'Client Zip is NULL! ', $r ); }
...
sub CheckProvider {
    ...
    if ( $rProvider->{LName} eq '' ) {
        $code++;
        $msg .= " Primary Provider LName is NULL! ";
    }
    if ( $rProvider->{FName} eq '' ) {
        $code++;
        $msg .= " Primary Provider FName is NULL! ";
    }
    if ( $rProvider->{SSN} eq '' ) {
        $code++;
        $msg .= " Primary Provider SSN is NULL! ";
    }
    $sProvider->finish();
    return ( $code, $msg );
}
...
qq|select xSC.*, xSCRates.*, xInsurance.Descr as InsDescr, xInsurance.Name as InsName,xInsurance.InsCode, xInsurance.Email as InsEmail from xSC left join xInsurance on xInsurance.ID=xSC.InsID left join xSCRates on xSCRates.SCID=xSC.SCID where xSC.SCID='${SCID}' and xSCRates.EffDate<='${ContDate}'  and ('${ContDate}'<=xSCRates.ExpDate or xSCRates.ExpDate is null) |;
    ...
    {
        $q =
qq|select xSC.*, xInsurance.Descr as InsDescr, xInsurance.Name as InsName ,xInsurance.InsCode, xInsurance.Email as InsEmail from xSC left join xInsurance on xInsurance.ID=xSC.InsID where xSC.SCID = ${SCID}|;
        $s = $dbh->prepare($q);
        $s->execute() || $form->dberror($q);
    ...
    my $Units    = 0;
    if    ( $rxSC->{UnitLbl} eq 'Each' ) { $Units = 1; }
    ...
    elsif ( $rxSC->{UnitLbl} eq 'Minutes' ) { $Units = $Duration / 60; }
    else { $Units = int( ( $Duration / 3600 / $rxSC->{HrsPerUnit} ) + .5 ); }
...
#warn qq|getServiceCode: Units=${Units} $rxSC->{Units}, ServiceRate=$rxSC->{ServiceRate}\n|;
    if ($TrID) {
        my $sTreatment = $dbh->prepare(
            "select BilledAmt,IncAmt,SchAmt,AmtDue from Treatment where TrID=?"
        );
        $sTreatment->execute($TrID)
          || $form->dberror("getServiceCode: select Treatment (${TrID})");
 ...
 # Recouped more than billed: (trans: +25,-35)=-10 paid) then 25-(-10) = 35 owed
        ...
        $rxSC->{AmtDue} =
          sprintf( "%.2f", $rxSC->{BillAmt} - abs( $rxSC->{RecAmt} ) );
    }
...
sub CheckInsurance {
    ...
    my ( $code, $msg ) = ( 0, '' );
    if ( my $rInsurance =
        $self->getInsurance( $form, $ClientID, $InsID, $InDate ) )
    {
        if ( $rInsurance->{InsIDNum} eq '' ) {
            $code = 1;
            $msg  = "InsNum is NULL! ";
        }
        if ( $rInsurance->{InsDescr} =~ /medicaid/i ) {
            ...
            if (   $rInsurance->{InsIDNum} !~ /^B\d{8}$/
                && $rInsurance->{InsIDNum} !~ /^\d{9}$/ )
            {
                $code = 1;
                $msg  = "InsNum MUST BE 9 digits or B followed by 8 digits! ";
            }
        }
    }
    else {
        $code = 1;
        $msg =
"Client Insurance not found around ${InDate}! Check Client Insurance Effective Date is before Note Contact Date. ";
    }
    return ( $code, $msg );
}
...
|;
    ...
    if ( $rContracts = $sContracts->fetchrow_hashref ) {
        unless ( $rContracts->{BillFlag} ) {
            $code++;
            $msg .= "Contract not set to BILL! ";
        }

#   Use the Agency as the BillTo/PayTo Provider Address? (not the incoming Clinic?)
        my $UseClinicID =
          $rContracts->{UseAgency}
          ? MgrTree->getAgency( $form, $ClinicID )
          : $ClinicID;
        my $sProvider = $dbh->prepare("select * from Provider where ProvID=?");
        ...
        if ( $rContracts->{UseReferring} ) {
            my $s = $dbh->prepare(
"select RefPhysNPI from ClientReferrals where ClientID='${ClientID}'"
            );
            $s->execute()
              || $self->dberror(
                "CheckContract: ERROR: select RefPhysNPI (${ClientID}");
            my ($NPI) = $s->fetchrow_array;
            $s->finish();

#warn qq|CheckClient: NPI=${NPI}, Referring\n| if ( $form->{'LOGINPROVID'} == 91 );
            ( $chkcode, $chkmsg ) =
              $self->CheckNPI( $form, $NPI, 'Referring Physician' );
            if    ( $chkcode && $chkmsg ) { $msg .= $chkmsg; $code++; }
            elsif ($chkmsg)               { $msg .= $chkmsg; }
        }
        ...
        if ( $rContracts->{UseRendering} ) {
            ...
            my $rInsurance =
              $self->getInsurance( $form, $ClientID, $InsID, $InDate );
            my $UseProvID =
              $rInsurance->{DesigProvID} ? $rInsurance->{DesigProvID} : $ProvID;
            ( $chkcode, $chkmsg ) =
              $self->CheckCredentials( $form, $UseProvID, $InsID,
                $rContracts->{NPI} );
            if    ( $chkcode && $chkmsg ) { $msg .= $chkmsg; $code++; }
            elsif ($chkmsg)               { $msg .= $chkmsg; }
        }
        ...
        if ( $rContracts->{UseSFacility} ) {
            my $s = $dbh->prepare(
"select FacIDNPI,Residence from ClientRelations where ClientID='${ClientID}'"
            );
            $s->execute()
              || $self->dberror(
                "CheckContract: ERROR: select FacIDNPI (${ClientID}");
...
#warn qq|CheckClient: NPI=${NPI}, Residence=$Residence\n| if ( $form->{'LOGINPROVID'} == 91 );
            ...
            if ( $ResCDC =~ /F|G/ ) {
                ( $chkcode, $chkmsg ) =
                  $self->CheckNPI( $form, $NPI, 'Service Facility' );
                if    ( $chkcode && $chkmsg ) { $msg .= $chkmsg; $code++; }
                elsif ($chkmsg)               { $msg .= $chkmsg; }
            }
        }
...
"select Provider.ProvID,Provider.LName,Provider.FName,Provider.SSN,ProviderControl.NPI from Provider left join ProviderControl on ProviderControl.ProvID=Provider.ProvID where Provider.ProvID=?"
    );
    $sProvider->execute($ProvID);
    my $rProvider = $sProvider->fetchrow_hashref;
    if ( $rProvider->{LName} eq '' ) {
        $code++;
        $msg .= 'Provider LName is NULL! ';
    }
    if ( $rProvider->{FName} eq '' ) {
        $code++;
        $msg .= 'Provider FName is NULL! ';
    }
    if ( $rProvider->{SSN} eq '' ) {
        $code++;
        $msg .= 'Provider SSN is NULL! ';
    }
    if ( $rProvider->{NPI} eq '' ) {
        $code++;
        $msg .= 'Provider NPI is NULL! ';
    }
...
"select Credentials.* from Credentials left join okmis_config.xCredentials on xCredentials.ID=Credentials.CredID where ProvID=? and InsID=? order by Credentials.Rank"
    );
    $sCredentials->execute( $ProvID, $InsID );
    my $rCredentials = $sCredentials->fetchrow_hashref;
    if ( $rCredentials->{Taxonomy} eq '' ) {
        $code++;
        $msg .= 'Provider Credential Taxonomy is NULL! ';
    }
    if ($code) {
        $msg .= qq| $rProvider->{FName} $rProvider->{LName} ($ProvID) |;
    }
    $sCredentials->finish();
    $sProvider->finish();
...
sub CheckNPI {
    ...
    if ( $rxNPI->{'NPI'} ne '' ) {
        if ( $Type =~ /physician/i ) {
            if ( $rxNPI->{ProvLastName} eq '' ) {
                $msg .= qq|${Type} LName is NULL! |;
                $code = 1;
            }
            if ( $rxNPI->{ProvFirstName} eq '' ) {
                $msg .= qq|${Type} FName is NULL! |;
                $code = 1;
            }
        }
        else {
            if ( $rxNPI->{ProvOrgName} eq '' ) {
                $msg .= qq|${Type} NPI ProvOrgName is NULL! |;
                $code = 1;
            }
            if ( $rxNPI->{Addr1} eq '' ) {
                $msg .= qq|${Type} NPI Address is NULL! |;
                $code = 1;
            }
            if ( $rxNPI->{City} eq '' ) {
                $msg .= qq|${Type} NPI City is NULL! |;
                $code = 1;
            }
            if ( $rxNPI->{ST} eq '' ) {
                $msg .= qq|${Type} NPI State is NULL! |;
                $code = 1;
            }
            if ( $rxNPI->{Zip} eq '' ) {
                $msg .= qq|${Type} NPI Zip is NULL! |;
                $code = 1;
            }
...
" );
    $sClientNoteProblems->execute($TrID)
      || $form->dberror("CheckNoteProblems: select ClientNoteProblems ($TrID)");
...
#foreach my $f ( sort keys %{$rClientNoteProblems} ) { warn "CheckProblems: CNP-${f}=$rClientNoteProblems->{$f}\n"; }
    ...
    return ( 0, '' )
      if ( substr( $rClientNoteProblems->{SCNum}, 0, 1 ) eq 'X' );
    my $rows = $sClientNoteProblems->rows;

    #warn qq|CheckProblems: TrID=$TrID, rows=$rows\n|;
    if ($rows) {
        $sCheckF->execute($TrID)
          || $form->dberror(
            "CheckNoteProblems: select ClientNoteProblems ($TrID) for F");
        my $Fs = $sCheckF->rows;
        ...
        unless ($Fs) {
            $code = 1;
            $msg =
qq|NO Mental Health Problems Addressed on Note, please check at least 1 ICD10 starting with F!|;
        }
    }
...
#foreach my $f ( sort keys %{$rPrAuth} ) { warn "CheckAuth: ${f}=$rPrAuth->{$f}\n"; }
        ...
        if ( !$MatchSCID ) {
            $msg .= 'NO Prior Authorization for Service Code. ';
        }
        elsif ( $rPrAuth->{PAnumber} eq '' ) {
            $msg .= 'Prior Authorization Number is NULL. ';
        }
        elsif ($rPrAuth->{InsDescr} != /medicaid/i
            && $rPrAuth->{InsDescr} ne 'medicare' )
        {
            $code = 0;
        }
        elsif ( $rPrAuth->{PAnumber} =~ /^\d{10}$/ ) { $code = 0; }
        elsif ( $rPrAuth->{PAnumber} =~ /^(PB|MC)\d{8}$/ ) {
            $code = 0;
        }    # DDSD waiver
...
sub CheckDiag() {
    ...
    if ( my $rPDDiag = $self->getDiag( $form, $ClientID, $InsID, $InDate ) ) {
        ...
        if    ( $tAxis1ACode eq '' ) { $msg .= 'Missing Diagnosis code. '; }
            ...
            $code = 1;
            $msg .=
"1: Diagnosis 1A Code format error! ($rPDDiag->{Axis1ACode}: =${tAxis1ACode}=)";
        }
        elsif ( $rPDDiag->{access} =~ /notfordate/i ) {
            $msg .= 'Diagnosis record not for Contact Date. ';
        }
        elsif ( $rPDDiag->{access} =~ /default/i ) {
            $msg .= 'Default Diagnosis record. ';
        }
...
sub getInsurance {
    ...
    my $InsPriority =
        $InsID =~ /primary/   ? qq| and Insurance.Priority=1 |
...
|;
        ...
        $sPDDiag->execute() || $form->dberror($qPDDiag);
        if ( $rPDDiag = $sPDDiag->fetchrow_hashref ) {
            $rPDDiag->{access} = 'notfordate';
        }
        else {
