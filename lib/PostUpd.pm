...
# set these...
    my $s = $dbh->prepare(
        "update Client set HmPh=MobPh where ClientID=? and HmPh is null");
    $s->execute($ClientID)
      || myDBI->dberror("newClient: update HmPh ${ClientID}!");
    $s->finish();
    ...
    unless ( $form->{'Insurance_EligibleType_1'} eq
        'No' )    # add Eligible for this month
    {
        my ( $FromDate, $ToDate ) = DBUtil->Date( '', 'monthly' );
        $sx = $dbh->prepare("select * from xEligibleType where Descr=?");
        $sx->execute( $form->{'Insurance_EligibleType_1'} )
          || myDBI->dberror("newClient: select xEligibleType!");
    ...
    if ( SysAccess->getRule( $form, 'EnableCDC' ) ) {
        ...
        $sClientReferrals = $dbh->prepare(
            "select RefDate from ClientReferrals where ClientID='${ClientID}'");
        $sClientReferrals->execute()
          || myDBI->dberror("newClient: select ClientReferrals ${ClientID}");
        my ($RefDate) = $sClientReferrals->fetchrow_array;
        $sClientReferrals->finish();

        #   calcPG is done again in setPrAuthTL (updPrAuth)
        my $PAgroup =
          DBA->isIndMedicaid( $form, $ClientID ) ? 'PG030' : 'PG038';
        ...
        DBA->doUpdate( $form, 'ClientPrAuthCDC', $r,
...
sub updPA         # only for UN-Authorized (Expired after today and No PAnumber)
{
  ...
  #foreach my $f ( sort keys %{$form} ) { warn "updPA: form-$f=$form->{$f}\n"; }
    ...
    my $qPrAuth =
qq|select ID from ClientPrAuth where ClientPrAuth.ClientID=${ClientID} and ClientPrAuth.ExpDate>='$form->{TODAY}' and ClientPrAuth.PAnumber is null|;
...
"select ClientPrAuth.*,Insurance.InsID from ClientPrAuth left join Insurance on Insurance.InsNumID=ClientPrAuth.InsuranceID where ClientPrAuth.ID='${PrAuthID}'"
    );
    ...
    if ( my $rPrAuth = $sPrAuth->fetchrow_hashref ) {
     ...
     #   and the PAgroup only for those Treatment Levels in xPAgroup.
        ...
        my $sUpdRVU = $dbh->prepare(
"update PrAuthRVU set EffDate='$rPrAuth->{EffDate}', ExpDate='$rPrAuth->{ExpDate}' where PrAuthID='${PrAuthID}'"
        );
        ...
        if ( $form->{'CLOSE'} eq $PrAuthID ) {
            my $sUpdCLOSE = $dbh->prepare(
"update ClientPrAuth set ExpDate='$rPrAuth->{EffDate}' where ID='${PrAuthID}'"
            );

#warn "update ClientPrAuth set ExpDate='$rPrAuth->{EffDate}' where ID='${PrAuthID}'\n";
            $sUpdCLOSE->execute()
              || myDBI->dberror("updPrAuth: CLOSE for dates");
            $sUpdCLOSE->finish();
            my $sUpdCLOSE = $dbh->prepare(
"update ClientPrAuthCDC set Status='Closed',StatusDate='$form->{TODAY}',Fail=NULL where ClientPrAuthID='${PrAuthID}'"
            );
            $sUpdCLOSE->execute()
              || myDBI->dberror("updPrAuth: CLOSE for dates");
            $sUpdCLOSE->finish();
        ...
        if ( $form->{'REOPEN'} eq $PrAuthID ) {
            my ( $months, $days ) =
              DBA->calcLOS( $form, $rPrAuth->{'InsID'}, $rPrAuth->{'PAgroup'} );
            my $ExpDate = DBUtil->Date( $rPrAuth->{'EffDate'}, $months, $days );
            my $sUpdREOPEN = $dbh->prepare(
"update ClientPrAuth set ExpDate='${ExpDate}' where ID='${PrAuthID}'"
            );

  #warn "update ClientPrAuth set ExpDate='${ExpDate}' where ID='${PrAuthID}'\n";
            $sUpdREOPEN->execute()
              || myDBI->dberror("updPrAuth: REOPEN for dates");
            $sUpdREOPEN->finish();
            my $sUpdREOPEN = $dbh->prepare(
"update ClientPrAuthCDC set Status='New',StatusDate='$form->{TODAY}' where ClientPrAuthID='${PrAuthID}'"
            );
            $sUpdREOPEN->execute()
              || myDBI->dberror("updPrAuth: REOPEN for dates");
            $sUpdREOPEN->finish();
...
sub setPrAuthTL {
    ...
    my $dbh = myDBI->dbconnect( $form->{'DBNAME'} );
    my ( $isAdult, $TL ) =
      TLevel->getTreatmentLevel( $form, $ClientID, '18', $PrAuthID );

    #warn qq|setPrAuthTL: isAdult=$isAdult, TL=$TL\n|;
    my $qUpd =
      $TL eq ''
      ? qq|update ClientPrAuth set TL=NULL|
...
#warn qq|NewJournal: ENTER: new=$form->{NEWJOURNAL}: ID=$form->{ClientJournals_ID} \n|;
    if ( $form->{NEWJOURNAL} eq 'new' ) {
        ...
        DBUtil->emailP( $form, $form->{Client_ProvID_1},
            $Subject, $Text, '', 1 );
    }
    return ($result);
}
...
sub updNote {
    ...
    my $rxSC =
      cBill->getServiceCode( $form, $SCID, $ContLogDate, $ContLogBegTime,
        $ContLogEndTime, $TrID, $BillDate );
...
#warn qq|PostUpd: updNote: delete=$form->{Treatment_DELETE_1}, Sch=$form->{note_scholarship} \n|;
    ...
    if ( $form->{rebill_note} || $form->{delete_note} || $form->{skip_note} ) {
        ...
        my ( $trid, $scid, $code, $type ) =
          uBill->postClaim( $form, $r835, 'MS', 'SR' );
        ...
        my $q =
qq|update Treatment set MgrProvID=NULL, MgrRevDate=NULL, MgrRevTime=NULL where TrID=?|;
        my $s = $dbh->prepare($q);
...
#warn qq|updNote: review own notes: $form->{Treatment_TrID_1},$form->{TODAY},$curtime\n|;
        ...
        PostUpd->setMgrDT( $form, $TrID, $form->{LOGINPROVID}, $form->{TODAY},
            $curtime );
...
#   RevStatus:  0 = Provider needed, 1 = Provider needed after changes, 2 = Mgr needed, 3 = Note approved
        if (
            !SysAccess->getRule(
                $form, 'NoteEdit', $form->{Treatment_ProvID_1}
            )
          )    # Rule set for signing notes?
        {
            if ( $form->{Treatment_ProvOKDate_1} ) {
                $Status = 3;
            }    # ready to bill
          ...
          # save and set for Provider to look at only if not this same provider.
            if ( $form->{Treatment_ProvID_1} != $form->{LOGINPROVID} ) {
                ...
                my $ID = DBA->insLog( $form, $inTable, 'NoteID', $TrID,
                    $inTable . 'Log' );
...
# add Manager dates to Log
                my $q =
qq|update ${inTable}Log set ProvID=?,ProvOKDate=?,ProvOKTime=?,MgrProvID=?,MgrRevDate=?,MgrRevTime=? where LogID=?|;
                my $s = $dbh->prepare($q);
                $s->execute(
                    $form->{Treatment_ProvID_1},
                    $form->{Treatment_ProvOKDate_1},
                    $form->{Treatment_ProvOKTime_1},
                    $form->{LOGINPROVID},
                    $form->{TODAY},
                    $curtime,
                    $ID
    ...
    #   as if Provider entered, not Data Entry.
        ...
        $form->{Treatment_ProvID_1} == $form->{LOGINPROVID}  # Owner of the note
        || $form->{Treatment_Type_1} == 3                    # Electronic note
        || (   $rxSC->{InsID} == 100
            && $rxSC->{'SCNum'} =~ /^G9/ )    # Medicaid or HH Gcode Bundle
      )
    {
...
# Owner approves or changes something reset DT.
        if (   $form->{'Treatment_ProvOKDate_1'} eq ''
            || $form->{"${inTable}_CHANGED"} )
        {
            PostUpd->setProvDT( $form, $TrID, $form->{TODAY}, $curtime );
            uBill->setRevStatus( $form, $TrID, 2 );
        }
...
# not the owner...flag for Provider...because MgrRevDate locks...so it's DataEntry?
    ...
    if (   $form->{Treatment_Type_1} == 3
        && $form->{Treatment_Path_1} eq '' )    # Electronic Note
    {
        $form->{fwdLINK} =
qq|/cgi/bin/mis.cgi?view=NoteUpload.cgi&Client_ClientID=$ClientID&Treatment_TrID=${TrID}&mlt=$form->{mlt}&misLINKS=$form->{misLINKS}|;
        $form->{misPOP} = 1;
    }
    if ( $form->{'AppointmentID'} ) {
        my $sDelete = $dbh->prepare(
            "delete from Appointments where ID='$form->{'AppointmentID'}'");
        $sDelete->execute()
          || myDBI->dberror("delete Appointments: $form->{'AppointmentID'}");
        $sDelete->finish();
    }
...
#warn qq|PostUpd: updNote: update SCNum=$rxSC->{SCNum}, ClientID=${ClientID}, ContLogDate=${ContLogDate}\n|;
        my $sUpdate = $dbh->prepare(
            "update ClientPHQ9 set TrID=? where ClientID=? and TestDate=?");
        $sUpdate->execute( $TrID, $ClientID, $ContLogDate )
          || myDBI->dberror("updNote: ${TrID} ${ClientID} ${ContLogDate}");
        $sUpdate->finish();
        my $sUpdate = $dbh->prepare(
            "update ClientTPHQ9 set TrID=? where ClientID=? and TestDate=?");
        $sUpdate->execute( $TrID, $ClientID, $ContLogDate )
          || myDBI->dberror("updNote: ${TrID} ${ClientID} ${ContLogDate}");
        $sUpdate->finish();
    }
 ...
 #warn qq|PostUpd: updNote: check BillStatus=$form->{Treatment_BillStatus_1}\n|;
    if ( $form->{'Treatment_BillStatus_1'} !~
        /3|4|5/ )    # inprocess/scholar/reconcile
    {
...
#  PRIMARY PROCEDURE codes 90791, 90792, 90832, 90834, 90837, or 90853.
            ...
            {
                my $sxSC = $dbh->prepare(
"select SCID from xSC where SCNum='90785' and InsID=? and CredID=?"
                );
                if ( $rxSC->{'CredID'} eq '' ) {
                    $sxSC = $dbh->prepare(
"select SCID from xSC where SCNum='90785' and InsID=? AND (CredID = ? OR CredID IS NULL)"
                    );
                }
                $sxSC->execute( $rxSC->{'InsID'}, $rxSC->{'CredID'} )
                  || myDBI->dberror(
                    "set90785: $rxSC->{'TrID'},$rxSC->{'SCID'}");
        ...
        if ($SCID3) {
            my $sUpdate =
              $dbh->prepare("update Treatment set SCID3=? where TrID=?");
            $sUpdate->execute( $SCID3, $TrID )
              || myDBI->dberror("set90785: ${SCID3} ${TrID}");
            $sUpdate->finish();
        }
        else {
            my $sUpdate =
              $dbh->prepare("update Treatment set SCID3=NULL where TrID=?");
            $sUpdate->execute($TrID)
              || myDBI->dberror("set90785: NULL ${TrID}");
            $sUpdate->finish();
        }
        $self->updNoteProblems( $form, $ClientID, $TrID,
            $form->{'NoteProblems'} );
        $self->updNoteTrPlanPG( $form, $ClientID, $TrID,
            $form->{'NoteTrPlanPG'} );
        $self->updNoteFamilyP( $form, $ClientID, $TrID, $form->{'NoteFamilyI'},
            'FamilyI' );
        $self->updNoteFamilyP( $form, $ClientID, $TrID, $form->{'NoteFamilyP'},
            'FamilyP' );
        $self->updNoteMeds( $form, $ClientID, $TrID )
          ;    # these are Active ClientMeds...
        if ( $form->{Treatment_Type_1} == 4 ) {

            # if medicare note...update the interventions performed
            $self->updIntPerformed( $form, $ClientID, $TrID );
        }
        if ( $form->{Treatment_Type_1} == 2 )    # only Physician notes...
        {
            $self->updNoteProcedures( $form, $ClientID, $TrID );
        }                                        # set the DocMeds Procedure...
        my ( $code, $msg ) =
          cBill->CheckNote( $form, $form->{'Treatment_TrID_1'} );
        if ( $code && $msg ne '' ) {
            my $sUnBill = $dbh->prepare(
                "update Treatment set BillStatus=2, StatusMsg=? where TrID=?");
            $sUnBill->execute( $msg, $form->{'Treatment_TrID_1'} )
              || myDBI->dberror(
                "updNote: set UnBill $form->{Treatment_TrID_1}");
            $sUnBill->finish();
        }
        my ( $BilledAmt, $IncAmt, $SchAmt, $AmtDue ) =
          uBill->setBilledAmt( $form, $form->{'Treatment_TrID_1'} );
    }
    return ($result);
}
...
sub setProvDT {
    ...
    my $q =
      $date eq ''
      ? qq|update Treatment set ProvOKDate=NULL, ProvOKTime=NULL where TrID=?|
...
sub setMgrDT {
    ...
    my $dbh = myDBI->dbconnect( $form->{'DBNAME'} );
    my $q =
qq|update Treatment set MgrProvID=?, MgrRevDate=?, MgrRevTime=? where TrID=?|;
    my $s = $dbh->prepare($q);
    $s->execute( $mgr, $date, $time, $form->{Treatment_TrID_1} )
      || myDBI->dberror($q);
    $s->finish();
    return (1);
}
...
"select ClientProblems.*,misICD10.ICD10 from ClientProblems left join okmis_config.misICD10 on misICD10.ID=ClientProblems.UUID where ClientProblems.ClientID=? and ClientProblems.UUID=?"
    );
    my $sLock =
      $dbh->prepare("update ClientNoteProblems set Locked=1 where ID=?");

    # first unlocked everyone...
    my $sUnLock =
      $dbh->prepare("update ClientNoteProblems set Locked=0 where TrID=?");
    $sUnLock->execute($TrID)
      || myDBI->dberror("updNoteProblems: UnLock ${TrID}");
    $sUnLock->finish();
 ...
 #foreach my $f ( sort keys %{$r} ) { warn "updNoteProblems: r-$f=$r->{$f}\n"; }
    ...
    my $sDelete =
      $dbh->prepare("delete from ClientNoteProblems where TrID=? and Locked=0");
    $sDelete->execute($TrID)
      || myDBI->dberror("updNoteProblems: delete ${TrID}");
    $sDelete->finish();
    return (1);
}
...
# we lock it now, but unLockTrPlan routine is needed to check all notes for Client/TrPlan
    my $sLock =
      $dbh->prepare("update ClientNoteTrPlanPG set Locked=1 where ID=?");

    # CHECK IF DELETED COMES THRU HERE??
    # first unlocked everyone...
    my $sUnLock =
      $dbh->prepare("update ClientNoteTrPlanPG set Locked=0 where TrID=?");
    $sUnLock->execute($TrID)
      || myDBI->dberror("updNoteTrPlanPG: UnLock ${TrID}");
    $sUnLock->finish();
 ...
 #foreach my $f ( sort keys %{$r} ) { warn "updNoteTrPlanPG: r-$f=$r->{$f}\n"; }
    ...
    # delete those left unlocked...
    my $sDelete =
      $dbh->prepare("delete from ClientNoteTrPlanPG where TrID=? and Locked=0");
    $sDelete->execute($TrID)
      || myDBI->dberror("updNoteTrPlanPG: delete ${TrID}");
    $sDelete->finish();

    # not needed here because lock based on Signed TP by PrimaryProvider
    DBA->lockTrPlan( $form, '', $TrID )
      ;    # double check all PGs are locked for this TrID...
    return (1);
}
...
#warn qq|updNote${table}: ClientID=$ClientID, TrID=$TrID, FPIDs=$FPIDs\n|;
    my $dbh = myDBI->dbconnect( $form->{'DBNAME'} );
    my $sClientFamily =
      $dbh->prepare("select * from ClientFamily where ClientID=? and ID=?");

# we lock it now, but unLockTrPlan routine is needed to check all notes for Client/TrPlan
    my $sLock =
      $dbh->prepare("update ClientNote${table} set Locked=1 where ID=?");

    # first unlocked everyone...
    my $sUnLock =
      $dbh->prepare("update ClientNote${table} set Locked=0 where TrID=?");
    $sUnLock->execute($TrID)
      || myDBI->dberror("updNote${table}: UnLock ${TrID}");
    $sUnLock->finish();
 ...
 #foreach my $f ( sort keys %{$r} ) { warn "updNote${table}: r-$f=$r->{$f}\n"; }
    ...
    # delete those left unlocked...
    my $sDelete =
      $dbh->prepare("delete from ClientNote${table} where TrID=? and Locked=0");
    $sDelete->execute($TrID)
      || myDBI->dberror("updNote${table}: delete ${TrID}");
    $sDelete->finish();
    return (1);
}
...
sub updNoteMeds {
    ...
    $sDelete->execute($TrID)
      || myDBI->dberror("updNoteMeds: delete ClientNoteMeds ${TrID}");
    $sDelete->finish();
    my $sClientMeds = $dbh->prepare(
"select * from ClientMeds where ClientID=? and Active=1 order by DrugInfo, PrescriptionDate"
    );
    $sClientMeds->execute($ClientID)
      || myDBI->dberror("updNoteMeds: select ClientMeds ${ClientID}");
...
#warn qq|updNoteProcedures: ClientID=$ClientID, TrID=$TrID\n|;
    ...
    $r->{'Active'}       = 1;
    $r->{'ProcedureID'} =
      '428191000124101';    # Documentation of current medications (procedure)
    $r->{'Rejected'} = $form->{'PhysNotes_DocMeds_1'};  # ie: SNOMEDCT_397745006

#foreach my $f ( sort keys %{$r} ) { warn "updNoteProcedures: r-$f=$r->{$f}\n"; }
    my $ID = DBA->doUpdate( $form, 'ClientProcedures', $r,
        "ClientID=${ClientID} and TrID=${TrID}" );
    return (1);
}
...
sub updClientVitalSigns {
    ...
    my $sClientVitalSigns =
      $dbh->prepare("select * from ClientVitalSigns where ID=?");
    $sClientVitalSigns->execute($ID)
      || myDBI->dberror("updClientVitalSigns: select ID=${ID}");
    ...
    my $sUpdate =
      $dbh->prepare("update ClientVitalSigns set BMI=?,BSA=? where ID=?");
    $sUpdate->execute( $BMI, $BSA, $ID )
      || myDBI->dberror("updClientVitalSigns: update ID=${ID}, BMI=${BMI}");
    $sUpdate->finish();
    return ();
}
############################################################################
sub setPrAuthNum {
    ...
    if ( $PANum eq '' ) {
        $s = $dbh->prepare(
            "update ClientPrAuth set Locked=0 where ID='${PrAuthID}'");
        $s->execute()
          || myDBI->dberror("setPrAuthNum: unlock ClientPrAuth ${PrAuthID}");
    }
    else {
        $PANum = 'MULTIPLE' if ($Many);
        $s     = $dbh->prepare(
"update ClientPrAuth set Locked=1,PAnumber='${PANum}' where ID='${PrAuthID}'"
        );
        $s->execute()
          || myDBI->dberror("setPrAuthNum: update ClientPrAuth ${PrAuthID}");
        $s = $dbh->prepare(
"update ClientPrAuthCDC set Status='Approved',StatusDate='$form->{TODAY}',Fail=NULL,Reason=NULL where ClientPrAuthID='${PrAuthID}'"
        );
        $s->execute()
          || myDBI->dberror("setPrAuthNum: update ClientPrAuthCDC ${PrAuthID}");
    }
    $s->finish();
    return ('');    # return no error message
}
...
sub updTrPlan {
    ...
    if ( $form->{TrPlan_Sign} ) {
        ...
        my $qTrPlanS =
qq|select * from TrPlanS where TrPlanS.ProvID=? and TrPlanS.ClientID=? and TrPlanS.TrPlanID=?|;

        #warn qq|qTrPlanS=$qTrPlanS\n${LOGINPROVID},${ClientID},${TrPlanID}|;
        my $sTrPlanS = $dbh->prepare($qTrPlanS);
        $sTrPlanS->execute( $LOGINPROVID, $ClientID, $TrPlanID )
          || myDBI->dberror($qTrPlanS);
...
sub updEDocs {
    ...
    elsif ($form->{'DBNAME'} eq 'okmis_mms'
        && $form->{"${table}_ProvID_1"} == 90 )
    {
        warn qq|updEDocs sync: ID=$form->{"${table}_ID_1"}\n|;
        DBUtil->sysfile( 'syncEDoc', $form->{"${table}_ID_1"},
            $form->{LOGINID} );
    }
...
sub updDischarge {
    ...
    my $ClientID =
      $inClientID ? $inClientID : $form->{'ClientDischarge_ClientID_1'};
...
"select InsNumID from Insurance where ClientID=? and Insurance.Priority=1 order by InsNumEffDate desc"
    );
    $sInsurance->execute($ClientID)
      || myDBI->dberror("updDischarge: select Client Insurance");
    ...
    my $qPrAuth =
qq|select ClientPrAuth.ID,ClientPrAuth.InsuranceID from ClientPrAuth left join ClientPrAuthCDC on ClientPrAuthCDC.ClientPrAuthID=ClientPrAuth.ID left join Insurance on Insurance.InsNumID=ClientPrAuth.InsuranceID where ClientPrAuth.ClientID='${ClientID}' and Insurance.Priority=1 and ClientPrAuth.ExpDate>='${IntDate}' and ClientPrAuthCDC.TransType!=21 order by ClientPrAuth.ID|;
...
qq|select ClientPrAuth.ID,ClientPrAuth.InsuranceID from ClientPrAuth left join ClientPrAuthCDC on ClientPrAuthCDC.ClientPrAuthID=ClientPrAuth.ID left join Insurance on Insurance.InsNumID=ClientPrAuth.InsuranceID where ClientPrAuth.ClientID='${ClientID}' and Insurance.Priority=1 and ClientPrAuth.EffDate<='${DisDate}' and ClientPrAuthCDC.TransType!=21 and ClientPrAuthCDC.TransType<60 order by ClientPrAuth.ID desc|;
   ...
   #warn qq|updDischarge inside2: ID=$DischargeID: set-DisPrAuthID=$PrAuthID\n|;
    ...
    if ( $form->{'CLOSE'} eq $DischargeID ) {
        my $sUpdCLOSE = $dbh->prepare(
"update ClientDischargeCDC set Status='Closed',StatusDate='$form->{TODAY}',Fail=NULL where ClientDischargeID='${DischargeID}'"
        );
        $sUpdCLOSE->execute()
          || myDBI->dberror("updDischarge: CLOSE ${DischargeID}");
        $sUpdCLOSE->finish();
    }
    if ( $form->{'REOPEN'} eq $DischargeID ) {
        my $sUpdREOPEN = $dbh->prepare(
"update ClientDischargeCDC set Status='New',StatusDate='$form->{TODAY}' where ClientDischargeID='${DischargeID}'"
        );
        $sUpdREOPEN->execute()
          || myDBI->dberror("updDischarge: REOPEN ${DischargeID}");
        $sUpdREOPEN->finish();
    }
    return ();
}
...
sub updAppt {
    ...
    if ( $form->{'CreateNote'} ) {
        ...
        $form->{'AppointmentContactDate'} =
          $form->{'Appointments_ContactDate_1'};
...
sub updContracts {
    ...
    if (   $rxInsurance->{'PayID'} eq ''
        || $rxInsurance->{'ClearingHouse'} eq '' )
    {
        if ( $form->{'Contracts_BillType_1'} eq 'EL' ) {
            $rContracts->{'BillType'} = 'BH';
            my $ID = DBA->doUpdate( $form, 'Contracts', $rContracts,
                "ID='$rContracts->{ID}'" );

#foreach my $f ( sort keys %{$rContracts} ) { warn qq|rContracts-${f}=$rContracts->{$f}\n|; }
            DBA->setAlert( $form,
"Contract not available for Electronic Billing, PayerID or ClearingHouse missing, reset to BH."
            );
        }
    }
...
|;
    DBUtil->email( $form, 'support@okmis.com', 'Add/Update: Insurance Contract',
        $Message, '', 1 );
    $sxInsurance->finish();
    $sProvider->finish();
    return ();
...
sub updSOGS {
    ...
    if ( my $rSOGS = $sSOGS->fetchrow_hashref ) {
        my $cnt = 0;
        $cnt++
          if ( $rSOGS->{'A6'} eq 'Most of the time I lost'
            || $rSOGS->{'A6'} eq 'Every time I lost' );

        #warn qq|cnt=$cnt\n|;
        $cnt++
          if ( $rSOGS->{'A7'} eq 'Yes in the past but not now'
            || $rSOGS->{'A7'} eq 'Yes' );

        #warn qq|cnt=$cnt\n|;
        $cnt++
          if ( $rSOGS->{'A8'} eq 'Yes less than half the time I lose'
...
sub updSOGSGSI {
 ...
 #warn qq|updSOGSGSI: YES UPDATE: TotalLost=$TotalLost, TotalPast=$TotalPast\n|;
        my $sUpdate = $dbh->prepare(
"update SOGSGSI set TotalLost=?,AvgLost=?,TotalPast=?,AvgPast=? where ID=?"
        );
        $sUpdate->execute( $TotalLost, $AvgLost, $TotalPast, $AvgPast,
            $form->{'SOGSGSI_ID_1'} );
        $sUpdate->finish();
    }
...
sub renumClientProblems {
    ...
    my $sClientProblems = $dbh->prepare(
        "select * from ClientProblems where ClientID=? order by Priority");
...
sub renumClientTrPlanPG {
    ...
    my $sClientTrPlanPG = $dbh->prepare(
...
sub renumClientFamilyProblems {
    ...
    my $num = 10;
    my $sUpdate =
      $dbh->prepare("update ClientFamilyProblems set Priority=? where ID=?");
    my $sClientFamilyProblems = $dbh->prepare(
"select * from ClientFamilyProblems where ClientFamilyID=? order by Priority"
    );
    $sClientFamilyProblems->execute($ClientFamilyID);
    while ( my $rClientFamilyProblems =
        $sClientFamilyProblems->fetchrow_hashref )
    {
...
sub renumProviderCDAparms {
    ...
    my $sProviderCDAparms = $dbh->prepare(
        "select * from ProviderCDAparms where ProvID=? order by Priority");
...
#my $udi = "=/W4146EB0010T0475=,000025=A99971312345600=>014032=%7D013032&,1000000000000XYZ123";
    ...
    my $url =
"https://accessgudid.nlm.nih.gov/api/v1/parse_udi.json?udi=${encoded_udi}";
    ...
    if ( $jsonData eq '' ) {
        ...
        my $NEWID = DBA->doUpdate( $form, 'ClientProcedures', $rudi,
            "ID=$form->{'ClientProcedures_ID_1'}" );
        return ();
    }
    ...
    my $url =
"https://accessgudid.nlm.nih.gov/api/v1/devices/lookup.json?di=$rudi->{di}";
    ...
    if ( $jsonData eq '' ) {
        ...
        DBA->setAlert( $form,
            "EMPTY data string on lookup for DEVICEID:\n${di}." );
    ...
    foreach my $f ( sort keys %{ $rparm->{'gudid'} } ) {
        warn "rparm gudid: $f=$rparm->{'gudid'}->{$f}\n";
    }
    foreach my $f ( sort keys %{ $rparm->{'gudid'}->{'device'} } ) {
        warn "rparm: $f=$rparm->{'gudid'}->{'device'}->{$f}\n";
    }
    foreach my $f (
        sort keys %{ $rparm->{'gudid'}->{'device'}->{'gmdnTerms'}->{'gmdn'} } )
    {
        warn
"rparm gmdnTerms-gmdn: $f=$rparm->{'gudid'}->{'device'}->{'gmdnTerms'}->{'gmdn'}->{$f}\n";
    }
    foreach
      my $f ( sort keys %{ $rparm->{'gudid'}->{'device'}->{'sterilization'} } )
    {
        warn
"rparm sterilization: $f=$rparm->{'gudid'}->{'device'}->{'sterilization'}->{$f}\n";
    }
    $rudi->{'error'} = $rparm->{'error'};
    $rudi->{'devicePublishDate'} =
      $rparm->{'gudid'}->{'device'}->{'devicePublishDate'};
    $rudi->{'catalogNumber'} = $rparm->{'gudid'}->{'device'}->{'catalogNumber'};
    $rudi->{'deviceHCTP'}    = $rparm->{'gudid'}->{'device'}->{'deviceHCTP'};
    $rudi->{'gmdnPTName'} =
      $rparm->{'gudid'}->{'device'}->{'gmdnTerms'}->{'gmdn'}->{'gmdnPTName'};
    $rudi->{'gmdnPTDefinition'} =
      $rparm->{'gudid'}->{'device'}->{'gmdnTerms'}->{'gmdn'}
      ->{'gmdnPTDefinition'};
    $rudi->{'brandName'} = $rparm->{'gudid'}->{'device'}->{'brandName'};
    $rudi->{'versionModelNumber'} =
      $rparm->{'gudid'}->{'device'}->{'versionModelNumber'};
    $rudi->{'companyName'} = $rparm->{'gudid'}->{'device'}->{'companyName'};
    $rudi->{'MRISafetyStatus'} =
      $rparm->{'gudid'}->{'device'}->{'MRISafetyStatus'};
    $rudi->{'labeledContainsNRL'} =
      $rparm->{'gudid'}->{'device'}->{'labeledContainsNRL'};
    $rudi->{'labeledNoNRL'} = $rparm->{'gudid'}->{'device'}->{'labeledNoNRL'};
    delete $rudi->{'udi'}
      ;    # returned udi is shorter than requested so DON'T update??
    foreach my $f ( sort keys %{$rudi} ) { warn "rudi2: $f=$rudi->{$f}\n"; }

    if ( $rudi->{'error'} eq '' ) {
        $rudi->{'error'} = '';
    }      # make sure we update error to NULL
    else {
        DBA->setAlert( $form, "error:\n$rudi->{error}." );
        $rudi->{'di'} = 'none' if $rudi->{'di'} eq '';
    }
    my $NEWID = DBA->doUpdate( $form, 'ClientProcedures', $rudi,
        "ID=$form->{'ClientProcedures_ID_1'}" );
    return ();
...
sub setClientRuleAlerts {
    ...
    my $dbh     = myDBI->dbconnect( $form->{'DBNAME'} );
    my $sDelete = $dbh->prepare(
        "delete from ClientRuleAlerts where ClientID='${ClientID}'");
    $sDelete->execute()
      || myDBI->dberror(
        "setClientRuleAlerts: delete ClientRuleAlerts ${ClientID}=${ClientID}");
    $sDelete->finish();
    my $sInsert = $dbh->prepare(
"insert into ClientRuleAlerts (ClientID,RuleID,ChangeProvID) values (?,?,?)"
    );
    ...
    while ( my $rCDSrules = $sCDSrules->fetchrow_hashref ) {
        ...
        if ($cnt) {
            ...
            $sInsert->execute( $ClientID, $RuleID, $ProvID )
              || myDBI->dberror(
"setClientRuleAlerts: insert ClientRuleAlerts ${ClientID}/${RuleID}"
              );
            push( @ruleids, $RuleID );
        }
    }
...
##crontab -l -u okmms | grep "genCCDAs DBNAME=okmms_dev\\\&ID=2"
    ...
    my $jobname =
      DBA->getxref( $form, 'xCronJobs', $form->{'ProviderJobs_Command_1'},
        'Descr' );
    my $job = $jobname . ' DBNAME=' . $form->{DBNAME} . "\\\\\\\&ID=" . $ID;
    warn qq|Remove Job: job=${job}\n|;
    my $outfile =
      qq|/home/${user}/logs/${jobname}.$form->{'DBNAME'}.${ID}.cronout|;
    warn qq|outfile=${outfile}\n|;
    my $delcron =
      qq|crontab -l | . '|' . qq| grep -v "${job}" > ${outfile} 2>&1|;
    ...
    {
        my ( $hrs, $min, $sec ) =
          split( ':', $form->{'ProviderJobs_CronTime_1'}, 3 );
        my $dom =
          $form->{'ProviderJobs_CronDay_1'} eq ''
          ? '*'
          : $form->{'ProviderJobs_CronDay_1'};
        my $month =
          $form->{'ProviderJobs_CronMonth_1'} eq ''
          ? '*'
          : $form->{'ProviderJobs_CronMonth_1'};
        my $week =
          $form->{'ProviderJobs_CronWeek_1'} eq ''
          ? '*'
          : $form->{'ProviderJobs_CronWeek_1'};
        my $addcron =
qq|(crontab -l; echo "${min} ${hrs} ${dom} ${month} ${week} ${BIN}/${jobname} DBNAME=$form->{'DBNAME'}\\&ID=${ID} >> ${outfile} 2>&1")|;
...
#warn qq|updDepressionTrID: ${table}, ${ID}\n|;
    ...
    $sPHQ9->execute($ID)
      || myDBI->dberror("setDepressionTrID: select ID=${ID}");
...
"select * from Treatment left join xSC on xSC.SCID=Treatment.SCID where Treatment.ClientID=? and ContLogDate=? and (SCNum='G8431' or SCNum='G8432' or SCNum='G8434' or SCNum='G8510')"
        );
        $sTreatment->execute( $ClientID, $ContDate )
          || myDBI->dberror("updTreatment: select ${ClientID}/${ContDate}");
        if ( my $rTreatment = $sTreatment->fetchrow_hashref ) {

            #warn qq|setDepressionTrID: TrID=$rTreatment->{TrID}\n|;
            my $sUpdate =
              $dbh->prepare("update ${table} set TrID=? where ID=?");
            $sUpdate->execute( $TrID, $ID )
              || myDBI->dberror(
                "setDepressionTrID: update ID=${ID}, TrID=${TrID}");
            $sUpdate->finish();
        }
        $sTreatment->finish();
    }
...
sub dupxTable {
    ...
    $sxTables->execute($TableID)
      || myDBI->dberror("dupxTable: select xTables ${TableID}!");
    if ( my $rxTables = $sxTables->fetchrow_hashref ) {
        ...
        my $sxTableFields =
          $cdbh->prepare("select * from xTableFields where TableID=?");
        warn qq|dupxTable: TableID=${TableID}\n|;
        $sxTableFields->execute($TableID)
          || myDBI->dberror("dupxTable: select xTableFields ${TableID}");
...
sub reseqxTable {
    ...
    warn qq|\n\nENTER reseqxTable: \n|;
    foreach my $f ( sort keys %{$form} ) {
        warn "reseqxTable: form-$f=$form->{$f}\n";
    }
    my $TableID = $form->{'xTables_ID_1'};
    my $cdbh    = myDBI->dbconnect('okmis_config');
    my $seq     = 0;
    my $uxTableFields =
      $cdbh->prepare("update xTableFields set theSeq=? where ID=?");
    my $sxTableFields = $cdbh->prepare(
        "select * from xTableFields where TableID=? order by theSeq");
    $sxTableFields->execute($TableID);
    while ( my $rxTableFields = $sxTableFields->fetchrow_hashref ) {

   #warn qq| update: ${seq}:$rxTableFields->{theField}: $rxTableFields->{ID}\n|;
        $uxTableFields->execute( $seq, $rxTableFields->{ID} )
          || $form->dberror("FAILED: $rxTableFields->{ID}: update");
        $seq++;
    }
...
sub addxTableQues {
    ...
    warn qq|\n\nENTER addxTableQues: \n|;
    foreach my $f ( sort keys %{$form} ) {
        warn "addxTableQues: form-$f=$form->{$f}\n";
    }
    my $TableID      = $form->{'xTables_ID_1'};
    ...
    foreach my $ques ( split( "\n", $NewQuestions ) ) {
        ...
        $seq++;
        my $q =
qq|INSERT INTO `xTableFields` (`ID`, `CreateProvID`, `CreateDate`, `ChangeProvID`, `ChangeDate`, `FormID`, `Locked`, `TableID`, `theField`, `theSeq`, `agent`, `theType`, `theText`, `thePreText`, `thePostText`, `theValues`, `descriptors`, `onchange`, `colspan`, `theSize`, `theStyle`, `theArgs`) VALUES (NULL,NULL,'$form->{'TODAY'}',NULL,NULL,NULL,NULL,${TableID},'q${seq}',${seq},'0','radio','${ques}',NULL,NULL,${v},${d},NULL,'3',NULL,NULL,${theArgs})|;
