...
sub styleQRDA {
    ...
    my $QRDAxsl =
      $form->{'DOCROOT'} . myConfig->cfgfile( "QRDAIII_INDIV.xsl", 1 );
...
sub setXML {
    ...
    $sProvider->execute($ProvID)
      || $form->dberror("setXML: select Provider ${ProvID}");
...
sub prepareMeasures {
    ...
    if ( $TOP_MEASUREID eq '138' ) {
        ...
        $numerator_two_measures =
          $self->prepare_numerator_two_measures(%measures);
        ...
        $numerator_three_measures =
          $self->prepare_numerator_three_measures(%measures);
    }
...
sub prepare_numerator_two_measures {
    ...
    my $Not_Hisp_Lati =
      $measures{'NUMERATOR_TWO'}{'Ethnicity'}{'Not_Hisp_Lati'};
...
sub prepare_numerator_three_measures {
    ...
    my $Not_Hisp_Lati =
      $measures{'NUMERATOR_THREE'}{'Ethnicity'}{'Not_Hisp_Lati'};
    ...
    my $Native_Hawaiian =
      $measures{'NUMERATOR_THREE'}{'Race'}{'Native_Hawaiian'};
...
sub genProvider {
    ...
    $sProviderControl->execute($ProvID)
      || $form->dberror("genProvider: select ProviderControl ${ProvID}");
    my $rProviderControl = $sProviderControl->fetchrow_hashref;
    $sProviderLicenses->execute($ProvID)
      || $form->dberror("genProvider: select ProviderLicenses ${ProvID}");
    ...
    $sProvider->execute($ClinicID)
      || $form->dberror("genProvider: select Clinic $ClinicID");
