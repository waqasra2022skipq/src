...
sub list_zip_files_in_table {
    my ($self, $form, $type, $id) = @_; # The folder path where zip files are located

    my $dbh = myDBI->dbconnect($form->{'DBNAME'});
    my $qEdocs =   qq|Select Title,Path From ProviderEDocs Where ProvID = ? and Type = ?|;
    ...
    while (my ($Title, $Path) = $sEdocs->fetchrow_array) {
      ...
      $html .= "<td> <a STYLE='cursor:pointer;' onclick=\"deleteFile('$Path', '$form->{DBNAME}', '$count')\">Delete</a> </td>";
      $html .= '</tr>';
    }
...
# the subtitle since we start the html as a FRAMESET
  ...
  my $lhcautocomplete = $flags =~ /lhcautocomplete/i ? qq|
...
#             ie:   PrintPHQ:Scores
          ...
          $display = qq|<A HREF="javascript:ReportWindow('/cgi/bin/${print_routine}.cgi?IDs=$r->{$RECID}&mlt=$form->{mlt}&action=${table}\&page=${page}','PrintWindow')" TITLE="${msg}" ><IMG ALT="print" SRC="/img/print-ok.png" HEIGHT="21" WIDTH="21" BORDER="0" ></A><BR>${fldval}|;
        }
...
#warn qq|ListSel: CHECK: Privilege=Agent\n|;
      ...
      {
        $show_id = qq|<A HREF="javascript:ReportWindow('/cgi/bin/show_id.cgi?IDs=$r->{$RECID}&mlt=$form->{mlt}&action=${table}\&page=${page}','PrintWindow')" TITLE="${TABLEID}: $r->{$RECID}" ><IMG ALT="show" SRC="/img/zoom.png" HEIGHT="20" WIDTH="20" ></A>| if ( $form->{'LOGINPROVID'} == 91 );
        $show_id .= qq|<A HREF="javascript:ReportWindow('/cgi/bin/show_log.cgi?IDs=$r->{$RECID}&mlt=$form->{mlt}&action=${table}\&page=${page}','PrintWindow')" TITLE="${TABLEID}: $r->{$RECID}" ><IMG ALT="history" SRC="/img/history.png" HEIGHT="20" WIDTH="20" ></A>| if ( myDBI->getTableLogFlag($form,$table) );
      }
...
#  my $s=$cdbh->prepare("select * from misTables where theTable='${table}' order by seq");
  ...
  {
    $html .= qq|<SCRIPT LANGUAGE="JavaScript">newtextMsg('help$rxTables->{theField}','$rxTables->{Defn}');</SCRIPT>\n|;
...
sub clientAddressForm
{
  ...
  if ($form->{LOGINPROVID} ne 91 && $form->{LOGINPROVID} ne 90  && $form->{LOGINPROVID} ne 89)
  {
    return;
  }
