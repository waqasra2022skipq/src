...
#warn qq|setLogin: q=${q}\n|;
    ...
    unless ( $s->rows() ) {
      if ( $form->{'office_365_login'} ) {
        $q = qq|select * from UserLogin where BINARY UserLogin.email='$self->{LOGINID}' and UserLogin.dbname='$form->{DBNAME}' |;
        $s = $dbh->prepare($q);
        $s->execute() || myLogin->error("sql error: setLogin checkpass");
      }
    }
...
#warn qq|setLogin: loginid=$r->{loginid}, renew=$r->{renew}\n|;
      $s->finish();
      myLogin->renewPasswd($form,$r) if ( $r->{renew} && $ENV{HTTP_USER_AGENT} );
...
#warn qq|getLogin: exit: ok=$ok, DBNAME=$form->{DBNAME}\n|;
  myDBI->cleanup();
  print myLogin->login($form,'System Unavailable: Upgrades/Enhancements: <BR>Please try again around 5am on 03/05/2019.');
  exit;
}
############################################################################
sub renewPasswd
...
#warn qq|renewLogin: DBNAME=$form->{DBNAME}, user=$form->{user}\n|;
  if ( $form->{user} )     # because first link is <A> does not send user.
  {
    ...
    $sUserLogin->execute() || myLogin->error("renewLogin: $form->{user} $form->{DBNAME}");
...
#warn qq|renewLogin: DB=${DB}, ID=${ID}, qUser=${qUser}\n|;
      ...
      $sUser->execute($rUserLogin->{'UserID'}) || myLogin->error("renewLogin: ${qUser} $rUserLogin->{'UserID'}");
...
#warn qq|renewLogin: ssn=$form->{ssn}, SSN=$rUser->{SSN}\n|;
      if ( $form->{ssn} eq $rUser->{SSN} )
      {
        ...
        $sUpd->execute() || myLogin->error("renewLogin: ERROR: update $form->{user}/$rUserLogin->{UserID}, Password=${Password}");
        $sUpd->finish();
        ...
        myForm->saveLINK('renewLogin',"$ENV{SCRIPT_NAME}?$ENV{QUERY_STRING}");
      }
...
{ 
  ...
  {
    my $sLogin=$dbh->prepare("insert into Login (Token,UserID,Name,Browser) values (?,?,?,'SysAdmin')");
