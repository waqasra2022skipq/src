...
class ntsCsrf
{
	static function get_token_name()
	{
		$return = 'nts_csrf_token';
		return $return;
	}

	static function get_cookie_name()
	{
		$return = 'nts_csrf_cookie';
		return $return;
	}

	static function set_cookie()
	{
		$cookie_name = ntsCsrf::get_cookie_name();
		$hash = ntsCsrf::get_hash();
		$cookie_path = '/';
		$cookie_domain = '';
		$csrf_expire = 7200;
		...
		setcookie( $cookie_name, $hash, $expire, $cookie_path, $cookie_domain, $secure_cookie);
	}
	...
	static function get_hash()
	{
		...
		if( $hash == ''){
			if( isset($_COOKIE[$cookie_name]) &&
				preg_match('#^[0-9a-f]{32}$#iS', $_COOKIE[$cookie_name]) === 1){
	...
	static function show_error( $error, $requested_panel = NULL )
	{
		echo 'CSRF ERROR: ' . $error;

		$outFile = NTS_APP_DIR . '/../csrflog.txt';
		if( file_exists($outFile) ){
			$fp = fopen( $outFile, 'a' );
			$referer = ( ! isset($_SERVER['HTTP_REFERER']) OR $_SERVER['HTTP_REFERER'] == '') ? '' : trim($_SERVER['HTTP_REFERER']);

			$msg = '';
			$msg .= 'pan = ' . $requested_panel . "\n";
			$msg .= 'ref = ' . $referer . "\n";
			$msg .= $error;
	...
	static function verify( $requested_panel = NULL )
	{
		if( defined('NTS_NO_CSRF_CHECK') ){
			return TRUE;
		}

		$csrf_token_name = ntsCsrf::get_token_name();
		$csrf_cookie_name = ntsCsrf::get_cookie_name();

		// If it's not a POST request we will set the CSRF cookie
		if (strtoupper($_SERVER['REQUEST_METHOD']) !== 'POST')
		{
			return ntsCsrf::set_cookie();
		}

		// Do the tokens exist in both the _POST and _COOKIE arrays?
		if ( ! isset($_POST[$csrf_token_name], $_COOKIE[$csrf_cookie_name]))
		{
			// _print_r( $_POST );
			// _print_r( $_COOKIE );
		...
		if( isset($this->sanitizer[$pName]) ){
			...
			if( is_array($return) ){
				...
				foreach( $return as $r ){
					if( ! preg_match($re, $r) ){
					/* sanitizer failed */
						echo "invalid value for '$pName' detected<br>";
						var_dump( $return );
						exit;
						}
					}
...
class ntsSession {
	...
	public $sess_cookie_name			= 'nts_session';
	public $cookie_prefix				= '';
	public $cookie_path				= '';
	public $cookie_domain				= '';
	public $cookie_secure				= FALSE;
	public $sess_time_to_update		= 300;
	public $encryption_key				= 'ntsencrypt';
	...
	public $session_key_name = 'nts_userdata';
