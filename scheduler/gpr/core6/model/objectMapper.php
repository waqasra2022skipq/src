...
class objectMapper extends ntsObjectMapper {
	...
	function makeTags_Appointment( $object, $access = 'external', $show_end_time = TRUE, $force_tz = NULL ){
		...
		$timeFormatted_StartOnly = $t->formatDateFull() . ' ' . $t->formatTime(0, $showTimezone);
		...
		$tags[0][] = '{APPOINTMENT.START_ONLY}';
		$tags[1][] = $timeFormatted_StartOnly;
...
//		$tags[1][] = $dueView;
	...
	/* location */
		...
		if( function_exists('get_userdata') ){
			$user_info = get_userdata($customer->getId());
			$customerRolesView = implode(', ', $user_info->roles);
			$tags[0][] = '{APPOINTMENT.CUSTOMER.ROLES}';
			$tags[1][] = $customerRolesView;
		}
