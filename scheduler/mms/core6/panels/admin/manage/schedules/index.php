		...
		<?php foreach( $dis as $di ) : ?>
			...
			/* timeblock */
				if( isset($b[0]) )
				{
					...
					$t->setTimestamp( $startDay )
						->modify( '+' . $b[0]['starts_at'] . ' seconds' )
						;
							...
							<div class="row">
								...
								<div class="col-md-5">
									...
									if( count($ress2) > 1 ){
										...
										$more_views[] = ntsView::objectTitle( $resource, TRUE );
									}
								
									if( count($b) <= 1 ){
										$this_sid = $b[0]['service_id'];
										if( $this_sid ){
											$service = ntsObjectFactory::get( 'service', $this_sid );
											$more_views[] = ntsView::objectTitle( $service, TRUE );
										}
									}
									?>
									
									<?php if( $more_views ) : ?>
										<?php echo join('<br/>', $more_views); ?>
									<?php endif; ?>
								</div>
