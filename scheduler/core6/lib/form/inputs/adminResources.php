...
$archivedResources = array();

reset( $allResources );
foreach( $allResources as $e ){
	$isArchive = $e->getProp('archive');
	if( $isArchive ){
		$archivedResources[] = $e;
	}
	else {
...
$allResources = array_merge( $activeResources, $archivedResources );
...
switch( $inputAction ){
	case 'display':
		...
		foreach( $allResources as $e ){
			...
			$resTitle .= '<br><small class="text-muted">' . 'id: ' . $id . '</small>';
			$input .= '<tr';
			if( $on )
				$input .= ' class="success"';
			$input .= '>';
