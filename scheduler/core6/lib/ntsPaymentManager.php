...
class ntsPaymentManager {
	...
	public $couponCountLeft = array();
	public $gotPromotions = array();
	...
	function getRKey( $r )
	{
		$return = array(
			...
			'customer_id'	=> isset($r['customer_id']) ? $r['customer_id'] : 0,
			);
		if( isset($r['date'])){
			$return['date'] = $r['date'];
		}
	...
	function getPromotionsOfCustomer( $customer_id )
	{
		$return = array();
		...
		foreach( $this->promotions as $cp ){
			...
			if( isset($rule['customer']) && in_array($customer_id, $rule['customer']) ){
				$return[] = $cp;
			}
		}
		return $return;
		...
		foreach( $this->promotions as $cp )
		{
			...
			foreach( $rule as $key => $options )
			{
				...
				switch( $key )
				{
					...
					case 'customer':
						if( isset($r['customer_id']) && in_array($r['customer_id'], $options) )
						{
							$on = TRUE;
						}
						break;
					...
					case 'time':
						if( isset($r['starts_at']) OR isset($r['date']) )
						{
							if( isset($r['date']) ){
								$on = TRUE;
							}
	...
	function getBasePrice( $r = array() )
	{
		...
		if( isset($r['pack_id']) ){
			...
			$pack->setId( $r['pack_id'] );
			$return = $pack->getProp('price');
		}
		else {
