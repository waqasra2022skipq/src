...
if( ! $groupRef )
{
	$groupRef = $object->generate_group_ref();
...
if( strlen($price) )
{
	...
	$promotions = $ntspm->getPromotions( $object, $coupon, TRUE );
	foreach( $promotions as $pro )
	{
		if( (! $coupon) && ($pro->getCouponCodes()) ){
			continue;
		}
		$this->runCommand( 
			$pro,
...
/* now promotions without coupon */
$promotions = $ntspm->getPromotions( $object );
foreach( $promotions as $pro )
{
	$this->runCommand( 
		$pro,
