...
EOT;
			...
			case 'customerNoEndTimeNotification':
				if( ! $rawValue )
					$return = 0;
				else
					$return = $rawValue;
...
{
	if( 
		( substr($str, 0, strlen('[m]')) == '[m]' ) &&
		( substr($str, -strlen('[/m]')) == '[/m]' )
		){
		$str = substr($str, strlen('[m]'), -strlen('[/m]'));
	}
...
//			$ntsdb->_debug = FALSE;
	...
	/* problem with this file */
		...
		{
			echo "Language file error: " . $languageConf['error'] . '<br>';
...
//echo "loading lang file $fullFileName<br>";
		if( ($lng != 'en-builtin') && $fullFileName ){
			...
			if( $lng == 'languageTemplate' ){
				...
				if( is_array($currentCountries) && (count($currentCountries) > 1) ){
		...
		// if template then load custom terms too
			if( $lng == 'languageTemplate' ){
				$add_more = array();
				$ntsdb =& dbWrapper::getInstance();

				$take_from = array(
					'form_controls'	=> array('title', 'description'),
					'services'		=> array('title', 'description'),
					'resources'		=> array('title', 'description'),
					'promotions'	=> array('title'),
					'packs'			=> array('title'),
					);

				foreach( $take_from as $take_table => $take_props ){
					if( ! $ntsdb->tableExists($take_table) ){
						continue;
					}

					$more = $ntsdb->get_select( 
						$take_props,
						$take_table
						);
					...
					foreach( $more as $mo ){
						foreach( $mo as $mo2 ){
							if( preg_match('/\[m\](.+)\[\/m\]/', $mo2, $ma) ){
								if( ! in_array($ma[1], $add_more) ){
									$add_more[] = $ma[1];
								}
							}
						}
				...
				foreach( $add_more as $am ){
					if( ! array_key_exists($am, $thisLangStrings) ){
						$thisLangStrings[$am] = $am;
					}
				}
				ksort( $thisLangStrings );
			}
