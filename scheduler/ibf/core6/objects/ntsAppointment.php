...
class ntsAppointment extends ntsObject {
	...
	public function generate_group_ref()
	{
		...
		while( $exists ){
			...
			$exists = $ntsdb->get_select( 'group_ref', 'appointments', array('group_ref' => array('=', $groupRef)) );
		}
		return $groupRef;
	}
	...
	public function getStatusActions()
	{
		...
		if( ! $completedStatus )
		{
			if( $approvedStatus )
			{
				...
				$actions[] = array( 'checked-in',	ntsAppointment::_statusLabel(0, HA_STATUS_CHECKED_IN, '', 'i') . ' ' . M('Checked In') );
		...
		else
		{
			...
			if( $completedStatus == HA_STATUS_CHECKED_IN )
			{
				$actions[] = array( 'complete',	ntsAppointment::_statusLabel(0, HA_STATUS_COMPLETED, '', 'i') . ' ' . M('Completed') );
			}
		}
			...
			if( (! $show_fields) OR in_array('payment_balance', $show_fields) ){
				...
				if( $due_amount < $amount ){
					...
					foreach( $postings as $e ){
						...
						if( $key == 'appointment::fund' ){
							$paid_by_balance = 1;
							$paid_by_balance_amount += $e['asset_value'];
						}
					}
				}
			}
	...
	/* transactions */
		if( ((! $show_fields) OR in_array('invoice_ref', $show_fields)) && ($part_id < 2) ){
			if( $paid_by_balance ){
				$return['invoice_ref'] = '';
				$return['paid_amount'] = $paid_by_balance_amount;
				$return['paid_through'] = M('Pay By Balance');
	...
	function getOrder(){
		...
		$order_ids = $this->getProp('_order');
		if( $order_ids && isset($order_ids[0]) ){
			$return = ntsObjectFactory::get( 'order' );
			$return->setId( $order_ids[0] );
		}
	...
	static function _statusText( $approved, $completed )
	{
		...
		$completed_conf = array(
			...
			HA_STATUS_CHECKED_IN=> array( 'default',	M('Checked In') ),
			);
