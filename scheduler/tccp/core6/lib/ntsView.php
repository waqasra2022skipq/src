...
class ntsListingTable {
	...
	function __construct( $fields, $entries ){
		$this->fields = $fields;
			...
			case 'user':
				...
				if( $return )
					...
					if( $object->getId() == 0 ){
						...
						if( $object->getProp( 'display_name' ) ){
							$return = $object->getProp( 'display_name' );
						}
						else {
...
//				$html = "<META http-equiv=\"refresh\" content=\"0;URL=$to\">";
	...
	static function redirect_later( $to )
	{
		global $NTS_LATER_REDIRECT;
		$NTS_LATER_REDIRECT = $to;
	}

	static function redirect( $to, $force = false, $parent = false )
	{
		global $NTS_VIEW, $NTS_LATER_REDIRECT;
		if( $NTS_LATER_REDIRECT ){
			$to = $NTS_LATER_REDIRECT;
			$NTS_LATER_REDIRECT = NULL;
