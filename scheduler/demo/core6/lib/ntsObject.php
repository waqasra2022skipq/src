...
class ntsObject {
	public $className;
	public $props = array();
	public $updatedProps = array();
	public $id = 0;
	public $notFound = false;
	public $_change_reason = '';
	...
	function getProp( $pName, $unserialize = FALSE ){
		...
		if( $unserialize )
		{
			$return = hc_mb_unserialize( $return );
		}
		...
		if( isset($NTS_OBJECT_PROPS_CONFIG[$this->className][$pName]) ){
			if( 
				...
				if( $fromStorage ){
					$pValue = trim($pValue);
					if( strlen($pValue) )
						$pValue = hc_mb_unserialize( $pValue );
					else
						$pValue = array();
...
class ntsObjectMapper {
	...
	public $service2form = array();
	public $fieldsCache = array(); 
	...
	public function makeDetails_Customer( $object, $access = 'external' ){
...
//			echo "fields '$cacheString' already set<br>";
	...
	function getControl( $className, $name, $forSearch = false ){
		...
		if( isset($c['description']) ){
			if( preg_match('/\[m\](.+)\[\/m\]/', $c['description'], $ma) ){
				$c['description'] = M($ma[1]);
			}
		}

		if( ! $forSearch )
		{
			if( isset($c['validators']) && $c['validators'] && is_array($c['validators']) )
			{
		...
		if( $result ){
			while( $c = $result->fetch() ){
				if( isset($c['attr']) && $c['attr'] )
					$c['attr'] = hc_mb_unserialize($c['attr']);
				else
					$c['attr'] = array();

				if( isset($c['validators']) && $c['validators'] ){
					$validators = @hc_mb_unserialize($c['validators']);
					if( $validators ){
						reset( $validators );
						$c['validators'] = array();
