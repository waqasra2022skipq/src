...
//		$conf['error'] = $this->_getErrorForInput( $conf['id'] );
	...
	// add csrf protection
		$csrf_name = ntsCsrf::get_token_name();
		$csrf_value = ntsCsrf::get_hash();
		$postParts[] = '<input type="hidden" id="' . $csrf_name . '" name="' . $csrf_name . '"' . ' value="' . $csrf_value . '">';
...
class ntsForm {
	function __construct( $formFile, $defaults = array(), $idPrefix = '' ){
...
//			$return .= '<div class="col-md-10">';
	...
	function buildInput( $type, $inputArray, $validators = array() )
	{
		...
		if( ($type == 'date/Calendar') && (! (isset($conf['default']) && $conf['default'])) ){
			if( array_key_exists('default', $conf) && ($conf['default'] === NULL) ){
				$conf['default'] = '';
			}
			else {
				$t = new ntsTime;
				$t->setNow();
				$conf['default'] = $t->formatDate_Db();
			}
		}
	...
	// NOW VALIDATE
		...
		/* built-in control validation */
			...
			if( ! isset($controlConf['validators']) )
				continue;
			if( ! is_array($controlConf['validators']) )
				continue;
