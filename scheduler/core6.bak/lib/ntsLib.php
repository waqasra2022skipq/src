...
if( ! function_exists('hc_mb_unserialize') ){
	function hc_mb_unserialize( $string )
	{
		// $string = preg_replace('!s:(\d+):"(.*?)";!se', "'s:'.strlen('$2').':\"$2\";'", $string);
		$func = create_function( '$m', 'return "s:" . strlen($m[2]) . ":\"" . $m[2] . "\";";' );
		$string = preg_replace_callback( '!s:(\d+):"(.*?)";!s', $func, $string );
...
class ntsLib {
	...
	static function profiler( $label = '' )
	{
		...
		if( (! $NTS_SQL_PROFILER) OR (! is_array($NTS_SQL_PROFILER)) ){
			return;
		}

		foreach( $NTS_SQL_PROFILER as $a ){
			echo '<span style="text-decoration: underline;">' . $a[1] . ' sec</span>' . ': ' . $a[0] . '<br>';
		}
	}
...
//		printf("%.2fs", $time );
	...
	static function checkLicenseUrl()
	{
		...
		if( $ri && ($ri == 'wordpress') )
		{
			...
			$slug = isset($_REQUEST['page']) ? $_REQUEST['page'] : '';
