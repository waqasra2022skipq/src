		...
		*/
			case 'promotion::apply':
				...
				if( isset($params['appointment']) ){
					$apply_on = $params['appointment'];
				}
				elseif( isset($params['order']) ){
					$apply_on = $params['order'];
				}
				if( ! $apply_on ){
					break;
				}
				$account_type = $apply_on->getClassName();

				$coupon = isset($params['coupon']) ? $params['coupon'] : '';

				$ntspm =& ntsPaymentManager::getInstance();
				$amount = $ntspm->applyPromotion( $apply_on, $object );
					...
					default:
						switch( $asset['type'] )
						{
							...
							case 'duration':
								...
								if( ! $app_duration ){
									$sid = $app->getProp('service_id');
									$service = ntsObjectFactory::get( 'service' );
									$service->setId( $sid );
									$app_duration = $service->getProp('duration');
								}
